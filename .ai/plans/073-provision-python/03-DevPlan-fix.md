...
# DevPlan 073 (Revised): provision-environment.sh → Python — F1-F7 fixes

$ARTIFACT_CONTRACT
PURPOSE: Migrate provision-environment.sh (442 LOC, 13 inline python3 calls) to Python module `core/internal/provisioner.py`. Eliminate all inline python3 from shell script, reduce shell to <55 LOC thin wrapper. Preserve existing tests (628 LOC) and consumer compatibility.
...
ACCEPTANCE_CRITERIA:
  ...
  - `core/internal/provision-environment.sh` — reduced to <55 LOC thin wrapper (arg parsing + audit_step + dispatch)
  ...
  - `tests/unit/test_provisioner.py` — 18 unit tests for Python module (parsing, planning, dry-run, idempotency)
  - Existing tests (28 unit + 2 smoke) continue to pass — subprocess-level integration coverage preserved
  - `make gate MODE=fast` — green
IMPLEMENTS: Wave 6B — Tier 1 shell → Python migration (Strangler-Fig discipline)
IMPACTS:
  - core/internal/provisioner.py (NEW — ~340 LOC)
  - core/internal/provision-environment.sh (REDUCE: 442 → ~50-55 LOC)
  - tests/unit/test_provisioner.py (NEW — ~250-300 LOC)
  - tests/test_unit_provision_environment.py (RETAINED AS-IS — 471 LOC, 28 tests, subprocess-level integration coverage)
  - tests/test_smoke_provision_environment.py (RETAINED AS-IS — 157 LOC, 2 tests, Docker-dependent E2E coverage)
  - core/internal/bootstrap/deploy-modules.sh (CONSUMER — lines 36-38, interface compatible, NO CHANGE)
  - core/internal/bootstrap/lifecycle/state_machine.py (CONSUMER — lines 1164-1168, interface compatible, NO CHANGE)
  - core/entrypoint-manifest.yaml (UPDATE — delegates_to chain: provision-environment.sh → provisioner.py)
...
### 2.2 Shell Wrapper: `core/internal/provision-environment.sh` (TARGET: ~50-55 LOC)
...
- LDD log format `[IMP:9][provision]` — **unchanged block name** for backward compatibility with 20 existing test assertions
...
make provision SCOPE=all
  → makefiles/helpers.mk → bash provision-environment.sh --scope all --platform-env platform-env.yaml
...
The same flow applies for consumers `deploy-modules.sh` and `state_machine.py` (see §6), which call `provision-environment.sh --scope networks --scope volumes` — the shell wrapper preserves this interface exactly.

---

## §5. Test Strategy

### 5.1 Layered Test Architecture

The migration introduces a **two-layered test architecture**:

| Layer | Test file | LOC | Tests | Type | What it validates |
|-------|-----------|-----|-------|------|-------------------|
| **Unit** | `tests/unit/test_provisioner.py` (NEW) | ~250-300 | 18 | Native Python imports | `provisioner.py` module: dataclasses, YAML parsing, function logic, CLI args — via monkeypatch (no subprocess, no Docker) |
| **Integration (retained)** | `tests/test_unit_provision_environment.py` | 471 | 28 | Subprocess via `bash provision-environment.sh` | Shell wrapper CLI contract: `--scope`, `--dry-run`, `--platform-env`, multi-scope, LDD logging format |
| **E2E (retained)** | `tests/test_smoke_provision_environment.py` | 157 | 2 | Subprocess + Docker | Full provision cycle: networks + volumes + GITHUB_ENV export + idempotency |

**Total: 48 tests (18 new + 28 existing unit + 2 existing smoke).**

### 5.2 Existing Test Migration Strategy

The two existing test files test the provisioner through the **shell wrapper interface** via `subprocess.run(["bash", "provision-environment.sh", ...])`. Since the shell wrapper preserves its CLI interface (same `--scope`, `--dry-run`, `--platform-env` flags), these tests require **minimal to zero changes**.

**`tests/test_unit_provision_environment.py` (28 tests):**

| Test class | Tests | What it tests | Post-migration status |
|-----------|-------|---------------|---------------------|
| `TestYamlParsing` | 9 tests | `platform-env.yaml` structure via `yaml.safe_load()` — **does NOT call provisioner**, reads YAML directly | **NO CHANGE** — data validation, independent of provisioner implementation |
| `TestProvisionerDryRun` | 9 tests | Shell CLI via `_run_provisioner("--scope", "networks", "--dry-run")` — validates `--dry-run` output, `--platform-env` path, `--help` flag | **NO CHANGE** — shell wrapper preserves CLI interface. 20 assertions grep for `[IMP:9][provision]` which the wrapper still emits (unchanged block name — see §8.1) |
| `TestProvisionerWithDocker` | 2 tests | Docker-dependent: network creation + idempotency | **NO CHANGE** — wrapper delegates to Python, same Docker commands |
| `TestProvisionerLDDLogging` | 1 test | Checks `[IMP:9][provision] Provision complete` in stderr | **NO CHANGE** — wrapper's final `echo` preserves this exact format |
| `TestMultiScopeDispatch` | 6 tests | `--scope all`, `--scope networks --scope volumes`, multi-scope idempotency | **NO CHANGE** — wrapper iterates scopes identically |
| `TestProvisionerErrors` | 1 test | Missing `--platform-env` path → error | **NO CHANGE** — wrapper validates args before Python call |

**`tests/test_smoke_provision_environment.py` (2 tests):**

| Test | What it tests | Post-migration status |
|------|---------------|---------------------|
| `test_provision_and_compose_up` | Networks + volumes + idempotency with real Docker | **NO CHANGE** — calls `bash provision-environment.sh --scope networks ...`, wrapper preserves interface |
| `test_provisioner_env_defaults_export` | GITHUB_ENV export via temporary file | **NO CHANGE** — wrapper passes through `$GITHUB_ENV` env var to Python module |
...
```yaml
# test_platform_env.yaml (minimal — real platform-env.yaml has 13 networks, 17 volumes)
networks:
  - name: proxy-net
    driver: bridge
  - name: shared-db-net
...
**Note:** Real `platform-env.yaml` (189 LOC) contains 13 networks, 17 volumes, 22 env_defaults, 14 profiles. The sample YAML above is minimal for unit tests. Use exact-count assertions against real data in integration tests (already covered by `TestYamlParsing`), not in unit tests.
...
- Log format: `[IMP:9][provision]` (matching shell wrapper and existing tests — see §8.1)

---

## §6. Consumers (Make Targets + Direct Callers)
...
### 6.2 Direct Callers (Previously Unlisted)

| Consumer | File | Line(s) | Call | Change |
|----------|------|---------|------|--------|
| deploy-modules.sh | `core/internal/bootstrap/deploy-modules.sh` | 36-38 | `bash provision-environment.sh --scope networks \|\| true` + `--scope volumes \|\| true` | **NO CHANGE** — wrapper preserves `--scope` interface. `\|\| true` tolerance remains. |
| state_machine.py | `core/internal/bootstrap/lifecycle/state_machine.py` | 1164-1168 | `["bash", provision_script, "--scope", "networks", "--scope", "volumes"]` | **NO CHANGE** — wrapper preserves multi-`--scope` dispatch |

Both consumers call the provisioner with the same CLI interface (`--scope networks`, `--scope volumes`). The shell wrapper preserves this interface exactly — the only change is that the shell no longer contains business logic, delegating to `python3 provisioner.py` internally. Both consumers are **transparently compatible**.

### 6.3 entrypoint-manifest.yaml
...
**Post-migration update:** delegation chain becomes two-level. The manifest should reflect:
```yaml
- make_target: provision
  delegates_to: core/internal/provision-environment.sh → core/internal/provisioner.py
```

Additionally, `core/internal/provision-environment.sh` appears in the manifest's `allowed_scripts` section (line 456). No change needed there — the shell wrapper file still exists.
...
**Docker availability check:** `provision_networks()` checks `shutil.which("docker")` before any network operations. Returns exit code 2 (Docker unavailable) matching current shell behavior.
...
**Critical design decision — block name preserved:** All Python functions use the **existing** `[provision]` block name (NOT `[provisioner]`). This preserves backward compatibility with 20 existing test assertions that grep for `[IMP:9][provision]` across `test_unit_provision_environment.py` (17 occurrences) and `test_smoke_provision_environment.py` (3 occurrences).

```python
logger.info("[IMP:7][provision][networks] Reading networks from %s", yaml_path)
logger.info("[IMP:9][provision][networks] Networks provisioned: %d created, %d skipped", created, skipped)
logger.error("[IMP:10][provision][networks] FATAL: Docker is not available")
```

```python
logger.info("[IMP:7][provision][volumes] Creating volume directories from %s", yaml_path)
logger.info("[IMP:9][provision][volumes] Volumes provisioned: %d created, %d skipped", created, skipped)
```

Shell wrapper final log line (unchanged):
```bash
echo "[IMP:9][provision] Provision complete (scope=$SCOPE)"
```

> `## @rationale` The block name `provision` is the canonical identifier used by 5 consumers (helpers.mk, modules.mk, deploy-modules.sh, state_machine.py, entrypoint-manifest.yaml) and 30 test assertions. Changing to `provisioner` would require updating all consumers and tests — unnecessary blast radius for a cosmetic rename. The module file is named `provisioner.py` to distinguish it as a Python module, but the LDD block name stays `provision` matching the Make target and operational verb.
...
- **Output:** `core/internal/provisioner.py` (+~190 LOC, total ~290 LOC)
...
- **Output:** `core/internal/provision-environment.sh` (rewrite: ~50-55 LOC)
...
- **Output:** `tests/unit/test_provisioner.py` (~250-300 LOC)
- **Deliverables:** 18 test functions covering all scenarios from §5.3
- **Dependencies:** TASK-1, TASK-2
- **Complexity:** 5/10
...
- **Complexity:** 2/10
- **Acceptance:** Gate passes, zero new failures, existing tests unchanged

### TASK-6: Update entrypoint-manifest.yaml
- **Output:** `core/entrypoint-manifest.yaml` — updated `delegates_to` field for `provision`
- **Deliverables:** `delegates_to: core/internal/provision-environment.sh → core/internal/provisioner.py`
- **Dependencies:** TASK-3 (shell wrapper must exist)
- **Complexity:** 1/10
- **Acceptance:** `make check-manifests` passes, `grep "provision-environment.sh → provisioner.py" core/entrypoint-manifest.yaml` matches

---

## §10. Parallel Groups (CORRECTED)

### Wave 1 (independent, no shared files)
- Tasks: TASK-1
- Command: `coder Read DevPlan.md, implement Wave 1: TASK-1`

### Wave 2 (depends on Wave 1 — TASK-2 implementation)
- Tasks: TASK-2
- Command: `coder Read DevPlan.md, implement Wave 2: TASK-2`

### Wave 3 (depends on Wave 2 — shell wrapper + tests after functions exist)
- Tasks: TASK-3, TASK-4 (parallel — no shared files: TASK-3 edits `.sh`, TASK-4 writes `tests/unit/`)
- Command: `coder Read DevPlan.md, implement Wave 3: TASK-3 and TASK-4`

### Wave 4 (final validation + manifest)
- Tasks: TASK-5, TASK-6 (parallel — no shared files)
- Command: `coder Read DevPlan.md, implement Wave 4: TASK-5 and TASK-6`

> `## @rationale` (F3 fix) Original DevPlan grouped TASK-2 and TASK-4 as parallel in Wave 2, but TASK-4 tests `provision_networks()`, `provision_volumes()`, `provision_env()`, `provision_profiles()` — all implemented in TASK-2. Tests cannot be written against functions that don't exist. Corrected: TASK-2 executes first (Wave 2), then TASK-3 + TASK-4 in parallel (Wave 3 — no file collision: `.sh` vs `tests/unit/`). TASK-6 (manifest update) added as post-implementation step in Wave 4.

---

## §11. LOC Estimates (CORRECTED)

| Item | Original DevPlan | Corrected (F5) | Rationale |
|------|-----------------|----------------|-----------|
| Shell wrapper | ~45 LOC | **~50-55 LOC** | DevPlan's own sample (§2.2) is 48 lines of code + shebang + GREP_SUMMARY = ~55 LOC total |
| provisioner.py | ~250 LOC | **~340 LOC** | Dataclasses (~40) + 5 functions with full LDD logging (~250) + CLI with argparse (~50) |
| test_provisioner.py | ~180 LOC | **~250-300 LOC** | 18 tests with fixtures, monkeypatch, caplog assertions, tmp_path setup per test |
| Existing tests (retained) | Not mentioned | **628 LOC** (471 + 157) | Retained as-is — subprocess-level integration + E2E coverage |
...
| File | LOC | Type |
|------|-----|------|
| `core/internal/provisioner.py` | ~340 | Python module — business logic |
| `tests/unit/test_provisioner.py` | ~250-300 | Python unit tests |

### §12.2 Files Modified
| File | Change | LOC delta |
|------|--------|-----------|
| `core/internal/provision-environment.sh` | Full rewrite: 442 LOC → ~50-55 LOC | −390 |
| `core/entrypoint-manifest.yaml` | Update `delegates_to` field for `provision` | +0 (inline change) |

### §12.3 Files Unchanged (Verified Compatible)
| File | Why unchanged |
|------|--------------|
| `tests/test_unit_provision_environment.py` (471 LOC) | Shell wrapper preserves CLI interface — 28 subprocess tests continue to pass |
| `tests/test_smoke_provision_environment.py` (157 LOC) | Shell wrapper preserves CLI interface — 2 E2E tests continue to pass |
| `makefiles/helpers.mk` | Calls `bash provision-environment.sh --scope ...` — interface unchanged |
| `makefiles/modules.mk` | Calls `bash provision-environment.sh --scope networks --scope volumes` — interface unchanged |
| `core/internal/bootstrap/deploy-modules.sh` | Calls `bash provision-environment.sh --scope networks \|\| true` — interface unchanged |
| `core/internal/bootstrap/lifecycle/state_machine.py` | Calls `["bash", provision_script, "--scope", "networks", "--scope", "volumes"]` — interface unchanged |
...
### Wave 1
```
coder Read .ai/plans/073-provision-python/03-DevPlan-fix.md, implement Wave 1: TASK-1
```

### Wave 2
```
coder Read .ai/plans/073-provision-python/03-DevPlan-fix.md, implement Wave 2: TASK-2
```

### Wave 3
```
coder Read .ai/plans/073-provision-python/03-DevPlan-fix.md, implement Wave 3: TASK-3 and TASK-4
```

### Wave 4
```
coder Read .ai/plans/073-provision-python/03-DevPlan-fix.md, implement Wave 4: TASK-5 and TASK-6
```
