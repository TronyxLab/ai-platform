...
### Отклонения, зафиксированные при реализации (2026-08-01, волна G)

| Задача | Отклонение | Причина |
|--------|-----------|---------|
| 51 | `node_yaml.py` сохранил PEP 562 `__getattr__` lazy re-export для всех `_cli_*` + `main` | `tests/unit/test_node_yaml_cli_get_many.py` (U-52) импортирует `_cli_get_many` напрямую из `node_yaml`; без re-export падала collection → inventory-гейт RED. Re-export ленивый (AC-G5), символы загружаются только по обращению. |
| 52 | Классы `CircuitBreakerService`/`CircuitEvent`/`CircuitBreaker`/`DockerManager` переименованы в `circuit_breaker.py`/`docker_ops.py` с сохранением исходных приватных имён | Гейт приватных межмодульных импортов (T6.1, allowlist пуст) запрещает `from X import _private`. Имена остались приватными внутри новых модулей; agent_watchdog импортирует их через `from circuit_breaker import CircuitBreaker` (публичный) и lazy-импорт в методах. |
| 52 | `tests/test_cross_layer_imports.py` allowlist обновлён: line-номера agent_watchdog сдвинуты (+docker_compose запись удалена, т.к. импорт ушёл в docker_ops.py) + добавлены circuit_breaker.py:31 и docker_ops.py:24 | Импорт `shared.docker_compose` переехал из agent_watchdog.py в docker_ops.py; allowlist привязан к (path, lineno) — без обновления гейт падал. Новые файлы — легитимные D1-паттерны (контейнерный модуль → shared/internal). |
| 55 | Функции collectors/renderer переименованы в публичные: `load_status_metrics`, `compute_staleness`, `enrich_projects`, `enrich_containers`, `compute_uptime_human`, `format_bytes`; app.py импортирует их через `from X import public as _private` | Тот же гейт T6.1: app.py импортировал приватные `_load_status_metrics` и т.д. из соседних модулей. Публичные имена в home-модулях + приватные алиасы в app.py сохраняют прежний `app._*` API (tests/test_status_page.py не изменён). |
| 57 | `_get_env_val_required` переименован в `_get_val_required` | Гейт `test_gate_env_example_drift.py::test_platform_domain_default` ищет литерал `get_val_required` в sync_env_defaults.py (fail-fast маркер DevPlan 116 T3). Имя `_get_val_required` содержит искомый substring и остаётся приватным. |
| 54 | `_render_template` → `render_template_file`, `_parse_retention_hours` → `parse_retention_hours` в monitoring_config_renderer.py; генераторы импортируют через `from X import public as _private`; legacy private alias `_parse_retention_hours` сохранён для white-box теста | Тот же гейт T6.1 — генераторы импортировали приватные хелперы из monitoring_config_renderer. Публичные имена в home-модуле + приватные алиасы в генераторах; `_parse_retention_hours` alias сохраняет `test_monitoring_config_renderer.py::test_parse_retention_hours` без изменений. |
