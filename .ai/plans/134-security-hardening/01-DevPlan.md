...
W2 ── security_posture.py (unit-тесты S1-S7) ──► build_check_security_ssh_cmd + execute_remote_check_security
     ──► remote_executor execute-check-security ──► entrypoint check-security.sh ──► make check-security NODE=<n>
     │ цикл: S1-S7 → агрегация {0 healthy, 1 warnings (S2 pending>0), 2 errors} → текст-отчёт + --json
...
1. `security_posture.py` (CLI `--json`, exit 0/1/2):
   - 7 проверок S1-S7 (см. D4), каждая — pure-функция `check_*(ctx) -> CheckResult(status, message)`; агрегация: любой FAIL → 2, иначе любой WARN → 1, иначе 0
...
1. `make check-security NODE=<test>` на подготовленной ноде: exit 0, все S1-S7 PASS (после того как unattended-upgrades установлен волной W1)
...
- L4: cadence пересборки L2-образов (weekly CI) — подхват security-фиксов базовых образов (content-hash skip не триггерит пересборку)
