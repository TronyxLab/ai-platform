#!/usr/bin/env python3
# GREP_SUMMARY: ci-ops cycle-2 poller gh runs r2 state tsv monitor
# STRUCTURE: load state json → gh api runs → diff transitions → render ci-runs-r2.tsv → ⎋ print NEW/CHANGED
# Usage: python3 ci_poller_r2.py [--limit N]  (state: ci-logs-r2/.ci-state-r2.json, tsv: ci-runs-r2.tsv, same dir)
"""ci-ops poller for session 141 cycle 2 — tracks GitHub Actions runs, renders ci-runs-r2.tsv.

Own files (untracked session tooling, not platform code):
  ci-logs-r2/.ci-state-r2.json — run_id -> last recorded status (persistence between polls)
  ci-runs-r2.tsv               — one row per run: run_id, workflow, status, started, duration_s, cause
"""
...
LOGS2 = HERE / "ci-logs-r2"
STATE = LOGS2 / ".ci-state-r2.json"
TSV = HERE / "ci-runs-r2.tsv"
...
def main(limit: int) -> int:
    ...
    print(f"[IMP:1][ci_poller_r2] {now} snapshot: {len(runs)} runs, {len(state)} tracked")
    if not changed:
        print("[IMP:7][ci_poller_r2] no transitions since last poll")
    for c in changed:
        print(f"[IMP:9][ci_poller_r2] {' '.join(str(x) for x in c)}")
    return 0
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"[IMP:10][ci_poller_r2] BLOCKED gh api rc={e.returncode}: {e.stderr.strip()}", file=sys.stderr)
