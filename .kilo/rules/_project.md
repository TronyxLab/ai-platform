...
3. `make gate MODE=fast` исполняется ТОЛЬКО pre-push hook'ом (blocking) при `git push` — ручной прогон не требуется (это дубль, ~350 s впустую). Исключение: `--no-verify` или отсутствие hooks (новый клон без `make pre-commit-install`) — тогда защита остаётся только на CI (`platform-gate-fast.yml` на push в main)
4. Диагностические ветки — от origin/main: `git checkout -b <branch> origin/main`, не от локального main
5. После merge/конфликтов: `make fix-gate && git add -u && make check`

## Верификация реализации (Code-агент)

**Цикл (DevPlan 120):** per-task `make test-summary TEST_FILE=...` / `make check-diff` → фикс-цикл `make check` → финальная верификация `make check` до чистоты. `make gate MODE=fast` в dev-цикле НЕ запускается — он исполняется ТОЛЬКО pre-push hook'ом при push (арбитр, без кэша, fail-fast; содержательно = суперсет check). Все проверки читаются из единого SoT `core/check-suite.yaml` — ручные check-manifests/ruff из инструкции удалены (дыры закрыты манифестом).

**Батчинг вместо серийных циклов:** `make gate MODE=fast` падает на первой ошибке (fail-fast) и дублирует pre-push hook — поэтому в dev-цикле он не гоняется вовсе. Работай батчами: `make check` собирает ВСЕ ошибки за один проход.
...
4. Финальная верификация — `make check` до чистоты; pre-push hook прогонит `make gate MODE=fast` автоматически (без кэша). Ручной `make gate MODE=fast` НЕ нужен — это точный дубль hook'а (~350 s впустую); вручную — только при `--no-verify` или отсутствии hooks
...
6. В промтах Code-субагентам последовательность писать шагами: «`make check` (до чистоты)», не одной цепочкой через `&&`; `make gate MODE=fast` в промптах НЕ упоминать — арбитр pre-push hook
