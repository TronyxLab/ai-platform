...
**Anti-Loop Protocol**
    ...
    **Batched verification (anti-serial):**
    - Prefer the project's batched verification command — a single invocation that collects ALL failures in one pass (e.g., `make check` — единственная тестовая команда ai-platform, DevPlan 165; `npm test` — для Node-проектов).
