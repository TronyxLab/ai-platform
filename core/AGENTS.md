...
| `make core-deliver` | Fallback-деплой core (локальное зеркало core-deploy.yml) | make core-deliver NODE=\<n\> [AGE_SECRET_KEY_FILE=\<f\>] [DRY_RUN=1] | core/entrypoints/core-deliver.sh → rsync core/+scripts/+makefiles/+platform-env.yaml → ssh make provision SCOPE=networks,volumes → ssh make node-update NODE=\<n\> |
...
| `docker_ops` | `core/internal/shared/docker_ops.py` | Единый слой docker-операций (DevPlan 128 W1, P2-5/D6) — docker ps/inspect/exec/stop/rm/tag/image/network/volume/stats/info/manifest/pull + CLI `--shell` для shell-фасадов (lib/docker.sh, паттерн ssh_opts 116 B5 D1). Дедуплицирует 17+ вызовов (deploy_engine, docker_orchestrator, observability, orphan_reconciler, converge/*, modules_healthcheck, docker_collector, deploy/orchestrator, provisioner, reconciler_projects, preflight, hermes_workflow, phases/docker, docker_registry_auth, state_store, security_posture, docker_compose примитивы); гейт docker_sole_path (allowlist пуст). |
...
| `compose_profiles` | `core/internal/shared/compose_profiles.py` | Единый loader COMPOSE_PROFILES — load_profiles() из platform-infra.yaml env_defaults (SoT). Заменяет чтение platform-env.yaml (scaffold_helpers) + platform_config (docker_orchestrator), DevPlan 118 C3. |
| `llm_paths` | `core/internal/shared/llm_paths.py` | Единый источник пути litellm-config.yml — litellm_config_path(core_dir)/litellm_template_path(core_dir). Заменяет 4 копии вывода + 1 шаблон (context_deployer, deploy_orchestrator, llm_provision, phases, config_renderer), DevPlan 118 C6. |
| `module_interface` | `core/internal/shared/module_interface.py` | Единая bash-обёртка invoke_module_interface — invoke(module, interface, *args, timeout) → (bool, output). Заменяет docker_orchestrator._invoke_healthcheck_full + deploy_orchestrator._invoke_module_interface; вход для B8 wire module-hooks (DevPlan 118 C5). |
