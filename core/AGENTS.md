...
| `make validate-node-input` | Локальная проверка входного контракта ноды (AGE/sops/required) | make validate-node-input NODE=\<name\> [NODE_CONFIGS_DIR=\<dir\>] | core/entrypoints/validate-node-input.sh → python3 -m core.internal.bootstrap.preflight --scope input (AGE-форма single-line, env-vs-file приоритет, SOPS enc-file наличие, required-ключи; 0 remote — входной контракт ДО любого SSH, DevPlan 029 T7) |
...
| `make parity-db` | Create/drop temporary parity database via privileged path (Plan 019 TASK-6, AC5) | make parity-db ACTION=\<create|drop\> PROJECT=\<name\> NODE=\<node\> | core/entrypoints/parity-db.sh → python3 -m core.internal.deploy.parity_db (ssh root@\<node\> → docker exec postgres psql -U postgres; привилегированный путь, DevPlan 019 TASK-6) |
...
| `make ai-instructions-sync` | Пересборка инструкций (канон + проектные дополнения) | make ai-instructions-sync [PROJECT=\<dir\>] [TEMPLATE=\<all|backend|frontend|ai-project\>] [CANON_PATH=\<dir\>] | python3 -m ai_instructions sync --config core/internal/ai-instructions/ai-instructions-pins.yaml (canon + .ai/ -\> .kilo/ + hermes platform-профиль + ai-instructions.lock; DevPlan 001 R16/T4.2) |
