...
# Типы: feat|fix|docs|style|refactor|test|chore|perf|ci|build|revert (не комментарий # type: —
#   vulture парсит "# type:" как аннотацию и падает, DevPlan 163 W-G)
...
def main(argv: list[str] | None = None) -> None:
    ...
    ## @changes 2026-08-14 | DevPlan 167 D1 — +argv DI-параметр (AF-4): None = sys.argv (backward compat)
    """
    # 🧐 TRAP[DI-SEAM] · 2026-08-14 · — · argv-параметр (DevPlan 167 D1)
    # · Rejected: прямое чтение sys.argv · Reason: seam = тестируемость реального вызова
    # ·   (unit-тесты вызывают main(argv=[...]) вместо monkeypatch sys.argv) · Rev: при
    # ·   изменении CLI-контракта хука
    # Канон AF-4 (163 W-H): явный argv НЕ содержит имя программы (как sys.argv[1:])
