...
# ⚠️ AGE_SECRET_KEY (env сессии) перекрывает локальную цепочку node_detect/--age-secret-key-file —
# корень 2 неудачных core-deliver (сессии 141, 156). Warning informational, не блокирует (DevPlan 157 W2 T2).
if [[ -n "${AGE_SECRET_KEY:-}" ]]; then
    echo "[core-deliver][warn] AGE_SECRET_KEY (env сессии) ПЕРЕКРЫВАЕТ локальную цепочку node_detect/--age-secret-key-file — node-update получит env-ключ. Для файла: unset AGE_SECRET_KEY" >&2
fi
