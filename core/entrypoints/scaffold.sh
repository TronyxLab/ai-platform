...
## @changes  2026-08-15 · DevPlan 170 W9-F2 — sync-env: `case " $* "` (хрупкий, ломался на
##           --project-dir=value/пробелах) → флаговый цикл по "$@" (интерфейс subcommands сохранён)
...
case "$CMD" in
    ...
    project-sync-env|sync-env)
        ...
        # 170 W9-F2: хрупкий `case " $* "` (ломается при пробелах/спецсимволах в значениях,
        # не различает --project-dir=value) заменён флаговым циклом по "$@" — интерфейс тот же.
        _has_project_dir=false
        for _arg in "$@"; do
            case "$_arg" in
                --project-dir|--project-dir=*) _has_project_dir=true; break ;;
            esac
        done
        unset _arg
        if $_has_project_dir; then
