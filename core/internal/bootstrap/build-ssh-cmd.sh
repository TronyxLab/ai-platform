...
# GREP_SUMMARY: build-ssh-cmd build_ssh_cmd build_update_ssh_cmd build_converge_ssh_cmd ssh-command quoting printf PLATFORM_ROOT export ci_deploy_key D3 remote-cmd
# STRUCTURE: ▶ ┌node_name+keys┐ → ○ export AGE/PLATFORM_ROOT/CI_DEPLOY_KEY/DOMAIN/CONTEXT (printf %q) → ○ bash node-lifecycle --mode {init,update} | bash converge.sh → ○ passthrough args → ⎋ echo remote cmd
...
##           PLATFORM_CI_DEPLOY_KEY, PLATFORM_DOMAIN, CONTEXT, AGE_SECRET_KEY).
