...
# AUDIT_LOG_FILE — ЕДИНСТВЕННЫЙ источник shared/audit_logger.DEFAULT_LOG_FILE
...
from core.internal.shared.audit_logger import (
    DEFAULT_LOG_FILE as AUDIT_LOG_FILE,  # ruff: ignore[F401] — публичный re-export канона (C8)
)

# Канонические константы путей/таймаутов — shared/deploy_paths + shared/timeouts
# (литералы удалены; re-export для доменных модулей converge/*, которые импортируют их из infra —
# публичные алиасы канона).
from core.internal.shared.deploy_paths import (
    DEFAULT_PROJECTS_BASE as PROJECTS_BASE,  # ruff: ignore[F401] — публичный re-export канона (B2)
)
from core.internal.shared.deploy_paths import converge_cooldown_file

# Единый канон run_subprocess — shared/subprocess_io (делегирование, check=False).
...
from core.internal.shared.timeouts import (
    FILE_OP_TIMEOUT,  # ruff: ignore[F401] — публичный re-export канона (B7)
)
...
# 170 W1-A2: канон deploy_paths (литерал /var/lib/platform/.converge_cooldown.json удалён)
COOLDOWN_FILE: str = str(converge_cooldown_file())
...
SEVERITY_ERROR: int = 2  # severity-порог: >= 2 → ошибка (exit 2)
...
def try_chmod(path: str, unit: str) -> bool:
    ...
    try:
        pathlib.Path(path).chmod(pathlib.Path(path).stat().st_mode | 0o110)  # ug+x
