...
def reconcile_projects(
    ...
    for proj in projects:
        ...
        if not dry_run and not report_only and proj_dir.is_dir():
            run_subprocess(["chown", "ci-deploy:ci-deploy", str(proj_dir)], timeout=FILE_OP_TIMEOUT)
        ...
        if not stub_file.is_file():
            if dry_run or report_only:
                ...
                try:
                    ...
                    run_subprocess(["chown", "ci-deploy:ci-deploy", str(stub_file)], timeout=FILE_OP_TIMEOUT)
            ...
            if is_stub_ai_platform_yaml(str(stub_file)):
...
def parse_projects_yaml(node_yaml_path: str) -> list[dict]:
    """Parse projects from node.yaml.
...
def create_empty_env_file(env_file: Path, unit: str) -> bool:
    ...
    try:
        env_file.write_text("")
        run_subprocess(["chmod", "0640", str(env_file)], timeout=FILE_OP_TIMEOUT)
        run_subprocess(["chown", "ci-deploy:ci-deploy", str(env_file)], timeout=FILE_OP_TIMEOUT)
...
def reconcile_env_platform(
    ...
    try:
        ...
        create_empty_env_file(env_file, unit)
