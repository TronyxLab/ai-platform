...
from core.internal.bootstrap.converge.infra import SUDOERS_DIR, report_add, run_subprocess, set_exit
...
# R8-канон таймаута — прямой импорт из shared SoT (pyright reportPrivateLocalImportUsage)
...
## @param replace_fn   Callable | None (DI, 167 D6 — atomic rename impl; None = os.replace)
## @return  True if write + validation succeeded, False otherwise
def atomic_write_sudoers(
    target_path: Path,
    content: str,
    module_name: str,
    ...
    try:
        ...
        # · Rejected: прямой Path.replace с monkeypatch.setattr(os, "replace", ...) в тестах R8
        # · Reason: seam = тестируемость реального вызова — replace_fn инжектируется тестом
        # ·   (трекер os.replace / сбой), default = os.replace (поведение неизменно)
        # · Rev: при миграции на shared/atomic_writer (validator=visudo) — убрать локальный шов
        commit = os.replace if replace_fn is None else replace_fn
        commit(tmp_path, str(target_path))
        Path(str(target_path)).chmod(0o440)
...
## @param sudoers_dir     Override sudoers.d directory (DI, DevPlan 163 W-H); None → SUDOERS_DIR
## @param replace_fn      Callable | None (DI, 167 D6 — atomic rename impl для self-heal;
##                        None = os.replace)
...
def reconcile_sudoers(
    ...
    for mod in modules:
        ...
        # Read actual content
        resolved_sudoers_dir = SUDOERS_DIR if sudoers_dir is None else sudoers_dir
        sudoers_file = Path(resolved_sudoers_dir) / f"platform-{mod_name}"
        actual_content = ""
        try:
        ...
        success = atomic_write_sudoers(sudoers_file, desired_content, mod_name, replace_fn=replace_fn)
