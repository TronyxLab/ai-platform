...
## @scope    Вызывается напрямую из `make up` (makefiles/modules.mk, preflight-шаг,
##           План 175 W4.2 — compose-wrapper.sh/up-safe удалены) перед `docker compose up`.
...
##            The wrapper is opt-in via `make up` preflight-шаг (SKIP_PREFLIGHT=1 — обход).
...
##          2026-08-16 | План 175 W4.2 — preflight встроен в `make up` (up-safe удалён)
## @usecases
##   - `make up MODULES=postgres,litellm` → preflight check → docker compose up
##   - `make up SKIP_PREFLIGHT=1` → preflight bypassed (осознанный обход)
...
## @io       profiles (set[str]), env: Mapping[str, str] | None = None (DI, W-H DevPlan 163 —
##           override COMPOSE_PROFILES; None = os.environ) → set[str] of module names
...
def resolve_modules(profiles: set[str], env: Mapping[str, str] | None = None) -> set[str]:
    ...
    env_profiles = source.get("COMPOSE_PROFILES", "")
    if env_profiles:
...
def load_manifest(manifest_path: str) -> list[dict[str, object]] | None:
    ...
    with p.open(encoding="utf-8") as f:
        # W11: yaml.safe_load → Any — каст к dict-контракту манифеста
    ...
    # W11: элемент манифеста — dict[str, object] (поля строковые/списковые)
    secrets_list = cast(list[dict[str, object]], data.get("secrets", []))
...
##           Combines env-source (os.environ или DI-дикт) with env_file_map (precedence: env wins).
## @io       target_modules (set[str]), secrets_list (list[dict]), env_file_map (dict[str,str]),
##           env: Mapping[str, str] | None = None (DI, W-H DevPlan 163 — override os.environ;
##           None = os.environ) → list[str] of missing variable names
...
##   - env-source (os.environ/DI) takes precedence over env_file_map
def check_secrets(
    target_modules: set[str],
    secrets_list: list[dict[str, object]],
    ...
    for s in secrets_list:
        ...
        tier = cast(str, s.get("tier", ""))
        consumers = cast(list[object], s.get("consumers", []))
        ...
        # Check availability: env-source first, then env_file_map
        env_val = source.get(name, "")
        if not env_val:
...
## @io       secrets_list (list[dict]), env_file_map (dict[str,str]),
##           env: Mapping[str, str] | None = None (DI, W-H DevPlan 163 — override os.environ;
##           None = os.environ) → list[str] of errors
...
def validate_charsets(
    ...
    for s in secrets_list:
        charset = cast(str, s.get("charset", ""))
        if not charset:
            continue
...
def main(test_args: list[str] | None = None) -> int:
    ...
    args = cast(_CliArgs, cast(object, parser.parse_args(test_args)))
