...
# exit_code_from_results: deployed — контракт-параметр (сигнатура pure-API, DEPLOY_BEST_EFFORT);
# не используется в логике (WARN→0 независимо); ruff: ignore[ARG001] — тот же контракт
...
_HC_DONE_MARKER = str(
    Path(os.environ.get("PLATFORM_STATE_DIR", str(deploy_paths.bootstrap_state_dir())), ".hc_done_in_deploy")
)
# 142 W2 (B21): persistent /var/lib/platform/run (tmpfs /run/platform не переживает reboot).
# Pure-модуль (инвариант: 0 I/O) — deploy_paths — pure data-резолвер (без I/O и сайд-эффектов).
_STATUS_METRICS_PATH = str(deploy_paths.status_metrics_json())
_STATUS_METRICS_TEMPLATE: dict[str, object] = {
    "schema_version": 2,
    "generated_at": None,
    "containers": [],
    "certs": [],
...
## @io       ⇥ context: str | None = None (CONTEXT env var) → ⎋ str
## @complexity O(1)
## @invariants
...
##   - CONTEXT пуст → /var/lib/platform/.bootstrap/.hc_done_in_deploy (1 нода = 1 контекст:
##     писатель и читатель в одном env-процессе резолвят одинаково — консистентность
##     важнее суффикса; update-mode без --context-флага покрыт этим путём)
def hc_marker_path(context: str | None = None) -> str:
    """Return the healthcheck-done marker path (per-context; 1:1 fallback без суффикса)."""
    if context:
