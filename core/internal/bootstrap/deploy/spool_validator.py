...
# 170 W1-A2: spool/state-пути — shared/deploy_paths резолверы (литералы /var/lib/platform удалены)
...
# region TYPEDEF_SpoolResult
class _DirEntry(TypedDict):
    """Одна запись отчёта: проверяемая директория и контекст (W11: типизация JSON-отчёта)."""
...
class _SpoolResult(TypedDict):
    """Структура JSON-отчёта verify_spool_dirs (W11: замена dict[str, Any])."""

    status: str
    missing: list[_DirEntry]
    stateless: list[str]
    ok: list[_DirEntry]
    checked: int


# endregion TYPEDEF_SpoolResult
...
FALLBACK_DIRS = [
    ...
    str(backup_spool_dir()),
    str(backup_spool_postgres_dir()),
    str(backup_spool_appdata_dir()),
]
...
def verify_spool_dirs(modules_dir: str) -> _SpoolResult:
    ...
    for mod_path in sorted(mods.iterdir()):
        ...
        with Path(yaml_path).open(encoding="utf-8") as f:
            # W11: yaml.safe_load → Any — каст к dict-контракту module.yaml
            cfg = cast(dict[str, object], yaml.safe_load(f) or {})
...
def _verify_dirs(dirs: list[str], context_label: str, result: _SpoolResult) -> None:
...
def _verify_single_dir(dirpath: str, context_label: str, result: _SpoolResult) -> None:
