...
##            Атомарная запись (tmp + os.replace) предотвращает повреждение daemon.json при прерывании.
...
def merge_live_restore(daemon_json: str) -> bool:
    ...
    """
    try:
        with pathlib.Path(daemon_json).open(encoding="utf-8") as f:
            config = cast("object", json.load(f))  # W11-G3: json.load → Any; JSON-граница (isinstance-гард ниже)
    ...
    try:
        _atomic_write_json(
            daemon_json, cast("dict[str, object]", config)
        )  # W11-G3: isinstance-сужение → dict[Unknown, Unknown]; каст к контракту atomic_writer
    except OSError as e:
