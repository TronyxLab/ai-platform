#!/usr/bin/env python3
# GREP_SUMMARY: firewall ufw declarative idempotent 22 80 443 5432-deny extra_ports deny-incoming allow-outgoing port-validation docker-user DOCKER-USER iptables bridge defense-in-depth zabbix-monitoring timeweb 10050
...
##           zabbix_monitoring: bool = True → ⎋ list[list[str]] — команды для subprocess
## @complexity O(B + P + M + Z) — B = baseline, P = extra, M = module-deny, Z = zabbix IPs
...
## @io       ⇥ status_text: str, tor_enabled: bool = False, zabbix_monitoring: bool = True → ⎋ bool
## @complexity O(1) — parse + проверки
def verify_firewall(status_text: str, tor_enabled: bool = False, zabbix_monitoring: bool = True) -> bool:
...
##           для тестируемости чистых функций). 162 W2-4: zabbix_monitoring=True по умолчанию.
## @io       ⇥ extra_ports: list[str], source_ip: str|None, tor_enabled: bool|None,
##           zabbix_monitoring: bool = True → ⎋ bool
## @complexity O(R + L)
def run(
    ...
    try:
        ...
        rules = build_rules(ports, source_ip, tor_enabled=tor_enabled, zabbix_monitoring=zabbix_monitoring)
    ...
    try:
        ...
        status_text = ""
    return verify_firewall(status_text, tor_enabled=tor_enabled, zabbix_monitoring=zabbix_monitoring)
