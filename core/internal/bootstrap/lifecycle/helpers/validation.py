...
## @complexity O(1)
_SUDOERS_MODE_MAX: int = 0o440  # максимальная допустимая маска sudoers-файла
...
def verify_core_files(core_dir: str) -> None:
    ...
    if not os.path.isfile(marker):
        msg = f"Core bootstrap not found at {marker}. Deploy first:\n  rsync -avz core/ root@<server>:{core_dir}/"
        raise ConfigNotFoundError(msg)
    ver_file = os.path.join(core_dir, "VERSION")
    if os.path.isfile(ver_file):
        with Path(ver_file).open(encoding="utf-8") as f:
...
def validate_node_yaml(node_yaml: str, core_dir: str) -> None:
    ...
    if not node_yaml or not os.path.isfile(node_yaml):
        msg = f"node.yaml not found: {node_yaml}"
        raise ConfigNotFoundError(msg)
    ...
    # ruff: ignore[PLW0717] — except использует имена, импортированные в try-теле (json/jsonschema) — извлечение лома...
    try:
        import jsonschema
        ...
        with Path(schema_file).open(encoding="utf-8") as f:
            schema = cast("object", json.load(f))  # W11-G3: json.load → Any; JSON-граница схемы
        instance = NodeYaml(node_yaml).raw()
        jsonschema.validate(instance, schema)  # pyright: ignore[reportArgumentType] — W11-G3: jsonschema.Schema = bool | Mapping[str, Any] (тип внешней библиотеки с Any в сигнатуре)
...
""",
    ...
    except (json.JSONDecodeError, jsonschema.ValidationError) as e:  # pyright: ignore[reportPossiblyUnboundVariable] — jsonschema optional (subprocess fallback); ветка достижима только после успешного `import jsonschema`
