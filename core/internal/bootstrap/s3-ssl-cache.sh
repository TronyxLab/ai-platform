...
##   - upload saves: fullchain.pem, privkey.pem, [optional] chain.pem, account.tar.gz
...
##   - chain.pem is NOT required — acme.sh --install-cert outputs only fullchain.pem + privkey.pem
##   - download validates issuer (Let's Encrypt) to reject mkcert/self-signed certs
##   - Account path uses <domain>_ecc/ (acme.sh default), fallback to data/<domain>/ (legacy)
...
##              - G2 fix: chain.pem removed from required_files, best-effort upload
##              - G3 fix: account path uses <domain>_ecc/ (acme.sh default), fallback data/<domain>/
##              - G3 fix: download tar extract to ACME_HOME/ (not ACME_HOME/data)
##              - LE issuer validation: reject non-LetsEncrypt certs in download
...
_s3_upload() {
    ...
    # ⚠️ TRAP[BUG] · 2026-07-23 · G2 · chain.pem not required — acme.sh --install-cert outputs only fullchain.pem + privkey.pem
    # · Observed: _s3_upload() always FAILed because chain.pem is not generated by acme.sh --install-cert
    # · Root: --install-cert outputs --fullchain-file + --key-file, no separate --chain-file
    # · Fix: Remove chain.pem from required_files; fullchain.pem already includes the chain (concatenated)
    # · When: StatusReport 057 — S3 cache never succeeded after cert issuance
    ...
    # ⚠️ TRAP[BUG] · 2026-07-23 · G2 · chain.pem optional — acme.sh --install-cert doesn't generate chain.pem
    # · Upload chain.pem if it exists on disk (best-effort). Not required — fullchain.pem contains the chain.
    if [[ -f "${cert_dir}/chain.pem" ]]; then
        log_imp 8 "-" "Uploading chain.pem (best-effort)"
        python3 "$UPLOAD_PY" \
            --config-source ssl-cache \
            "${cert_dir}/chain.pem" \
            "${S3_SSL_CERT_PREFIX}/${domain}/chain.pem" 2>&1 || \
            log_step "upload" "WARN" "Failed to upload chain.pem for ${domain} — S3 may be unavailable"
    else
        log_imp 8 "-" "chain.pem not found (expected for acme.sh certs) — skipping optional upload"
    fi

    # Upload acme.sh account data for domain persistence across restores
    local account_tar="/tmp/acme-account-${domain}.tar.gz"
    # ⚠️ TRAP[BUG] · 2026-07-23 · G3 · acme.sh account data path uses <domain>_ecc/, not data/<domain>/
    # · Observed: account data never uploaded because ${ACME_HOME}/data/${domain} doesn't exist
    # · Root: acme.sh stores account data in ${ACME_HOME}/${domain}_ecc/ directory structure
    # · Fix: try ${domain}_ecc first (acme.sh default), fall back to data/${domain} (legacy)
    # · When: StatusReport 057 — account.tar.gz never uploaded, cert restore always fresh issue
    local acme_domain_dir=""
    if [[ -d "${ACME_HOME}/${domain}_ecc" ]]; then
        acme_domain_dir="${ACME_HOME}/${domain}_ecc"
        log_imp 8 "-" "Using acme.sh account path: ${domain}_ecc"
    elif [[ -d "${ACME_HOME}/data/${domain}" ]]; then
        acme_domain_dir="${ACME_HOME}/data/${domain}"
        log_imp 8 "-" "Using legacy acme.sh account path: data/${domain}"
    fi
    if [[ -n "$acme_domain_dir" && -d "$acme_domain_dir" ]]; then
...
_s3_download() {
    ...
    # ⚠️ TRAP[BUG] · 2026-07-23 · G4 · Validate issuer to reject mkcert/self-signed certs from S3 cache
    # · Observed: S3 cache could restore non-LE (mkcert/dev) certificates, causing nginx to reject them
    # · Root: no issuer validation — any valid x509 cert in S3 was accepted
    # · Fix: check that issuer contains "Let's Encrypt", reject (return 1) if not
    local cert_issuer
    cert_issuer="$(openssl x509 -in "$tmp_fullchain" -issuer -noout 2>/dev/null)"
    if ! echo "$cert_issuer" | grep -qi "Let's Encrypt"; then
        log_step "download" "WARN" "Downloaded cert issuer '${cert_issuer}' is not Let's Encrypt — rejecting non-LE cert"
        rm -f "$tmp_fullchain"
        return 1
    fi
    log_imp 8 "-" "Issuer validated: ${cert_issuer}"
    ...
    # · Observed: account data extracted to wrong directory, never restored correctly
    # · Root: tar contains <domain>_ecc/ directory, but code extracted to ${ACME_HOME}/data/
    # · Fix: extract to ${ACME_HOME}/ (tar already has ${domain}_ecc/ inside it)
    ...
    if [[ $? -eq 0 ]]; then
        ...
        mkdir -p "${ACME_HOME}"
        tar xzf "$account_tar" -C "${ACME_HOME}" 2>/dev/null && \
