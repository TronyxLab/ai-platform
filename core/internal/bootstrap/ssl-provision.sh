#!/usr/bin/env bash
# GREP_SUMMARY: ssl-provision, acme.sh, letsencrypt, tls, dns-01, webnames, dnsapi, wildcard-cert, idempotent, cron, cert-expiry, project-certs
# STRUCTURE: ▶ ┌NODE_YAML env┐ → python3 parse → ○ cert exists? → SKIP exit 0 → ◇ validate env → install_acme → issue_tls_cert → _acme_install_cron → _acme_verify_cert → ◇ _issue_project_certs → ⎋ exit 0|1
# region MODULE_CONTRACT
## @purpose  Standalone, idempotent SSL/TLS certificate provisioning via acme.sh DNS-01.
##           Extracted from nginx/install.sh — operates independently of nginx install type.
## @scope    Called from node-lifecycle.sh update step 3 (ssl-provision), BEFORE docker compose up.
##           Does NOT deploy nginx config — that is the docker compose step's responsibility.
##           All functions from the original install.sh are preserved with original contracts.
## @invariants
##   - Idempotent: if /etc/letsencrypt/live/$domain/fullchain.pem exists → SKIP, exit 0
##   - DNS-01 only — HTTP-01 NOT supported (cannot issue wildcard certs via HTTP)
##   - Requires PLATFORM_ACME_DNS_PLUGIN — no auto-detection of DNS provider
##   - webnames plugin requires WEBNAMES_API_KEY env var; key is shredded from disk after use
##   - acme.sh cron installed AFTER cert issuance (nothing to renew without cert)
##   - Cert expiry verified via openssl x509 — read-only check, no re-issue
##   - LETSENCRYPT_DIR env override supported (for testing: sets alternate cert root)
## @rationale nginx was converted to docker (install_type: docker), but acme.sh logic
##   remained in install.sh which is only called for system-type modules. Extracting SSL
##   provisioning as a standalone pre-deploy step ensures certs exist before docker nginx
##   starts — breaking the chicken-and-egg restart loop that caused the production incident.
##   See DevPlan 002 §Root Cause Analysis for details.
## @changes   CREATED: 2026-07-17 · T1' — Extract from nginx/install.sh (DevPlan 002)
...
__LOG_PREFIX="ssl-provision"
source "${SCRIPT_DIR}/../../lib/logging.sh"

# ═══════════════════════════════════════════════════════════════════
# NOTE: All functions below are extracted from core/modules/nginx/install.sh.
# Original TRAP[BUG], TRAP[BUSINESS], TRAP[DECISION] comments are preserved.
# See nginx/install.sh DEPRECATED header for full migration rationale.
# ═══════════════════════════════════════════════════════════════════
...
_acme_install_cron() {
    ...
    # [IMP:9][ssl-provision][acme-cron] BUSINESS INVARIANT: idempotency — skip if already installed
    ...
    # [IMP:9][ssl-provision][acme-cron] Verify cronjob was actually installed
    if crontab -l 2>/dev/null | grep -q "${acme_sh}.*--cron"; then
...
_acme_verify_cert() {
    ...
    # [IMP:9][ssl-provision][acme-verify] BUSINESS INVARIANT: cert must be valid >30 days
    local expiry_date
...
issue_tls_cert() {
    ...
    # [IMP:9][ssl-provision][acme.sh] Idempotency: do NOT re-issue existing valid certificate
    local cert_path="/etc/letsencrypt/live/${domain}/fullchain.pem"
    if [[ -f "$cert_path" ]]; then
    ...
    # [IMP:9][ssl-provision][acme.sh] BUSINESS INVARIANT: DNS plugin required for wildcard cert
    if [[ -z "$dns_plugin" ]]; then
        log_step "tls" "FAIL" "TLS certificate requires DNS plugin (set PLATFORM_ACME_DNS_PLUGIN in env)"
        return 1
    fi

    # [IMP:9][ssl-provision][acme.sh] BUSINESS INVARIANT: WEBNAMES_API_KEY required for webnames
...
PYEOF
    ...
    if ! install_acme; then
        log_fail "acme.sh installation failed — cannot issue TLS certificate"
        exit 1
    fi

    # ── Step 2: Issue wildcard TLS certificate ─────────────────────
    if ! issue_tls_cert "$domain" "$email" "$dns_plugin" "true"; then
        log_fail "TLS certificate issuance failed for ${domain}"
        exit 1
    fi

    # ── Step 3: Install acme.sh cron for daily renewal ─────────────
    # [IMP:9][ssl-provision][main] BUSINESS INVARIANT: cron must be installed when TLS cert exists
    if [[ -f "$cert_path" ]]; then
        _acme_install_cron || log_warn "acme.sh cron install failed — cert still valid, renew manually"
    fi

    # ── Step 4: Verify certificate expiry >30 days ─────────────────
    # [IMP:9][ssl-provision][main] BUSINESS INVARIANT: cert must be valid >30 days
    _acme_verify_cert "$domain" || log_warn "Certificate expires within 30 days — renew soon"

    # ── Step 5 (Optional): Issue project domain certs ─────────────
    # [IMP:9][ssl-provision][main] BUSINESS INVARIANT: independent project domains
