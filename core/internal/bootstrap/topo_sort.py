...
## @io       str (modules_dir path) → List[dict] (parsed YAML contents)
## @complexity 2 — file I/O with sorted glob
def load_module_yamls(modules_dir: str) -> list[dict[str, Any]]:
...
## @io       List[dict] → List[dict]
## @complexity 1 — simple list comprehension
def filter_docker_modules(modules: list[dict[str, Any]]) -> list[dict[str, Any]]:
...
## @io       dict (module) → List[str]
## @complexity 1 — type-checking branch
def extract_depends_on(module: dict[str, Any]) -> list[str]:
...
## @io       modules (List[dict]), filter_names (Optional[List[str]]) → Dict[str, List[str]]
...
def build_dag(modules: list[dict[str, Any]], filter_names: list[str] | None = None) -> dict[str, list[str]]:
