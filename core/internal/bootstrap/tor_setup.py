...
## @scope    Вызывается install_tor_proxy.py install_packages (native import; shell-фасад
##           install-tor-proxy.sh — тонкая обёртка). Чистая логика
...
def apt_cache_has(pkg: str) -> bool:
    ...
    result = subprocess.run(["apt-cache", "show", pkg], capture_output=True, text=True, check=False)
    return result.returncode == 0
...
def apt_update() -> None:
    ...
    apt-get на слабых VPS (аналогично system.py install_apt_packages / install_acme.py).
    """
    result = subprocess.run(
        ["apt-get", "update", "-qq"], capture_output=True, text=True, timeout=APT_TIMEOUT, check=False
    )
...
def apt_install(packages: list[str]) -> None:
    ...
    """
    result = subprocess.run(
        ["apt-get", "install", "-y", "-qq", *packages], capture_output=True, text=True, timeout=APT_TIMEOUT, check=False
    )
    if result.returncode != 0:
        logger.error(
        ...
        msg = f"apt-get install failed: {' '.join(packages)}"
        raise TorSetupError(msg)
