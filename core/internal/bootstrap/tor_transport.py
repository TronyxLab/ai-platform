...
def parse_bridges(content: str, available_binaries: set[str]) -> BridgeParseResult:
    ...
    for line in content.splitlines():
        ...
        if transport not in TRANSPORT_BIN:
            ...
            raise TorTransportError(f"Unknown transport '{transport}' — no registered binary path")
...
## @io       ⇥ None → ⎋ set[str] — транспорты с существующим бинарником
## @complexity O(T) — T = транспорты
def resolve_available_binaries() -> set[str]:
    """Return set of transports whose binary exists on the system (shutil.which)."""
    import shutil

    available: set[str] = set()
    for transport, binary in TRANSPORT_BIN.items():
        if shutil.which(binary) or __import__("os").path.isfile(binary):
...
def main() -> int:
    ...
    try:
        with open(args.bridges_file, encoding="utf-8") as f:
            content = f.read()
