...
def collect_hosts(
    ...
    if not cert_file.is_file():
        ...
        sans_hosts = _sans_to_hosts(cert_file)
        hosts |= sans_hosts
...
def _sans_to_hosts(cert_file: Path) -> set[str]:
    ...
    """
    hosts: set[str] = set()
    for entry in get_cert_sans(cert_file):  # get_cert_sans требует Path (dev_cert_generator контракт)
...
def block_diff(etc_hosts: str, hosts: set[str]) -> tuple[str, bool]:
    ...
    if begin_idx is not None and end_idx is not None:
        if end_idx < begin_idx:
            raise ConfigParseError("dev-hosts marker block malformed: END before BEGIN in hosts file")
...
def _read_file(target: Path) -> str:
    ...
    if not target.is_file():
        raise ConfigNotFoundError(f"hosts file not found: {target}")
...
def _atomic_write(target: Path, content: str) -> None:
    ...
    try:
        ...
        _sudo_move(Path(tmp_name), target)
    finally:
...
def _sudo_move(src: Path, dest: Path) -> None:
    ...
    try:
        ...
        raise PlatformError(f"sudo mv failed (rc={exc.returncode}) — /etc/hosts not updated: {exc}") from exc
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        # ── default / explicit dry-run ─────────────────────────────
        content = _read_file(Path(hosts_path))
        _, changed = block_diff(content, hosts)  # merged content re-rendered by _render_diff
        if changed:
