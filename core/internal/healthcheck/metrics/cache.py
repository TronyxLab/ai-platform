...
class CacheManager:
    ...
    def __init__(self, cache_dir: str = "/var/cache/platform/metrics"):
        ...
        try:
            ...
            _logger.info("[IMP:8][cache][init] Cache directory ensured: %s", cache_dir)
        except OSError as exc:
            _logger.warning("[IMP:8][cache][init] Cannot create cache dir %s: %s", cache_dir, exc)
    ...
    def get(self, key: str, ttl_seconds: int = 3600, source_mtime: float | None = None) -> dict | None:
        ...
        if not os.path.isfile(cache_path):
            _logger.info("[IMP:8][cache][get] Cache MISS (no file): %s", key)
            return None
        ...
        try:
            ...
            _logger.warning("[IMP:8][cache][get] Cache MISS (corrupt): %s: %s", key, exc)
            return None
        ...
        if cache_timestamp + ttl_seconds < now:
            _logger.info("[IMP:8][cache][get] Cache MISS (TTL expired): %s (age=%.0fs)", key, now - cache_timestamp)
            return None
        ...
        _logger.info("[IMP:9][cache][get] Cache HIT: %s (age=%.0fs)", key, now - cache_timestamp)
    ...
    def clear(self, key: str) -> None:
        ...
        try:
            ...
            _logger.info("[IMP:8][cache][clear] Cache cleared: %s", key)
        except OSError as exc:
            _logger.warning("[IMP:8][cache][clear] Cache clear failed for %s: %s", key, exc)
