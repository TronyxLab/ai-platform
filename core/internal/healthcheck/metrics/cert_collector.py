...
def _load_cert(path: str) -> dict | None:
    ...
    try:
        ...
        _logger.warning("[IMP:8][cert_collector][_load_cert] Cannot read cert file %s: %s", path, exc)
        return None
...
def get_certs(node_yaml_path: str) -> list[dict]:
    ...
    """
    _logger = logging.getLogger(__name__)
    _logger.info("[IMP:8][cert_collector][get_certs] Starting certificate collection")
    ...
    if not domains:
        _logger.info("[IMP:8][cert_collector][get_certs] No exposed domains in node.yaml")
        return []

    _logger.info("[IMP:8][cert_collector][get_certs] Found %d domain(s) to check", len(domains))

    # Step 2: Try direct path for each domain
    cert_map: dict[str, dict] = {}  # cert_id → cert dict
    for domain in domains:
        cert_path = os.path.join(_LETSENCRYPT_LIVE, domain, "fullchain.pem")
        if os.path.isfile(cert_path):
            cert_data = _load_cert(cert_path)
            if cert_data:
                cid = cert_data["cert_id"]
                if cid in cert_map:
                    # Merge domain into existing cert
                    if domain not in cert_map[cid]["domains"]:
                        cert_map[cid]["domains"].append(domain)
                else:
                    cert_data["domains"] = [domain]
                    cert_map[cid] = cert_data
                continue

        # Step 3: Domain not found at direct path — search all live certs
        _logger.info("[IMP:8][cert_collector][get_certs] No direct cert for %s — searching live/", domain)
        domain_cert = _search_wildcard_cert(domain)
        if domain_cert:
            cid = domain_cert["cert_id"]
            if cid in cert_map:
                if domain not in cert_map[cid]["domains"]:
                    cert_map[cid]["domains"].append(domain)
            else:
                domain_cert["domains"] = [domain]
                cert_map[cid] = domain_cert
        else:
...
def _search_wildcard_cert(domain: str) -> dict | None:
    ...
    try:
        ...
        _logger.warning("[IMP:8][cert_collector][_search_wildcard] Cannot list live dir: %s", exc)
        return None
