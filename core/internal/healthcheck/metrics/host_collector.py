...
# region DATA_HostDisk
class HostDisk(TypedDict):
    """Дисковая статистика хоста (граница status-metrics.json)."""
...
# endregion DATA_HostDisk


# region DATA_HostUptime
class HostUptime(TypedDict):
    """Uptime + load-average хоста (граница status-metrics.json; None при недоступности /proc)."""
...
# endregion DATA_HostUptime


# region DATA_HostMemory
class HostMemory(TypedDict):
    """RAM/Swap статистика хоста (граница status-metrics.json)."""
...
# endregion DATA_HostMemory


# region DATA_HostUname
class HostUname(TypedDict):
    """OS/kernel/arch хоста (граница status-metrics.json)."""
...
_LOADAVG_FIELDS_MIN: int = 3  # /proc/loadavg: 1m 5m 15m ...
_MEMINFO_FIELDS: int = 2  # /proc/meminfo: "Key: value ..."


def get_host_disk(path: str = "/opt") -> HostDisk:
    ...
    logger_.info("[IMP:8][host_collector][get_host_disk] Checking disk usage for %s", path)
    ...
    try:
        ...
        logger_.warning("[IMP:8][host_collector][get_host_disk] Failed to get disk usage for %s: %s", path, exc)
...
def get_host_uptime(open_fn: Callable[..., TextIO] = open) -> HostUptime:
    ...
    🧐 TRAP[DI-SEAM] · 2026-08-14 · — · /proc-чтение через open_fn (167 D6)
    · Rejected: прямой builtins.open с monkeypatch-патчем в тестах (fake /proc)
    · Reason: seam = тестируемость реального вызова — open_fn инжектируется тестом
    ·   (fake /proc-файлы/FileNotFoundError), default = open (поведение неизменно)
    · Rev: при выносе /proc-абстракции в shared — единый DI-параметр
    """
    logger_ = logging.getLogger(__name__)
    logger_.info("[IMP:8][host_collector][get_host_uptime] Reading host uptime & load")
    ...
    # ruff: ignore[PLW0717] — тело try присваивает имена, читаемые except/после — извлечение ломает видимость
    try:
        with open_fn("/proc/uptime", encoding="utf-8") as f:
            parts = f.read().strip().split()
            if parts:
                uptime_val = float(parts[0])
                result["uptime_seconds"] = uptime_val
                logger_.info("[IMP:9][host_collector][get_host_uptime] Uptime: %.2f seconds", uptime_val)
...
def get_host_uname() -> HostUname:
    ...
    try:
        ...
        logger_.warning("[IMP:8][host_collector][get_host_uname] os.uname failed: %s", exc)
