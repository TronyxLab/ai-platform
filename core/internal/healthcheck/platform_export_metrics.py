...
from core.internal.healthcheck.metrics.backup_collector import BackupStatus
...
# · W4e (DevPlan 160 E2): env= Mapping параметр — тесты передают env-дикт
#   напрямую (DI), reload/importlib больше не требуется.


def _get_node_yaml_path(env: Mapping[str, str] | None = None) -> str:
    """Get node.yaml path from env (или DI-дикта), with fallback."""
    source: Mapping[str, str] = os.environ if env is None else env
    # ⚠️ TRAP[BUG] · 2026-08-13 · P2 · str(Path) — PTH118-автофикс вернул PosixPath (DevPlan 163)
    # · Symptom: test_auto_detect_node_yaml_path_default: 'PosixPath' has no attribute 'endswith'
    # · Root: W-B autofix os.path.join → Path(...); контракт функции -> str (shell-потребители)
    # · Fix: str(Path) — контракт сохранён, pathlib-канон внутри
    # · Prevention: unit-тест TestNodeYamlAutoDetect покрывает оба пути
    return source.get(
        "NODE_YAML_PATH",
        str(
            Path(
                source.get("NODE_CONFIGS_DIR", str(node_configs_remote())),
                source.get("NODE_NAME", "unknown"),
                "node.yaml",
            )
        ),
    )
...
def _get_status_metrics_json(env: Mapping[str, str] | None = None) -> str:
    """Get status-metrics.json output path from env (или DI-дикта).
    ...
    """
    source: Mapping[str, str] = os.environ if env is None else env
    return source.get("STATUS_METRICS_JSON", str(_status_metrics_json()))


def _get_node_name(env: Mapping[str, str] | None = None) -> str:
    """Get node name from env (или DI-дикта).
    ...
    ## @io — ⇥ env: Mapping | None (DI, DevPlan 160 E2) → ⎋ str (NODE_NAME or "unknown" fallback)
    ## @complexity — O(1)
    """
    source: Mapping[str, str] = os.environ if env is None else env
    return source.get("NODE_NAME", "unknown")


def _get_cache_dir(env: Mapping[str, str] | None = None) -> str:
...
## @io       ⇥ env: Mapping[str, str] | None (DI, DevPlan 160 E2; None = os.environ)
...
## @io       ⇥ host, image_sizes, logger → ⎋ результат try-тела
## @complexity O(1) — извлечение управляющего потока
def _plw_body_main(host: dict[str, object], image_sizes: dict[str, int], logger: logging.Logger) -> None:
    if image_sizes:
        total_bytes = sum(image_sizes.values())
        host["docker_images_size_gb"] = round(total_bytes / (1024**3), 2)
        logger.info("[IMP:9][coordinator][main] Docker images total size: %.2f GB", host["docker_images_size_gb"])
    else:
...
def main(env: Mapping[str, str] | None = None) -> int:
    ...
    node_name = _get_node_name(env)
    status_metrics_json = _get_status_metrics_json(env)
    node_yaml_path = _get_node_yaml_path(env)
    cache_dir = _get_cache_dir(env)

    start = time.monotonic()
    logger.info("[IMP:9][coordinator][main] Starting metrics export for node=%s", node_name)
    ...
    _load_node_yaml(node_yaml_path, logger)
    ...
    try:
        ...
        logger.info("[IMP:9][coordinator][main] Docker containers collected: %d", len(containers))
    except (ImportError, OSError, FileNotFoundError) as exc:
        logger.warning("[IMP:8][coordinator][main] Docker container collection failed: %s", exc)
    ...
    cache_mgr: CacheManager | None = None  # init ДО try — шаг 5 читает cache_mgr после возможного отказа шага 3
    try:
        cache_mgr = _get_cache_manager(logger, cache_dir)
        image_sizes = _get_image_sizes_cached(image_ids, cache_mgr, logger)
        logger.info("[IMP:9][coordinator][main] Image sizes collected: %d", len(image_sizes))
    except (ImportError, OSError, FileNotFoundError) as exc:
        logger.warning("[IMP:8][coordinator][main] Image size collection failed: %s", exc)
    ...
    try:
        ...
        logger.info("[IMP:9][coordinator][main] Certificates collected: %d", len(certs))
    except (ImportError, OSError, FileNotFoundError) as exc:
        logger.warning("[IMP:8][coordinator][main] Cert collection failed: %s", exc)
    ...
    try:
        ...
        logger.info("[IMP:9][coordinator][main] Projects collected: %d", len(projects))
    except (ImportError, OSError, FileNotFoundError) as exc:
        logger.warning("[IMP:8][coordinator][main] Project collection failed: %s", exc)
    ...
    try:
        ...
        host = cast(
            "dict[str, object]", cast(object, get_host_disk())
        )  # W11: HostDisk (TypedDict) → dict через object-мост
        logger.info("[IMP:9][coordinator][main] Host disk collected")
    except (ImportError, OSError, FileNotFoundError) as exc:
        logger.warning("[IMP:8][coordinator][main] Host disk collection failed: %s", exc)
    ...
    try:
        ...
        logger.info("[IMP:9][coordinator][main] Host uptime & load collected")
    except (ImportError, OSError, FileNotFoundError) as exc:
        logger.warning("[IMP:8][coordinator][main] Host uptime collection failed: %s", exc)
        errors.append(f"host_uptime: {exc}")

    # ── 6c. Docker images total size (from image_sizes dict, zero-cost) ──
    try:
        _plw_body_main(host, image_sizes, logger)
    except (OSError, FileNotFoundError) as exc:
        logger.warning("[IMP:8][coordinator][main] Docker images size calc failed: %s", exc)
    ...
    try:
        ...
        logger.info("[IMP:9][coordinator][main] Host memory & swap collected")
    except (ImportError, OSError, FileNotFoundError) as exc:
        logger.warning("[IMP:8][coordinator][main] Host memory collection failed: %s", exc)
    ...
    try:
        ...
        logger.info("[IMP:9][coordinator][main] Host OS/kernel collected")
    except (ImportError, OSError, FileNotFoundError) as exc:
        logger.warning("[IMP:8][coordinator][main] Host OS collection failed: %s", exc)
        errors.append(f"host_os: {exc}")

    # ── 7. Backup status ──
    backup: BackupStatus = {"last_postgres_at": None, "last_app_data_at": None, "status": "unknown"}
    try:
        from core.internal.healthcheck.metrics.backup_collector import get_backup_status

        backup = get_backup_status()
        logger.info("[IMP:9][coordinator][main] Backup status collected: %s", backup.get("status", "unknown"))
    except (ImportError, OSError, FileNotFoundError) as exc:
        logger.warning("[IMP:8][coordinator][main] Backup collection failed: %s", exc)
        errors.append(f"backup: {exc}")

    # ── 7b. Deploy status (170 W12 C5, SLO-минимум) ──
    deploy: dict[str, object] = {"status": "unknown"}
    try:
        from core.internal.healthcheck.metrics.deploy_collector import get_deploy_status

        deploy = dict(get_deploy_status())
        logger.info("[IMP:9][coordinator][main] Deploy status collected: %s", deploy.get("status", "unknown"))
    except (ImportError, OSError, FileNotFoundError) as exc:
    ...
    try:
        ...
        logger.error("[IMP:9][coordinator][main] Atomic write failed: %s", exc)
...
def _load_node_yaml(path: str, _logger: logging.Logger) -> dict[str, object]:
...
def _get_cache_manager(_logger: logging.Logger, cache_dir: str | None = None) -> CacheManager | None:
...
def _get_image_sizes_cached(
    image_ids: set[str], cache_mgr: CacheManager | None, _logger: logging.Logger
) -> dict[str, int]:
    ...
    if cache_mgr is not None:
        ...
        if cached is not None:
            ...
            return cast("dict[str, int]", cached)  # W11: кэш-граница JSON (dict[str, object] → dict[str, int])
