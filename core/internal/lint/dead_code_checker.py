...
##           2026-08-14 | DevPlan 167 D2 — CommandRunner-seam: get_line_add_timestamp(runner=),
##           main(project_root=) path-injection (0 monkeypatch в тестах)
...
class BlameProc(Protocol):
    """Минимальный контракт результата CommandRunner (W11-G4): subprocess.run |
    тестовый fake (167 D2 seam) — только returncode/stdout используются ниже."""

    returncode: int
    stdout: str


CommandRunner = Callable[..., BlameProc]


def get_line_add_timestamp(
    project_root: Path | str,
    rel: str,
    line_num: int,
    mtime: int,
    runner: CommandRunner | None = None,  # CommandRunner-seam (167 D2): subprocess.run fake
) -> int:
    ...
    ##       ⇥ runner: DI-fn (None → subprocess.run)
    ##       → ⎋ int — epoch seconds
    ...
    ## 🧐 TRAP[DI-SEAM] · 2026-08-14 · — · CommandRunner-seam: git blame через runner-параметр
    ## · Rejected: прямой вызов subprocess.run (тест патчил subprocess-модуль monkeypatch.setattr)
    ## · Reason: seam = тестируемость реального вызова (fake-runner параметром, 0 патчей)
...
def check_dead_code(
    ...
    """Orchestrator: scan → blame → age → filter; per-marker report_lines (stdout-контракт P9/P10), violations → return.

    ▶ scan-start (stderr) → find_marker_files → ○ per file: find_deprecated_lines → ○ per hit: blame/mtime →
    ◇ age > T ? → ⊕ violation + STALE | OK → report_lines (interleaving) → ⎋ list[DeadCodeViolation]

    ## @purpose — главный конвейер: собирает per-marker строки в report_lines (байт-в-байт как оригинальный
    ##             while-loop) и возвращает список нарушений (контракт для _print_report и main).
    ## @io — ⇥ project_root; ⇥ threshold_days: default 30; ⇥ report_lines: list[str] | None — аккумулятор
    ##       stdout-отчёта (main передаёт и печатает; None = чистый вызов без side-effect)
    ##       → ⎋ list[DeadCodeViolation] — stale-нарушения
    ## @complexity — O(F * L + H) — F файлов, L строк, H hits (per-hit git blame subprocess)
    ## @invariants — violation iff age_days > threshold_days (СТРОГО, P8); OK/STALE накапливаются в порядке
    ##               обхода (интерливинг как в оригинале, AC3) — main печатает их в stdout byte-identical (P11);
    ##               unreadable/deleted file → skip; stderr-телеметрия на DEBUG — CLI-вывод остаётся byte-identical (P11)
    ## @rationale — печать вынесена в CLI-обёртку (main): бизнес-функция не печатает (T20-clean);
    ##               report_lines сохраняет интерливинг — main печатает строки в том же порядке;
    ...
    for file in find_marker_files(root, SELF_EXCLUSIONS):
        ...
        try:
            mtime = int(Path(file).stat().st_mtime)
        except OSError as e:
        ...
        for line_num, line_text in find_deprecated_lines(file):
            ...
            if age_days > threshold_days:
                ...
                if report_lines is not None:
                    ...
                    report_lines.append(f"  >>> {line_text[:120]}")
            else:
                ok_count += 1
                if report_lines is not None:
                    report_lines.append(
...
def _attach_stderr_handler() -> logging.StreamHandler[typing.TextIO]:
...
def main(argv: list[str] | None = None, project_root: Path | None = None) -> int:
    ...
    ## @io — ⇥ argv: None → sys.argv[1:]; ⇥ project_root: Path | None — path-injection DI
    ##       (None → _default_project_root()) → ⎋ int — 0 clean / 1 violations
    ...
    ## 🧐 TRAP[DI-SEAM] · 2026-08-14 · — · Path-injection: main(project_root=) вместо _default_project_root
    ## · Rejected: чтение корня из _default_project_root() (тест патчил её monkeypatch.setattr)
    ## · Reason: seam = тестируемость реального вызова — корень сканирования параметром
    ## · Rev: при появлении CLI --root-параметра (scripts_audit-style) — консолидировать
    """
    ...
    try:
        threshold = cast(int, args.threshold)  # W11-G4: argparse Namespace → Any → cast на границе CLI
        violations = check_dead_code(
            project_root if project_root is not None else _default_project_root(),
            threshold,
            report_lines,
        )
        # stdout-контракт CLI: печать per-marker строк в порядке обхода (P11 byte-identical)
        for line in report_lines:
            print(line)
        _print_report(violations, threshold)
