...
#            ▶ namelint: yaml.safe_load(manifest) + .PHONY (Makefile + makefiles/*.mk) → ◇ forbidden/allowed/lifecycle/exceptions → ⎋ errors
...
# ⚠️ TRAP[DECISION] · 2026-07-31 · — · namelint-исключения из секции name_linter манифеста
...
def check_grep_summary_presence(file: Path) -> list[str]:
    ...
    """
    # 🧐 TRAP[BUG] · 2026-07-22 · pipefail + head|grep -qE race (история из check-doc-headers.sh:47-51)
    # · Reason: `set -euo pipefail` + `head -5 | grep -qE` — timing-dependent: head exits 0 до
    # ·   grep match → pipefail=0 → !0 = 1 → false [FAIL]. В Python race отсутствует
    # ·   (прямой файловый I/O, pipes нет). Fix в shell был `grep -cE || true`.
    text = _read_text(file)
    if text is None:
        return []
...
def check_structure(file: Path) -> list[str]:
    ...
    """
    # 🧐 TRAP[BUG] · 2026-07-22 · pipefail + head|grep -qE race (та же бага, что check_grep_summary)
    # · Fix в shell был `grep -cE || true` — pipefail-safe; в Python race отсутствует (история).
    text = _read_text(file)
    if text is None:
        return []
    first_10 = text.splitlines()[:10]
...
def check_md_sh_refs(file: Path, repo_root: Path | None = None) -> list[str]:
    ...
    ##               skip-list для удалённых скриптов отклонён (TRAP[DECISION]) — фикс документации лучше.
    """
    ...
    # 🧐 TRAP[BUG] · 2026-07-11 · SIGPIPE fix · subshell +o pipefail (find | grep -q)
    # · Reason: `set -o pipefail` + `grep -q` мог вызвать SIGPIPE (141) — pipefail локализован
    # ·   subshell'ом. В Python — Path.rglob, pipes отсутствуют (история).
...
def _find_in_internal(root: Path, ref: str) -> bool:
    ...
    if not internal.is_dir():
        return False
    return any(path.is_file() and len(path.relative_to(internal).parts) <= 5 for path in internal.rglob(ref))
...
def validate_make_target_names(repo_root: Path) -> list[str]:
    """Validate .PHONY make targets against manifest allowed/lifecycle/forbidden + system exceptions.

    ▶ ┌repo_root┐ → ○ manifest exists → ○ yaml.safe_load → ○ .PHONY (Makefile + makefiles/*.mk) →
    ◇ forbidden → ◇ allowed → ◇ lifecycle → ◇ prefixes → ◇ literals → ⊕ errors → ⎋ list[str]
    ...
    forbidden = set(data.get("forbidden_verbs") or [])
    ...
    for target in sorted(targets):
        if target in forbidden:
            msg = f"[FAIL] Target '{target}' is FORBIDDEN (listed in manifest forbidden_verbs)"
            logger.info("[IMP:9][validate_make_target_names][fail] %s", msg)
            errors.append(msg)
        elif (
            target in allowed
            ...
            or target in system_exceptions
        ):
            continue
        else:
...
def main() -> int:
    ...
    if args.command == "doc-headers":
        errors, checked = validate_files(args.files, repo_root)
        if errors:
            for err in errors:
                print(err)
