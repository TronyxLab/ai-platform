...
# W1-A1 (план 170): timeout=30 литерал (git ls-files) → канон SoT
...
def extract_keywords(line: str, mode: str = "scan") -> list[str]:
    ...
    if mode == "staged":
        ...
        return tokens
    cleaned: list[str] = []
    for raw_kw in tokens:
        kw = raw_kw.replace("#", "").replace("-->", "").replace("<!--", "")
...
def extract_sh_refs(text: str, backtick_only: bool = False) -> list[str]:
    ...
    """
    if backtick_only:
        # W11-G4: typeshed Pattern.findall → list[Any] (паттерн без capture-групп — полные
        # совпадения str); cast — граница stdlib re (групп нет — тип гарантирован)
        matches = cast(list[str], _BACKTICK_SH_REF_RE.findall(text))
        refs = sorted({m.replace("`", "") for m in matches})
