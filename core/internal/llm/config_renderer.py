...
##            enables CI gate for manifest freshness (make check MARKER=check-manifests).
...
# ── Data builders ────────────────────────────────────────────────────────────
_DIFF_LINES_MAX: int = 20  # обрезка дифф-вывода при --check


# region DATA_ModelListEntry
class ModelListEntry(TypedDict):
    """Одна запись model_list litellm-config.yml (из активного alias-деплоймента).

    ## @purpose  Типизированная граница entry model_list: name (alias), litellm_model
    ##            (provider/model), api_key (env-ссылку), access_groups, mode.
    """
...
# endregion DATA_ModelListEntry


def _build_model_list(policy: LLMPolicy) -> list[ModelListEntry]:
    ...
    """
    model_list: list[ModelListEntry] = []
        ...
        if isinstance(deployments, DeploymentList):
            ...
            if deployments.primary is not None:
                ...
                if not provider:
                    ...
                    msg = f"Provider '{provider_name}' for alias '{alias_name}' not found"
            ...
            if deployments.fallback is not None:
                ...
                if not provider:
                    ...
                    msg = f"Fallback provider '{provider_name}' for alias '{alias_name}' not found"
                    raise ConfigValidationError(msg)
                api_key = f"os.environ/{provider.key_env}"
                entry: ModelListEntry = {
...
def render_litellm_config(
    ...
    if not template_path.exists():
        ...
        msg = f"Jinja2 template not found: {template_path}"
    ...
    try:
        _validate_rendered_yaml(rendered)
...
def _validate_rendered_yaml(rendered: str) -> None:
    """Парс-валидация rendered YAML — mapping-структура (Step 6, вынесено для TRY-лимита).

    ▶ ┌rendered┐ → ○ yaml.safe_load → ◇ не mapping → ConfigValidationError → ⎋ None

    ## @purpose  Гарантия инварианта «Output is valid YAML mapping» (render_litellm_config):
    ##            шаблон может отрендерить список/скаляр — fail-fast на рантайме.
    ## @io — ⇥ rendered: str → ⎋ None | ConfigValidationError
    ## @complexity — O(N) — YAML parse
    ## @changes  2026-08-15 | DevPlan 170 W11 — извлечение из try (TRY-лимит) + типизация yaml-границы
    """
    raw_parsed = cast("object", yaml.safe_load(rendered))  # W11: yaml → Any → object
    if not isinstance(raw_parsed, dict):
        msg = "Rendered output is not a valid YAML mapping"
        raise ConfigValidationError(msg)
...
def check_freshness(
    ...
    # Render to temp file
    with tempfile.NamedTemporaryFile(encoding="utf-8", mode="w", suffix=".yml", delete=False) as tmp:
        ...
        try:
            ...
            if is_fresh:
                ...
                if len(diff_lines) > _DIFF_LINES_MAX:
                    sys.stderr.write(f"... ({len(diff_lines) - _DIFF_LINES_MAX} more lines)\n")
        finally:
...
class CliArgs(argparse.Namespace):
    ...
    output: str | None  # pyright: ignore[reportUninitializedInstanceVariable] — W11 argparse fills (без class-value дефолтов)
    check: bool  # pyright: ignore[reportUninitializedInstanceVariable] — W11 argparse fills (без class-value дефолтов)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        content = render_litellm_config(policy_path, template_path)
        print(content, end="")
    # ruff: ignore[BLE001] — top-level CLI render handler — catch-all (FileNotFoundError/ValueError уже покрыты ветко...
    except Exception as e:  # noqa: EXC — top-level CLI handler — catch-all (FileNotFoundError/ValueError покрыты ветками)
