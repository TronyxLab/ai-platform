...
class AliasDef(BaseModel):
    ...
    @classmethod
    def _validate_deployments(cls, v: Any) -> Any:
...
class LLMPolicy(BaseModel):
    ...
    @classmethod
    def _validate_default_profile_exists(cls, v: AutoProvisionDef, info: Any) -> AutoProvisionDef:
    ...
    def from_yaml(cls, path: str | pathlib.Path) -> "LLMPolicy":
        ...
        if schema_path.exists():
            ...
            if errors:
                ...
                raise jsonschema.ValidationError(f"Policy JSON Schema validation failed: {error_messages}")
