...
def load_history(history_dir: str | Path) -> list[dict]:
    ...
    try:
        ...
        raise ConfigParseError(f"history.json битый JSON ({path}): {exc}") from exc
    runs = data.get("runs", []) if isinstance(data, dict) else []
    if not isinstance(runs, list):
        raise ConfigParseError(f"history.json ({path}): runs не является списком")
...
def append_run(history_dir: str | Path, run: dict) -> None:
