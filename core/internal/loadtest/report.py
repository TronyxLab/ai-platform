...
from core.internal.loadtest.baseline import (  # структурный twin BaselineBlock (report-слой)
    BaselineComparison,
    HistoryRun,
)
...
# region DATA_TaskStats
class TaskStats(TypedDict):
    """Per-task статистика (148 TASK-6): строка stats.csv для Name != Aggregated.

    ## @purpose  Единица per-task breakdown отчёта (read_query/write_query для db —
    ##            скорость записи vs чтения, SC_DB_RW). Все перцентили в СЕКУНДАХ.
    """
...
# endregion DATA_TaskStats


# region DATA_StepStats
class StepStats(TypedDict, total=False):
    """Метрики одного шага прогона (контракт step_runner capacity + _run_one_step).

    ## @purpose  Единый носитель метрик шага: capacity.run_capacity получает его от
    ##            step_runner, runner_cli собирает в _run_one_step. Ключ "error" —
    ##            сигнал отказа прогона (runner error / locust rc != 0).
    """
...
# endregion DATA_StepStats


# region DATA_StatsJson
class StatsJson(TypedDict, total=False):
    """Блок stats отчёта (report.json) — зеркало dataclass Stats."""
...
# endregion DATA_StatsJson


# region DATA_BaselineJson
class BaselineJson(TypedDict, total=False):
    """Блок baseline отчёта (report.json) — зеркало BaselineComparison/HistoryRun.prev."""

    prev: HistoryRun | None
    delta_p95: float | None
    delta_error_pp: float | None
    first_run: bool
    baseline_reset: bool


# endregion DATA_BaselineJson


# region DATA_CapacityStep
class CapacityStep(TypedDict, total=False):
    """Строка capacity_profile отчёта — зеркало dataclass StepResult."""
...
# endregion DATA_CapacityStep


# region DATA_SaturationAgg
class SaturationAgg(TypedDict, total=False):
    """Агрегат одной метрики saturation-секции (avg/max, cpu_* → pct)."""
...
# endregion DATA_SaturationAgg


# region DATA_ReportJson
class ReportJson(TypedDict, total=False):
    """Полный report.json — единая структура всех режимов (DevPlan 146 §3.5 + 148)."""
    ...
    stats: StatsJson
    tasks: dict[str, TaskStats] | None
    saturation: dict[str, SaturationAgg]
    missing_metrics: list[str]
    insufficient_metrics: list[str]
    baseline: BaselineJson
    max_rps: int | None
    capacity_profile: list[CapacityStep]
    verdict: str
    warnings: list[str]


# endregion DATA_ReportJson
...
def parse_stats_csv(path: str | Path) -> tuple[Stats, dict[str, TaskStats]]:
...
def verdict_regression(stats: Stats, max_p95: float, baseline: BaselineComparison | BaselineBlock) -> str:
...
def build_report(
    *,
    ...
    version: str,
    stats: Stats,
    saturation_aggregates: dict[str, SaturationAgg] | None = None,
    missing_metrics: list[str] | None = None,
    insufficient_metrics: list[str] | None = None,
    baseline: BaselineComparison | BaselineBlock | None = None,
    max_rps: int | None = None,
    capacity_profile: list[CapacityStep] | None = None,
    duration_s: float | None = None,
    tasks: dict[str, TaskStats] | None = None,
    verdict: str,
...
def render_markdown(report: ReportJson) -> str:
    ...
    stats_block: StatsJson = stats if stats is not None else {}
    lines = [
        ...
        "|-----|-----|-----|-----|-----------|----------|-----------|",
        (
            f"| {_fmt(stats_block.get('rps'))} | {_fmt(stats_block.get('p50'))} | {_fmt(stats_block.get('p95'))} | "
            f"{_fmt(stats_block.get('p99'))} | {_fmt(stats_block.get('error_rate'))} | "
            f"{stats_block.get('total_requests')} | {stats_block.get('total_failures')} |"
        ),
    ]
    if report.get("duration_s") is not None:
        lines += ["", f"- **Duration: `{_fmt(report.get('duration_s'))}s`**"]
    tasks_block = report.get("tasks")
    if tasks_block:
        lines += [
            "",
            "## Tasks (per-task)",
        ...
        for name, task_stats in sorted(tasks_block.items()):
            lines.append(
    ...
    profile_block = report.get("capacity_profile")
    if profile_block:
        lines += [
            "",
            "## Capacity profile",
    ...
    if saturation_block:
        ...
        for name, agg in sorted(saturation_block.items()):
            lines.append(f"| {name} | {_fmt(agg.get('avg'))} | {_fmt(agg.get('max'))} | {_fmt(agg.get('pct'))} |")
    missing_block = report.get("missing_metrics")
    if missing_block:
        lines += ["", f"- ⚠️ Missing metrics (WARN): {', '.join(missing_block)}"]
    insufficient_block = report.get("insufficient_metrics")
    if insufficient_block:
        lines += ["", f"- ⚠️ Insufficient metrics (WARN): {', '.join(insufficient_block)}"]
    baseline_block = report.get("baseline")
    base_block: BaselineJson = baseline_block if baseline_block is not None else {}
    prev_block = base_block.get("prev")
    if prev_block is not None:
        lines += [
            "",
            "## Baseline",
            "",
            f"- prev p95: `{_fmt(prev_block.get('p95'))}` · delta_p95: `{_fmt(base_block.get('delta_p95'))}`",
            f"- prev error: `{_fmt(prev_block.get('error_rate'))}` · delta: `{_fmt(base_block.get('delta_error_pp'))}pp`",
        ]
    if base_block.get("first_run"):
        lines += ["", "- ℹ️ First run — baseline записан, сравнение недоступно"]
    if base_block.get("baseline_reset"):
        lines += ["", "- ℹ️ baseline_reset: нода пересоздана (host изменился) — сравнение недействительно"]
    warnings_block = report.get("warnings")
    if warnings_block:
        lines += ["", "## Warnings", ""]
        lines += [f"- ⚠️ {w}" for w in warnings_block]
    return "\n".join(lines) + "\n"
...
def write_junit_xml(report: ReportJson, path: str | Path) -> None:
...
def write_report_json(report: ReportJson, path: str | Path) -> None:
    ...
    """
    # W11: atomic_write_json ждёт dict[str, Any] | list[Any] (shared — W11-G5 cross-file);
    # TypedDict → dict() копия: shallow, сериализация идентична (типовая граница, не поведение)
