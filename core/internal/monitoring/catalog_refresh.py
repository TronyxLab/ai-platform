...
try:
    ...
    from monitoring.constants import CATALOG_SCRIPT  # pyright: ignore[reportImplicitRelativeImport]
...
def refresh_catalog(platform_root: Path) -> RenderResult:
    ...
    try:
        ...
        err_text = cast("str | None", e.stderr) or ""  # W11: typeshed CalledProcessError.stderr — Any → str | None
        logger.info("[IMP:6][catalog] Catalog generation failed (exit %s): %s", e.returncode, err_text.strip())
        return RenderResult(component="catalog", status="failed", detail=err_text.strip())
