...
# invocation (`python3 core/internal/monitoring/config_renderer.py`, make target).
# File is at core/internal/monitoring/config_renderer.py → root = 4 levels up (172 W5.1).
...
# · Rev: if config_renderer changes its packaging namespace (172 W5.1: переехал В monitoring/), drop the direct-import fallback
try:
    # Imported as core.internal.monitoring.config_renderer (tests, python3 -m) — single module instance
    ...
    # Invoked as `python3 ${PLATFORM_ROOT}/core/internal/monitoring/config_renderer.py`:
    ...
    from template_engine import (  # ruff: ignore[F401]  # pyright: ignore[reportImplicitRelativeImport] — direct-script fallback (sys.path[0]=core/internal/), TRAP[DECISION] 094 §7.1 dual-path
        TemplateError,
        render_template,
    )
...
from core.internal.monitoring.constants import (  # ruff: ignore[F401] — публичный re-export констант (DevPlan 117 G ...
    ALERT_RULES_DIR,
    CATALOG_SCRIPT,
...
class ProjectMonitoringConfig:
    ...
    merged_config: dict[str, object] = field(default_factory=dict)
...
def deep_merge(base: dict[str, object], override: dict[str, object]) -> dict[str, object]:
    ...
    for key, val in override.items():
        if key in result and isinstance(result[key], dict) and isinstance(val, dict):
            # W11: isinstance-narrowed dict is dict[Unknown, Unknown] → cast for recursive merge
...
def load_yaml_config(yaml_path: Path) -> dict[str, object]:
    ...
    return cast(dict[str, object], yaml.safe_load(raw)) or {}


def load_l1_defaults(defaults_path: Path, project_type: str) -> dict[str, object]:
    ...
    # W11: dict[str, object].get → object → cast nested mapping boundaries
    monitoring = dict(cast(dict[str, object], data.get("monitoring", {})))
    type_defaults = cast(
        dict[str, object],
        cast(dict[str, object], data.get("type-defaults", {})).get(project_type, {}),
    )
    if type_defaults:
...
def load_l2_overrides(override_path: Path) -> dict[str, object]:
    ...
    if not data:
        return {}
    # W11: dict[str, object].get → object → cast nested mapping boundary
    return dict(cast(dict[str, object], data.get("monitoring", {})))


def load_l3_project_config(project_yaml: dict[str, object]) -> dict[str, object]:
    ...
    """
    # W11: dict[str, object].get → object → cast nested mapping boundary
    return dict(cast(dict[str, object], project_yaml.get("monitoring", {})))
...
def build_merged_config(
    ...
    # W11: shared.load_project_yaml уже возвращает dict[str, object] (G1) — direct assignment
    ...
    config = ProjectMonitoringConfig(
        ...
        metrics_port=int(cast(int, merged.get("metrics_port", 3000))),
        dashboard_enabled=_str_to_bool(merged.get("dashboard", False)),
        alerting_enabled=_str_to_bool(merged.get("alerting", False)),
        needs_llm=_str_to_bool(cast(dict[str, object], needs).get("llm", False)),
...
def _str_to_bool(val: object) -> bool:
...
def _parse_ai_retention(val: object) -> int:
...
# private alias — white-box tests import _parse_retention_hours (gate does not scan tests/).
...
def generate_prometheus_target(
    ...
    """
    from monitoring.prometheus_targets import (  # pyright: ignore[reportImplicitRelativeImport]
        generate_prometheus_target as _impl,  # pyright: ignore[reportImplicitRelativeImport]
    )
...
def generate_grafana_dashboard(
    ...
    from monitoring.grafana_dashboards import (  # pyright: ignore[reportImplicitRelativeImport]
        generate_grafana_dashboard as _impl,  # pyright: ignore[reportImplicitRelativeImport]
    )
...
def update_loki_retention(
    ...
    from monitoring.loki_retention import (  # pyright: ignore[reportImplicitRelativeImport]
        update_loki_retention as _impl,  # pyright: ignore[reportImplicitRelativeImport]
    )
...
def create_langfuse_project(
    ...
    from monitoring.langfuse_projects import (  # pyright: ignore[reportImplicitRelativeImport]
        create_langfuse_project as _impl,  # pyright: ignore[reportImplicitRelativeImport]
    )
...
def generate_alert_rules(
    ...
    from monitoring.alert_rules import generate_alert_rules as _impl  # pyright: ignore[reportImplicitRelativeImport]
...
def refresh_catalog(platform_root: Path) -> RenderResult:
    ...
    from monitoring.catalog_refresh import refresh_catalog as _impl  # pyright: ignore[reportImplicitRelativeImport]
...
def reload_monitoring_services() -> list[RenderResult]:
    ...
    from monitoring.service_reload import (  # pyright: ignore[reportImplicitRelativeImport]
        reload_monitoring_services as _impl,  # pyright: ignore[reportImplicitRelativeImport]
    )
...
def _render_step(step_name: str, fn: Callable[..., object], *args: object) -> None:
    ...
    # ruff: ignore[BLE001] — best-effort контракт post-deploy chain — произвольная render-функция
...
_DEFAULT_RENDER_STEPS: dict[str, Callable[..., object]] = {
    ...
    "langfuse": create_langfuse_project,
...
def run_monitoring_reconfig(
    ...
    platform_root: Path,
    *,
    render_steps: dict[str, Callable[..., object]] | None = None,
) -> int:
    ...
    ## @param render_steps  DI (DevPlan 167 D3): dict {шаг: callable} — fake render-шаги для
    ##                      тестов (запись вызовов, симуляция сбоев); None → _DEFAULT_RENDER_STEPS.
    ## 🧐 TRAP[DI-SEAM] · 2026-08-14 · — · render_steps на run_monitoring_reconfig (render-recording seam)
    ## · Rejected: прямой вызов module-level render-фасадов
    ## · Reason: seam = тестируемость реального вызова (порядок/continue/skip) без глобального
    ## ·   monkeypatch module-атрибутов; поведение по умолчанию (None → канон) неизменно
    ## · Rev: появление второго оркестратора render-шагов → общий шаговый runner
    ...
    ##           ⇥ render_steps: dict | None — override render-шагов (DI)
    ...
    _render_step("langfuse", steps["langfuse"], config)
    _render_step("catalog", steps["catalog"], platform_root)
...
def main(argv: list[str] | None = None) -> int:
    ...
    ## @changes 2026-08-14 | DevPlan 167 D1 — +argv DI-параметр (AF-4): None = sys.argv
    """
    ...
    # core/internal/monitoring/config_renderer.py → monitoring/ → internal/ → core/ → platform_root
