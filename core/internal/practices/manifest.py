...
# ── TypedDict-граница YAML-канона (W11-G4: после schema-валидации структура гарантирована;
#    функциональный синтаксис — ключ "class" (keyword) не доступен как атрибут класса) ──
_CheckEntry = TypedDict(
    "_CheckEntry",
    {
        "id": str,
        "level": str,
...
class _ManifestData(TypedDict):
    """Структура practices_manifest.yaml (schema-валидирована Draft7 до парсинга)."""

    version: int
    maturity: dict[str, int]
    pins: dict[str, str]
    allowed_external_networks: list[str]
    checks: list[_CheckEntry]
...
def load_manifest(path: Path | None = None) -> PracticesManifest:
    ...
    if not manifest_path.is_file():
        ...
        msg = f"Practices manifest not found: {manifest_path}"
    ...
    try:
        ...
        msg = f"Practices manifest is not valid YAML: {exc}"
...
## @complexity O(C)
def _read_manifest(path: Path) -> _ManifestData:
    """Parse manifest YAML → _ManifestData (safe_load; schema already validated structure)."""
    import yaml

    with Path(path).open(encoding="utf-8") as f:
        loaded = yaml.safe_load(f)  # pyright: ignore[reportAny] W11-G4: pyyaml без stubs → Any (граница untyped-библиотеки; isinstance-гейт + cast ниже)
    if not isinstance(loaded, dict):
        msg = f"Practices manifest root must be an object: {path}"
        raise ConfigValidationError(msg)
    # W11-G4: двухшаговый cast (dict → object → _ManifestData) — pyright запрещает прямой
    # dict[Unknown, Unknown] → TypedDict (reportInvalidCast); структура гарантирована schema-валидацией
    return cast(_ManifestData, cast(object, loaded))
