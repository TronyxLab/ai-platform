...
## 🧐 TRAP[DI-SEAM] · 2026-08-14 · — · audit_writer на set_practices (audit-logging seam)
## · Rejected: прямой вызов shared/audit_logger.write_audit_entry (пишет в /var/log/platform)
## · Reason: seam = тестируемость реального audit-контракта (from/to/reason) без патча
## ·   audit_logger; поведение по умолчанию (None → канон) неизменно
## · Rev: появление общего audit-объекта (transaction-контекст) → единый DI-параметр
...
def set_practices(project_dir: Path, level: str, *, audit_writer: Callable[..., bool] | None = None) -> SetReport:
    ...
    _audit_transition(project_dir, level, prev_state, sync.state, audit_writer=audit_writer)
    logger.info(
...
def _audit_transition(
    ...
    new_state: str,
    *,
    audit_writer: Callable[..., bool] | None = None,
) -> bool:
    """Write audit entry for practices state transition (non-raising).

    Args:
        project_dir: Project directory.
        level: Practices level.
        prev_state: Previous state (from practices.lock).
        new_state: New state.
        audit_writer: Optional audit-writer callable (167 D3 DI-seam). None → канон audit_logger.

    Returns:
        SetReport: результат установки уровня практик.
    """
    # dict[str, str]-сплат: ключи from/to/reason уходят в **extra audit_logger'а (W11-G4:
    # значения — str; тип совместим с **extra: dict[str, object])
    extra_fields: dict[str, str] = {"from": prev_state, "to": new_state, "reason": f"manual:{level}"}
    # ruff: ignore[PLW0717] — try содержит import + writer-резолв + аудит-запись (6 операторов, 167 D3)
    try:
        from core.internal.shared.audit_logger import write_audit_entry

        # cast → Callable[..., bool]: pyright не может статически сопоставить ключи
        # **extra_fields (dict[str, str]) с типизированными опциональными параметрами
        # write_audit_entry (log_file/raise_on_error) — ложное "raise_on_error ← str"
        # (W11-G4; рантайм-семантика: ключи from/to/reason уходят в **extra как есть)
        writer = cast(Callable[..., bool], write_audit_entry if audit_writer is None else audit_writer)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        project_dir = Path(cast(str, args.project_dir))
        level_arg = cast(str, args.level)
        report = set_practices(project_dir, level_arg)
    ...
    print(
        f"[PRACTICES:SET][level:{level_arg}][state:{report.sync.state}][yaml:{report.yaml_status}][lock:{report.sync.lock_status}]"
    )
