...
## @param parse_fn      DI (DevPlan 163 W-H): fake-парсер для тестов; None → try_parse_compose
## @param which_fn      DI (DevPlan 167 D3): fake shutil.which для тестов; None → канон.
##                      Пробрасывается в try_parse_compose (docker-доступность → PyYAML fallback).
## 🧐 TRAP[DI-SEAM] · 2026-08-14 · — · which_fn на validate_compose_networks (docker-detect seam)
## · Rejected: прямой вызов shutil.which в try_parse_compose
## · Reason: seam = тестируемость реального вызова (docker-отсутствие → PyYAML path) без
## ·   глобального патча shutil.which; поведение по умолчанию (None → канон) неизменно
## · Rev: появление второго I/O-канала в compose-парсинге → общий runner-параметр
## @io        ⇥ compose_path: Path, domain: str, compose_profiles: str, log_prefix: str,
##                parse_fn: Callable|None, which_fn: Callable|None → ⎋ ValidationResult
...
def validate_compose_networks(
    ...
    *,
    ...
    parse_fn: Callable[..., dict[str, object] | None] | None = None,
    ...
    DI (W-H DevPlan 163): parse_fn=None → try_parse_compose (канон); тесты передают
    fake-парсер (dict/None) вместо monkeypatch.setattr — поведение по умолчанию неизменно.
    which_fn (167 D3): пробрасывается в try_parse_compose, когда parse_fn=None.
    """
    ...
    # Step 1: Parse compose (parse_fn канон → try_parse_compose c which_fn DI, 167 D3)
    if parse_fn is not None:
        data = parse_fn(compose_path, compose_profiles=compose_profiles)
    else:
        data = try_parse_compose(compose_path, compose_profiles=compose_profiles, which_fn=which_fn)
    if data is None:
...
## @param which_fn     DI (DevPlan 163 W-H): fake shutil.which для тестов; None → канон
## @param compose_config_fn  DI (DevPlan 163 W-H): fake docker-compose-config канал; None → канон
...
def try_parse_compose(
    compose_path: Path,
    *,
    compose_profiles: str,
    which_fn: Callable[[str], str | None] | None = None,
    compose_config_fn: Callable[..., subprocess.CompletedProcess[str]] | None = None,
) -> dict[str, object] | None:
    """Try to parse compose file via docker compose config, then PyYAML.

    DI (W-H DevPlan 163): which_fn/compose_config_fn — None = канонические shutil.which/
    _shared_docker_compose_config; тесты передают fake-каналы вместо monkeypatch (0 патчей).
    """
    which_bin = shutil.which if which_fn is None else which_fn
    compose_config = _shared_docker_compose_config if compose_config_fn is None else compose_config_fn
    # Method 1: docker compose config (resolves anchors, aliases, extends) — shared sole path
    if which_bin("docker"):
        cfg_r = compose_config(
        ...
        if cfg_r.returncode == 0 and cfg_stdout.strip():
            ...
            try:
                # W11: yaml.safe_load returns Any → cast to compose payload boundary (docker emits mapping)
                return cast(dict[str, object], yaml.safe_load(cfg_stdout))
    ...
    try:
        with Path(compose_path).open(encoding="utf-8") as f:
            # W11: yaml.safe_load returns Any → raw guard → cast to compose payload boundary
            data_raw: object = cast(object, yaml.safe_load(f))
...
def analyze_proxy_net(data: dict[str, object]) -> tuple[bool, int, str]:
    """Analyze compose data for proxy-net external:true and service connections."""
    networks_raw: object = data.get("networks", {})
    ...
    networks = cast(dict[str, object], networks_raw)

    proxy_net_raw = networks.get("proxy-net", {})
    if not isinstance(proxy_net_raw, dict):
        return False, 0, "proxy-net is not a valid network entry"
    proxy_net = cast(dict[str, object], proxy_net_raw)
    ...
    services_raw: object = data.get("services", {})
    ...
    for svc_config in services.values():
        if not isinstance(svc_config, dict):
            continue
        svc = cast(dict[str, object], svc_config)
