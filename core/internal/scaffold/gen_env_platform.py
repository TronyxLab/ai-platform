...
##   - ≥8 PLATFORM_* variables (gate test validates; только legacy-путь без enabled_modules —
##     фильтр по ноде осознанно уменьшает набор, ложный WARNING не эмитится)
...
##   - enabled_modules (P2-фикс D6 sync-env, launch-validation asi-team-vps): если задан и
##     непустой → PLATFORM_PROVIDES и per-service HOST/PORT/DSN/URL эмитятся ТОЛЬКО для
##     svc_name ∈ enabled_modules (пересечение с provides-каталогом; nginx попадает через
##     пересечение, принудительной эмиссии НЕТ). None/пустой → legacy-путь (байт-идентично).
##     Сети/volumes emission НЕ фильтруется (остаётся полный каталог — инвариант не трогается).
...
##            P2 (D6): нода без postgres/litellm НЕ обязана информировать проект о них как
##            available — фильтр по enabled-модулям ноды убирает фантомные сервисы из
##            .env.platform и AI-PLATFORM.md; legacy-путь (None) сохраняет байт-совместимость.
...
## @changes  2026-09-01 · P2 D6 (launch-validation asi-team-vps) — +enabled_modules фильтр
##           provides по enabled-модулям ноды (NODE_YAML); None → legacy полный каталог
...
# ═══════════════════════════════════════════════════════════════════
# region FUNC__resolve_enabled_modules_from_node_yaml
## @purpose  P2-fix (D6 sync-env, launch-validation asi-team-vps): enabled-модули ноды из
##           NODE_YAML env — тот же NodeYaml-резолв, что resolve_placement_for_project
##           (канон чтения node.yaml, DevPlan 117 D20). NODE_YAML не задан / нечитаем →
##           None → legacy-путь (полный provides-каталог, обратная совместимость).
## @io       ⇥ (env: NODE_YAML) → ⎋ list[str] | None
## @complexity O(M), M = записи modules в node.yaml
## @invariants
##   - None ТОЛЬКО при недоступности NODE_YAML (нода не определима ≠ 0 enabled-модулей)
##   - enabled-семантика строгая: m.get("enabled") is True (только явные включения;
##     совпадает с [m.name for m in modules if m.enabled] из FIX-спецификации P2)
##   - Записи без name / с пустым name пропускаются (не участвуют в фильтре)
def _resolve_enabled_modules_from_node_yaml() -> list[str] | None:
    """Resolve enabled module names from the NODE_YAML env var (None → legacy full catalog)."""
    node_yaml_path = os.environ.get("NODE_YAML", "")
    if not node_yaml_path:
        logger.info("[IMP:7][gen_env_platform][enabled_modules] NODE_YAML not set — legacy full provides catalog")
        return None
    try:
        from core.internal.shared.node_yaml import NodeYaml
        ...
        logger.warning(
            "[IMP:7][gen_env_platform][enabled_modules] NODE_YAML unreadable (%s) — legacy full provides catalog",
            exc,
        )
        return None
    enabled = [str(m.get("name")) for m in modules if m.get("enabled") is True and str(m.get("name", "")).strip()]
    logger.info(
        "[IMP:8][gen_env_platform][enabled_modules] %d enabled module(s) from %s: %s",
        len(enabled),
        node_yaml_path,
...
# endregion FUNC__resolve_enabled_modules_from_node_yaml
...
## @param enabled_modules Optional enabled-модули ноды (P2 D6): Sequence[str] | None.
##                      Задан и непустой → PLATFORM_PROVIDES/per-service эмитятся только для
##                      пересечения provides-каталога с enabled_modules (фантомных сервисов нет);
##                      None/пустой → legacy-путь (полный каталог, байт-идентично)
...
def generate(
    ...
    Args:
        ...
        enabled_modules: Optional enabled module names of the node (P2 D6). When set and
            non-empty, provides emission is filtered to the intersection with the provides
            catalog — a node without postgres/litellm does not advertise them as available.
            None/empty → legacy path (full catalog, byte-identical).
    ...
    # ⚠️ TRAP[BUG] · 2026-09-01 · P2 · Фантомные сервисы в .env.platform / AI-PLATFORM.md (D6 sync-env)
    # · Symptom: нода asi-team-vps (enabled: nginx, platform-secrets, logging, status-page)
    # ·   информировала о postgres/litellm/... как available — docker ps показывал только
    # ·   roadmap/status-page/nginx/loki; .env.platform содержал несуществующие DSN/URL.
    # · Root: provides-эмиссия брала ВЕСЬ платформенный каталог (postgres, redis, litellm,
    # ·   langfuse, clickhouse, minio, nginx) БЕЗ фильтра по enabled-модулям ноды.
    # · Fix: enabled_modules (пересечение с каталогом); main() резолвит из NODE_YAML тем же
    # ·   NodeYaml-резолвом, что resolve_placement_for_project. None → legacy (байт-идентично).
    # · Prevention: negative-тест (enabled=[nginx,...] → PLATFORM_PROVIDES=nginx) + legacy-тест.
    # PLATFORM_PROVIDES — фильтр по enabled-модулям ноды (P2 D6); None/пустой → полный каталог
    provides_keys = sorted(provides.keys())
    if enabled_modules:
        enabled_set = set(enabled_modules)
        provides_keys = sorted(k for k in provides_keys if k in enabled_set)
        logger.info(
            "[IMP:8][gen_env_platform][generate] provides filtered by node enabled_modules: %s",
            ",".join(provides_keys) or "<none>",
        )
    lines.append(f"PLATFORM_PROVIDES={','.join(provides_keys)}")

    # Per-service HOST/PORT/DSN/URL
    for svc_name in provides_keys:
    ...
    # Validation — порог ≥8 осмыслен ТОЛЬКО для полного каталога (legacy-путь); при
    # enabled_modules-фильтре набор осознанно меньше (нода без сервисов) — WARNING не эмитится
    plat_count = sum(1 for line in lines if line.startswith("PLATFORM_"))
    if not enabled_modules and plat_count < _PLATFORM_VARS_MIN:
...
## @param enabled_modules Optional enabled-модули ноды (P2 D6): Sequence[str] | None.
##                      Задан и непустой → provides фильтруется пересечением с каталогом;
##                      None/пустой → legacy-путь (полный каталог, байт-идентично)
## @return  List of .env.platform lines (newline-terminated)
## @raises FileNotFoundError               If yaml_path does not exist
## @raises yaml.YAMLError                   If YAML content is malformed
...
def generate_env_platform(
    ...
    Args:
        ...
        enabled_modules: Optional enabled module names of the node (P2 D6). When set and
            non-empty, provides emission is filtered to the intersection with the provides
            catalog. None/empty → legacy path (full catalog, byte-identical).
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.placement:
            ...
            if args.target_node:
                consumer_node = args.target_node
            # P2-fix (D6 sync-env): PLATFORM_PROVIDES фильтруется по enabled-модулям ноды —
            # тот же NodeYaml-резолв NODE_YAML, что resolve_placement_for_project; NODE_YAML
            # не задан/нечитаем → None → legacy полный каталог (обратная совместимость).
            enabled_modules = _resolve_enabled_modules_from_node_yaml()
