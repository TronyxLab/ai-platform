...
class ProjectAdopter:
    ...
    def generate_minimal_ai_platform_yaml(self) -> str:
        ...
        data: dict[str, Any] = {
            ...
            "needs": {
                "domain": bool(self.domain),
                "expose": bool(self.domain),
            },
            "monitoring": {
                "metrics": False,
                "logs_retention": "7d",
        ...
        try:
            ...
            logger.info("[IMP:10][%s][gen_yaml] PyYAML not available — cannot write ai-platform.yaml", self._log_prefix)
            return "exists"

        with open(self.yaml_file, "w") as f:
            yaml.dump(data, f, default_flow_style=False, sort_keys=False)

        logger.info("[IMP:9][%s][gen_yaml] ai-platform.yaml generated: %s", self._log_prefix, self.yaml_file)
        logger.info("[IMP:8][%s][gen_yaml]  REVIEW and adjust type/domain/monitoring values", self._log_prefix)
        return "generated"
...
"""
    ...
    def gen_project_makefile(self) -> str:
        ...
        logger.info("[IMP:7][%s][makefile] Generating project Makefile", self._log_prefix)

        # Use tab for Makefile indentation
        tab = "\t"
        makefile_content = f"""# GENERATED by adopt-project.sh — DO NOT EDIT manually
# Project: {self.name}
...
{tab}@$(MAKE) -C $(PLATFORM_DIR) project-sync-env NAME={self.name} DOMAIN={self.domain or ""}
...
{tab}@$(MAKE) -C $(PLATFORM_DIR) project-status NAME={self.name}
...
"""
        ...
        logger.info("[IMP:9][%s][makefile] Project Makefile generated: %s", self._log_prefix, makefile)
        return "generated"
    ...
    def gen_project_agents(self) -> str:
        ...
        if agents_file.exists():
            if not self.force:
                logger.info("[IMP:6][%s][agents] AGENTS.md exists — SKIP (use --force to regenerate)", self._log_prefix)
                return "exists"
            logger.info("[IMP:7][%s][agents] Force mode: overwriting existing AGENTS.md", self._log_prefix)

        logger.info("[IMP:7][%s][agents] Generating project AGENTS.md", self._log_prefix)

        agents_content = f"""# AGENTS.md — {self.name} (ai-platform project)
...
"""
        ...
        logger.info("[IMP:9][%s][agents] Project AGENTS.md generated: %s", self._log_prefix, agents_file)
        return "generated"
