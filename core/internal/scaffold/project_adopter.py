...
##                        тег из github.sha
##           2026-08-25 · DevPlan 14 T1.1 (QA C2/R9) — деплой-канал генерируется ЗАПИНЕННЫМ:
##                        deploy-project.yml AT channel_pin.DEPLOY_CHANNEL_PIN вместо mutable
##                        @main (SoT: channel_pin.py, freshness-гейт G6); actions переведены
##                        на те же full-SHA pins, что в шаблонах; top-level permissions +
##                        concurrency добавлены (REF-0011 паритет: top-level concurrency
##                        reusable-файла при uses НЕ действует на caller'а)
...
# T1.1 (DevPlan 14, QA C2): SoT-пин деплой-канала — единственный источник SHA для генерации;
# freshness-гейт test_gate_workflow_sha_pins сверяет значение с шаблонами и git-историей
...
class ProjectAdopter:
    ...
    def simplify_deploy_yml(self) -> bool:
        ...
        # registry требует lowercase org/repo (TRAP в docker-compose.yml шаблона — --org в lowercase)
        image_ref = f"ghcr.io/{workflow_org}/{self.name}".lower()

        # T1.1 (DevPlan 14, QA C2/R9): генерация = SHA-pin канала (channel_pin SoT) + full-SHA
        # pins actions (паритет шаблона) + top-level permissions/concurrency (REF-0011/0012
        # паритет — reusable top-level concurrency при `uses` на caller'а не действует).
...
# REF-0011: сериализация деплоев проекта. Top-level concurrency reusable-файла при `uses`
# НЕ действует на caller'а — группа задаётся здесь (паритет шаблона).
concurrency:
  group: deploy-${{{{ github.workflow }}}}
  cancel-in-progress: false

# REF-0012 hygiene: минимум прав на уровне workflow; packages:write — только у build-job.
permissions:
  contents: read
...
jobs:
  ...
  deploy:
    ...
    uses: {workflow_org}/ai-platform/.github/workflows/deploy-project.yml@{DEPLOY_CHANNEL_PIN} # {PIN_COMMENT}
    with:
...
"""
        ...
        logger.info(
            "[IMP:9][%s][simplify] deploy.yml simplified with pinned channel %s…: %s",
            self._log_prefix,
            DEPLOY_CHANNEL_PIN[:12],
            self.deploy_yml,
        )
        return True
