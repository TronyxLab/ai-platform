...
class _ProjectInfo(TypedDict):
    """Typed result of find_project_in_node_yaml (W11: no dict[str, Any]).

    Контракт «Returns {} if project not found» сохранён — not-found возвращает {}.
    """
...
class _SshRunner(Protocol):
    """SSH runner injectable for tests: (host, user, cmd, timeout) → (rc, output)."""

    def __call__(self, host: str, user: str, cmd: str, timeout: int = COMPOSE_UP_TIMEOUT) -> tuple[int, str]: ...
...
def find_project_in_node_yaml(
    ...
    yaml_files = sorted(projects_root.glob(search_pattern))
    ...
    for ny in yaml_files:
        ...
        try:
            ...
            # W11: NodeYaml.get_project returns dict[str, object] → cast str fields at boundary
            domain = cast(str, project.get("domain", ""))
            repo = cast(str, project.get("repo", ""))
...
def ssh_compose_down(
    host: str,
    project: str,
    ssh_runner: _SshRunner | None = None,
) -> bool:
    ...
    if ssh_runner is None:
        ...
        def _default_ssh(host: str, user: str, cmd: str, timeout: int = COMPOSE_UP_TIMEOUT) -> tuple[int, str]:
...
# region FUNC_main
# ⚠️ TRAP[BUG] · 2026-08-15 · HI · Namespace-подкласс с class-атрибутом-значением глушит argparse-дефолты
# · Symptom: `projects_root: str = ""` в _RemoverArgs → parse_args(namespace=...) НЕ применял
#   default=_DEFAULT_PROJECTS_ROOT (argparse: `if not hasattr(namespace, dest)` — class-атрибут
#   виден через hasattr) → find_project_in_node_yaml искал в Path("") = cwd → «Found 0 node.yaml»
#   → тихий idempotent-SKIP exit 0 вместо удаления (поймано test_unregister_removes_project_entry).
# · Root: W11-типизация ввела паттерн `attr: Type = value` в 56 Namespace-подклассов.
# · Fix: ClassVar-аннотации БЕЗ значений (hasattr=False → argparse ставит parser-дефолты).
# · Prevention: tests/gates/test_gate_typed_namespace.py (AST-скан, R5-negative-пара) — RED
#   на любой Namespace-подкласс с AnnAssign+значением в core/.
class _RemoverArgs(argparse.Namespace):
...
def main(argv: list[str] | None = None) -> int:
    ...
    args = parser.parse_args(argv, namespace=_RemoverArgs())
    ...
    unregister_from_node_yaml(cast(str, project_info["node_yaml"]), args.name)
    ...
    ssh_done = False
    host = cast(str, project_info.get("host", ""))
    if host:
