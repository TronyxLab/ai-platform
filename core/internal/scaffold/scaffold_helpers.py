...
import yaml  # module-level — except-ветки читают yaml.YAMLError (reportPossiblyUnboundVariable)
...
def gen_ai_platform_yaml(
    ...
    # Write YAML
    # ruff: ignore[PLW0717] — тело try >5 операторов (yaml.dump блок) — извлечение неразумно
    try:
...
## @purpose  Прочитать и провалидировать template.yaml шаблона (DevPlan 141 A7):
##           сверка requires_practices_version против practices_manifest.version.
##           Graceful degradation: отсутствующий файл → {} (не блокирует scaffold).
## @io       ⇥ template_dir: str | Path → ⎋ dict[str, Any]
##           ⚡ raises ConfigValidationError если шаблон требует новее практики
## @complexity O(1) — единичный YAML-load + сверка версий
## @invariants
##   - Отсутствие template.yaml НЕ блокирует scaffold (обратная совместимость)
##   - requires_practices_version > practices_manifest.version → ConfigValidationError
##   - Только чтение — никаких side-эффектов
def read_template_yaml(template_dir: str | Path) -> dict[str, object]:
    ...
    with Path(template_yaml).open(encoding="utf-8") as f:
        # W11: yaml.safe_load returns Any → cast to template.yaml boundary
    ...
    practices = load_manifest()
    # W11: dict[str, object] boundary — requires_practices_version is a YAML int
    required = cast(int, data.get("requires_practices_version", 0))
...
def gen_project_makefile(
    ...
    # ⚠️ doxygen-safe: НЕ f-string целиком (парсер ломается на парах "..." строк без
    # f-string-интерполяций внутри) — список строк + join (паттерн render_test_health).
    tab = "\t"
    makefile_lines: list[str] = [
        ...
        "sync-env:",
        f'{tab}@echo "[IMP:7][project] Syncing .env.platform..."',
        f"{tab}@$(MAKE) -C $(PLATFORM_DIR) project-sync-env PROJECT=$(CURDIR)",
        f'{tab}@echo "[IMP:9][project] .env.platform sync complete"',
        "",
        "## status: Show live project status from target node",
        "status:",
        f"{tab}@$(MAKE) -C $(PLATFORM_DIR) project-status PROJECT=$(CURDIR)",
        "",
        "## project-check: Run practices checks (K1) — автофикс: project-check --fix",
        "project-check:",
        f"{tab}@$(MAKE) -C $(PLATFORM_DIR) project-check PROJECT=$(CURDIR)",
        "",
        "## project-sync-practices: Regenerate GENERATED practices files",
        "project-sync-practices:",
        f"{tab}@$(MAKE) -C $(PLATFORM_DIR) project-sync-practices PROJECT=$(CURDIR)",
        "",
        "## project-set-practices: Set practices level (LEVEL=baseline|full|auto)",
        "project-set-practices:",
        f"{tab}@$(MAKE) -C $(PLATFORM_DIR) project-set-practices PROJECT=$(CURDIR) LEVEL=$(LEVEL)",
        "",
        "## ai-sync: Rebuild project instructions (.kilo/) from live canon (DevPlan 001 T5.3)",
        "ai-sync:",
        f"{tab}@$(MAKE) -C $(PLATFORM_DIR) ai-instructions-sync PROJECT=$(CURDIR)",
        "",
        "## dev: Run app against platform services (requires 'make up' in ai-platform)",
        "dev:",
        f"{tab}@echo \"Starting dev app against platform services (requires 'make up' in ai-platform)...\"",
        f"{tab}docker compose -f docker-compose.dev.yml up -d",
        f'{tab}@echo "Dev app ready on http://localhost:8000. Ensure .env.platform is synced (make sync-env)."',
        "",
        "## help: Show all available commands",
        "help:",
        f"{tab}@grep -E '^## ' $(MAKEFILE_LIST) | column -t -s ':'",
        "",
    ]
    makefile_content = "\n".join(makefile_lines)
...
See `.env.platform` for exact host/port/DSN/URL (reference: `.env.example`).
...
- `make project-check` — run practices checks (K1)
- `make project-check --fix` — auto-fix practices checks
- `make project-sync-practices` — regenerate GENERATED practices files
- `make project-set-practices` — set practices level (LEVEL=baseline|full|auto)
- `make dev` — run app against platform services (backend; requires platform stack)
...
def gen_project_platform_md(
    ...
    # ── Resolve node.yaml: PROJECTS_BASE/org/node-configs/node/node.yaml ──
    node_yaml_path = ""
