...
##   - resolve_node_configs_dir: projects/<org>/node-configs → PROJECTS_BASE env fallback
...
##           2026-08-14 · DevPlan 167 D2 — CommandRunner-seam: runner/update_yaml/resolve_configs
##           DI-параметры (0 monkeypatch в тестах)
...
## @param runner            CommandRunner-seam (DevPlan 167 D2): subprocess.run fake (None → real)
## @param update_yaml       DI-fn: update_yaml_for_vhost seam (None → реальная функция)
## @param resolve_configs   DI-fn: resolve_node_configs_dir seam (None → реальная функция)
...
def configure_vhost(
    ...
    *,
    ...
    update_yaml: Callable[..., None] | None = None,
    resolve_configs: Callable[..., Path | None] | None = None,
) -> bool:
    ...
    # 🧐 TRAP[DI-SEAM] · 2026-08-14 · — · DI-швы configure_vhost: runner/update_yaml/resolve_configs
    # · Rejected: прямой вызов (тест патчил vc.subprocess.run / vc.update_yaml_for_vhost /
    # ·   vc.resolve_node_configs_dir через monkeypatch.setattr)
    # · Reason: seam = тестируемость реального вызова (fake-runner/fake-fn параметром, 0 патчей)
    # · Rev: при введении общего CommandRunner (subprocess_io) — консолидировать runner-швы
    update_fn = update_yaml if update_yaml is not None else update_yaml_for_vhost
    update_fn(yaml_file, domain, log_prefix=log_prefix)
    ...
    return configure_vhost_via_subprocess(
        project_dir,
        domain,
        org,
        node_configs_dir,
...
## @complexity O(1)
# region FUNC__plw_body_update_yaml_for_vhost
## @purpose  Тело try-блока (PLW0717 extraction из update_yaml_for_vhost) — семантика except не меняется.
## @io       ⇥ domain, log_prefix, shared_project_yaml, yaml, yaml_file → ⎋ результат try-тела
## @complexity O(1) — извлечение управляющего потока
def _plw_body_update_yaml_for_vhost(domain: str, log_prefix: str, yaml_file: Path) -> None:
    import yaml
...
# endregion FUNC__plw_body_update_yaml_for_vhost
...
def update_yaml_for_vhost(yaml_file: Path, domain: str, log_prefix: str = "adopt") -> None:
    ...
    try:
        _plw_body_update_yaml_for_vhost(domain, log_prefix, yaml_file)
...
def configure_vhost_via_subprocess(
    ...
    *,
    ...
    if node_configs_dir is None:
        resolve_fn = resolve_configs if resolve_configs is not None else resolve_node_configs_dir
        node_configs_dir = resolve_fn(project_dir, org=org)
...
## @purpose  Derive node-configs path: projects/{org}/node-configs/ (или PROJECTS_BASE env).
## @param project_dir  Project root directory
...
def resolve_node_configs_dir(project_dir: Path, org: str) -> Path | None:
    ...
    # Try alternative: PROJECTS_BASE env var
    projects_root_env = os.environ.get("PROJECTS_BASE")
