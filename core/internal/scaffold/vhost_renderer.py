...
##           - add-vhost.sh (shell facade → exec python3 -m vhost_renderer, DevPlan 173 W2.3)
...
## @rationale (D2) FQDN-conflict проверка — check_duplicate_domains в Python покрывает её.
...
# NOTE: канон ProjectEntry — core.internal.shared.node_yaml.ProjectEntry (единственное определение в core/).
...
def load_vhost_config(project_dir: str) -> ProjectConfig | None:
    ...
    ## @purpose — Parse project YAML for vhost eligibility (единственный reader — shared project_yaml).
    ##            Имя read_project_yaml зарезервировано за shared/project_yaml каноном.
    ...
    # W11: cfg is dict[str, object] → cast str fields at boundary
    domain = cast(str, cfg.get("domain", ""))
    target_node = cast(str, cfg.get("target_node", ""))
    ...
    logger.info("[IMP:9][load_vhost_config] Parsed: expose=true, domain=%s, target_node=%s", domain, target_node)
...
server {{
    ...
    location /health {{
        # Переменная обязана быть определена В ЭТОМ location (set в / не виден —
        # location-скоуп); без set $upstream_* пуст → proxy_pass "" → 500.
        set $upstream_{nginx_safe_name} http://{project_name}:80;
        proxy_pass $upstream_{nginx_safe_name};
        access_log off;
    }}
...
# region FUNC_validate_vhost_identifiers

# H18 (security hardening): строгая валидация fqdn/project_name ПЕРЕД подстановкой в nginx-конфиг
# f-string и в имя выходного файла <fqdn>.conf. RFC-совместимый label: [a-z0-9] start/end,
# [a-z0-9-] middle, ≤63; TLD — только буквы (≥2). Rejects config-injection (${...}, ;, пробелы)
# и path traversal (../). fqdn уже lower() на входе render_vhost.
_FQDN_LABEL_RE = re.compile(r"^[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?$")
_TLD_RE = re.compile(r"^[a-z]{2,}$")
# project_name: тот же класс, что validate_project_name (^[a-zA-Z0-9][a-zA-Z0-9_-]*$) —
# БЕЗ verb-reserve (имя уже проверено на scaffold), но БЕЗ инжекционных символов.
_PROJECT_NAME_RE = re.compile(r"^[a-zA-Z0-9][a-zA-Z0-9_-]*$")
_MIN_FQDN_LABELS = 2


def validate_vhost_identifiers(fqdn: str, project_name: str) -> None:
    """Strict validation of fqdn/project_name before nginx substitution (H18).

    ▶ ┌{fqdn, project_name}┐ → ◇ fqdn labels (≥2, RFC-label, TLD alpha) → ◇ project_name class
    → ⊕ ConfigValidationError (exit 4) | ⎋ None (PASS)

    ## @purpose — Block config-injection/path traversal через fqdn/project_name (H18, security
    ##            hardening). nginx -t ловит синтаксис, но не семантику — `${...}` в f-string
    ##            подставляется ДО nginx и даёт произвольный конфиг; `../` в fqdn = path traversal
    ##            в <fqdn>.conf.
    ## @io — ⇥ fqdn: str (already lowercase), project_name: str
    ##       → ⊕ ConfigValidationError on violation | ⎋ None on PASS
    ## @complexity — O(N) — regex per label
    ## @invariants
    ##   - fqdn: ≥2 labels, каждый — RFC DNS label ([a-z0-9] start/end, ≤63), TLD — [a-z]{2,}
    ##   - project_name: ^[a-zA-Z0-9][a-zA-Z0-9_-]*$ (инжекционные символы запрещены)
    ##   - Violation → ConfigValidationError (exit 4, не тихий рендер)
    ## @rationale  Аудит 2026-08-15 H18: fqdn/project_name в nginx f-string без валидации формата.
    """
    labels = fqdn.split(".")
    if len(labels) < _MIN_FQDN_LABELS:
        msg = f"Invalid FQDN (single label, no TLD): {fqdn!r}"
        raise ConfigValidationError(msg)
    for label in labels:
        if not _FQDN_LABEL_RE.match(label):
            msg = f"Invalid FQDN label {label!r} in {fqdn!r}"
            raise ConfigValidationError(msg)
    if not _TLD_RE.match(labels[-1]):
        msg = f"Invalid FQDN TLD {labels[-1]!r} in {fqdn!r}"
        raise ConfigValidationError(msg)
    if not _PROJECT_NAME_RE.match(project_name):
        msg = f"Invalid project name for vhost: {project_name!r}"
        raise ConfigValidationError(msg)


# endregion FUNC_validate_vhost_identifiers
...
def render_vhost(
    ...
    # H18 (security hardening): строгая валидация fqdn/project_name ДО подстановки в nginx-конфиг.
    validate_vhost_identifiers(fqdn, entry.name)
...
def remove_vhost(project_name: str, overlays_dir: str, platform_root: str | None = None) -> bool:  # ruff: ignore[ARG001] — keyword-контракт (platform_root резерв совместимости)
...
def configure_vhost_for_project(
    ...
    ## @purpose — Python-first API для adopt-project (D4 primary path). Реализует
    ##            configure_vhost без subprocess add-vhost.sh.
...
def render_all(
    ...
    ##   - dev_domain_suffix=None/"" and output_dir=None → behavior byte-for-byte
    """
...
# ── CLI-фасад (T3.7 god-file trim) ────────────────────────────────────────────────
# CLI-блок (build_parser/_normalize_mode/main, ~230 LOC) переехал в vhost_cli.py.
# РЕБРО ОТСУТСТВУЕТ: ядро НЕ импортирует CLI ни статически, ни лениво (acyclic KEPT);
# CLI импортирует ядро лениво внутри main(). Точка входа `-m vhost_renderer`
# резолвит CLI через import_module (dotted name, обычная система импорта — не DEP-0018).
if __name__ == "__main__":
    # 🧐 TRAP[DECISION] · 2026-08-22 · — · CLI-фасад через import_module, не статический импорт
    # · Rejected: from .vhost_cli import main (даже в __main__-гварде) — grimp/import-linter
    # ·   считает гвард-импорт ребром → цикл vhost_renderer⇄vhost_cli (RED acyclic).
    # · Reason: CLI импортирует ядро лениво внутри main(); обратное ребро держим динамическим.
    # ·   import_module — обычная система импорта (dotted name), НЕ DEP-0018 by-path loading.
    # · Rev: если CLI переедет в отдельный домен или ядро перестанет быть нужен CLI — статика ок.
    import importlib

    _cli = importlib.import_module("core.internal.scaffold.vhost_cli")
    sys.exit(_cli.main())  # pyright: ignore[reportAny] — ModuleType attr (динамический фасад)
