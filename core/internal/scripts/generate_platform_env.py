...
##           единственный data-артефакт; env_defaults_generated.py — re-export шим, DevPlan 171 W1.3).
##           Собирает статическую инфраструктуру (networks, volumes,
...
##   - smoke_env_generated.py — единственный носитель ci_default-значений; helpers-файл
##     проецирует их в _-константы (литералы значений НЕ дублируются, DevPlan 171 W1.3)
...
##           2026-08-15 | DevPlan 171 W1.3 — helpers → re-export shim (smoke = единственный
##                      data-артефакт)
...
# W11: infra/platform-env YAML payloads — opaque mappings on the data boundary.
_InfraData = dict[str, object]
...
def load_infra(infra_path: Path) -> _InfraData:
    ...
    if not infra_path.is_file():
        msg = f"Infra file not found: {infra_path}"
        raise FileNotFoundError(msg)

    with Path(infra_path).open(encoding="utf-8") as f:
        # W11: yaml.safe_load returns Any → cast to infra boundary
        data: _InfraData | None = cast(_InfraData | None, yaml.safe_load(f))

    if not isinstance(data, dict):
        msg = f"Infra file {infra_path} is not a valid YAML dict"
    ...
    logger.info(
        ...
        len(cast(list[object], result["networks"])),
        len(cast(list[object], result["volumes"])),
        len(cast(list[object], result["provides"])),
    )
    return result
...
def load_ci_defaults(secret_defs_path: Path | str) -> dict[str, str]:
    ...
    with Path(secret_defs_path).open(encoding="utf-8") as f:
    ...
    for secret in cast(list[object], secrets):
        if not isinstance(secret, dict):
            continue
        secret_typed = cast(dict[str, object], secret)
        name = secret_typed.get("name", "")
        ci_val = secret_typed.get("ci_default")

        if name and ci_val is not None and ci_val:
            # W11: name is a YAML string key — cast (no runtime coercion)
            ci_defaults[cast(str, name)] = str(ci_val)
...
def generate_helpers_py(ci_defaults: dict[str, str]) -> str:
    """Generate Python re-export shim over SMOKE_ENV_GENERATED (DevPlan 171 W1.3).

    ## @purpose  env_defaults_generated.py — тонкий re-export шим: значения живут ТОЛЬКО
    ##            в smoke_env_generated.py (SMOKE_ENV_GENERATED), шим проецирует их в
    ##            _-префиксные константы для backward compatibility (helpers/__init__.py,
    ##            _conftest/env.py fallback). Литералы значений НЕ дублируются.
    ## @io        ⇥ ci_defaults: dict[str, str] → ⎋ str: Python module content (re-export shim)
    ## @complexity O(K) where K = number of ci_defaults
    ## @invariants
    ##   - Каждый источник _<SECRET> = _SMOKE_ENV["<SECRET>"] (без литералов значений)
    ##   - Includes __all__ list for clean imports
    ##   - Детерминирован: sorted keys
    ## @changes  2026-08-15 | DevPlan 171 W1.3 — был самостоятельный GENERATED-артефакт
    ##            значений; теперь re-export шим (единственный data-артефакт — smoke)
    """
    lines: list[str] = [
    ...
    lines.extend(f'_{key}: str = _SMOKE_ENV["{key}"]' for key in sorted(ci_defaults.keys()))
...
def _check_generated_content(content: str, path: Path, label: str) -> int:
    ...
    ##            полный unified diff вместо среза [:20]; rc-семантика --check сохранена.
    ...
    return check_generated(path, content, to_label=f"{label} (generated)")
...
class _GenEnvArgs(argparse.Namespace):
    ...
    output: ClassVar[str]
    smoke_env_output: ClassVar[str | None]
    helpers_output: ClassVar[str | None]
    check: ClassVar[bool]
...
def main(
    ...
    # ── Load static infra ──
    infra: _InfraData = load_infra(infra_path)
    ...
    non_secret: dict[str, str] = {k: str(v) for k, v in cast(dict[str, object], infra.get("env_defaults", {})).items()}
    ...
    # ⚠️ TRAP[DECISION] · 2026-08-13 · MED · builtins.open вместо Path.open (DevPlan 163 W-G)
    # · Rejected: Path(output_path).open("w") (PTH118-автофикс W-B) — pathlib держит прямую ссылку
    # ·   на io.open и НЕ перехватывается fault-injection → seam гейта ломается (R1:
    # ·   "генератор не писал через перехватываемые пути")
    # · Reason: тестовый seam (краш после первой записи) — контракт гейта; PTH118 для этого
    # ·   файла в per-file-ignores ruff.toml.new (см. ruff_policy.md)
    # · 2026-08-14 | DevPlan 167 D1 — механизм мигрировал с патча builtins.open на open-fn DI:
    # ·   write-сайты используют open_fn-параметр (гейт передаёт fault-wrapped open напрямую).
    # ·   Rev: если fault-injection-механизм гейта мигрирует на Path-перехват — вернуть Path.open
    ...
    if smoke_content is not None and smoke_env_path is not None:
        ...
        with open_impl(smoke_env_path, "w", encoding="utf-8") as f:  # open_fn DI seam (DevPlan 167 D1)
    ...
    if helpers_content is not None and helpers_path is not None:
        ...
        with open_impl(helpers_path, "w", encoding="utf-8") as f:  # open_fn DI seam (DevPlan 167 D1)
