...
def generate_env_example(env_defaults: dict[str, str], secret_defs: dict[str, dict[str, str]]) -> str:
    ...
    # ── Header ──
    lines.append("# GREP_SUMMARY: env-example docker-compose variables grouped-by-module platform context platform-secrets postgres redis clickhouse minio s3 litellm langfuse hermes telegram nginx webnames ssl dns proxy monitoring compose-profiles misc")
    lines.append("# STRUCTURE: Platform/Context → Platform secrets → Postgres → PgBouncer → Redis → ClickHouse → MinIO → S3/Backup → LLM Provider API Keys → LiteLLM → Langfuse → Hermes Dashboard → Hermes API → Telegram → Nginx → SSL/DNS (webnames) → Proxy → Monitoring/Observability → Compose Profiles → Misc")
    lines.append("")
    lines.append("# region MODULE_CONTRACT")
    lines.append("## @purpose  Env-шаблон для docker compose — единый источник всех переменных окружения для platform-модулей.")
    lines.append("##           Канонический документ для локальной разработки и CI тестов.")
    lines.append("## @scope    Все compose-переменные, сгруппированные по модулям: postgres, pgbouncer, redis, clickhouse,")
    lines.append("##           minio, s3/backup, litellm, langfuse, hermes-agent, telegram, nginx, proxy, monitoring, misc.")
    lines.append("## @invariants")
    lines.append("##   1. ⚠️ CONSTRAINT-комментарии — единый формат \\`# ⚠️ CONSTRAINT: <VAR> must match <regex>\\` для всех")
    ...
    lines.append("##   6. GENERATED by sync_env_defaults.py — DO NOT EDIT. SoT: platform-env.yaml env_defaults section.")
    lines.append("## @rationale Charset constraint предотвращает баг pgbouncer-краша при спецсимволах в POSTGRES_PASSWORD")
    ...
    lines.append("##   2026-07-21 — TASK-3: добавлены CONSTRAINT-комментарии для POSTGRES_PASSWORD, CLICKHOUSE_PASSWORD,")
    lines.append("##               MINIO_ROOT_USER, MINIO_ROOT_PASSWORD, PLATFORM_MASTER_PASSWORD, HERMES_DASHBOARD_PASSWORD")
    lines.append("##   2026-07-22 — Добавлен WEBNAMES_API_KEY (SSL/DNS Challenge секция) для acme.sh DNS-01 wildcard TLS")
    ...
    lines.append("# Master credentials — unified auth for all platform services (status-page, Prometheus, Loki, Grafana, Langfuse, Hermes).")
    lines.append("# Overridable per-service via service-specific env var (e.g. GF_SECURITY_ADMIN_PASSWORD overrides PLATFORM_MASTER_PASSWORD for Grafana).")
    ...
    lines.append("AGE_SECRET_KEY=" + get_val("AGE_SECRET_KEY", "AGE-SECRET-KEY-TEST1234567890ABCDEFGHIJKLMNOPQRSTUVWXYZ"))
    ...
    lines.append("# Токен для Prometheus metrics scraping (LiteLLM /metrics endpoint)")
    lines.append("LITELLM_METRICS_TOKEN=" + get_val("LITELLM_METRICS_TOKEN", ""))
    ...
    lines.append("NO_PROXY=" + get_val("NO_PROXY", "localhost,127.0.0.1,.local,postgres,pgbouncer,redis,clickhouse,litellm,langfuse,minio,grafana,prometheus"))
    ...
    lines.append("COMPOSE_PROFILES=" + get_val("COMPOSE_PROFILES", "postgres,redis,nginx,clickhouse,backup-cron,hermes-agent,monitoring,logging,litellm,langfuse,infra-metrics,minio"))
    ...
    lines.append("# CI_DEPLOY_KEY — SSH deploy key for ci-deploy forced-command (used by deploy-project.yml, platform-deploy.yml)")
    lines.append("# DOCKER_HUB_USERNAME — Docker Hub username for authenticated pulls (used by platform-test.yml, platform-deploy.yml)")
    lines.append("# DOCKER_HUB_TOKEN — Docker Hub PAT for authenticated pulls (used by platform-test.yml, platform-deploy.yml)")
    lines.append("# SSH_HOST — SSH hostname passed as workflow_call secret (used by platform-deploy.yml)")
    lines.append("# SSH_KEY — SSH private key for ci-deploy passed as workflow_call secret. ≡ CI_DEPLOY_KEY (один ключ, две роли: rsync + forced-command)")
    ...
    lines.append("")
    lines.append("# GIT_MIRROR_TOKEN — Token for git mirror operations (optional, SSH fallback — used by platform-deploy.yml)")
...
def main() -> None:
    ...
    if args.check:
        ...
        if existing != generated:
            ...
            logger.error("[IMP:9][sync_env][CHECK] Divergence detected — .env.example is stale. Run: make sync-env-defaults")
            sys.exit(2)
