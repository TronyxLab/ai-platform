...
def validate(path: pathlib.Path) -> bool:
    ...
    if not isinstance(data, dict):
        print(f"[IMP:10][dora] ERROR: Dashboard root is not a JSON object (got {type(data).__name__})", file=sys.stderr)
        return False
    ...
    found = {p.get("title", "") for p in panels if isinstance(p, dict)}
