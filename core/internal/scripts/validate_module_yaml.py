...
# W11: YAML/JSON payloads — recursive union on the data boundary (reportExplicitAny=error).
YamlScalar: TypeAlias = str | int | float | bool | None
YamlData: TypeAlias = YamlScalar | list["YamlData"] | dict[str, "YamlData"]
...
def _load_yaml_file(path: Path) -> YamlData:
    """Load YAML file with safe_load. Raises FileNotFoundError/YAMLError on failure."""
    if not path.exists():
        msg = f"[validate_module_yaml] File not found: {path}"
        raise FileNotFoundError(msg)
    with Path(path).open(encoding="utf-8") as f:
        # W11: yaml.safe_load returns Any in stubs → cast to YamlData boundary type
        return cast(YamlData, yaml.safe_load(f))


def _load_json_file(path: Path) -> YamlData:
    """Load JSON file. Raises FileNotFoundError/JSONDecodeError on failure."""
    if not path.exists():
        msg = f"[validate_module_yaml] Schema file not found: {path}"
        raise FileNotFoundError(msg)
    with Path(path).open(encoding="utf-8") as f:
        # W11: json.load returns Any in typeshed → cast to YamlData boundary type
        return cast(YamlData, json.load(f))


def _normalize_env_requires_entry(entry: YamlData) -> dict[str, YamlData]:
    ...
    if isinstance(entry, dict):
        ...
        if not name or not isinstance(name, str):
            msg = f"env_requires object missing 'name' field: {entry}"
    ...
    msg = f"env_requires entry must be string or object, got {type(entry).__name__}: {entry}"
    raise ConfigValidationError(msg)
...
def _is_critical_with_always_carveout(module: dict[str, YamlData], service_restart: str) -> bool:
...
def load_module(path: Path) -> dict[str, YamlData]:
    ...
    try:
        ...
        msg = f"[load_module] YAML parse error in {path}: {e}"
        raise ConfigValidationError(msg) from e
    if not isinstance(module, dict):
        msg = f"[load_module] module.yaml must be a mapping, got {type(module).__name__}: {path}"
        raise ConfigValidationError(msg)
    env_req = module.get("env_requires", [])
    if env_req is None:
        env_req = []
    if not isinstance(env_req, list):
        msg = f"[load_module] env_requires must be a list, got {type(env_req).__name__}: {path}"
        raise ConfigValidationError(msg)
    normalized: list[YamlData] = []
    for entry in env_req:
        try:
            normalized.append(_normalize_env_requires_entry(entry))
        except ValueError as e:  # ruff: ignore[PERF203]
            msg = f"[load_module] {path}: {e}"
            raise ConfigValidationError(msg) from e
...
def validate_schema(module: dict[str, YamlData], schema_path: Path = DEFAULT_SCHEMA_PATH) -> list[str]:
    ...
    try:
        # W11: JSON schema from file is a nested mapping — cast YamlData to jsonschema.Schema-compatible shape
        jsonschema.validate(module, cast(dict[str, YamlData], schema))
...
def check_env_requires_presence(
    ...
    # W11-G1 cross-file: shared env_requires expects dict[str, object]; module dict is
    # dict[str, YamlData] (invariant value type) → cast at the delegation boundary
    return _impl(cast(dict[str, object], module), env_example_path, secrets_manifest_path)
...
def check_restart_drift(module: dict[str, YamlData], compose_base_path: Path) -> list[str]:
...
# region FUNC_main
class _ValidatorArgs(argparse.Namespace):
    ...
    all: ClassVar[bool]
    module: ClassVar[str | None]
    schema_strict: ClassVar[bool]
    schema_path: ClassVar[Path]
    modules_dir: ClassVar[Path]
    env_example: ClassVar[Path | None]
    secrets_manifest: ClassVar[Path]
...
def main(argv: list[str] | None = None) -> int:
    ...
    args = parser.parse_args(argv, namespace=_ValidatorArgs())
    ...
    if args.all:
        ...
        # W11: --all | --module are mutually exclusive (required=True) → module name is set here
        target = modules_dir / cast(str, args.module) / "module.yaml"
