...
# Scalar+container union covers everything yaml.safe_load / json.load can produce.
...
def yaml_get(path: pathlib.Path, key: str, default: YamlScalar | None = None) -> YamlData:
...
def yaml_query(path: pathlib.Path, jq_filter: str) -> YamlData:
...
def json_get(path: pathlib.Path, key: str, default: YamlScalar | None = None) -> YamlData:
    """Get value from JSON file by dotted-key path."""
    with pathlib.Path(path).open(encoding="utf-8") as f:
        # W11: json.load has no return annotation in typeshed → cast to YamlData boundary type
        data = cast(YamlData, json.load(f))
    return _dotted_get(data, key, default)
...
def _load_yaml(path: pathlib.Path) -> YamlData:
    if not path.exists():
        msg = f"[IMP:9][yaml_query] file not found: {path}"
    ...
    with pathlib.Path(path).open(encoding="utf-8") as f:
        try:
            ...
            msg = f"[IMP:9][yaml_query] malformed YAML in {path}: {e}"
            raise yaml.YAMLError(msg) from e


def _dotted_get(data: YamlData, key: str, default: YamlScalar | None = None) -> YamlData:
    current: YamlData = data
    for part in key.split("."):
        if isinstance(current, dict) and part in current:
            ...
            if default is not None:
                return default
            msg = f"[IMP:9][yaml_query] key not found: {key}"
            raise KeyError(msg)
    return current


def _jq_eval(data: YamlData, jq_filter: str) -> YamlData:
    ...
    current: YamlData = data
    for raw_segment in jq_filter.strip().lstrip(".").split("|"):
        segment = raw_segment.strip()
        if segment.endswith("[]"):
            key = segment[:-2]
            current = _dotted_get(current, key) if key else current
            if not isinstance(current, list):
                msg = f"[IMP:9][yaml_query] expected list for {segment}, got {type(current).__name__}"
                raise TypeError(msg)
...
def _format_item(item: YamlData, _json_output: bool = False) -> str:
...
class _CliArgs(argparse.Namespace):
    ...
    file: ClassVar[pathlib.Path | None]
    stdin: ClassVar[bool]
    get: ClassVar[str | None]
    query: ClassVar[str | None]
    default: ClassVar[str | None]
    json_output: ClassVar[bool]
    items: ClassVar[bool]
    keys: ClassVar[bool]
    ...
    try:
        ...
        if args.stdin:
            ...
            if not raw:
                ...
                return 2
            try:
                # W11: json.loads returns Any in typeshed → cast to YamlData boundary type
                data: YamlData = cast(YamlData, json.loads(raw))
            ...
            # W11: --stdin and --file are mutually exclusive (required=True) → file is set here
            data = _load_yaml(cast(pathlib.Path, args.file))
            if isinstance(data, dict):
                for k in data:
                    print(k)
                return 0
            ...
            # W11: --stdin and --file are mutually exclusive (required=True) → file is set here
            value = yaml_get(cast(pathlib.Path, args.file), args.get, args.default)
        elif args.query:
            # W11: --stdin and --file are mutually exclusive (required=True) → file is set here
            value = yaml_query(cast(pathlib.Path, args.file), args.query)
        else:
