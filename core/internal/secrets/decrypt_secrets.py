...
def resolve_enc_path(enc_path: str | None, *, secrets_dir: str = _NODE_CONFIGS_SECRETS_DIR) -> str:
    """Resolve encrypted secrets file path (env → explicit → node-configs glob fallback).

    ▶ ┌enc_path┐ → ◇ isfile? → ⎋ enc_path · ◇ glob <secrets_dir>/*.enc.yaml → ⊕ первый match → ⎋ · ✗ → raise

    ## @purpose — Порт резолва SECRETS_FILE из удалённого decrypt-secrets.sh (DevPlan 173 W1.3):
    ##            точный путь (или SECRETS_FILE env) → иначе <secrets_dir>/*.enc.yaml.
    ...
    ##   - glob детерминированный (sorted) — первый match, как `matches[0]` в shell
    ##   - Ни один источник не дал файл → FileNotFoundError (exit 1 в main)
    """
    ...
    msg: str
    if enc_path:
        msg = f"Encrypted secrets file not found: {enc_path} (and no {secrets_dir}/*.enc.yaml fallback)"
    else:
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        "then /opt/node-configs/secrets/*.enc.yaml glob (DevPlan 173 W1.3).",
    )
    parser.add_argument(
        "output_path",
        nargs="?",
    ...
    # env/positional → точный путь → /opt/node-configs/secrets/*.enc.yaml glob.
    try:
