...
| `subprocess_io.py` | Единый канон run_subprocess (DevPlan 118 C10 — дедупликация lifecycle/helpers/subprocess_io.py raise-семантики и converge/infra.py graceful-семантики; обе выражаются параметрами check/non_fatal) | `run_subprocess(cmd, *, timeout, check, non_fatal)` | converge/infra (делегирование, check=False) |
