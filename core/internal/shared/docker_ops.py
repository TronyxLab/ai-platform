...
##   7. W4d DI: runner: CommandRunner | None = None во всех публичных функциях — дефолт (None) →
##      прямой subprocess.run (поведение НЕ изменено: env/exception-семантика сохранены);
##      runner задан (тесты) → runner.run(cmd, timeout=..., check=False) — fake scripted
##      (env при runner не передаётся — fake-раннер не использует env).
...
_EXEC_ARGS_MIN: int = 2  # exec: <container> <command>
...
def _run_docker(
    ...
    ▶ ┌cmd, timeout, env, runner┐ → ◇ subprocess.run(cmd, capture, text, timeout) → ⎋ CompletedProcess|None
    ...
    ##            W4d: runner задан (тесты) → runner.run(cmd, timeout, check=False) — fake scripted
    ##            (канон CommandRunner W4b); env при runner не передаётся (fake не использует env).
    ## @io — ⇥ cmd: list[str], timeout: int, env: dict|None, runner: CommandRunner | None
    ...
    ##   - runner≠None: семантика канона subprocess_io check=False (graceful rc=127/124) —
    ##     fake имитирует failed-запуск rc=127/124 (НЕ None) для non-fatal кейсов.
    """
...
# ruff: ignore[A002]  # `all`/`format` = canonical docker CLI flag names (--all/-a, --format), public keyword API
def docker_ps(
    *,
    all: bool = False,
    quiet: bool = False,
    ...
    ▶ ┌all, quiet, filters, format, runner┐ → ◇ build cmd → _run_docker → ⎋ CompletedProcess (never raise)

    ## @purpose — Единая точка `docker ps` (списки контейнеров: stale-cleanup, orphan-реконсиляция,
    ...
    ##       timeout: int, runner: CommandRunner | None → ⎋ CompletedProcess[str] (failed при сбое запуска)
...
# ruff: ignore[A002]  # `all` = canonical docker CLI flag name (--all/-a), public keyword API
def ps_container_names(
    *, all: bool = False, timeout: int = DOCKER_CMD_TIMEOUT, runner: CommandRunner | None = None
) -> list[str]:
    """Return container names from docker ps (--format {{.Names}}).

    ▶ ┌all┐ → docker_ps(format="{{.Names}}") → ○ splitlines → ⊕ non-empty → ⎋ list[str]

    ## @purpose — Typed helper для списков имён контейнеров (stale-cleanup, orphan-реконсиляция,
    ##            observability, converge/vhosts). Сбой → [] (graceful).
    ## @io — ⇥ all: bool, timeout: int, runner: CommandRunner | None → ⎋ list[str] (empty on failure)
    ## @complexity — O(L) — L строк docker ps
    ## @invariants — Non-fatal: возвращает [] при недоступности docker/таймауте
    """
    result = docker_ps(all=all, format="{{.Names}}", timeout=timeout, runner=runner)
    if result.returncode != 0:
...
def docker_inspect(
    ...
    ▶ ┌identifier, format, runner┐ → ◇ build cmd → _run_docker → ⎋ CompletedProcess (never raise)

    ## @purpose — Единая точка `docker inspect` (State.Health, State.Restarting, ...).
    ## @io — ⇥ identifier: str, format: str | None, timeout: int, runner: CommandRunner | None
...
def docker_inspect_many(
    ...
    ▶ ┌identifiers, format, runner┐ → _run_docker(["docker","inspect",(--format F),*ids]) → ⎋ CompletedProcess (never raise)

    ## @purpose — Batch docker inspect (docker_collector, security_posture: O(N) → O(1)).
    ## @io — ⇥ identifiers: list[str], format: str | None, timeout: int, runner: CommandRunner | None
    ##       ⎋ CompletedProcess[str]
...
def inspect_state_health(
    ...
    ▶ ┌identifier, runner┐ → docker_inspect(format="{{.State.Status}}|{{.State.Health.Status}}") →
    │   partition("|") → ⎋ (state, health)

    ## @purpose — Единый источник State.Status|State.Health.Status (healthcheck_poll, D5 канон).
    ## @io — ⇥ identifier: str, timeout: int, runner: CommandRunner | None → ⎋ tuple[str, str] (state, health)
    ## @complexity — O(1) + docker inspect I/O
...
def docker_exec(
    ...
    ▶ ┌container, command, runner┐ → ◇ build cmd → _run_docker → ⎋ CompletedProcess (never raise)

    ## @purpose — Единая точка `docker exec` (nginx -t / nginx -s reload / service checks).
    ## @io — ⇥ container: str, command: list[str], timeout: int, runner: CommandRunner | None
    ##       ⎋ CompletedProcess[str]
...
def docker_logs(
    ...
    ▶ ┌container, tail, runner┐ → _run_docker(["docker","logs","--tail",N,C]) → ⎋ CompletedProcess (never raise)
    ...
    ## @io — ⇥ container: str, tail: int (строк), timeout: int, runner: CommandRunner | None
    ##       ⎋ CompletedProcess[str]
...
def docker_stop(container: str, timeout: int = DOCKER_STOP_TIMEOUT, runner: CommandRunner | None = None) -> bool:
    """Stop a container by name (docker stop, graceful window DOCKER_STOP_TIMEOUT).

    ▶ ┌container, runner┐ → _run_docker(["docker","stop",C]) → ◇ rc==0? → ⎋ bool

    ## @purpose — Единая точка `docker stop` (stale-cleanup, observability, orphan).
    ## @io — ⇥ container: str, timeout: int, runner: CommandRunner | None → ⎋ bool (True = success)
...
def docker_rm(
    ...
    ▶ ┌container, force, runner┐ → _run_docker(["docker","rm",(-f),C]) → ◇ rc==0? → ⎋ bool

    ## @purpose — Единая точка `docker rm` (stale-cleanup, observability, orphan self-heal -f).
    ## @io — ⇥ container: str, force: bool, timeout: int, runner: CommandRunner | None
    ##       ⎋ bool (True = success)
...
def docker_tag(image_id: str, tag: str, timeout: int = DOCKER_CMD_TIMEOUT, runner: CommandRunner | None = None) -> bool:
    """Tag an image (docker tag <id> <tag>).

    ▶ ┌image_id, tag, runner┐ → _run_docker(["docker","tag",id,tag]) → ◇ rc==0? → ⎋ bool

    ## @purpose — Единая точка `docker tag` (deploy_engine rollback fallback-tag, orchestrator rollback).
    ## @io — ⇥ image_id: str, tag: str, timeout: int, runner: CommandRunner | None → ⎋ bool (True = success)
...
def docker_image_inspect(
    ...
    ▶ ┌image_id, format, runner┐ → _run_docker → ◇ rc==0? → ⊕ stdout strip → ⎋ str | None
    ...
    ## @io — ⇥ image_id: str, format: str, timeout: int, runner: CommandRunner | None
    ##       ⎋ str | None (None = сбой/пусто)
...
def docker_image_inspect_many(
    ...
    ▶ ┌identifiers, format, runner┐ → _run_docker(["docker","image","inspect",*ids,"--format",F]) → ⎋ CompletedProcess

    ## @purpose — Batch docker image inspect (docker_collector get_image_sizes: O(N) → O(1)).
    ## @io — ⇥ identifiers: list[str], format: str, timeout: int, runner: CommandRunner | None
    ##       ⎋ CompletedProcess[str]
...
def docker_manifest_inspect_raw(
    ...
    ▶ ┌image_ref, flags, runner┐ → _run_docker(["docker","manifest","inspect",*flags,ref]) → ⎋ CompletedProcess (never raise)
    ...
    ##       runner: CommandRunner | None → ⎋ CompletedProcess[str]
...
def docker_manifest_inspect(
    ...
    ▶ ┌image_ref, runner┐ → docker_manifest_inspect_raw → ◇ rc==0? → ⎋ bool
    ...
    ## @io — ⇥ image_ref: str, timeout: int, runner: CommandRunner | None → ⎋ bool (True = image exists in registry)
    ## @complexity — O(1) + network
    ## @invariants — Non-fatal: False на сбое/таймауте; stderr не логируется как error (ожидаем
    ##               на несуществующих образах)
    """
    result = docker_manifest_inspect_raw(image_ref, timeout=timeout, runner=runner)
    if result.returncode == 0:
...
def docker_network_inspect_raw(
    ...
    ▶ ┌name, runner┐ → _run_docker(["docker","network","inspect",name]) → ⎋ CompletedProcess (never raise)

    ## @purpose — Raw variant для caller'ов, которым нужен stdout (converge/networks driver-check).
    ## @io — ⇥ name: str, timeout: int, runner: CommandRunner | None → ⎋ CompletedProcess[str]
...
def docker_network_inspect(name: str, timeout: int = DOCKER_CMD_TIMEOUT, runner: CommandRunner | None = None) -> bool:
    """Check whether a docker network exists (docker network inspect <name> rc==0).

    ▶ ┌name, runner┐ → docker_network_inspect_raw → ◇ rc==0? → ⎋ bool

    ## @purpose — Единая точка `docker network inspect` (provisioner, converge/networks proxy-net).
    ## @io — ⇥ name: str, timeout: int, runner: CommandRunner | None → ⎋ bool (True = network exists)
    ## @complexity — O(1) + docker network inspect I/O
    ## @invariants — Non-fatal: False на сбое (caller решает create)
    """
    result = docker_network_inspect_raw(name, timeout=timeout, runner=runner)
    if result.returncode == 0:
...
def docker_network_create(
    name: str, driver: str = "bridge", timeout: int = DOCKER_CMD_TIMEOUT, runner: CommandRunner | None = None
) -> bool:
    """Create a docker network (docker network create --driver <d> <name>).

    ▶ ┌name, driver, runner┐ → _run_docker(["docker","network","create","--driver",d,name]) → ◇ rc==0? → ⎋ bool

    ## @purpose — Единая точка `docker network create` (provisioner, converge/networks proxy-net).
    ## @io — ⇥ name: str, driver: str, timeout: int, runner: CommandRunner | None → ⎋ bool (True = created)
...
def docker_volume_inspect(name: str, timeout: int = DOCKER_CMD_TIMEOUT, runner: CommandRunner | None = None) -> bool:
    """Check whether a docker volume exists (docker volume inspect <name> rc==0).

    ▶ ┌name, runner┐ → _run_docker(["docker","volume","inspect",name]) → ◇ rc==0? → ⎋ bool

    ## @purpose — Единая точка `docker volume inspect` (converge/volumes R7 detect-only check).
    ## @io — ⇥ name: str, timeout: int, runner: CommandRunner | None → ⎋ bool (True = volume exists)
...
def docker_info(
    timeout: int = DOCKER_CMD_TIMEOUT, runner: CommandRunner | None = None
) -> subprocess.CompletedProcess[str]:
    """Run docker info (daemon reachability check).

    ▶ ┌timeout, runner┐ → _run_docker(["docker","info"]) → ⎋ CompletedProcess (never raise)
    ...
    ## @io — ⇥ timeout: int, runner: CommandRunner | None → ⎋ CompletedProcess[str]
...
def docker_stats(
    ...
    ▶ ┌format, timeout, runner┐ → _run_docker(["docker","stats","--no-stream","--format",F]) → ⎋ CompletedProcess

    ## @purpose — Единая точка `docker stats` (docker_collector CPU%/memory metrics).
    ## @io — ⇥ format: str, timeout: int, runner: CommandRunner | None → ⎋ CompletedProcess[str]
...
def main(argv: list[str] | None = None, runner: CommandRunner | None = None) -> int:
    ...
    ## @io — ⇥ argv: list[str] | None (W4d: None → sys.argv), runner: CommandRunner | None (DI)
    ##       ⎋ int (0 ok | 1 docker failure | 2 usage)
    ## @invariants
    ...
    if args.op == "ps":
        result = docker_ps(all=args.all, format=args.format, runner=runner)
    ...
    if args.op == "inspect":
        ...
        result = docker_inspect(args.rest[0], format=args.format, runner=runner)
        sys.stdout.write(_stdout_str(result))
        return 0 if result.returncode == 0 else 1

    # exec <container> <cmd...>
    if len(args.rest) < _EXEC_ARGS_MIN:
        parser.error("exec requires a container and a command")
    result = docker_exec(args.rest[0], list(args.rest[1:]), runner=runner)
    sys.stdout.write(_stdout_str(result))
    return 0 if result.returncode == 0 else 1
