...
##           env-requirements модулей, ранее расходившиеся между validate_module_yaml.py
...
def _load_yaml_file(path: Path) -> Any:
    ...
    """
    if not path.exists():
        raise FileNotFoundError(f"[env_requires] File not found: {path}")
    with open(path) as f:
...
def env_var_in_dotenv(env_example_path: Path, var_name: str) -> tuple[bool, str]:
    ...
    ## @purpose  Единая реализация presence-проверки .env.example (ранее — приватный хелпер
    ##           validate_module_yaml._env_var_in_dotenv, DevPlan 118 D4).
...
def env_var_in_secrets_manifest(manifest_path: Path, var_name: str) -> bool:
    ...
    ## @purpose  Единая реализация manifest-registration проверки (ранее — приватный хелпер
    ##           validate_module_yaml._env_var_in_secrets_manifest, DevPlan 118 D4).
...
def check_requires_presence(
    ...
    Единая реализация — ранее validate_module_yaml.check_env_requires_presence (DevPlan 118 D4).
...
def check_runtime_env(module_name: str, secrets_manifest_path: str) -> list[str]:
    ...
    Единая реализация — ранее secrets_validator.check_env_requires (DevPlan 118 D4).
...
def check_env_requires(
    ...
    if env_example_path is not None:
        ...
        for req in module.get("env_requires", []):
            if not isinstance(req, dict):
                continue
            if req.get("type") == "secret" and req.get("required", True):
                name = req.get("name")
                if name and not env_var_in_secrets_manifest(Path(secrets_manifest_path), name):
                    violations.append(
    ...
    try:
        violations.extend(check_runtime_env(module.get("name", ""), str(secrets_manifest_path)))
