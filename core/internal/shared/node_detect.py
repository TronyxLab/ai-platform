...
##            default key file ~/.config/age/keys.txt → /etc/age/key.txt (restore-first fallback).
##            Mirrors the removed shell detect_age_key() from bootstrap.sh/node-update.sh.
## @io       ⇥ env: Mapping|None (None = os.environ), home_dir: str|None (None = Path.home()),
##           etc_key_file: str|None (None = _ETC_AGE_KEY_FILE) → ⎋ str | None (None = not found)
## @complexity O(1) — 3 env lookups + 2 file probes
## @invariants
...
##   - AGE_SECRET_KEY_FILE is read as first AGE-SECRET-KEY- line (comment-scan, head -1 equivalent)
...
def detect_age_key(
    *,
    ...
    # Файл содержит ключ AGE-SECRET-KEY-xxxxxxxx…; comment-строки (# created / # public key)
    # МОГУТ идти ВЫШЕ ключа — берётся ПЕРВАЯ строка с каноническим префиксом (единый канон
    # с Check 4; ⚠️ TRAP[BUG] 2026-08-12 · P1 · readline() без фильтра возвращал комментарий
    # `# created: ...` → sops получал невалидный ключ → «failed to create reader» (DevPlan 154 W5)).
    ...
    if file_path:
        try:
            ...
            if key:
                ...
                return key
            logger.warning("[IMP:8][node_detect] AGE_SECRET_KEY_FILE=%s has no AGE-SECRET-KEY- line", file_path)

    # ── Check 4: default key file (operator convenience, local E2E) ──
    # home_dir param (W5 T5.3) — тесты изолируют default-file probe в tmp_path без setenv HOME.
    ...
    # etc_key_file param (W5 T5.3) — тесты читают реальный tmp-файл вместо патчей Path.is_file/open.
    node_key = Path(etc_key_file if etc_key_file is not None else _ETC_AGE_KEY_FILE)
    if node_key.is_file():
        try:
            with Path(node_key).open(encoding="utf-8") as f:
                key = next((line.strip() for line in f if line.strip().startswith("AGE-SECRET-KEY-")), "")
        except OSError as e:
...
def _log_masked(key_name: str, key_value: str, source: str) -> None:
    ...
    masked = key_value[:_KEY_MASK_LEN] if len(key_value) >= _KEY_MASK_LEN else key_value
...
def auto_detect_node_name(node_configs_dir: str = DEFAULT_NODE_CONFIGS_DIR) -> str:
    ...
    if not configs_path.is_dir():
        msg = f"{node_configs_dir} does not exist"
        raise NodeDetectionError(msg)
    ...
    if not candidates:
        msg = "No node directories found"
        raise NodeDetectionError(msg)
    if len(candidates) > 1:
        msg = f"Multiple directories: {', '.join(sorted(candidates))} — use --node <name>"
        raise NodeDetectionError(msg)
...
## @io — ⇥ argv: list[str] | None, env: Mapping|None (W5 T5.3 — DI для CLI-тестов;
##       None = os.environ), etc_key_file: str | None (DI — 167 D4: путь /etc/age/key.txt
##       для Check 5 restore-first fallback; None = _ETC_AGE_KEY_FILE) → ⎋ int
##       (0 = success, 3 = key absent (module OK), 1 = detection failure / unexpected error,
...
## 🧐 TRAP[DI-SEAM] · 2026-08-14 · — · CLI not-found путь (exit 3) тестируется реальным main()
## · Rejected: прямой вызов detect_age_key без проброса пути (тест патчил _ETC_AGE_KEY_FILE —
## ·   1 monkeypatch.setattr; HOME остаётся setenv — default-file probe изоляция)
## · Reason: seam = тестируемость реального CLI-вызова main(["--detect-age-key"], env={},
## ·   etc_key_file=tmp) — детерминированный exit 3 на любой машине (без реального /etc/age)
## · Rev: при введении test-import-harness (full env изоляция) — etc_key_file может уйти
...
def main(
    ...
    *,
    ...
    args = cast(_CliArgs, cast(object, _build_parser().parse_args(argv)))

    if args.detect_age_key:
        key = detect_age_key(env=env, etc_key_file=etc_key_file)
        if key:
            print(key)
            return 0
