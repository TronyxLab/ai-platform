...
## @rationale Q: Почему shared/node_resolver.py, а не расширение node_yaml/cli.py?
##            A: node_yaml CLI — CLI-слой конкретного фасада (--get/--get-many/...). Резолв
...
##           core/internal/shared/node_yaml/cli.py (T6-нормализация — локальная копия _format_cli_value)
...
def _format_cli_value(value: object) -> str:
    ...
    ## @purpose  Parity с node_yaml/cli.py:_format_cli_value (DevPlan 123 T6) — bool → lowercase
    ...
    ##            санкционировано как единственный путь без изменения публичного API node_yaml.cli.
...
def resolve_node_yaml(
    ...
    resolved = NodeYaml.resolve(node_name=node_name, config_dir=platform_root, env=env)
