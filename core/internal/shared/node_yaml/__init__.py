...
##      Validation, Resolve. Legacy typed-геттеры (get_secrets_config/get_firewall/
...
##   4. CLI (node_yaml_cli) re-export'ится лениво через PEP 562 __getattr__ (паттерн
##      DevPlan 117 G T51): `python3 -m core.internal.shared.node_yaml` работает без
##      загрузки node_yaml_cli при импорте.
...
# Legacy тесты (test_node_yaml_cli_get_many.py) импортируют _cli_get_many из этого модуля.
# Ленивый резолв через PEP 562 __getattr__ сохраняет имя импортируемым без загрузки
# node_yaml_cli при импорте агрегатора (AC-G5).
