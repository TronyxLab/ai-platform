...
## @invariants
##   1. get_context(): ТОЛЬКО contexts[0].name (dict-form). Legacy top-level 'context' и str-form
##      contexts[0] удалены (DevPlan 116 B6, D4/D5). No raise — потребители полагаются на "".
##   2. get_domain_config(): flat schema (domain: string, DevPlan 116 B6 T7). Legacy dict-form
##      (domain: {platform: ...}) удалён (greenfield). Возвращает DomainConfig с defaults.
...
##            legacy-геттеры get_domain()/get_contexts()/get_email() НЕ пересоздаются (118 B3).
