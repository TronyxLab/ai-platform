...
##   3. NOT the same as get("node.host") — get_node_info() читает node.fqdn (field),
...
class NodeMixin:
    ...
    # ── Mixin-контракт: _load предоставляет NodeYamlCore (агрегатор) ──
    ...
    def get_node_info(self) -> NodeInfo:
        ...
        info = NodeInfo(
            # yaml-payload → str-граница: node.get(...) из dict[Any, Any] — str() конвертация (W11)
            fqdn=str(node.get("fqdn", "")),
            owner_key=str(node.get("owner_key", "")),
            docker_mirror=str(node.get("docker_mirror", "")),
        )
