...
## @io       ⇥ project_dir: Path → ⎋ dict[str, Any] — полное содержимое ai-platform.yaml
...
## @io       ⇥ data: dict → ⎋ dict[str, Any]
## @complexity O(1)
def get_needs(data: dict[str, Any]) -> dict[str, Any]:
...
## @purpose  Типизированный аксессор expose (B1): needs.expose → top-level expose (legacy fallback).
## @io       ⇥ data: dict → ⎋ bool
## @complexity O(1)
## @invariants
##   - needs.expose приоритетнее; top-level expose — legacy-фолбэк (vhost_renderer семантика)
def get_expose(data: dict[str, Any]) -> bool:
    """Extract expose flag (needs.expose → top-level expose legacy fallback, B1)."""
...
## @purpose  Типизированный аксессор domain (B1): needs.domain → top-level domain (legacy fallback).
##           False-y значения ("false"/False/"none"/"no"/"null"/"") → "" (унифицировано с
##           conflict_checks._FALSEY_DOMAINS; read_project_yaml legacy "false" → "").
...
## @complexity O(1)
def get_target_node(data: dict[str, Any], *, required: bool = False) -> str:
    ...
    if required and not val:
        raise ConfigValidationError("ai-platform.yaml: target_node is required (missing or empty)")
    return val
...
## @purpose  Типизированный аксессор name (B1): name → project (legacy fallback) → "".
## @io       ⇥ data: dict → ⎋ str
## @complexity O(1)
def get_name(data: dict[str, Any]) -> str:
    """Extract project name (name → project legacy fallback, B1)."""
...
## @io       ⇥ data: dict → ⎋ str
## @complexity O(1)
def get_project_type(data: dict[str, Any]) -> str:
...
## @io       ⇥ data: dict → ⎋ dict[str, Any]
## @complexity O(1)
def get_monitoring(data: dict[str, Any]) -> dict[str, Any]:
...
## @io       ⇥ data: dict → ⎋ dict[str, Any]
## @complexity O(1)
def get_expose_config(data: dict[str, Any]) -> dict[str, Any]:
