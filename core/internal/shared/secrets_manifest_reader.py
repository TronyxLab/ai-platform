...
def iter_secrets(path: Path | str) -> list[dict[str, object]]:
    ...
    if data is None:
        msg = f"[IMP:10][iter_secrets] Manifest {manifest_path} is empty — expected dict with 'secrets' key"
...
## @complexity O(1)
def tier(secret: dict[str, object]) -> str:
...
## @complexity O(1)
def consumers(secret: dict[str, object]) -> list[str]:
...
## @io       ⇥ secret: dict → ⎋ str ("" if absent)
## @complexity O(1)
def charset(secret: dict[str, object]) -> str:
...
## @io       ⇥ secret: dict → ⎋ str ("" if absent)
## @complexity O(1)
def gen_command(secret: dict[str, object]) -> str:
