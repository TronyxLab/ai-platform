...
## @purpose  Единый канон run_subprocess (DevPlan 118 C10) — единственный источник семантики
...
##           check=False (default) = graceful-стиль converge; check=True = raise-стиль.
## @scope    Импортируется converge/infra.py (делегирование, DevPlan 118 C10). Кандидат для
##           lifecycle/helpers/subprocess_io.py при будущей миграции bootstrap-фаз (вне скоупа C10).
## @invariants
##   1. Сигнатура: run_subprocess(cmd, *, timeout, check, non_fatal) → subprocess.CompletedProcess
...
##   5. capture_output=True + text=True — всегда (стандарт платформы)
...
## @io       ⇥ cmd: list[str]; timeout: int; check: bool; non_fatal: bool
##           ⎋ subprocess.CompletedProcess (check=False) ⚡ PlatformFatalError (check=True)
...
def run_subprocess(
    ...
    *,
    ...
    """Run a subprocess with the canonical graceful/raise semantics (DevPlan 118 C10).

    ▶ ┌cmd┐ → ○ subprocess.run(capture_output, text, timeout) → ◇ FileNotFound → ⎋ rc=127 | check? raise ┤
      → ◇ TimeoutExpired → ⎋ rc=124 | check? raise → ◇ rc!=0 → check? raise │ non_fatal? WARN → ⎋ result
    """
