...
class _GetMeUser(TypedDict):
    username: str


class _GetMeResult(TypedDict):
    """Типизированная граница getMe-ответа Telegram Bot API (W11, DevPlan 170)."""

    ok: bool
    result: _GetMeUser


# endregion TYPEDDICT_GetMe
...
# region FUNC__plw_body_get_me
# PLW0717 extraction из get_me — тело try-блока (семантика except не меняется).
# io: ⇥ opener, req → ⎋ результат try-тела; complexity O(1) — извлечение управляющего потока
def _plw_body_get_me(
    opener: urllib.request.OpenerDirector,
    req: urllib.request.Request,
) -> bool:
    # opener.open → Any (typeshed urllib); HTTPResponse-граница (W11)
    with cast(HTTPResponse, opener.open(req, timeout=SSH_CONNECT_TIMEOUT)) as resp:
        if resp.status == HTTP_OK:
            # json.loads → Any; TypedDict-граница getMe-ответа (W11)
            data = cast(_GetMeResult, json.loads(resp.read().decode("utf-8")))
            if data.get("ok"):
...
# endregion FUNC__plw_body_get_me
...
def get_me(
    ...
    try:
        return _plw_body_get_me(opener, req)
...
def notify(
    ...
    if pathlib.Path(secrets_file).is_file():
        try:
...
def main() -> int:
    ...
    ## @io — ⇥ argv + env (TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID, TELEGRAM_PROXY_URL)
    ##       → ⎋ exit 0|1
