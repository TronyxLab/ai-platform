...
class _VarDef(TypedDict, total=False):
    """Per-variable definition in template-manifest.yaml (W11: yaml payload boundary)."""
...
class _TemplateEntryBase(TypedDict, total=False):
    """Optional fields of a template-manifest entry (total=False база, Python 3.10 stdlib)."""

    output: str | None
    type: str
    vars: dict[str, _VarDef | str]


class _TemplateEntry(_TemplateEntryBase):
    """Single entry in template-manifest.yaml templates[] list (template обязателен)."""

    template: str


class _TemplateManifestBase(TypedDict, total=False):
    """Optional top-level fields (total=False база, Python 3.10 stdlib)."""

    standard_vars: dict[str, _VarDef]


class _TemplateManifest(_TemplateManifestBase):
    """Top-level template-manifest.yaml structure (templates обязателен)."""
...
def parse_vars(var_pairs: list[str]) -> dict[str, str]:
    ...
    for pair in var_pairs:
        if "=" not in pair:
            msg = f"Invalid variable format, expected KEY=value: {pair!r}"
            raise ConfigValidationError(msg)
        key, _, value = pair.partition("=")
        if not key:
            msg = f"Empty key in variable pair: {pair!r}"
            raise ConfigValidationError(msg)
...
# ruff: ignore[A002]  # `vars` — публичный keyword-API (многочисленные потребители + тесты), переименование = churn в...
def render_template(
    template_path: str,
    ...
    *,
    ...
    def _replacer(m: re.Match[str]) -> str:
        ...
        if not allow_missing:
            msg = f"unresolved placeholder: {m.group(0)}"
            raise TemplateError(
                msg,
                template_path=template_path,
                unresolved=[var_name],
...
def _atomic_write(content: str, output_path: str) -> None:
    ...
    try:
        ...
        msg = f"Atomic write failed for {output_path}: {exc}"
...
# ruff: ignore[A002]  # `vars` — публичный keyword-API (scaffold-потребители), переименование = churn вне скоупа
