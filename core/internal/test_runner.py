...
## @changes 2026-07-31 | DevPlan 099: --junit-output (явный путь, для routing executor'а),
...
# Зеркало бывшего ci.mk test-таргета (DevPlan 165: таргет удалён, порядок остаётся каноном _ALL_SUITES_ORDER). См. TRAP[DESIGN] ниже.
# not requires_node: DevPlan 095 AC9/AC12 — E2E pipeline тесты (tests/e2e/, маркер
# requires_node) НЕ входят в MARKER=static_audit/static, MARKER=all (бывш. make test) и
...
# e2e исключён (external *.tronyx.ru manual — НЕ входит в MARKER=all);
# local_auth не входит в MARKER=all; static — special handler
...
##           MARKER=smoke (make check MARKER=smoke) идёт через check_suite → прямой pytest-cmd;
##           (F11 — TEST_NO_XDIST прокидывается только check_suite-инвокациями).
## @io       ⇥ marker: str | None (имя маркерной суиты),
##          env: Mapping[str, str] | None = None (DI, W-H DevPlan 163 — override TEST_NO_XDIST;
##              None = os.environ),
##          has_xdist_fn: Callable[[str], bool] | None = None (DI — xdist-доступность;
##              None = _has_xdist) → ⎋ list[str] ([] или ["-n", "auto"])
## @complexity O(1)
## @invariants
##   - DI-параметры (None → os.environ/_has_xdist) — поведение без изменений; тесты
##     передают env-дикт/fake вместо monkeypatch (W-H)
def _xdist_args(
    marker: str | None = None,
    *,
    env: Mapping[str, str] | None = None,
    has_xdist_fn: Callable[[str], bool] | None = None,
) -> list[str]:
    """Return pytest xdist args (`-n auto`) unless docker marker, TEST_NO_XDIST=1, or xdist unavailable."""
    source: Mapping[str, str] = os.environ if env is None else env
    xdist_impl = _has_xdist if has_xdist_fn is None else has_xdist_fn
    if marker in _DOCKER_MARKERS:
        ...
        return []
    if source.get("TEST_NO_XDIST") == "1":
        return []
    if xdist_impl(sys.executable):
        return ["-n", "auto"]
    return []
...
# ·   flock-CLI сломал бы smoke-прогоны на macOS (command not found)
...
# TypedDict-граница failed-записи JUnit-агрегации (W11-G4): {name, type, message, text}
class FailedTestDict(TypedDict):
    """Одна упавшая проверка в TestSummary.failed_tests (type ∈ {FAIL, ERROR})."""
...
class TestSummary:
    ...
    skip_count: int
    error_count: int
    failed_tests: list[FailedTestDict] = field(default_factory=list)
    duration: float = 0.0
...
def parse_junit_xml(path: str) -> TestSummary:
    ...
    duration = 0.0
    suite_count = 0
    failed_tests: list[FailedTestDict] = []
...
def _build_pytest_args(marker: str) -> list[str] | None:
    ...
    if args is None:
        ...
        msg = f"Unknown MARKER='{marker}'. Valid values: {valid}"
...
##           Use-case: make check TEST_FILE=tests/unit/test_foo.py (DevPlan 165, AC6)
...
##           MARKER=static — validate.sh → validate.sh --lint → pytest static_audit
...
##           всех pytest-суит из _ALL_SUITES_ORDER (зеркало бывшего make test MARKER=all,
...
##   - e2e/local_auth/static НЕ входят: external manual / вне MARKER=all / special handler
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "--junit-output",
        default=None,
        help="Write JUnit XML to explicit path (no temp dir, no auto-cleanup). For executor routing (check-suite.yaml).",
    )
    parser.add_argument(
        "-f",
        "--test-file",
...
class _TestRunnerArgs(Protocol):
    """Типизированная проекция argparse.Namespace test_runner (W11-G4): все атрибуты
    заданы _parse_args с дефолтами — прямой доступ безопасен. Protocol с АННОТАЦИЯМИ
    БЕЗ значений (подклассы Namespace с class-атрибутами ломают hasattr/дефолты)."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    args = cast(
        _TestRunnerArgs, cast(object, args)
    )  # W11-G4: Namespace динамический → типизированная проекция (имя args сохранено — контракт-тест xdist-фрагментов матчит "*_xdist_args(args.marker),")
    ...
    try:
        ...
        if args.test_file:
            ...
            # Валидация до запуска pytest: невалидный TEST_FILE → usage + exit 2 (DevPlan 157 W2 T1)
            target = platform_root / args.test_file
            if not target.is_file():
                print(f"[test_runner] ERROR: test file not found: {args.test_file}", file=sys.stderr)
                print(
                    "[test_runner] Usage: make check TEST_FILE=tests/unit/test_x.py — "
                    "ОДИН файл; два файла = два вызова",
                    file=sys.stderr,
                )
                return 2
