...
def check_port_conflict(projects_base: str) -> tuple[bool, str]:
    ...
    for yaml_file in sorted(base.glob("*/ai-platform.yaml")):
        ...
        host_port_raw = monitoring.get("host_port", 0)
        if not host_port_raw:
            continue
        try:
            # W11: dict[str, object] boundary — host_port is YAML scalar (int|str|bool)
            host_port = int(cast(int | str | bool, host_port_raw))
...
def main() -> int:
    ...
    class _Args(argparse.Namespace):
        ...
        projects_base: ClassVar[str | None]

    args = build_parser().parse_args(namespace=_Args())
