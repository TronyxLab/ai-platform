...
##           — поллинг — через shared healthcheck_poller (Python); модульные
##             healthcheck.sh используют примитивы check_docker_health/check_http
...
##             модульные healthcheck.sh source-ят эту библиотеку напрямую
...
## @changes   — 2026-07-26 · DevPlan 083 — Added exec_check(); added timeout param to check_http()
## @verified  — 2026-07-26 · T1 — exec_check() + check_http(timeout) added
...
##             — Command string is NOT eval'd — split into array via IFS
exec_check() {
    local container="$1"
    local command="$2"
