...
##           Бизнес-логика htpasswd/ensure — в secrets_manager.py (Python), вызывается
##           из phases.py напрямую (shell-фасад их не дублирует).
## @invariants
...
## @changes   LAST_CHANGE: 2026-07-31 · DevPlan 102 — thin facade over Python secrets core
