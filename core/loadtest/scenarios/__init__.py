...
# W11: locust wait_time-функции без аннотаций (external untyped) → импорт даёт Unknown-типы
from locust import (
    between,  # pyright: ignore[reportUnknownVariableType] — W11 external locust untyped
    constant_throughput,  # pyright: ignore[reportUnknownVariableType] — W11 external locust untyped
)


# region FUNC_rps_wait_time
def rps_wait_time(target_rps: float, users: int) -> Callable[[], float]:
    ...
    if target_rps > 0 and users > 0:
        # W11: locust wait_time-функции без аннотаций (untyped) → cast к контракту wait_time
        return cast("Callable[[], float]", constant_throughput(target_rps / users))
    return cast("Callable[[], float]", between(0.05, 0.2))
