...
from scenarios.pgwire import connect  # чистый stdlib PG wire protocol (148 TASK-1)
...
class DbUser(User):
    ...
    def _fire_query(self, name: str, fn) -> None:
        ...
        ## @io — ⇥ name: str (имя задачи = строка stats), fn: Callable[[], None]
        ...
        try:
            fn()
        except Exception as exc:  # locust fire(exception=) требует перехвата
            exception = exc
        finally:
