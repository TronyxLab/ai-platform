...
# Минимальные длины сообщений (PG wire protocol): DataRow = count(int16), backend-msg = length(int32)+type
_MSG_LEN_INT16: int = 2
_MSG_LEN_INT32: int = 4
_SQL_PREVIEW_MAX: int = 80  # обрезка SQL в логах/ошибках
...
def _parse_server_first(server_first: str) -> tuple[str, bytes, int]:
    ...
    try:
        ...
        msg = f"SCRAM server-first: iterations не число: {fields.get('i')!r}"
        raise PgError(msg) from exc
    if not nonce or not salt_b64 or iterations <= 0:
        msg = f"SCRAM server-first: неполные поля (r/s/i обязательны): {server_first!r}"
        raise PgError(msg)
    try:
        salt = base64.b64decode(salt_b64)
    except (ValueError, TypeError) as exc:
        msg = f"SCRAM server-first: salt не base64: {salt_b64!r}"
        raise PgError(msg) from exc
    if not salt:
        msg = "SCRAM server-first: пустая соль"
        raise PgError(msg)
    logger.info(
...
def scram_client_first(user: str, nonce: str) -> tuple[str, str]:
    ...
    if not _NONCE_RE.fullmatch(nonce):
        msg = "SCRAM client nonce содержит недопустимые символы (допустимо: A-Za-z0-9_-)"
        raise PgError(msg)
...
def _recv_exact(conn: socket.socket, n: int) -> bytes:
    ...
    while len(buf) < n:
        try:
            ...
            msg = f"pgwire: таймаут чтения от сервера ({n - len(buf)} байт не получены)"
            raise PgError(msg) from exc
        if not chunk:
            msg = "pgwire: сервер закрыл соединение (EOF) до полного сообщения"
            raise PgError(msg)
        buf += chunk
    return bytes(buf)
...
def parse_data_row(payload: bytes) -> list[object]:
    ...
    """
    if len(payload) < _MSG_LEN_INT16:
        msg = "pgwire: DataRow payload обрезан (нет count)"
        raise PgError(msg)
    ...
    for _ in range(count):
        if offset + 4 > len(payload):
            msg = "pgwire: DataRow payload обрезан (нет длины значения)"
            raise PgError(msg)
        ...
        if offset + length > len(payload):
            msg = "pgwire: DataRow payload обрезан (значение длиннее payload)"
            raise PgError(msg)
...
def _parse_error_fields(payload: bytes) -> tuple[str, str | None]:
...
def read_backend_messages(data: bytes):
    ...
    while i + 5 <= len(data):
        ...
        if length < _MSG_LEN_INT32:
            msg = f"pgwire: некорректная длина backend-сообщения: {length}"
            raise PgError(msg)
        end = i + 5 + (length - 4)
        if end > len(data):
...
class PGSocket:
    ...
    def _recv(self) -> tuple[int, bytes]:
        ...
        conn = self._conn
        if conn is None:
            msg = "pgwire: connection closed — backend-сообщение нечитаемо"
            raise PgError(msg)
        ...
        if length < _MSG_LEN_INT32:
            msg = f"pgwire: некорректная длина backend-сообщения: {length}"
            raise PgError(msg)
        return mtype, _recv_exact(conn, length - 4)
    ...
    def authenticate(self, user: str, password: str, database: str) -> None:
        ...
        while True:
            ...
            if mtype == _MSG_AUTH:
                if len(payload) < _MSG_LEN_INT32:
                    msg = "pgwire: AuthenticationRequest без кода"
                    raise PgError(msg)
                ...
                if code == _AUTH_OK:
                    ...
                    salt = payload[4:8]
                    if len(salt) != _MSG_LEN_INT32:
                        msg = "pgwire: MD5-salt не 4 байта"
                        raise PgError(msg)
                    ...
                    if SASL_MECHANISM not in mechanisms:
                        msg = f"сервер не предлагает SCRAM-SHA-256: {mechanisms}"
                        raise PgError(msg)
                    ...
                    if self._client_first_bare is None:
                        msg = "pgwire: SASLContinue без client-first (нарушен порядок диалога)"
                        raise PgError(msg)
                    ...
                    if self._server_verify and verify and verify != self._server_verify:
                        msg = "SCRAM server signature (v=) не совпала — подмена сервера"
                        raise PgError(msg)
                    logger.info("[IMP:8][pgwire][auth] SASLFinal принят (код 12)")
                else:
                    msg = f"неподдерживаемый механизм аутентификации (код {code})"
                    raise PgError(msg)
    ...
    def query(self, sql: str) -> list[list[object]]:
        ...
        if not self._authenticated:
            msg = "pgwire: query до authenticate() — нарушен порядок протокола"
            raise PgError(msg)
...
def _sanitize_sql(sql: str) -> str:
    ...
    return first[:_SQL_PREVIEW_MAX] + ("..." if len(first) > _SQL_PREVIEW_MAX else "")
...
def connect(
    ...
    try:
        ...
        msg = f"pgwire: не удалось подключиться к {host}:{port} — {exc}"
        raise PgError(msg) from exc
