...
# W11: json.loads → Any → cast к list[str] (граница JSON)
LT_PATHS: list[str] = cast("list[str]", json.loads(os.environ.get("LT_PATHS", '["/", "/status"]')))
...
class WebUser(HttpUser):
    ...
    def get_paths(self) -> None:
        ...
        path = random.choice(LT_PATHS) if LT_PATHS else "/"  # ruff: ignore[S311] — нагрузочный выбор пути, НЕ криптография
