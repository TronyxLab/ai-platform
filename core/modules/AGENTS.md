...
## Compose-include контракт (DevPlan 123 T10, FL1)
...
3. **dev-оверрайды:** `.env` (локальный, не коммитится в payload) или `docker-compose.platform-dev.yml`/`docker-compose.macos.yml` — единственное место dev-переопределений путей. Полный перечень маунтов с macOS-статусом — `.ai/plans/123-nightly-hardening/files/compose-mounts-mapping.md`.
4. **Контракт локального стека:** локальный `make up` работает 21/21 healthy с dev-оверрайдами `.env`; новые модули обязаны следовать п.2 (env-параметризация) — литерал `/var/lib/platform/*` в новом модульном base-файле = нарушение.

# ⚠️ TRAP[DECISION] · 2026-08-03 · MED · Compose-include резолюция путей (DevPlan 123 T10, FL1)
# · Rejected: «не документировать, все знают» (верификация RC-121 показала: 9/13 false-lead
# ·   рекомендаций не выполнены именно из-за отсутствия зафиксированных контрактов)
# · Reason: расхождение «локальный стек зелёный → прод 502» коренится в незадокументированных
# ·   путях/резолюции; правило зафиксировано как обязательный контракт новых модулей.
# · Rev: 2026-10-21 — если docker compose изменит резолюцию относительных путей (v3), обновить правило.
