#!/usr/bin/env bash
# GREP_SUMMARY: logging healthcheck loki liveness lib/healthcheck deep-check-http
# STRUCTURE: source lib/healthcheck.sh → healthcheck_mode → check_containers (loki only) → exit0/1
# region MODULE_CONTRACT
## @purpose  Liveness healthcheck for logging module: Loki singleton-хранилище (DevPlan 010 T3.1)
...
##   - ⚠️ DevPlan 010 T3.1: Alloy УДАЛЁН — healthcheck проверяет ТОЛЬКО loki (коллектор — в
##     log-collector healthcheck.sh); локальный loki /ready остаётся deep-проверкой хранилища
##   - Имена контейнеров и порты env-параметризованы (паттерн infra-metrics, W10 T10.12):
##     LOKI_CONTAINER_NAME/LOKI_PORT
...
## @changes — 2026-08-22 | DevPlan 010 T3.1 — alloy удалён (→ log-collector); только loki
...
if [ "$MODE" = "deep" ]; then
    ...
    # Step 2: Service-specific diagnostics via check_http (Loki /ready)
