#!/usr/bin/env python3
# GREP_SUMMARY: postgres hook on-project-deploy auto-create-db database project needs.database role GRANT credentials platform-db.env idempotent
...
def auto_create_db(
    ...
    ensure_project_db_access(project_dir, project, db_name, env)
    return 0
...
def ensure_project_db_access(
    ...
    if role_exists:
        ...
        create_out = _psql("-c", f"CREATE ROLE \"{role}\" LOGIN PASSWORD '{password}'")
    ...
    _psql("-c", f'GRANT CONNECT ON DATABASE "{db_name}" TO "{role}"')
    _psql("-c", f'GRANT CREATE, USAGE ON SCHEMA public TO "{role}"')
    ...
    if created:
        _regenerate_env_platform(project_dir, project)
...
def _psql(*psql_args: str) -> str | None:
...
def _regenerate_env_platform(project_dir: str, project: str) -> bool:
    ...
    if os.path.isfile(ai_yaml):
        try:
            ...
            if d:
                domain = str(d)
        except Exception as exc:  # noqa: EXC — graceful, non-fatal (best-effort)
