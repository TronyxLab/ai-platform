# GREP_SUMMARY: deploy.mk, deploy, deploy-project, context-promote, hermes-build-platform, hermes-build-context, hermes-push-l1, hermes-push-l2, verify-domains
# STRUCTURE: ┌variables┐ → ◇ deploy → ◇ deploy-project → ◇ context-promote → ◇ hermes-build-* → ◇ verify-domains
...
.PHONY: deploy deploy-project context-promote hermes-build-platform hermes-build-context hermes-push-l1 hermes-push-l2 verify-domains
...
##   Локально (CI != 1) — non-fatal DR-backup.
# ⚠️ TRAP[DECISION] · 2026-08-15 · — · GHCR_OWNER — отложенная (=), НЕ ?= с $(shell) (172 W2.5)
# · Rejected: parse-time $(shell echo ... | tr ...) — выполнялся при ЛЮБОМ вызове make
# ·   (включая make help/check), замедляя не-hermes таргеты.
# · Reason: `=` — lazy-раскрытие только в рецептах, использующих GHCR_OWNER
# ·   (hermes-push-l1/hermes-push-l2); env-override сохраняется (env > makefile-присваивание).
...
hermes-push-l2:
	...
	echo "[IMP:7][make][hermes-push-l2] Pushing L2 hermes-agent-context to ghcr.io/$${L2_ORG} (tags: latest, v$${PLATFORM_VERSION})..."; \
