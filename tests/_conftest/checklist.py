...
def _print_checklist() -> None:
    ...
    print("\n=== ATTEMPT CHECKLIST (common errors) ===", file=sys.stderr)
    for i, item in enumerate(_CHECKLIST, 1):
        print(f"  {i}. {item}", file=sys.stderr)
    print("=== END CHECKLIST ===", file=sys.stderr)
...
def _print_reflection() -> None:
    ...
    print(
        ...
        " Are you repeating a failed strategy?"
        " Consider alternatives (Superposition).",
        file=sys.stderr,
    )
