...
_COUNTER_FILE = Path(__file__).parent.parent / ".test_counter.json"
_LOCK_FILE = _COUNTER_FILE.with_name(_COUNTER_FILE.name + ".lock")
...
def _warn_worker_write(op: str) -> bool:
    ...
    logger.info(
        "%s",
        f"[IMP:7][counter] Worker {os.environ.get('PYTEST_XDIST_WORKER')} — '{op}' SKIPPED (counter is master-only, DevPlan 139 W5)",
    )
    return True
...
class _CounterLock:
    ...
    def __enter__(self) -> "_CounterLock":
        ...
        self._fh = pathlib.Path(_LOCK_FILE).open("a+", encoding="utf-8")
...
def _read_counter() -> dict:
    ...
    with _CounterLock():
        if not pathlib.Path(_COUNTER_FILE).is_file():
            return {"attempts": 0}
        try:
            with pathlib.Path(_COUNTER_FILE).open(encoding="utf-8") as f:
                return json.load(f)
...
def _write_counter(data: dict) -> None:
    ...
    if _warn_worker_write("_write_counter"):
        return
    with _CounterLock(), pathlib.Path(_COUNTER_FILE).open("w", encoding="utf-8") as f:
        json.dump(data, f)
        f.write("\n")
...
def _increment_counter() -> int:
    ...
    with _CounterLock():
        counter = {"attempts": 0}
        if pathlib.Path(_COUNTER_FILE).is_file():
            try:
                with pathlib.Path(_COUNTER_FILE).open(encoding="utf-8") as f:
                    loaded = json.load(f)
        ...
        with pathlib.Path(_COUNTER_FILE).open("w", encoding="utf-8") as f:
            json.dump(counter, f)
            f.write("\n")
