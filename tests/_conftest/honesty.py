...
def _honesty_mode(env: Mapping[str, str] | None = None) -> HonestyMode:
    """Read REQUIRE_HONESTY_MODE env var. Default: 'marker' (soft Wave 1)."""
    source: Mapping[str, str] = os.environ if env is None else env
    mode = source.get("REQUIRE_HONESTY_MODE", "marker").lower().strip()
    if mode not in {"marker", "xfail", "fail"}:
        msg = f"[IMP:10][honesty] invalid REQUIRE_HONESTY_MODE={mode!r}, expected one of: marker, xfail, fail"
        raise ValueError(msg)
    return mode  # type: ignore[return-value]


def _docker_available(which_fn: Callable[[str], str | None] | None = None) -> bool:
    """Check if Docker daemon is reachable.

    DI (W-H DevPlan 163): which_fn=None → shutil.which (канон); тесты передают fake
    (0 патчей shutil.which/subprocess в test_honesty).
    """
    which_bin = shutil.which if which_fn is None else which_fn
    if not which_bin("docker"):
        return False
    try:
...
def _dispatch(mode: HonestyMode, reason: str) -> None:
    ...
    logger.info("%s", f"[IMP:10][honesty] mode={mode} action={action_map[mode]} reason={reason}")
    if mode == "marker":
...
def require_docker_or_fail(
    reason: str = "Docker daemon required",
    *,
    mode_env: Mapping[str, str] | None = None,
    docker_available_fn: Callable[..., bool] | None = None,
) -> None:
    """R4 fix: skip/fail when Docker not available. Mode controlled by REQUIRE_HONESTY_MODE.

    DI (W-H DevPlan 163): mode_env=None → os.environ; docker_available_fn=None → _docker_available.
    """
    available = _docker_available if docker_available_fn is None else docker_available_fn
    if available():
        logger.info("[IMP:9][honesty] Docker available, proceeding")
        return
    _dispatch(_honesty_mode(env=mode_env), f"Docker daemon not available — {reason}")


def require_script_or_fail(
    script_path: pathlib.Path, reason: str = "", *, mode_env: Mapping[str, str] | None = None
) -> None:
    """R4 fix: skip/fail when required script not found."""
    if script_path.exists() and os.access(script_path, os.X_OK):
        return
    _dispatch(_honesty_mode(env=mode_env), f"Script not found or not executable: {script_path} — {reason}")


def require_env_or_fail(var: str, reason: str = "", *, mode_env: Mapping[str, str] | None = None) -> None:
    """R4 fix: skip/fail when required env var not set."""
    source: Mapping[str, str] = os.environ if mode_env is None else mode_env
    if source.get(var):
        return
    _dispatch(_honesty_mode(env=mode_env), f"Env var not set: {var} — {reason}")
...
def require_service_healthy(host: str, port: int, reason: str = "", timeout: int = 5) -> None:
    ...
    try:
        with socket.create_connection((host, int(port)), timeout=timeout):
            logger.info("%s", f"[IMP:9][honesty] Service healthy: {host}:{port} — proceeding")
            return
    except OSError:
