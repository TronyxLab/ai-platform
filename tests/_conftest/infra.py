...
def _load_test_infra() -> list[dict]:
    ...
    if not _DISCOVER_SCRIPT.exists():
        msg = f"Discover script not found: {_DISCOVER_SCRIPT}"
...
class _TestInfra:
    ...
    def __new__(cls, loader: Callable[[], list[dict]] | None = None):
        # 🧐 TRAP[DI-SEAM] · 2026-08-14 · — · loader-параметр: data-загрузка через DI вместо
        # ·   monkeypatch _load_test_infra (DevPlan 167 D0)
        # · Rejected: прямой вызов _load_test_infra (тест не мог бы наблюдать 1-load семантику)
        # · Reason: loader=None → канонический _load_test_infra (поведение без изменений);
        # ·   тесты передают call-recording fake (0 патчей модульной функции)
        # · Rev: если discover-механизм сменится на файловый кэш — loader-сигнатура сохранится
        if cls._instance is None:
            cls._instance = super().__new__(cls)
            cls._instance._data = loader() if loader is not None else _load_test_infra()
    ...
    def get_container_name(self, module_name: str, service: str | None = None) -> str:
        ...
        if not names:
            msg = f"Module '{module_name}' has no container_names in test infrastructure"
            raise KeyError(msg)
        return names[0]
    ...
    def get_test_port(self, module_name: str, service: str | None = None, port_name: str | None = None) -> int | dict:
        ...
        if service:
            if service not in ports:
                ...
                msg = f"Service '{service}' not found in module '{module_name}'. Available services: {available}"
                raise KeyError(msg)
...
class _LazyTestInfraProxy:
    ...
    def __init__(self, loader: Callable[[], list[dict]] | None = None) -> None:
        self._loader = loader
    ...
    def _get_delegate(self) -> _TestInfra:
        ...
        if self._delegate is None:
            self._delegate = _TestInfra(self._loader)
