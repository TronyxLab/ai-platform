...
def _find_node_yaml() -> str | None:
    ...
    if env_path:
        abs_env = Path(env_path).resolve()
        if Path(abs_env).is_file():
    ...
    pr = Path(platform_root)

    # 2. Scan node-configs/ for any node.yaml
    node_configs = pr / "node-configs"
...
def _parse_projects_from_node_yaml(node_yaml_path: str) -> list[dict]:
    ...
    """
    try:
        with Path(node_yaml_path).open(encoding="utf-8") as f:
...
def _load_platform_env_networks() -> list[str]:
    ...
    yaml_path = Path(platform_root) / "platform-env.yaml"
    try:
...
def project_compose_files(node_yaml_projects: list[dict]) -> list[Path]:
    ...
    test_data_dir = os.environ.get(
        "TEST_PROJECTS_DIR",
        Path(os.environ.get("PLATFORM_ROOT", Path(Path(__file__).parent / ".." / "..").resolve()))
        / "tests"
        / "test_data"
        / "projects",
    )
