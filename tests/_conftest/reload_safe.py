...
##   - Патчи (monkeypatch) применяются ТОЛЬКО ПОСЛЕ reload — к актуальному объекту модуля
...
for _ln in _SELF_SOURCE.splitlines():
    ...
    if _stripped.startswith(("del sys.modules", "sys.modules.pop")):
        msg = f"Запрещённый вызов в reload_safe.py: {_stripped} — канон W4 нарушен"
        raise AssertionError(msg)
...
def _reload_doc_example() -> Iterator[ModuleType]:
    ...
    with reload_guard({"NODE_NAME": "test-node", "NODE_YAML_PATH": None}):
        yield reload_module("app", expected_file_substring="status-page")
    # Канон (W4): monkeypatch-патчи применяются ПОСЛЕ reload — к актуальному объекту модуля
    # (в реальном тесте — после `with`/возобновления генератора, к перезагруженному объекту)
