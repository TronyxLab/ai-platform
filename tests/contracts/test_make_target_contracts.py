...
## @usecases — T2.3 contract gate: CI runs make check MARKER=contract to validate
...
_SYSTEM_EXCEPTIONS = {
    ...
    "help-all",  # name_linter.system_exceptions (План 175 W1.3 — полный реестр глаголов)
...
def _parse_delegates_to_files(manifest_path: pathlib.Path) -> list[str]:
    ...
    for entries in data.values():
        ...
        for entry in entries:
            ...
            if any(delegates_to.startswith(p) for p in non_file_phrases):
                continue
...
# endregion TEST: bidirectional parity
...
# endregion TEST: delegates_to script existence
...
# endregion TEST: deploy NODE/LAUNCH/preflight registration contract (139 W1 K2)
