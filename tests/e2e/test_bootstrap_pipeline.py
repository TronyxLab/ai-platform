...
def test_04_deploy_test_project(
    ...
    # 176 A.2: фикстура БЕЗ host-порта (L1 ports-published запрещает публикацию) — HTTP 200
    # проверяется docker exec ВНУТРИ контейнера (тот же сигнал «nginx обслуживает»; wget в образе)
    http = node_ssh.ssh_read(
        "docker exec test-project-web wget -q -O /dev/null http://127.0.0.1/ && echo 200", timeout=60
    )
    logger.info("[IMP:9][T9][deploy] in-container HTTP -> %s", http.stdout.strip())
    assert http.stdout.strip() == "200", f"test-project HTTP not 200 (in-container): {http.stdout} {http.stderr}"
...
def test_07_restore_roundtrip(requires_node: str, node_ssh: NodeSSHClient, test_project_dir: str, caplog) -> None:
    ...
    ##            HTTP 200 внутри контейнера (docker exec — фикстура без host-порта, 176 A.2)
    ##            + контейнер running. Restore-механизм: передоставка payload
    ...
    # 176 A.2: фикстура БЕЗ host-порта (L1 ports-published) — HTTP 200 внутри контейнера
    http = node_ssh.ssh_read(
        "docker exec test-project-web wget -q -O /dev/null http://127.0.0.1/ && echo 200", timeout=60
    )
    logger.info("[IMP:9][T12][restore] in-container HTTP after restore -> %s", http.stdout.strip())
