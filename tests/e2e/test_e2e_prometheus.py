...
def test_prometheus_targets_up(PROMETHEUS_PROXY_URL: str, grafana_credentials: tuple[str, str], caplog) -> None:
    ...
    if resp.status_code != 200:
        ...
        # авторизация сломана — это конфигурационная ошибка → FAIL безусловно.
    ...
    if down_targets:
        ...
        # ⚠️ TRAP[LOCAL] · 2026-07-08 · — · Local dev may have DOWN targets (node-exporter, alloy)
...
def test_prometheus_metrics(label, query, metric_hint, PROMETHEUS_PROXY_URL, grafana_credentials, caplog):
    ...
    if resp.status_code != 200:
        ...
        if resp.status_code == 401:
            # R4 (DevPlan 160 W5 T5.1): 401 = auth misconfiguration → FAIL (см. targets test)
            pytest.fail("Prometheus query API returned 401 — auth rejected (configuration error, not environment)")
