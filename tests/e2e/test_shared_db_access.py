...
def test_shared_db_full_cycle_via_pgbouncer(shared_db_stack, tmp_path, caplog) -> None:
    ...
    env_text = env_platform.read_text()
    dsn_line = next((_l for _l in env_text.splitlines() if _l.startswith("PLATFORM_POSTGRES_DSN=")), "")
...
def test_idempotent_redeploy_role_unchanged(shared_db_stack, tmp_path, caplog) -> None:
    ...
    try:
        ...
        creds1 = (project_dir / ".platform-db.env").read_text()
        rc2 = on_project_deploy.main(argv=[str(project_dir), _TEST_PROJECT, "local"])
        creds2 = (project_dir / ".platform-db.env").read_text()
    finally:
        os.environ.pop("POSTGRES_PASSWORD", None)

    assert rc1 == 0 and rc2 == 0
    pw1 = next(_l.split("=", 1)[1] for _l in creds1.splitlines() if _l.startswith("PLATFORM_POSTGRES_PASSWORD="))
    pw2 = next(_l.split("=", 1)[1] for _l in creds2.splitlines() if _l.startswith("PLATFORM_POSTGRES_PASSWORD="))
