...
# fast (прежний ci.mk:138-168): pre-commit → validate → check-dead-code →
...
# full (прежний ci.mk:169-213): pre-commit → validate → check-dead-code → lint →
...
def test_no_hardcoded_checks_in_makefiles(caplog) -> None:
    ...
    for name, (def_line, body) in portals.items():
        ...
        if re.search(r"pytest", body) or re.search(r'-m\s+"', body):
...
def test_registration_in_entrypoint_manifest(caplog) -> None:
    ...
    with open(_MANIFEST_PATH) as f:
        manifest = yaml.safe_load(f)
    with open(_ENTRYPOINT_MANIFEST_PATH) as f:
    ...
    for c in manifest.get("checks", []):
        ...
        for raw in [cmd, *cmds.values()]:
            if not isinstance(raw, str) or not raw.startswith("make "):
                continue
            target = raw.split()[1]
            if target not in allowed_verbs and target not in system_exceptions:
                violations.append(f"check {c.get('id')!r}: make-таргет {target!r} не в allowed_verbs/system_exceptions")
