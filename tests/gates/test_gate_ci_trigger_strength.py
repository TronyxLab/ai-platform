...
def _gate_strength_findings(wf_dir: pathlib.Path) -> list[str]:
    ...
    for name in _DOWNSTREAM_WORKFLOWS:
        ...
        try:
            wf = load_yaml(path)
        except Exception as exc:  # ruff: ignore[BLE001] — parse-failure = violation (fail loud): цель — поймать ЛЮБУЮ ошибку парсинга
        ...
        if not any(_GATE_WORKFLOW in {f"{ref}.yml", ref} for ref in refs):
...
def test_gate_strength_scanner_yaml_parseable(caplog) -> None:
    ...
    for yml in sorted(_WORKFLOW_DIR.glob("*.yml")):
        try:
            with pathlib.Path(yml).open(encoding="utf-8") as f:
                yaml.safe_load(f)
        except Exception as exc:  # ruff: ignore[BLE001] — parse-failure = violation (fail loud): цель — поймать ЛЮБУЮ ошибку парсинга
