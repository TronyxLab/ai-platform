# GREP_SUMMARY: gate context-contract contexts-canon invariant-3 node-yaml top-level-context removed flat-domain single-read-point
# STRUCTURE: ▶ (a) node.yaml-образцы: 0 × top-level ^context:, contexts: present → ◇ (b) 0 × extract-alias в core/ → ◇ (c) 0 × node._data в core/ → ◇ (d) validate() negative (context, dict-domain) → ◇ (e) domain flat-only в образцах → ⎋ pass|fail
# region MODULE_CONTRACT
## @purpose  Контрактный gate (DevPlan 116 B6 T9.1, D6): инвариант 3 кодируется в коде —
##           contexts[] канон, top-level 'context' удалён. Самоверификация волны B6:
##           (a) образцы node.yaml не содержат поля, (b) extract-алиас удалён из core/,
##           (c) приватный кэш-атрибут node._data не используется, (d) validate() отклоняет
##           фикстуры, (e) domain строго flat-строка.
...
##   - (d) NodeYaml.validate() на `context:` → error «'context'»;
...
# region TEST_A_SAMPLES_NO_TOP_LEVEL_CONTEXT
@pytest.mark.gate
@ldd_trajectory
def test_node_yaml_samples_contexts_canon(caplog) -> None:
    ...
    for sample in samples:
        ...
        # top-level context field — ЗАПРЕЩЁН (invariant 3, DevPlan 116 B6 T1)
        assert not re.search(r"^context:\s", content, re.M), (
            f"{sample}: top-level 'context:' present — use contexts[] canon"
        )
    ...
    logger.critical("[IMP:9][gate_context_contract][a] PASS: %d samples, 0 context", len(samples))


# endregion TEST_A_SAMPLES_NO_TOP_LEVEL_CONTEXT
...
# endregion TEST_B_NO_EXTRACT_ALIAS
...
# endregion TEST_C_NO_PRIVATE_CACHE_ACCESS


# region TEST_D_VALIDATE_NEGATIVE_CONTRACT
@pytest.mark.gate
@ldd_trajectory
def test_validate_rejects_top_level_context(tmp_path, caplog) -> None:
    """(d) negative-контракт: `context:` → validate() error «'context' field is rejected»."""
    ctx_file = tmp_path / "old-format.yaml"
    ctx_file.write_text("context: old\ncontexts:\n  - name: canon\nnode:\n  name: n\n  host: 1.2.3.4\nmodules: []\n")
    errors = NodeYaml(str(ctx_file)).validate()
    assert any("'context' field is rejected" in e for e in errors), f"expected 'context' error, got {errors}"
    logger.critical("[IMP:9][gate_context_contract][d] PASS: validate() rejects top-level 'context' field")
...
# endregion TEST_D_VALIDATE_NEGATIVE_CONTRACT
...
# endregion TEST_E_DOMAIN_FLAT_ONLY
