...
_CORE_DIR = str(Path(_PROJECT_ROOT) / "core")

# Ensure shared/ is importable
_SHARED_DIR = str(Path(_CORE_DIR) / "internal" / "shared")
...
def _load_entrypoint_manifest() -> dict:
    ...
    manifest_path = str(Path(_CORE_DIR) / "entrypoint-manifest.yaml")
    try:
...
def test_letsencrypt_live_sole_resolver():
    ...
    deploy_paths_file = str(Path(_CORE_DIR) / "internal" / "shared" / "deploy_paths.py")
    with pathlib.Path(deploy_paths_file).open(encoding="utf-8") as f:
        text = f.read()
    ...
    consumers = [
        str(Path(_CORE_DIR) / "internal" / "bootstrap" / "s3_ssl_cache.py"),
        str(Path(_CORE_DIR) / "internal" / "healthcheck" / "metrics" / "cert_collector.py"),
        str(Path(_CORE_DIR) / "internal" / "bootstrap" / "cert_orchestrator.py"),
        str(Path(_CORE_DIR) / "internal" / "bootstrap" / "core_deliverer.py"),
        str(Path(_CORE_DIR) / "internal" / "bootstrap" / "overlay_deliverer.py"),
    ]
    offenders = []
    for p in consumers:
        try:
...
def test_litellm_config_path_sole_resolver():
    ...
    llm_paths_file = str(Path(_CORE_DIR) / "internal" / "shared" / "llm_paths.py")
    with pathlib.Path(llm_paths_file).open(encoding="utf-8") as f:
        llm_text = f.read()
    ...
    for p in sorted(f for f in __import__("pathlib").Path(_CORE_DIR).rglob("*.py") if "__pycache__" not in f.parts):
        ...
        if rel == str(Path("core") / "internal" / "shared" / "llm_paths.py"):
            continue
        try:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Constant) and isinstance(node.value, str) and node.value == "litellm-config.yml":
                offenders.append(f"{rel}:{node.lineno}")  # ruff: ignore[PERF401] — вложенные циклы, extend нечитаем
    assert not offenders, (
