...
MODULES_DIR = Path(ROOT_DIR) / "core" / "modules"
...
def _parse_container_names():
    ...
    for module_name in discover_docker_modules(MODULES_DIR):
        base_yml = Path(MODULES_DIR) / module_name / "docker-compose.base.yml"
        if not pathlib.Path(base_yml).is_file():
            continue
        with pathlib.Path(base_yml).open(encoding="utf-8") as f:
            content = f.read()
...
def test_env_example_hostnames_match_containers(caplog):
    ...
    env_path = Path(ROOT_DIR) / ".env.example"
    assert pathlib.Path(env_path).is_file(), f".env.example not found at {env_path}"
