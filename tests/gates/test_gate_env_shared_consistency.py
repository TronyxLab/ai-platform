...
PLATFORM_ENV_PATH = Path(ROOT_DIR) / "platform-env.yaml"
ENV_EXAMPLE_PATH = Path(ROOT_DIR) / ".env.example"
ENV_DOT_PATH = Path(ROOT_DIR) / ".env"
...
def _load_module_yaml(module_name: str) -> dict | None:
    ...
    path = Path(MODULES_DIR) / module_name / "module.yaml"
...
def _load_compose_yaml(module_name: str) -> dict | None:
    ...
    path = Path(MODULES_DIR) / module_name / "docker-compose.base.yml"
    if not pathlib.Path(path).is_file():
        return None
    with pathlib.Path(path).open(encoding="utf-8") as f:
        return yaml.safe_load(f)
