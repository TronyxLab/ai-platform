...
def test_no_self_read(caplog) -> None:
    ...
    (forbidden-тройка упразднена DevPlan 171 W3.3 — секции удалены из генератора)
    ...
    try:
        with pathlib.Path(_GENERATOR_PATH).open(encoding="utf-8") as f:
            source = f.read()
    ...
    # Verify the merge() function preserves module_lifecycle, etc.
    preserve_keys = [
        "name_linter",
        "module_lifecycle",
