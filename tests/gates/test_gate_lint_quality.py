...
## @invariants
##   - Module output must equal the spec oracle on every target category: unknown → FAIL,
...
def _run_python_linter(test_targets: dict[str, str]) -> list[str]:
    ...
    ## @purpose — Independent re-implementation of the namelint spec (manifest data only):
    ##            allowed/module_lifecycle/system_exceptions/system_prefixes → pass; else FAIL.
    ##            (forbidden-тройка упразднена DevPlan 171 W3.3 — запрещён = не зарегистрирован).
    ...
    # Категорийное правило (DevPlan 171 W3.6): служебные таргеты + _-префикс
...
# ·   makefiles/*.mk) → module flags exactly unknown targets and passes
...
def test_linter_parity(caplog, tmp_path) -> None:
    ...
    lint_sh = _LINT_SH_PATH.read_text(encoding="utf-8")
