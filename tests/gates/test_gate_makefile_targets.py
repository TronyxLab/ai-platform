...
# STRUCTURE: ┌extract_phony_targets()┐ → ◇ test_recipe_lines_use_tab → ◇ test_root_makefile_line_limit → ◇ make -n targets
# region MODULE_CONTRACT
## @purpose  CI gate для W4-E4 Makefile include-split: validate `make -n <target>` works
##           для всех .PHONY targets и recipe lines use TAB (not spaces).
##           Семантика test_all_phony_targets_discovered (Makefile .PHONY ↔ allowed_verbs
##           parity, G1.2) МИГРИРОВАНА в core/internal/static/verb_register.py (rule
##           verb-register, DevPlan 163 W-C P4, parity files/static_parity_p4.md) и удалена.
...
## @changes 2026-08-13 | DevPlan 163 W-C P4 — test_all_phony_targets_discovered удалён (migrated → verb_register.py)
...
def _is_removed_make() -> bool:
    ...
    try:
        result = subprocess.run(
            ...
            text=True,
            timeout=5,
            check=False,  # probe: missing make handled by except branch below
        )
        import re as _re
...
_REMOVED_MAKE = _is_removed_make()
...
class TestMakefileIncludeSplit:
    ...
    def test_makefiles_directory_exists(self):
        ...
        expected = {
            ...
            "ai-instructions.mk",  # DevPlan 001 T4.2 — фасад конвенционного компилятора инструкций
        }
    ...
    def test_make_n_for_simple_targets(self):
        ...
        for target in sorted(targets):
            ...
            args = ["make", "-n", target]
            if target in ("bootstrap-node", "converge", "render-vhosts", "verify"):  # ruff: ignore[PLR6201] — tuple: set-literal = verb-register RED
            ...
            if target in ("new-project", "remove-project", "project-status"):  # ruff: ignore[PLR6201] — tuple: set-literal = verb-register RED
