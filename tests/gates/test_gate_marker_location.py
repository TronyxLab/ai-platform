...
def _find_gate_markers_outside_gates() -> list[tuple[str, int]]:
    ...
    ## @io — ⎋ list[tuple[str, int]]: (rel-путь от tests/, lineno) зомби-маркеров
...
def test_no_gate_markers_outside_gates_dir(caplog) -> None:
    ...
    if offenders:
        ...
        pytest.fail(
            ...
            "Перенеси gate-функции в tests/gates/ ИЛИ сними маркер с legacy-unit (DevPlan 119 A1)."
        )
...
def test_gate_marker_outside_detected_negative(caplog) -> None:
    ...
    try:
        offenders = _find_gate_markers_outside_gates()
        hits = [o for o in offenders if "test_zombie_probe" in o[0]]
