...
##   - YAML !override tag (test.yml) обрабатывается конструктором (как test_gate_smoke_test_isolation)
...
def _find_non_canonical_networks(compose_files: list[Path] | None = None) -> dict[str, set[str]]:
    ...
    for cf in compose_files:
        ...
        if bad:
            offenders[str(cf.relative_to(PROJECT_ROOT))] = bad
    return offenders
...
class TestGateNetworksSot:
    ...
    #   _find_non_canonical_networks детектирует
    ...
    def test_non_canonical_network_detected_negative(self, tmp_path: Path):
        ...
        # Детектор: имя вне канона ∪ allowlist = неканоничное (tmp-path probe —
        # relative_to(PROJECT_ROOT) неприменим, проверяем напрямую через канон-сверку)
        bad = names - canon - _ALLOWLIST_NETWORKS
        assert "evil-custom-net" in bad, "R5 FAIL: non-canonical network name was NOT detected"
