...
##           Non-generator scripts with inline python3 are tracked separately.
...
##   - inline python3 in non-generator scripts (install-docker, validate, project-list,
...
_PROJECT_ROOT: str = os.path.normpath(Path(__file__).resolve().parent / ".." / "..")
...
def _check_shell_generator_violations(filepath: Path) -> list[str]:
    """Проверить один shell-файл на shell-генераторы (PLR1702-хелпер).

    ## @purpose — Извлечение тела вложенных проверок 2a/2b: shell-функция с
    ##             именем генератора ИЛИ ссылка на генератор без python3-вызова.
    ## @io — ⇥ filepath → ⎋ list[str] violations
    ## @complexity — O(L*G) где L = строки, G = генераторы
    """
    errors: list[str] = []
...
def test_no_shell_generators(caplog) -> None:
    ...
    scripts_dir = Path(_PROJECT_ROOT) / "core" / "internal" / "scripts"
    if not pathlib.Path(scripts_dir).is_dir():
        pytest.fail(f"Scripts directory not found: {scripts_dir}")
        return

    py_files: set[str] = {p.name for p in scripts_dir.iterdir()}
    ...
    shell_dirs = [
        Path(_PROJECT_ROOT) / "core" / "entrypoints",
        Path(_PROJECT_ROOT) / "core" / "internal",
    ]

    for scan_dir in shell_dirs:
        if not pathlib.Path(scan_dir).is_dir():
        ...
        for root, _dirs, files in os.walk(scan_dir):
            for filename in sorted(files):
                ...
                errors.extend(_check_shell_generator_violations(filepath))
