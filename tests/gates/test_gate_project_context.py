...
##           2. Absence of `context:` field in the YAML body
## @invariants
...
_PROJECT_ROOT = os.path.normpath(Path(__file__).parent / ".." / "..")
_PROJECTS_DIR = Path(_PROJECT_ROOT) / "projects"
...
##           no `context:` field). Возвращает список проблем; пусто = чисто.
...
def _scan_project_context(projects_dir: str) -> list[str]:
    ...
    yaml_pattern = Path(projects_dir) / "*" / "*" / "ai-platform.yaml"
    yaml_files = sorted(Path(projects_dir).glob("*/*/ai-platform.yaml"))
    ...
    for yaml_path in sorted(yaml_files):
        ...
        context_from_path = Path(Path(abs_path).parent.parent).name
        _logger.info(
        ...
        # Read YAML and check for context field
        try:
        ...
        if "context" in data:
            issues.append(f"{yaml_path}: contains 'context: {data['context']}' field — D2 requires removal")
            _logger.error("[IMP:9][gate][context] TOP-LEVEL FIELD: %s → context=%s", yaml_path, data["context"])
...
def test_project_context_valid(caplog, tmp_path) -> None:
    ...
    if pathlib.Path(_PROJECTS_DIR).is_dir():
