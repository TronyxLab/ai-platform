...
_PROJECTS_DIR = Path(repo_root()) / "projects"
_PLATFORM_ENV_YAML = Path(repo_root()) / "platform-env.yaml"
...
def _scan_project_env(projects_dir: str) -> list[str]:
    ...
    for yaml_path in sorted(yaml_files):
        project_dir = Path(yaml_path).parent
        env_platform_path = Path(project_dir) / ".env.platform"
        ...
        if not pathlib.Path(env_platform_path).is_file():
            issues.append(
        ...
        try:
            with pathlib.Path(env_platform_path).open(encoding="utf-8") as f:
                env_content = f.read()
        ...
        for line_raw in env_content.splitlines():
            line = line_raw.strip()
