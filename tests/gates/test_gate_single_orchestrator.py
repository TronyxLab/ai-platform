...
_CORE_INTERNAL_DIR = str(Path(_CORE_DIR) / "internal")

# ── Whitelists ────────────────────────────────────────────────────────────────
# SoT: core/internal/shared/deploy_paths.py (DevPlan 171 W3.5) — разрешения живут
# в каноне путей, а не в гейт-литералах. Гейт читает константы.
from core.internal.shared.deploy_paths import DOCKER_COMPOSE_ALLOWED_MODULES, SCP_RSYNC_ALLOWED_PATHS
...
ALLOWED_DOCKER_COMPOSE_MODULES: list[str] = list(DOCKER_COMPOSE_ALLOWED_MODULES)

# Layer 2: Shell/Python files that are ALLOWED to use scp/rsync directly
# NOTE: relpath is relative to core/ directory (no "core/" prefix)
ALLOWED_SCP_RSYNC_PATHS: list[str] = list(SCP_RSYNC_ALLOWED_PATHS)
...
# receive/deploy-many — прежние/локальные subcommands, остаются разрешёнными.
...
FORCED_COMMAND_DOC_ONLY: list[str] = [
    # deploy.sh УДАЛЁН (164 W3-1) — doc-only список пуст; forced-command определён
    # ТОЛЬКО в orchestrator_cli dispatch (канон 116 B1)
]
...
def _read_file_content(root: str, rel_path: str) -> str:
    ...
    try:
        with pathlib.Path(full).open(encoding="utf-8", errors="replace") as f:
            return f.read()
