...
PROJECT_ROOT = os.path.normpath(Path(__file__).parent / ".." / "..")
...
def test_all_templates_resolvable(caplog):
    ...
    manifest_path = Path(PROJECT_ROOT) / "core" / "templates" / "template-manifest.yaml"
    ...
    except Exception as e:  # ruff: ignore[BLE001] — fail loud: любая ошибка check_all = RED gate
...
# endregion FUNC_test_all_templates_resolvable
