...
##            удалены (все <150 LOC, 0 binary calls); bootstrap.sh остаётся; deploy.sh удалён (164 W3-1)
##            (>150 LOC); + test_allowlist_current (R5 negative на возврат удалённых)
## @changes — 2026-08-15 | DevPlan 170 W9-F1: bootstrap.sh 160 → 99 LOC (tab-парсинг извлечён
##            в bootstrap_resolver.py); allowlist-запись остаётся — exec ssh (бинарный вызов)
...
ALLOWLIST: frozenset[str] = frozenset({
    # bootstrap.sh (99 LOC, 1 func, 170 W9-F1): резолв параметров извлечён в
    # core/internal/bootstrap/bootstrap_resolver.py — остаётся в allowlist из-за exec ssh
    # (прямой бинарный вызов, языковая политика: entrypoint = тонкий фасад над
    # scp-deliver/build-ssh-cmd/bootstrap_resolver). Бинарный вызов justify-ит allowlist (G1).
    "bootstrap.sh",
    # deploy.sh УДАЛЁН (164 W3-1): dispatch — единственный канал; allowlist больше
    # не содержит переходных entrypoint-ов
})
...
def count_functions(filepath: pathlib.Path) -> int:
    ...
    result: subprocess.CompletedProcess = subprocess.run(
        ...
        capture_output=True,
        text=True,
        check=False,  # grep rc=1 (no match) → count 0, documented in @invariants
    )
...
def test_allowlist_current() -> None:
    ...
    assert ALLOWLIST, "Allowlist must not be empty — it protects bootstrap.sh (G1)"
