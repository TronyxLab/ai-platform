...
def test_gate_verify_contracts_l1_always_blocks() -> None:
    """L1-контракты блокируют при ЛЮБОМ уровне: _severity_for не гейтит L1 по state."""
    src = _VERIFY_SRC.read_text(encoding="utf-8")
    assert "def _severity_for" in src, "verify_contracts.py обязан иметь централизованную политику severity"
    # L1 ветка всегда возвращает block (политика §4.5 — переходный grace-период удалён)
    assert "if klass == KLASS_L1:" in src
    assert "return SEVERITY_BLOCK" in src
...
@pytest.mark.gate
def test_gate_verify_contracts_unmanaged_negative() -> None:
    """R5: unmanaged (без practices.lock → L1 блок) покрыт negative-тестом."""
    test_src = _UNIT_TEST_SRC.read_text(encoding="utf-8")
    assert "test_verify_contracts_unmanaged_blocked" in test_src, "R5: нет negative-теста unmanaged"
    assert "PRACTICES:UNMANAGED" in test_src, "unmanaged тест обязан проверять маркер PRACTICES:UNMANAGED"
