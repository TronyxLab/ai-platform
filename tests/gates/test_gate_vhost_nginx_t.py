...
def _patch_cert_paths(vhost_content: str) -> str:
    ...
    content = content.replace("/var/www/acme", "/tmp/acme-stub")
    return content
...
def test_gate_vhost_nginx_t(tmp_path: Path) -> None:
    ...
    # ── Act: generate vhosts via render_all ──
    # Mock nginx_t_harness to skip internal Docker validation (which has a known
    # harness file-isolation issue). The gate test does its OWN independent Docker
    # nginx -t validation below — belt-and-suspenders approach.
    function_call = (
        'nginx_t_harness() { log_imp 7 "harness" "MOCK: skipping nginx -t for gate test"; return 0; }; '
    ...
    assert len(vhost_files) == 3, (
        f"Expected 3 vhost files, got {len(vhost_files)}: {[f.name for f in vhost_files]}"
    )
    ...
    for vf in vhost_files:
        ...
        assert "GENERATED by add-vhost.sh" in content, (
            f"Vhost {vf.name} missing GENERATED marker:\n{content}"
        )

        # No self-signed default.crt
        assert "default.crt" not in content, (
            f"Vhost {vf.name} uses self-signed default.crt:\n{content}"
        )
    ...
    openssl_result = subprocess.run(
        [
            ...
            "-newkey", "rsa:2048",
            "-keyout", str(certs_dir / "privkey.pem"),
            "-out", str(certs_dir / "fullchain.pem"),
            "-subj", "/CN=localhost",
        ],
        capture_output=True,
        text=True,
    ...
    docker_result = subprocess.run(
        [
            ...
            "-v", f"{patched_dir}:/etc/nginx/conf.d/overlay:ro",
            "-v", f"{certs_dir}:/etc/nginx/dev-certs:ro",
            "-v", f"{includes_dir}:/etc/nginx/includes:ro",
            "-v", f"{acme_stub}:/tmp/acme-stub:ro",
