...
# DevPlan 005: dead-man's switch cron reader УДАЛЁН — count 9→8
...
# DevPlan 005: dead-man's switch cron workflow удалён — dead-man's switch removal)
...
def _extract_module_list_from_content(content: str) -> set[str]:
    ...
    """
    # Dynamic generation patterns (StatusReport 046 T2: module_discovery.py + inline)
    # - module_discovery.py call (new, T2): core/internal/scripts/module_discovery.py
    # - inline python3: Path('core/modules').glob (replaced in T2, kept for compat)
    ...
    has_old_inline = "Path('core/modules').glob" in content
    if "Generate module list" in content and (uses_module_discovery or has_old_inline):
        # Dynamic generation — return actual modules from filesystem
        return {p.parent.name for p in _MODULES_DIR.glob("*/docker-compose.base.yml")}
    # Fall back to hardcoded pattern for workflows
...
def test_platform_test_has_single_job():
    ...
    logger.info("[IMP:9][test] jobs (static-gate, basedpyright-tests, platform-integration) correctly removed")
