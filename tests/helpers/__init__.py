...
# STRUCTURE: ┌package marker┐ → ◇ re-exports from helpers.py (flat module) → ⎋ backward-compat for `from helpers import load_schema`
...
##           which shadowed the flat tests/helpers.py module. To preserve
##           backward compatibility with ~4 existing test files using
##           `from helpers import load_schema, FQDNRegistry, FQDNConflictError`,
##           this __init__.py re-exports the helpers.py content.
## @scope    All test files importing from `helpers` (flat) or `helpers.gate_helpers` (submodule)
## @invariants
##   - `from helpers import load_schema` works (re-exported from helpers.py)
##   - `from helpers.gate_helpers import ...` works (submodule)
##   - helpers.py moved here; tests/helpers.py file removed
## @rationale Namespace collision fix: package + module can't coexist with the same
##            name in Python. Wave 1 chose package; code needs flat import.
##            Solution: move flat content INTO the package's __init__.py — both work.
## @changes  2026-07-21 | Wave 2 W2-E2 — merged helpers.py into package __init__
# endregion MODULE_CONTRACT
# ruff: file-ignore[RUF067]  # backward-compat bridge (W2-E2): helpers.py намеренно слит в __init__
# для `from helpers import load_schema, FQDNRegistry` (см. @rationale). Перенос в подмодуль
# сломал бы flat-импорты ~4 тест-файлов.

import json
from pathlib import Path

# Preserve SCHEMAS_DIR resolution: helpers.py was at tests/helpers.py,
...
from helpers.env_defaults_generated import _CLICKHOUSE_PASSWORD
