...
## @scope    Integration harness, маркер requires_node (НЕ в make test / make gate — фильтр
...
def test_3x_bootstrap_idempotent(caplog, tmp_path: Path) -> None:
    ...
    _init_phases = list(state.phases("init").keys())
    _done, pending = state.all_phases_done(_init_phases)
    del _done  # только pending важен для идемпотентности
