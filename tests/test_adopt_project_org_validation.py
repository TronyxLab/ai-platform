...
## @invariants
##   - Zero hardcoded paths — все тесты используют tmp_path для PROJECTS_ROOT
...
from core.internal.scaffold import project_adopter
...
def _isolate_projects_root(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """PROJECTS_ROOT → tmp_path so node.yaml casing validation resolves there."""
...
def test_context_mismatch_detected(caplog: pytest.LogCaptureFixture, tmp_path: Path) -> None:
    ...
    # Create node.yaml with lowercase context at PROJECTS_ROOT/TronyxLab/node-configs/tronyx-vps/
...
def test_detect_fails_without_org(
    ...
    path-derivation yields empty org → fail-fast (mock Path.resolve to root-level path).
    """
    caplog.set_level(logging.DEBUG)
    proj_dir = tmp_path / "solo-project"
    proj_dir.mkdir()

    # Simulate project at "/" (parent.name == "") — path-derivation yields empty org
    monkeypatch.setattr(
        project_adopter.Path,
        "resolve",
        lambda self, strict=False: Path("/") / self.name,
    )

    with pytest.raises(ConfigValidationError, match="PROJECT_ORG is not set"):
        detect_project_config(project_dir=proj_dir)
