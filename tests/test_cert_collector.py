...
# Add core/ to sys.path for import
...
class TestCertCollectorTimezone:
    ...
    def test_not_valid_after_naive_datetime(self, mock_cert_pem, caplog):
        ...
        naive_expiry = datetime(2030, 1, 1)  # No tzinfo — naive!
...
class TestSanMatch:
    """Tests for _san_match — exact and wildcard domain matching."""

    def test_exact_match(self):
        """Exact domain match returns True."""
        assert _san_match(["example.com", "www.example.com"], "example.com") is True

    def test_exact_match_case_insensitive(self):
        """Exact match is case-insensitive."""
        assert _san_match(["Example.COM"], "example.com") is True
    ...
    def test_wildcard_no_match_deep(self):
        """Wildcard *.example.com does NOT match deep.sub.example.com."""
        assert _san_match(["*.example.com"], "deep.sub.example.com") is False

    def test_no_match(self):
        """Unrelated domain returns False."""
        assert _san_match(["example.com"], "other.com") is False

    def test_empty_san(self):
        """Empty SAN list returns False."""
        assert _san_match([], "example.com") is False

    def test_trailing_dot(self):
        """Domains with trailing dots are normalized."""
        assert _san_match(["example.com."], "example.com.") is True
