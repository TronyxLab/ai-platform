...
def _remove_networks() -> None:
    ...
    for net in _EXTERNAL_NETWORKS:
        result = subprocess.run(["docker", "network", "rm", net], capture_output=True, text=True, check=False)
...
def clickhouse_up(
    ...
    compose_failed_log = tmp_path_factory.mktemp("clickhouse") / "compose-up-failed.log"

    # ── Resolve compose path ──────────────────────────────────────────────
    compose_dir = Path(modules_dir) / "clickhouse"
    compose_base = Path(compose_dir) / _COMPOSE_BASE
    compose_test = Path(compose_dir) / _COMPOSE_TEST

    if not pathlib.Path(compose_base).exists():
        pytest.fail(f"ClickHouse compose file not found: {compose_base}")
    if not pathlib.Path(compose_test).exists():
    ...
    subprocess.run(
        [*compose_args, "down", "--timeout", "5"], env=env, capture_output=True, text=True, timeout=60, check=False
    )
    ...
    try:
        ...
        with pathlib.Path(compose_failed_log).open("w", encoding="utf-8") as f:
            f.write(f"STDOUT:\n{full_stdout}\n\nSTDERR:\n{full_stderr}\n")
        logger.error("[IMP:9][clickhouse_up] docker compose up failed — %s", compose_failed_log)
        logger.info("%s", f"[IMP:9][clickhouse_up] STDERR (last 3000): ...{full_stderr[-3000:]}")
        pytest.fail(
            f"ClickHouse stack failed to start. "
            f"Full output: {compose_failed_log}. "
    ...
    try:
        ...
        subprocess.run(
            [*compose_args, "down", "-v", "--timeout", "1"], env=env, capture_output=True, text=True, check=False
        )
    except subprocess.CalledProcessError as exc:
...
@pytest.mark.component
@ldd_trajectory
# GUARD-PRESERVE (168): component-тест реального Docker-контейнера, уникальное покрытие AC-7
# (Prometheus /metrics endpoint); @pytest.mark.component + require_docker_or_fail — не редуцируется
def test_clickhouse_prometheus_metrics(clickhouse_up: dict, caplog) -> None:
    ...
    try:
        r = requests.get(url, timeout=10)
        assert r.status_code == 200, f"/metrics returned {r.status_code}"  # R1 in-body mechanism
        _assert_metrics_ok(r, url)
        logger.info("[IMP:9][test_clickhouse_metrics] PASS: ClickHouse /metrics OK — %d bytes", len(r.text))
    except requests.RequestException as exc:
        _handle_e2e_error(exc, url, caplog, logger)
        return


def _assert_metrics_ok(r: requests.Response, url: str) -> None:
    """Проверить /metrics: HTTP 200 + Prometheus-формат + sample-лог (PLW0717-хелпер).

    ## @io — ⇥ r, url → ⎋ None (pytest.fail при не-200/не-формате)
    ## @complexity — O(L) где L = строки body
    """
    logger.info("[IMP:8][test_clickhouse_metrics] ClickHouse /metrics returned HTTP %s", r.status_code)
    _check_metrics_status(r, url)
    _assert_prometheus_metrics_format(r.text)
    _log_sample_metrics(r.text)


def _check_metrics_status(r: requests.Response, url: str) -> None:
    """Fail при HTTP != 200 на /metrics (PLW0717-хелпер).

    ## @io — ⇥ r, url → ⎋ None (pytest.fail при !=200)
    ## @complexity O(1)
    """
...
def _log_sample_metrics(body: str) -> None:
    """Залогировать первые метрики для видимости (PLW0717-хелпер).

    ## @io — ⇥ body: str → ⎋ None (лог)
...
def _assert_prometheus_metrics_format(body: str) -> None:
    """Проверить Prometheus-формат (HELP/TYPE) и залогировать (PLW0717-хелпер).

    ## @io — ⇥ body: str → ⎋ None (asserts + лог)
    ## @complexity — O(L) где L = строки body
    """
    has_prometheus_format = any(line.startswith(("# HELP", "# TYPE")) for line in body.splitlines())
    logger.info("[IMP:8][test_clickhouse_metrics] Has HELP/TYPE: %s, size: %d bytes", has_prometheus_format, len(body))
...
@pytest.mark.component
@ldd_trajectory
# GUARD-PRESERVE (168): component-тест реального Docker-контейнера, уникальное покрытие AC-8
# (SQL SELECT 1 через HTTP + Basic Auth); @pytest.mark.component + require_docker_or_fail — не редуцируется
def test_clickhouse_basic_query(clickhouse_up: dict, caplog) -> None:
    ...
    try:
        ...
        assert r.status_code in {200, 401, 403}, f"SELECT 1 returned {r.status_code}"  # R1 in-body mechanism

        # Accept both 200 (success) and 401/403 (auth may be disabled or different mechanism)
        _handle_query_response(r, url, query, password)
    except requests.RequestException as exc:
        _handle_e2e_error(exc, url, caplog, logger)
        return


def _handle_query_response(r: requests.Response, url: str, query: str, password: str) -> None:
    """Обработать ответ SELECT 1 (200/401-403/иное) (PLW0717-хелпер).

    ## @io — ⇥ r, url, query, password → ⎋ None (asserts/logs/pytest.fail)
    ## @complexity O(1)
    """
    if r.status_code == 200:
        ...
        _try_header_auth_query(url, query, password)
    else:
        logger.error(
...
def _try_header_auth_query(url: str, query: str, password: str) -> None:
    """Fallback-запрос с X-ClickHouse-User/Key заголовками (PLW0717-хелпер, auth-ветка).

    ## @io — ⇥ url, query, password → ⎋ None (логирует PASS/предупреждение)
    ## @complexity O(1)
    """
    r2 = requests.get(
        url,
    ...
    if r2.status_code == 200 and "1" in r2.text:
        logger.info("[IMP:9][test_clickhouse_query] PASS: SELECT 1 with X-ClickHouse-* headers: %s", r2.text.strip())
    else:
        logger.warning(
