...
_VOLUME_BIND_DIRS = [
    Path(_VOLUME_BASE) / "postgres-data",
    Path(_VOLUME_BASE) / "hermes-agent" / "data",
]
...
def postgres_up(platform_services: dict[str, list[str]], modules_dir) -> None:
    ...
    compose_file = Path(modules_dir) / "postgres" / "docker-compose.base.yml"
    ...
    test_override = Path(compose_file).parent / "docker-compose.test.yml"
    ...
    try:
        ...
        pg_stderr = getattr(exc, "stderr", str(exc))[-500:] if hasattr(exc, "stderr") else str(exc)[-500:]
        logger.error("[IMP:9][postgres_up] Docker logs:\n%s\n%s", pg_diag.stdout[:2000], pg_diag.stderr[:2000])
        pytest.fail(
    ...
    if "Up" not in container_running.stdout:
        ...
        logger.error("[IMP:9][postgres_up] Docker compose logs:\n%s\n%s", pg_logs.stdout[:2000], pg_logs.stderr[:2000])
        pytest.fail(
    ...
    subprocess.run(down_args, env={**os.environ, **ENV}, capture_output=True, text=True, check=False)
...
def hermes_up(platform_services: dict[str, list[str]], postgres_up, modules_dir) -> None:
    ...
    """
    compose_file = Path(modules_dir) / "hermes-agent" / "docker-compose.base.yml"
    ...
    hermes_test_override = Path(compose_file).parent / "docker-compose.test.yml"
    # ⚠️ TRAP[BUG] · 2026-07-24 · Include test.yml in down for consistency with up
    down_cleanup_args = ["docker", "compose", "-f", compose_file]
    if pathlib.Path(hermes_test_override).exists():
        down_cleanup_args.extend(["-f", hermes_test_override])
    down_cleanup_args.extend(["--project-name", COMPOSE_PROJECT_HERMES, "down", "--timeout", "5"])
    subprocess.run(
        down_cleanup_args, env={**os.environ, **ENV_HERMES}, capture_output=True, text=True, timeout=30, check=False
    )
    ...
    if context_image:
        inspect = subprocess.run(
            ["docker", "image", "inspect", context_image.split("@")[0]],
            capture_output=True,
            text=True,
            timeout=10,
            check=False,
    ...
    try:
        ...
        h_logs_args = ["docker", "compose", "-f", compose_file]
        if pathlib.Path(hermes_test_override).exists():
            h_logs_args.extend(["-f", hermes_test_override])
        h_logs_args.extend(["--project-name", COMPOSE_PROJECT_HERMES, "logs", "--tail", "50"])
        hermes_diag = subprocess.run(
            h_logs_args, env={**os.environ, **ENV_HERMES}, capture_output=True, text=True, timeout=30, check=False
        )
        h_stderr = getattr(exc, "stderr", str(exc))[-500:] if hasattr(exc, "stderr") else str(exc)[-500:]
        logger.error("[IMP:9][hermes_up] Docker logs:\n%s\n%s", hermes_diag.stdout[:2000], hermes_diag.stderr[:2000])
        pytest.fail(
    ...
    if "Up" not in container_running.stdout:
        ...
        h_ps_args = ["docker", "compose", "-f", compose_file]
        if pathlib.Path(hermes_test_override).exists():
            h_ps_args.extend(["-f", hermes_test_override])
        h_ps_args.extend(["--project-name", COMPOSE_PROJECT_HERMES, "ps"])
        h_ps = subprocess.run(
            h_ps_args, env={**os.environ, **ENV_HERMES}, capture_output=True, text=True, timeout=30, check=False
        )
        h_logs_args = ["docker", "compose", "-f", compose_file]
        if pathlib.Path(hermes_test_override).exists():
            h_logs_args.extend(["-f", hermes_test_override])
        h_logs_args.extend(["--project-name", COMPOSE_PROJECT_HERMES, "logs", "--tail", "50"])
        h_logs = subprocess.run(
            h_logs_args, env={**os.environ, **ENV_HERMES}, capture_output=True, text=True, timeout=30, check=False
        )
        logger.error("[IMP:9][hermes_up] Compose ps:\n%s", h_ps.stdout[:1000])
        logger.error("[IMP:9][hermes_up] Compose logs:\n%s\n%s", h_logs.stdout[:2000], h_logs.stderr[:2000])
        pytest.fail(
    ...
    subprocess.run(down_args, env={**os.environ, **ENV_HERMES}, capture_output=True, text=True, check=False)
...
def test_hermes_agent_starts(hermes_up, caplog) -> None:
    ...
    with caplog.at_level(logging.DEBUG):
        ...
        container_status = (ps_result.stdout or "").strip()
...
@pytest.mark.component
@ldd_trajectory
# GUARD-PRESERVE (168): component-тест реального hermes-контейнера, уникальное покрытие AC-1
# (dashboard 302 → /auth/login); содержит TRAP[BUG] retry-логику (P1, 2026-08-14) — не редуцируется
def test_hermes_dashboard_auth(hermes_up, caplog) -> None:
    ...
    with caplog.at_level(logging.DEBUG):
        ...
        for attempt in range(3):
            try:
                resp_noauth = _get_noauth_redirect(dashboard_url, attempt)
...
def _get_noauth_redirect(dashboard_url: str, attempt: int) -> requests.Response:
    """GET без auth: ожидается 302 redirect (TRAP[BUG] 2026-07-23 retry, PLW0717-хелпер).

    ## @purpose — Извлечение тела try: запрос + статус-проверка 302. Бросает
    ##             RequestException — retry-цикл в вызывающем.
    ## @io — ⇥ dashboard_url, attempt → ⎋ requests.Response (302)
    ## @complexity O(1)
    """
    ...
    for retry in range(3):
        try:
            ...
            return resp_noauth  # ruff: ignore[TRY300] — retry-loop body: перенос в else ломает retry-семантику (return/break одной попытки)
        except requests.RequestException as exc:
            if retry < 2:
                wait_s = 2**retry  # 1s, 2s backoff
                logger.warning(
                    "[IMP:7][test_hermes_dashboard_auth] Attempt %d failed (%s), retrying in %ds...",
                    retry + 1,
                    exc,
                ...
                raise
    raise AssertionError("unreachable: retry-цикл завершается return или raise")  # ruff: ignore[EM101] — терминальный unreachable-raise, не пользовательская ошибка; явный терминальный raise (implicit-return)
