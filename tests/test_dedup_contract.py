...
TEMPLATE_PATH_RELATIVE: str = os.path.join("core", "templates", "sudo-whitelist.template")
...
def _get_whitelist_path(platform_root: str, module_name: str) -> str:
    ...
    return os.path.join(platform_root, "core", "modules", module_name, "sudo-whitelist.conf")


def _get_template_path(platform_root: str) -> str:
    """Get path to sudo-whitelist template."""
    return os.path.join(platform_root, TEMPLATE_PATH_RELATIVE)
        ...
        for fname in files:
            if fname == "sudo-whitelist.conf":
                ...
                if not os.path.islink(fpath):
                    ...
                    logger.info("[IMP:8][test_no_duplicates] Symlink OK: %s -> %s", rel_path, os.readlink(fpath))
...
def test_module_whitelist_equals_listing(
    ...
    """
    modules_dir = os.path.join(platform_root, "core", "modules")
    assert os.path.isdir(modules_dir), f"modules_dir not found: {modules_dir}"
