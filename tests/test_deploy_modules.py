...
# STRUCTURE: ▶ test_skip_provision_flag (static grep phases.py) → ▶ test_merge_deploy_steps (static grep node-lifecycle.sh) → ▶ testtopo_sort_enriched (native topo_sort.py call with mock yamls) → ▶ фасад-генераторы (sudoers/orphan/overlay) + CI rsync → ⎋ LDD [IMP:9]
...
def testtopo_sort_enriched_output(caplog, tmp_path) -> None:
    ...
    logger.info("[IMP:8][testtopo_sort_enriched_output] Building enriched output ...")
    ...
    logger.info("[IMP:9][testtopo_sort_enriched_output] Module metadata OK (docker/system + severities)")
