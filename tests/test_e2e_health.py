...
# GUARD-PRESERVE (168): e2e-проверка реальных сервисов (Grafana/Langfuse /health) — единственное e2e-покрытие health-слоя
@ldd_trajectory
@pytest.mark.e2e
...
def test_service_health(
    ...
    try:
        resp = _http_get_with_status(full_url, service, **kwargs)
        assert resp.status_code == 200, f"{service} returned {resp.status_code}"  # R1 in-body mechanism
    except (
...
def _http_get_with_status(full_url: str, service: str, **kwargs) -> requests.Response:
    """GET + проверка 200 + тайминг-лог (PLW0717-хелпер).

    ## @io — ⇥ full_url, service, kwargs (auth/timeout) → ⎋ requests.Response (200)
    ## @complexity O(1) — single HTTP request
    """
    start = time.time()
    resp = requests.get(full_url, timeout=kwargs.pop("timeout", 10), **kwargs)
