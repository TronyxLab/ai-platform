...
##   - "TRAP[BUG]" present documenting the FAIL→build decision
##   - "Local L1→L2 build failed" present for build failure handling
...
def test_hermes_fallback_code_present(caplog: pytest.LogCaptureFixture) -> None:
    ...
    assert "Pre-built image not found" in content and "will build locally" in content, (
        "W4 violation: hermes-agent handle_hermes_agent must WARN on missing image with local build fallback"
    )
...
def test_hermes_pull_success(caplog: pytest.LogCaptureFixture) -> None:
    ...
    try:
        result = subprocess.run(
            ["docker", "manifest", "inspect", image_ref], capture_output=True, text=True, timeout=60, check=False
        )
...
def test_hermes_pull_404_build(
    ...
    subprocess.run(
        ["docker", "rmi", "hermes-agent-fallback-test:latest"], capture_output=True, text=True, timeout=30, check=False
    )
...
def test_hermes_build_fallback_fail(
    ...
    if build_result.returncode == 0:
        ...
        # Cleanup
        subprocess.run(
            ["docker", "rmi", "hermes-broken-test:latest"], capture_output=True, text=True, timeout=30, check=False
        )
