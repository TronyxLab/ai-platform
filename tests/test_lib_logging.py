...
_LIB_PATH: Path = Path(__file__).resolve().parent.parent / "core" / "lib" / "logging.sh"
...
""",
    ...
    assert result.returncode == 0, f"Script failed: {result.stderr}"
    assert result.stdout == "", f"log_imp wrote to stdout: {result.stdout!r}"
...
""",
    ...
    assert result.returncode == 0, f"Script failed: {result.stderr}"
    assert result.stdout == "", f"stdout not empty after source: {result.stdout!r}"
    assert result.stderr == "", f"stderr not empty after source: {result.stderr!r}"
