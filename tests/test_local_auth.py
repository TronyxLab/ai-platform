...
@pytest.mark.local_auth
@ldd_trajectory
def test_grafana_login_local_correct(caplog) -> None:
    ...
    logger.info("[IMP:8][test_grafana_login_local_correct] HTTP 200 OK")
    ...
    logger.info(
        "[IMP:9][test_grafana_login_local_correct] PASSED — login=%s role=%s",
        user_info.get("login"),
...
@pytest.mark.local_auth
@ldd_trajectory
def test_grafana_login_local_wrong(caplog) -> None:
    ## @purpose — GET 127.0.0.1:3000/api/user with wrong password → 401
    ## @complexity — O(1)
    _skip_if_port_unreachable("127.0.0.1", 3000)
    logger.info("[IMP:9][test_grafana_login_local_wrong] Starting local Grafana wrong-password test")

    try:
        resp = requests.get(
            GRAFANA_USER_URL,
            auth=(GRAFANA_USER, "WRONG_PASSWORD_12345"),
            timeout=REQUEST_TIMEOUT,
        )
    except requests.ConnectionError:
        require_service_healthy("127.0.0.1", 3000, reason="Grafana HTTP service")
    ...
    logger.info(
        "[IMP:9][test_grafana_login_local_wrong] PASSED — wrong password rejected, HTTP %d",
        resp.status_code,
    )
...
@pytest.mark.local_auth
@ldd_trajectory
def test_langfuse_login_local_correct(caplog) -> None:
    ## @purpose — Langfuse CSRF login with correct credentials → 200, session cookie / no signin redirect
    ## @complexity — O(1)
    logger.info("[IMP:9][test_langfuse_login_local_correct] Starting local Langfuse auth test")

    resp = _langfuse_login(LANGFUSE_EMAIL, LANGFUSE_PASS, caplog)
    if resp is None:
        return

    logger.info(
        "[IMP:8][test_langfuse_login_local_correct] status=%d",
        resp.status_code,
    )
    ...
    logger.info(
        "[IMP:8][test_langfuse_login_local_correct] session cookie found=%s, url=%s", has_session_cookie, redirect_url
    )
    logger.info("[IMP:9][test_langfuse_login_local_correct] PASSED — login succeeded (no signin redirect)")


@pytest.mark.local_auth
@ldd_trajectory
def test_langfuse_login_local_wrong(caplog) -> None:
    ## @purpose — Langfuse CSRF login with wrong password → redirect to signin (failure)
    ## @complexity — O(1)
    logger.info("[IMP:9][test_langfuse_login_local_wrong] Starting local Langfuse wrong-password test")

    resp = _langfuse_login(LANGFUSE_EMAIL, "WRONG_PASSWORD_12345", caplog)
    if resp is None:
        return

    logger.info(
        "[IMP:8][test_langfuse_login_local_wrong] status=%d",
        resp.status_code,
    )
    ...
    logger.info("[IMP:9][test_langfuse_login_local_wrong] PASSED — wrong password correctly rejected")
