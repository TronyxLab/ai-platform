...
##               loki/promtail/prometheus/grafana YAML, dashboards JSON, alert-rules,
...
MODULES_DIR = os.path.join(PROJECT_ROOT, "core", "modules")
...
def _module_dir(module: str) -> str:
    ...
    return os.path.join(MODULES_DIR, module)
...
def test_compose_services_have_profile(domain: dict, caplog) -> None:
    ...
    """
    compose = os.path.join(_module_dir(domain["module_dir"]), "docker-compose.base.yml")
    assert os.path.exists(compose), f"compose base not found: {compose}"
...
def test_healthcheck_sh_sources_lib(domain: dict, caplog) -> None:
    ...
    ## @complexity — O(1)
    """
    healthcheck_sh = os.path.join(_module_dir(domain["module_dir"]), "healthcheck.sh")
    ...
    with open(healthcheck_sh) as fh:
        content = fh.read()
...
CLICKHOUSE_CONFIG_D = os.path.join(CLICKHOUSE_DIR, "config", "config.d")
CLICKHOUSE_USERS_D = os.path.join(CLICKHOUSE_DIR, "config", "users.d")
...
def test_clickhouse_healthcheck_contract(caplog) -> None:
    ...
    healthcheck_sh = os.path.join(CLICKHOUSE_DIR, "healthcheck.sh")
    assert os.path.isfile(healthcheck_sh), f"ClickHouse healthcheck.sh not found at {healthcheck_sh}"

    with open(healthcheck_sh, encoding="utf-8") as fh:
        content = fh.read()
...
def test_clickhouse_module_yaml_structure(caplog) -> None:
    ...
    module_yaml = os.path.join(CLICKHOUSE_DIR, "module.yaml")
    assert os.path.isfile(module_yaml), f"ClickHouse module.yaml not found at {module_yaml}"

    with open(module_yaml, encoding="utf-8") as fh:
        data = yaml.safe_load(fh)
...
def test_clickhouse_makefile_template(caplog) -> None:
    ...
    makefile = os.path.join(CLICKHOUSE_DIR, "Makefile")
    assert os.path.isfile(makefile), f"ClickHouse Makefile not found at {makefile}"
...
def test_clickhouse_users_xml_no_hardcoded_password(caplog) -> None:
    ...
    if not os.path.isdir(CLICKHOUSE_USERS_D):
    ...
    sole_xml = os.path.join(CLICKHOUSE_USERS_D, "10-users.xml")
...
INFRA_METRICS_COMPOSE = os.path.join(INFRA_METRICS_DIR, "docker-compose.base.yml")
...
LITELLM_CONFIG = os.path.join(LITELLM_DIR, "config", "litellm-config.yml")
...
def test_litellm_config_has_required_keys(caplog) -> None:
    ...
    assert os.path.exists(LITELLM_CONFIG), f"litellm-config.yml not found: {LITELLM_CONFIG}"
    logger.info("[IMP:7][test_litellm_static] Config found: %s", LITELLM_CONFIG)

    with open(LITELLM_CONFIG) as fh:
...
def test_litellm_healthcheck_sh_contract(caplog) -> None:
    ...
    healthcheck_sh = os.path.join(LITELLM_DIR, "healthcheck.sh")
    assert os.path.exists(healthcheck_sh), f"healthcheck.sh not found: {healthcheck_sh}"
...
def test_litellm_module_yaml_has_required_fields(caplog) -> None:
    ...
    module_yaml = os.path.join(LITELLM_DIR, "module.yaml")
...
LOGGING_COMPOSE = os.path.join(LOGGING_DIR, "docker-compose.base.yml")
LOGGING_COMPOSE_TEST = os.path.join(LOGGING_DIR, "docker-compose.test.yml")
LOGGING_LOKI_CONFIG = os.path.join(LOGGING_DIR, "config", "loki-config.yml")
LOGGING_PROMTAIL_CONFIG = os.path.join(LOGGING_DIR, "config", "promtail-config.yml")
...
def test_logging_compose_contract(caplog) -> None:
    """logging compose: loki healthcheck (/usr/bin/loki -version), promtail depends_on service_healthy, networks."""
    assert os.path.exists(LOGGING_COMPOSE), f"compose base not found: {LOGGING_COMPOSE}"

    with open(LOGGING_COMPOSE) as fh:
        data = yaml.safe_load(fh)
...
def test_loki_config_valid(caplog) -> None:
    ...
    assert os.path.exists(LOGGING_LOKI_CONFIG), f"loki-config.yml not found: {LOGGING_LOKI_CONFIG}"

    with open(LOGGING_LOKI_CONFIG) as fh:
        data = yaml.safe_load(fh)
...
def test_promtail_config_valid(caplog) -> None:
    """promtail-config.yml: positions/clients/scrape_configs sections."""
    assert os.path.exists(LOGGING_PROMTAIL_CONFIG), f"promtail-config.yml not found: {LOGGING_PROMTAIL_CONFIG}"

    with open(LOGGING_PROMTAIL_CONFIG) as fh:
        data = yaml.safe_load(fh)
...
def test_logging_docker_compose_test_overlay(caplog) -> None:
    ...
    assert os.path.exists(LOGGING_COMPOSE_TEST), f"compose test overlay not found: {LOGGING_COMPOSE_TEST}"

    with open(LOGGING_COMPOSE_TEST) as fh:
...
MONITORING_COMPOSE = os.path.join(MONITORING_DIR, "docker-compose.base.yml")
MONITORING_PROMETHEUS_YML = os.path.join(MONITORING_DIR, "config", "prometheus.yml.tmpl")
MONITORING_GRAFANA_DIR = os.path.join(MONITORING_DIR, "config", "grafana")
MONITORING_DATASOURCES_YML = os.path.join(MONITORING_GRAFANA_DIR, "datasources.yml")
MONITORING_DASHBOARDS_YML = os.path.join(MONITORING_GRAFANA_DIR, "dashboards.yml")
MONITORING_DASHBOARDS_DIR = os.path.join(MONITORING_DIR, "config", "dashboards")
MONITORING_ALERT_RULES_YML = os.path.join(MONITORING_DIR, "config", "alert-rules.yml")
MONITORING_ALERTING_DIR = os.path.join(MONITORING_DIR, "config", "alerting")
...
def test_monitoring_compose_networks(caplog) -> None:
    ...
    with open(MONITORING_COMPOSE) as fh:
        data = yaml.safe_load(fh)
...
def test_prometheus_yml_valid(caplog) -> None:
    ...
    assert os.path.exists(MONITORING_PROMETHEUS_YML), f"prometheus.yml not found: {MONITORING_PROMETHEUS_YML}"

    with open(MONITORING_PROMETHEUS_YML) as fh:
        data = yaml.safe_load(fh)
...
def test_grafana_datasources_yml(caplog) -> None:
    ...
    assert os.path.exists(MONITORING_DATASOURCES_YML), f"datasources.yml not found: {MONITORING_DATASOURCES_YML}"

    with open(MONITORING_DATASOURCES_YML) as fh:
        data = yaml.safe_load(fh)
...
def test_grafana_dashboards_yml(caplog) -> None:
    ...
    assert os.path.exists(MONITORING_DASHBOARDS_YML), f"dashboards.yml not found: {MONITORING_DASHBOARDS_YML}"

    with open(MONITORING_DASHBOARDS_YML) as fh:
        data = yaml.safe_load(fh)
...
def test_dashboards_exist_and_valid_json(caplog) -> None:
    ...
    assert os.path.isdir(MONITORING_DASHBOARDS_DIR), f"Dashboards dir not found: {MONITORING_DASHBOARDS_DIR}"

    present = sorted(os.listdir(MONITORING_DASHBOARDS_DIR))
    ...
    for db_file in present:
        ...
        if db_file in TEMPLATE_DASHBOARDS:
            ...
            continue
        path = os.path.join(MONITORING_DASHBOARDS_DIR, db_file)
        with open(path) as fh:
            try:
                json.load(fh)
...
def test_alert_rules_yml_valid(caplog) -> None:
    ...
    assert os.path.exists(MONITORING_ALERT_RULES_YML), f"alert-rules.yml not found: {MONITORING_ALERT_RULES_YML}"

    with open(MONITORING_ALERT_RULES_YML) as fh:
        data = yaml.safe_load(fh)
...
def test_alerting_provisioning_dir(caplog) -> None:
    ...
    assert os.path.isdir(MONITORING_ALERTING_DIR), f"Alerting provisioning dir not found: {MONITORING_ALERTING_DIR}"

    files = sorted(os.listdir(MONITORING_ALERTING_DIR))
...
def hermes_agent_compose_path(tmp_path_factory):
    ...
    src = os.path.join(MODULES_DIR, "hermes-agent", "docker-compose.base.yml")
...
def test_pgbouncer_no_proxy_includes(hermes_agent_compose_path, caplog) -> None:
    ...
    with open(PLATFORM_ENV_PATH) as fh:
    ...
    with open(hermes_agent_compose_path) as fh:
        data = yaml.safe_load(fh)
...
REDIS_PROMETHEUS_YML = os.path.join(REDIS_MONITORING_DIR, "config", "prometheus.yml.tmpl")
REDIS_DASHBOARDS_DIR = os.path.join(REDIS_MONITORING_DIR, "config", "dashboards")
...
def redis_infra_metrics_compose_path(tmp_path_factory):
    ...
    src = os.path.join(REDIS_INFRA_METRICS_DIR, "docker-compose.base.yml")
...
def redis_dashboards_dir_path(tmp_path_factory):
    ...
    if os.path.isdir(REDIS_DASHBOARDS_DIR):
        for fname in os.listdir(REDIS_DASHBOARDS_DIR):
            src = os.path.join(REDIS_DASHBOARDS_DIR, fname)
...
def test_redis_dashboard_exists(redis_dashboards_dir_path, caplog) -> None:
    ...
    redis_dash_path = os.path.join(redis_dashboards_dir_path, "redis.json")
    ...
    with open(redis_dash_path) as fh:
...
def test_redis_healthcheck_deep_exit(redis_healthcheck_path, caplog) -> None:
    ...
    with open(redis_healthcheck_path) as fh:
        content = fh.read()
