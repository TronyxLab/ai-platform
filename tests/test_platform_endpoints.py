...
def test_langfuse_health_endpoint(caplog, platform_services) -> None:
    ...
    if not _check_port_forwarded(port, "/api/public/health"):
        ...
        diag_parts = [f"Langfuse port {port} is not accessible via _check_port_forwarded."]
        ...
        diag_parts.append(f"Container status: {ps.stdout.strip() or 'container not found'}")

        # Check listening ports in relevant test range
        lsof = _sp.run(
            ["lsof", "-iTCP", "-sTCP:LISTEN", "-P", "-n"], capture_output=True, text=True, timeout=5, check=False
        )
        port_lines = [
            line
            for line in lsof.stdout.splitlines()
            if any(p in line for p in ("13000", "13030", "19090", "19119", "14000", "18642"))
        ]
        if port_lines:
            diag_parts.append("Listening ports in test range:\n" + "\n".join(port_lines))
        else:
            diag_parts.append("No relevant ports listening on localhost")

        # R4 (DevPlan 160 W5 T5.1): skip-as-bug-masking запрещён. Диагностика сохраняется
        # как причина для канонического honesty-диспетчера require_service_healthy:
        # TCP-пробу (port не forward-ится) → marker-режим skip (локальный стек не поднят),
        # fail-режим (CI) → FAIL (NO_SERVICE = FAIL, Rule R4).
        logger.warning(
            "[IMP:9][test_langfuse_health_endpoint] Langfuse port %d not accessible: %s", port, " | ".join(diag_parts)
        )
        require_service_healthy(
            "127.0.0.1",
            port,
            reason=f"Langfuse container not accessible on port {port}: {'; '.join(diag_parts)}",
        )
