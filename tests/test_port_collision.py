...
def test_no_hardcoded_ports(all_compose_files, caplog) -> None:
    ...
    """## @purpose — WARN (not FAIL) if ports are hardcoded as numbers instead of env var pattern.
    ##            Preferred pattern: ${PORT:-default}
    ## @io — ⇥ all_compose_files: dict[str, str] → ⎋ None (WARN, pass always)
    ## @complexity — O(N * Y) where N = compose count, Y = YAML parse size
    """
