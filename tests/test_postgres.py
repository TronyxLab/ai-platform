...
def test_module_yaml_no_per_module_version_file(postgres_fixtures, caplog) -> None:
    ...
    with caplog.at_level(logging.DEBUG):
        ...
        version_file = os.path.join(postgres_fixtures["MODULE_DIR"], "VERSION")
...
@ldd_trajectory
def test_compose_no_ports(postgres_fixtures, caplog) -> None:
    with caplog.at_level(logging.DEBUG):
        logger.info("[IMP:7][test_postgres][compose_no_ports] START")
        ...
        logger.critical(
            "[IMP:9][test_postgres][compose_no_ports] ASSERT: ports=%s (expected None/absent)",
            ports,
        )
        assert ports is None, f"R1 violation: ports must be absent, got: {ports}"


@ldd_trajectory
def test_compose_restart_unless_stopped(postgres_fixtures, caplog) -> None:
    with caplog.at_level(logging.DEBUG):
        logger.info("[IMP:7][test_postgres][compose_restart] START")
        ...
        logger.critical(
            "[IMP:9][test_postgres][compose_restart] ASSERT: restart=%s (expected unless-stopped)",
            restart,
        )
        assert restart == "unless-stopped", f"R1 violation: restart must be 'unless-stopped', got: {restart}"


@ldd_trajectory
def test_compose_stop_grace_period(postgres_fixtures, caplog) -> None:
    with caplog.at_level(logging.DEBUG):
        logger.info("[IMP:7][test_postgres][compose_grace] START")
        ...
        logger.critical(
            "[IMP:9][test_postgres][compose_grace] ASSERT: stop_grace_period=%s (expected 60s)",
            grace,
        )
        assert grace == "60s", f"06§13 violation: stop_grace_period must be '60s', got: {grace}"
...
@ldd_trajectory
def test_compose_shared_db_net_external(postgres_fixtures, caplog) -> None:
    with caplog.at_level(logging.DEBUG):
        logger.info("[IMP:7][test_postgres][compose_network] START")

        data = _load_compose(postgres_fixtures["COMPOSE_FILE"])
        networks = data.get("networks", {})
        shared_db_net = networks.get("shared-db-net", {})
        is_external = shared_db_net.get("external")

        logger.critical(
            "[IMP:9][test_postgres][compose_network] ASSERT: shared-db-net.external=%s (expected True)",
            is_external,
        )
        assert is_external is True, f"00§9 violation: shared-db-net must be external: true, got: {is_external}"
...
def test_postgresql_conf_owner_tuning(postgres_fixtures, caplog) -> None:
    ...
    """
    conf_path = os.path.join(postgres_fixtures["MODULE_DIR"], "config", "postgresql.conf")

    with caplog.at_level(logging.DEBUG):
        logger.info("[IMP:7][test_postgres][conf_tuning] START: validating postgresql.conf against owner reference")

        assert os.path.isfile(conf_path), f"postgresql.conf not found: {conf_path}"
...
def test_pg_hba_covers_docker_bridge_range(postgres_fixtures, caplog) -> None:
    ...
    """
    hba_path = os.path.join(postgres_fixtures["MODULE_DIR"], "config", "pg_hba.conf")

    with caplog.at_level(logging.DEBUG):
        logger.info("[IMP:7][test_postgres][pg_hba] START: checking pg_hba.conf covers Docker bridge range")

        assert os.path.isfile(hba_path), f"pg_hba.conf not found: {hba_path}"
...
def test_sudo_whitelist_agent_cannot_stop_or_restore(postgres_fixtures, caplog) -> None:
    ...
    with caplog.at_level(logging.DEBUG):
        ...
        whitelist_path = os.path.join(postgres_fixtures["MODULE_DIR"], "sudo-whitelist.conf")
