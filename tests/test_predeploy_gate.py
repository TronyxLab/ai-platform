# GREP_SUMMARY: predeploy-gate compose-env-vars yaml-valid external-networks hardcoded-credentials predeploy
# STRUCTURE: ▶ all_compose_files → ∋ (yaml.safe_load|regex) → ◇ test_required_env_vars_present ⊕ test_all_compose_configs_valid ⊕ test_docker_networks_precreated ⊕ test_no_hardcoded_credentials → ∑ IMP:7-10 logs → ⎋ pass|fail
# @file test_predeploy_gate.py
# @purpose  Pre-deploy gate tests: static validation of docker-compose configs,
#           required env vars, external networks, and hardcoded credentials.
...
#           W2 T2.3 (DevPlan 160): test_docker_image_tags_pinned УДАЛЁН — дубль
#           image-pinning триплета: test_gate_image_tag_form.py (ghcr tag-form, canon, R5-negative)
#           + test_compose_contract.py::test_images_pinned_tag_digest (digest-pin ВСЕХ не-local образов;
#           :latest без digest → RED там). predeploy-вариант сканировал те же base.yml файлы
#           со слабее политикой (только :latest) и пустой детекционной поверхностью
#           (единственные local-build :latest — backup-cron/status-page — в его own исключениях).
...
#             AC-T2 from DevPlan — 4 acceptance criteria (image-pinning удалён W2 T2.3).
#

# region MODULE_CONTRACT
## @purpose  Pre-deploy gate: 4 lightweight tests that validate docker-compose configs
##           before `docker compose up`. Catches missing env vars, invalid
##           YAML, missing external networks, and hardcoded credentials.
##           AC-T2.1, AC-T2.3 through AC-T2.5 from DevPlan (AC-T2.2 image-pinning —
##           консолидирован W2 T2.3 в test_gate_image_tag_form.py + test_compose_contract.py).
...
def _parse_compose_ports(compose_file: str) -> list[int]:
    ...
    for svc_name, svc_config in services.items():
        if not isinstance(svc_config, dict):
            continue
        svc_ports = _extract_host_ports(svc_config.get("ports"))
        host_ports.extend(svc_ports)
        if svc_ports:
            logger.info(
...
def _extract_host_ports(ports: object) -> list[int]:
    """Извлечь host-порты из ports-секции сервиса (PLR1702-хелпер).

    ## @io — ⇥ ports (list|None) → ⎋ list[int] host-портов
    ## @complexity — O(P) где P = entries
    """
    host_ports: list[int] = []
...
def _find_project_ai_platform_yamls(node_yaml_projects: list[dict]) -> list[tuple[str, str]]:
    ...
    for proj in node_yaml_projects:
        ...
        primary = Path(projects_dir) / name / "ai-platform.yaml"
        if Path(primary).is_file():
            results.append((name, primary))
            continue

        # Fallback: tests/test_data/projects/<name>/ai-platform.yaml
        fallback = Path(test_data_dir) / name / "ai-platform.yaml"
...
def test_required_env_vars_present(
    ...
    for var_name in sorted(required_vars):
        ...
        env_path = Path(modules_dir) / "hermes-agent" / ".env"
...
def _docker_network_names() -> set[str] | None:
    """Имена Docker-сетей через `docker network ls` (PLW0717-хелпер).

    ## @io — ⎋ set[str] | None (None при сбое CLI/timeout/rc!=0)
    ## @complexity O(N) где N = сети
    """
    try:
...
def _validate_compose_config(compose_path: pathlib.Path, errors: list[str]) -> None:
    """`docker compose config --dry-run` — валидность compose-файла (PLW0717-хелпер).

    ## @io — ⇥ compose_path, errors (мутируется) → ⎋ None
...
def test_docker_networks_precreated(
    ...
    if not external_networks:
        ...
        return
    # endregion BLOCK_CollectExternalNetworks

    # region BLOCK_GetDockerNetworks
    existing_networks = _docker_network_names()
    if existing_networks is None:
        logger.warning("[IMP:8][test_docker_networks_precreated] docker network ls failed — skipping network check")
        return
    logger.info(
    ...
    # endregion BLOCK_GetDockerNetworks
...
def test_project_compose_configs_valid(
    ...
    for compose_path in project_compose_files:
        ...
        try:
            _validate_compose_config(compose_path, errors)
        except FileNotFoundError:
...
def test_project_ports_no_conflict(
    ...
    # endregion BLOCK_CollectProjectPorts
