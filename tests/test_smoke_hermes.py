...
def test_hermes_auth_login(caplog, platform_services) -> None:
    ...
    try:
        _assert_password_login(url, payload)
    except requests.RequestException as exc:
        _handle_e2e_error(exc, url, caplog, logger)


def _assert_password_login(url: str, payload: dict) -> None:
    """POST /auth/password-login: ok:true + session cookie (PLW0717-хелпер).

    ## @io — ⇥ url, payload → ⎋ None (asserts)
...
def test_hermes_api_completions(caplog, platform_services) -> None:
    ...
    try:
        _assert_chat_completion(url, payload, headers)
    except requests.RequestException as exc:
        _handle_e2e_error(exc, url, caplog, logger)


def _assert_chat_completion(url: str, payload: dict, headers: dict) -> None:
