...
def test_langfuse_health(caplog, platform_services):
    ...
    for attempt in range(3):
        try:
            _assert_langfuse_health(attempt)
            break
...
def _assert_langfuse_health(attempt: int) -> None:
    """GET health: HTTP 200 + status OK (TRAP[BUG] 2026-07-23 retry, PLW0717-хелпер).

    ## @io — ⇥ attempt → ⎋ None (asserts; бросает RequestException в retry-цикл)
...
def test_langfuse_ingestion(caplog, platform_services):
    ...
    now = time.time()
    basic = base64.b64encode(f"{langfuse_pk}:{langfuse_sk}".encode()).decode()
