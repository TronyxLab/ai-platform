...
def test_litellm_models_api(caplog, platform_services) -> None:
    ...
    try:
        # R1 in-body mechanism: статус проверяется и здесь, и в helper (200 + data)
        r = requests.get(url, headers=headers, timeout=10)
        assert r.status_code == 200, f"/v1/models returned {r.status_code}"
        _assert_litellm_models(url, headers)
...
def _assert_litellm_models(url: str, headers: dict) -> None:
    """GET /v1/models: HTTP 200 + non-empty data (PLW0717-хелпер).

    ## @io — ⇥ url, headers → ⎋ None (asserts)
