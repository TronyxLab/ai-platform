...
##   - Container names: loki-test, promtail-test (from test.yml override)
...
_LOGGING_MODULE = _PROJECT_ROOT / "core" / "modules" / "logging"
...
def logging_compose():
    ...
    _logger.info("[IMP:7][logging_compose][setup] Starting logging smoke fixture")

    # ── Step 1: Stop any running wave-logging project ──────────────────────
    _logger.info("[IMP:7][logging_compose][setup] Stopping existing wave-logging project")
    down_args = [
        "docker",
        "compose",
        "-p",
    ...
    try:
        ...
        _logger.warning("[IMP:8][logging_compose][setup] wave-logging down timed out")

    # ── Step 2: Ensure external networks exist ─────────────────────────────
    created_by_us = set()
    for net in _EXTERNAL_NETWORKS:
        _logger.info("[IMP:7][logging_compose][setup] Checking network %s", net)
        try:
            ...
            if inspect_result.returncode != 0:
                _logger.info("[IMP:8][logging_compose][setup] Creating network %s", net)
                subprocess.run(
                ...
                _logger.info("[IMP:9][logging_compose][setup] Created %s", net)
            else:
                _logger.info("[IMP:8][logging_compose][setup] %s already exists", net)
        except subprocess.TimeoutExpired:
    ...
    try:
        ...
        _logger.info("[IMP:8][logging_compose][setup] compose up rc=%d", up_result.returncode)
        ...
        _logger.info("[IMP:9][logging_compose][setup] wave-logging-smoke started successfully")
    except subprocess.TimeoutExpired:
        _logger.error("[IMP:9][logging_compose][setup] compose up timed out after %ds", _COMPOSE_UP_TIMEOUT)
    ...
    _logger.info("[IMP:7][logging_compose][teardown] Tearing down wave-logging-smoke")
    down_smoke_args = [
        "docker",
        "compose",
        "-p",
    ...
    try:
        ...
        _logger.warning("[IMP:8][logging_compose][teardown] compose down timed out")

    # ── Remove networks only if we created them ───────────────────────────
    for net in created_by_us:
        _logger.info("[IMP:7][logging_compose][teardown] Removing network %s (created by fixture)", net)
        try:
            subprocess.run(
            ...
            _logger.info("[IMP:9][logging_compose][teardown] %s removed", net)
        except subprocess.TimeoutExpired:
            _logger.warning("[IMP:8][logging_compose][teardown] Failed to remove %s", net)

    _logger.info("[IMP:9][logging_compose][teardown] Fixture teardown complete")
