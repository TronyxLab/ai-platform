...
def test_platform_starts_all_containers(
    ...
    logger.info(
        "[IMP:7][test_platform_starts_all_containers] Started modules: {started}, failed: {failed}",
    )
    # endregion
    ...
    for module_name in started:
        ...
        # ── DIAGNOSTIC: log raw docker compose ps command and output ──
        logger.info(
            "[IMP:9][DIAG][test_platform_starts_all_containers] ps_args=%s, COMPOSE_PROFILES=%s", ps_args, module_name
        )
        result = _run_docker(
            ps_args,
            timeout=30,
