...
def _remove_network(net_name: str) -> None:
    ...
    ## @complexity — O(1)
    """
    subprocess.run(["docker", "network", "rm", net_name], capture_output=True, text=True, timeout=30, check=False)
...
def test_all_compose_configs_valid(
    ...
    root_compose = Path(platform_root) / "docker-compose.yml"
    assert pathlib.Path(root_compose).is_file(), f"root docker-compose.yml not found: {root_compose}"
    # endregion BLOCK_Setup
    ...
    # endregion BLOCK_Assert_AtLeastOne
...
def test_platform_starts_all_containers(
    ...
    expected_services = [s for s in expected_services if s not in ONESHOT_CONTAINERS]
    # ── Deduplicate ───────────────────────────────────────────────────────────
    expected_services = sorted(set(expected_services))
    # endregion BLOCK_CollectExpected
...
def test_critical_services_healthy(
    ...
    # endregion BLOCK_ResolveContainers
    ...
    # endregion BLOCK_FilterCritical
...
def test_platform_cleanup(
    ...
    # endregion BLOCK_VerifyProjectName
    ...
    # endregion BLOCK_VerifyDownCommand
    ...
    # endregion BLOCK_VerifyFixtureTeardown
