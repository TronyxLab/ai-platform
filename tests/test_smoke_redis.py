...
def _ensure_redis_network(created_by_us: bool) -> bool:
    """docker network inspect/create test-shared-cache-net (TRAP[BUG] 2026-07-21, PLW0717-хелпер).

    ## @io — ⇥ created_by_us → ⎋ bool (обновлённый флаг создания)
    ...
    try:
        ...
        if inspect_result.returncode != 0:
            created_by_us = _create_redis_network(created_by_us)
        else:
            logger.info("[IMP:8][redis_compose][setup] shared-cache-net already exists")
    except subprocess.TimeoutExpired:
        logger.error("[IMP:9][redis_compose][setup] Timeout checking/creating shared-cache-net")
        pytest.fail("Failed to ensure shared-cache-net exists")
    return created_by_us


def _create_redis_network(created_by_us: bool) -> bool:
    """docker network create test-shared-cache-net (PLW0717-хелпер).

    ## @io — ⇥ created_by_us → ⎋ bool (обновлённый флаг)
    ## @complexity O(1) — один subprocess
    """
    logger.info("[IMP:8][redis_compose][setup] Creating test-shared-cache-net")
    subprocess.run(
    ...
    logger.info("[IMP:9][redis_compose][setup] Created shared-cache-net")
    return True


def _redis_compose_up(compose_up_args: list[str], env_up: dict) -> None:
...
def redis_compose(platform_services: dict[str, list[str]]) -> dict:
    ...
    """
    logger = logging.getLogger(__name__)
    logger.info("[IMP:7][redis_compose][setup] Starting redis smoke fixture")
    ...
    if foreign:
        logger.info("[IMP:8][redis_compose] Reusing redis from platform_services")
    ...
    logger.info("[IMP:7][redis_compose][setup] Stopping existing wave-redis project")
    down_args = [
        "docker",
        "compose",
        "-p",
    ...
    try:
        ...
        logger.warning("[IMP:8][redis_compose][setup] wave-redis down timed out — continuing")
    ...
    created_by_us = False
    logger.info("[IMP:7][redis_compose][setup] Checking test-shared-cache-net")
    created_by_us = _ensure_redis_network(created_by_us)

    # ── Step 3: Remove stale container from shared stack ──────────────────────
    logger.info("[IMP:8][fixture][setup] Cleaning stale container: redis-test")
    subprocess.run(
    ...
    logger.info("[IMP:7][redis_compose][setup] Starting wave-redis-smoke compose")
    compose_up_args = [
        "docker",
        "compose",
        "-p",
    ...
    try:
        ...
        logger.info("[IMP:9][redis_compose][setup] wave-redis-smoke started successfully")
    except subprocess.TimeoutExpired:
        logger.error("[IMP:9][redis_compose][setup] compose up timed out after %ds", _COMPOSE_UP_TIMEOUT)
    ...
    logger.info("[IMP:7][redis_compose][teardown] Tearing down wave-redis-smoke")
    down_smoke_args = [
        "docker",
        "compose",
        "-p",
    ...
    try:
        ...
        logger.warning("[IMP:8][redis_compose][teardown] compose down timed out — continuing")

    # ── Remove test-shared-cache-net only if we created it ─────────────────────
    if created_by_us:
        logger.info("[IMP:7][redis_compose][teardown] Removing test-shared-cache-net (created by fixture)")
        try:
            subprocess.run(
            ...
            logger.info("[IMP:9][redis_compose][teardown] test-shared-cache-net removed")
        except subprocess.TimeoutExpired:
            logger.warning("[IMP:8][redis_compose][teardown] Failed to remove test-shared-cache-net")

    logger.info("[IMP:9][redis_compose][teardown] Fixture teardown complete")
