...
# STRUCTURE: ▶ sys.path+tmp_path → write module.yaml mocks → topo_sort.load_module_yamls → build_dag → kahn_topological_sort → ◇ assert groups → ⊕ LDD trajectory → ⎋ IMP:9 check
# region MODULE_CONTRACT [DOMAIN(TESTING):3; CONCEPT(TOPO-SORT):4; TECH(PYTEST):2]
## @purpose  Unit tests for core/internal/bootstrap/topo_sort.py topological sort
...
##   - Tests call topo_sort functions directly (native imports, no subprocess)
...
##   testtopo_sort_linear [W:2] — a->b->c linear chain -> 3 groups
##   testtopo_sort_parallel_groups [W:2] — diamond dep -> 3 groups with parallelism
##   testtopo_sort_cycle_detection [W:2] — a->b->a cycle -> RuntimeError
##   testtopo_sort_no_deps [W:1] — modules without depends_on -> single group
##   testtopo_sort_filter_names [W:2] — --filter-names restricts to subset
## @usecases
##   - CI: verifies that topological sort correctly orders dependencies
##   - Refactoring: ensures changes to topo_sort.py don't break deploy ordering
...
# Add core/internal/bootstrap to sys.path to import topo_sort
...
logger = logging.getLogger("testtopo_sort")
...
# region FUNC_testtopo_sort_linear
...
def testtopo_sort_linear(tmp_path: Path, caplog) -> None:
    ...
    modules = topo_sort.load_module_yamls(modules_dir)
    docker_mods = topo_sort.filter_docker_modules(modules)
    dag = topo_sort.build_dag(docker_mods)
...
# endregion FUNC_testtopo_sort_linear


# region FUNC_testtopo_sort_parallel_groups
...
def testtopo_sort_parallel_groups(tmp_path: Path, caplog) -> None:
    caplog.set_level(logging.INFO)
...
# endregion FUNC_testtopo_sort_parallel_groups


# region FUNC_testtopo_sort_cycle_detection
...
def testtopo_sort_cycle_detection(tmp_path: Path, caplog) -> None:
    caplog.set_level(logging.INFO)
    ...
    with pytest.raises(ConfigValidationError) as excinfo:
        topo_sort.kahn_topological_sort(dag)
...
# endregion FUNC_testtopo_sort_cycle_detection


# region FUNC_testtopo_sort_no_deps
...
def testtopo_sort_no_deps(tmp_path: Path, caplog) -> None:
    caplog.set_level(logging.INFO)
...
# endregion FUNC_testtopo_sort_no_deps


# region FUNC_testtopo_sort_filter_names
...
def testtopo_sort_filter_names(tmp_path: Path, caplog) -> None:
    caplog.set_level(logging.INFO)
    ...
    dag = topo_sort.build_dag(docker_mods, filter_names=["a", "c"])
...
# endregion FUNC_testtopo_sort_filter_names
