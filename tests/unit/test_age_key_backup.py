...
@pytest.fixture()
def sops_ok(monkeypatch: pytest.MonkeyPatch) -> None:
    """Patch sops presence + subprocess.run to return a fake encrypted container."""
    ...
    monkeypatch.setattr(akb.shutil, "which", lambda name: f"/usr/local/bin/{name}")
    monkeypatch.setattr(akb.subprocess, "run", _fake_run)
...
def s3_ready(monkeypatch: pytest.MonkeyPatch) -> FakeS3Client:
    """Patch get_s3_client factory + S3_BUCKET env; returns the fake client."""
    fake = FakeS3Client()
    monkeypatch.setattr("core.internal.shared.s3_client.get_s3_client", lambda **_: fake)
...
def test_full_pipeline_encrypt_upload_sha256(caplog, sops_ok, s3_ready: FakeS3Client) -> None:
    ...
    monkey.setattr(akb, "detect_age_key", lambda: _FAKE_KEY)
    try:
        rc = akb.run_backup(_args("--recipient", _FAKE_RECIPIENT, "--s3-key", "age-key-backup/test.enc"))
    finally:
        monkey.undo()
...
# region TEST_dry_run
def test_dry_run_no_mutation(caplog, sops_ok, monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    ...
    """
    caplog.set_level(logging.DEBUG)
    monkeypatch.setattr(akb, "detect_age_key", lambda: _FAKE_KEY)
    fake = FakeS3Client()
    monkeypatch.setattr("core.internal.shared.s3_client.get_s3_client", lambda **_: fake)
    # S3_BUCKET НЕ задан — dry-run не должен требовать S3-конфиг

    out_enc = tmp_path / "age-master-key.enc"
    rc = akb.run_backup(_args("--recipient", _FAKE_RECIPIENT, "--output-enc", str(out_enc), "--dry-run"))
...
def test_no_upload_encrypt_only(caplog, sops_ok, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    rc = akb.run_backup(_args("--recipient", _FAKE_RECIPIENT, "--no-upload"))
...
def test_no_secrets_in_stdout_and_caplog(caplog, sops_ok, s3_ready: FakeS3Client, capsys) -> None:
    ...
    monkeypatch.setattr(akb, "detect_age_key", lambda: _FAKE_KEY)
    try:
        rc = akb.main(["--recipient", _FAKE_RECIPIENT, "--s3-key", "age-key-backup/test.enc"])
    finally:
        monkeypatch.undo()
...
def test_output_enc_written(caplog, sops_ok, monkeypatch: pytest.MonkeyPatch, tmp_path) -> None:
    ...
    rc = akb.run_backup(_args("--recipient", _FAKE_RECIPIENT, "--output-enc", str(out_enc), "--no-upload"))
...
def test_exit_code_key_missing(caplog, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    caplog.set_level(logging.DEBUG)
    monkeypatch.setattr(akb, "detect_age_key", lambda: None)

    rc = akb.run_backup(_args("--recipient", _FAKE_RECIPIENT))
...
def test_exit_code_recipient_missing(caplog, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    rc = akb.run_backup(_args())

    assert rc == 2, "missing AGE_RECIPIENT must be ConfigNotFound (2)"
    _assert_ldd_imp9(caplog)


def test_exit_code_sops_missing(caplog, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    caplog.set_level(logging.DEBUG)
    monkeypatch.setattr(akb, "detect_age_key", lambda: _FAKE_KEY)
    monkeypatch.setattr(akb.shutil, "which", lambda name: None)

    # encrypt_age_key кидает PlatformFatalError — exit-код мапится в main()
    rc = akb.main(["--recipient", _FAKE_RECIPIENT, "--no-upload"])
...
def test_exit_code_sha256_mismatch(caplog, sops_ok, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("core.internal.shared.s3_client.get_s3_client", lambda **_: tampered)
    monkeypatch.setenv("S3_BUCKET", "chaos-dr-backup-bucket")
    monkeypatch.setattr(
        akb.subprocess,
        "run",
        lambda cmd, *a, **k: (
            SimpleNamespace(returncode=0, stdout="DIFFERENT-CONTENT", stderr="")
            if cmd and cmd[0] == "sops"
            else SimpleNamespace(returncode=0, stdout="", stderr="")
        ),
    )

    rc = akb.main(["--recipient", _FAKE_RECIPIENT, "--s3-key", "age-key-backup/corrupt.enc"])
...
def test_main_returns_int_and_ok(caplog, sops_ok, s3_ready: FakeS3Client, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    rc = akb.main(["--recipient", _FAKE_RECIPIENT, "--s3-key", "age-key-backup/main.enc"])
