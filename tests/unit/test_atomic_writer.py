...
# · Scenario: replace_fn injected to raise → target keeps ORIGINAL content, temp removed
# · Remove if: atomic_writer failure semantics change
def test_atomic_write_no_partial_write_negative(tmp_path: Path, caplog: pytest.LogCaptureFixture) -> None:
    ...
    # 167 D6 (DI-zero): replace_fn seam — fake-сбой rename вместо monkeypatch os.replace
    with caplog.at_level(logging.INFO), pytest.raises(OSError):
        atomic_write(str(target), "NEW=1\n", mode=0o600, replace_fn=_boom)
