...
## @rationale C-5 fix (debt 096) создал audit.sh; тест оставался stale на удалённый shell audit — 5 failures
...
_PROJECT_ROOT = str(Path(_TEST_DIR).parent)
_AUDIT_SH = str(Path(_PROJECT_ROOT) / "core" / "lib" / "audit.sh")
...
"""
    ...
    result = subprocess.run(["bash", str(wrapper_path)], capture_output=True, text=True, timeout=10, check=False)
...
def test_audit_step_success(tmp_path, caplog):
    ...
    for entry in entries:
        logger.info("%s", f"    {json.dumps(entry, ensure_ascii=False)}")
