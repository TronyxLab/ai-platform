...
MODULE_DIR = Path(__file__).parent / "../.." / "core" / "modules" / "backup-cron"
COMPOSE_FILE = Path(MODULE_DIR) / "docker-compose.base.yml"
MODULE_YAML = Path(MODULE_DIR) / "module.yaml"
CRONTAB_FILE = Path(MODULE_DIR) / "scripts" / "crontab"
HEALTHCHECK_SH = Path(MODULE_DIR) / "healthcheck.sh"
READY_CHECK_SH = Path(MODULE_DIR) / "ready-check.sh"
UPLOAD_S3_SH = Path(MODULE_DIR) / "scripts" / "upload-s3.sh"
BACKUP_POSTGRES_SH = Path(MODULE_DIR) / "scripts" / "backup-postgres.sh"
MINIO_COMPOSE_FILE = Path(__file__).parent.parent.parent / "core" / "modules" / "minio" / "docker-compose.base.yml"
...
def test_backup_postgres_uses_port_env(caplog) -> None:
    ...
    with caplog.at_level(logging.DEBUG):
        ...
        backup_postgres_py = Path(MODULE_DIR) / "scripts" / "backup_postgres.py"
        assert pathlib.Path(backup_postgres_py).is_file(), f"backup_postgres.py not found: {backup_postgres_py}"
        with pathlib.Path(backup_postgres_py).open(encoding="utf-8") as f:
            py_content = f.read()
        ...
        assert pathlib.Path(COMPOSE_FILE).is_file(), f"compose file not found: {COMPOSE_FILE}"
        with pathlib.Path(COMPOSE_FILE).open(encoding="utf-8") as f:
        ...
        assert pathlib.Path(MINIO_COMPOSE_FILE).is_file(), f"minio compose file not found: {MINIO_COMPOSE_FILE}"
        with pathlib.Path(MINIO_COMPOSE_FILE).open(encoding="utf-8") as f:
...
def test_compose_spool_volume_declared(caplog) -> None:
    ...
    with caplog.at_level(logging.DEBUG):
        ...
        root_compose_path = Path(__file__).parent / "../.." / "docker-compose.yml"
        with pathlib.Path(root_compose_path).open(encoding="utf-8") as f:
...
def test_crontab_has_three_schedule_entries(caplog) -> None:
    ...
    with caplog.at_level(logging.DEBUG):
        ...
        assert pathlib.Path(CRONTAB_FILE).is_file(), f"crontab not found: {CRONTAB_FILE}"
        with pathlib.Path(CRONTAB_FILE).open(encoding="utf-8") as f:
            content = f.read()
...
def test_crontab_scripts_exist(caplog) -> None:
    ...
    with caplog.at_level(logging.DEBUG):
        ...
        assert pathlib.Path(CRONTAB_FILE).is_file()
        with pathlib.Path(CRONTAB_FILE).open(encoding="utf-8") as f:
            content = f.read()
        ...
        scripts_dir = Path(MODULE_DIR) / "scripts"
        missing = []
...
def test_liveness_uses_pgrep_cron(caplog) -> None:
    ...
    with caplog.at_level(logging.DEBUG):
        ...
        assert pathlib.Path(HEALTHCHECK_SH).is_file(), f"healthcheck.sh not found: {HEALTHCHECK_SH}"
        with pathlib.Path(HEALTHCHECK_SH).open(encoding="utf-8") as f:
            content = f.read()
...
def test_readiness_check_removed(caplog) -> None:
    ...
    with caplog.at_level(logging.DEBUG):
        ...
        assert not pathlib.Path(READY_CHECK_SH).is_file(), (
            f"B7 FAIL: ready-check.sh должен быть удалён (0 callers), найден: {READY_CHECK_SH}"
        )
        dockerfile = Path(MODULE_DIR) / "Dockerfile"
...
def test_upload_s3_real_implementation(caplog) -> None:
    ...
    with caplog.at_level(logging.DEBUG):
        ...
        assert pathlib.Path(UPLOAD_S3_SH).is_file(), f"upload-s3.sh not found: {UPLOAD_S3_SH}"
        with pathlib.Path(UPLOAD_S3_SH).open(encoding="utf-8") as f:
            content = f.read()
...
def test_sudo_whitelist_agent_cannot_stop_or_restore(caplog) -> None:
    ...
    with caplog.at_level(logging.DEBUG):
        ...
        whitelist_path = Path(MODULE_DIR) / "sudo-whitelist.conf"
