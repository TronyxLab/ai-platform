...
class _FakeFacts:
    """EnvironmentFacts-fake (W-H DevPlan 163 / 167 D0): path_isfile через параметр.

    ## @purpose — DI канал disk-present seam: тест передаёт path_isfile override напрямую
    ##             в _process_single_domain(facts=...) вместо monkeypatch os.path.isfile.
    """

    def __init__(self, path_isfile) -> None:
...
@ldd_trajectory
def test_upload_called_on_skip(caplog, tmp_path):
    """_process_single_domain() calls _upload_to_s3() when cert exists on disk (skip path)."""
    # DI (W-H + 167 D0): cert_validity_fn/s3_cache/upload_fn/facts-канал — 0 патчей
    upload_called = False

    def mock_upload(domain, **kwargs):
    ...
    # disk-present seam (fullchain.pem) через facts.path_isfile (DI, DevPlan 167 D0) —
    # заменяет monkeypatch os.path.isfile: EnvironmentFacts-канал уже существует
    facts = _FakeFacts(path_isfile=mock_isfile)
...
def test_upload_called_after_issue(caplog, tmp_path, monkeypatch):
    ...
    # DI (W-H): cert_validity_fn + s3_cache параметрами (0 патчей модуля/env)
    mock_s3 = MagicMock()
