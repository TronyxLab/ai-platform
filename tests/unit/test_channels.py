...
class TestPayload:
    ...
    # endregion FUNC_test_payload_creation
    ...
    # endregion FUNC_test_payload_empty_project_name
    ...
    # endregion FUNC_test_payload_empty_tar_path
    ...
    def test_payload_defaults(self, temp_tar: Path) -> None:
        ...
        assert not payload.version
        assert payload.metadata == {}

    # endregion FUNC_test_payload_defaults
...
class TestDeliveryResult:
    ...
    # endregion FUNC_test_delivery_result_defaults
    ...
    # endregion FUNC_test_delivery_result_failure
...
class TestDeliveryChannelABC:
    ...
    # endregion FUNC_test_abc_cannot_instantiate
    ...
    # endregion FUNC_test_concrete_must_implement_deliver
    ...
    def test_concrete_works(self, sample_payload: Payload) -> None:
        ...
        class TestChannel(DeliveryChannel):
            def deliver(self, _payload) -> DeliveryResult:
    ...
    # endregion FUNC_test_concrete_works

    # region FUNC_test_retry_deliver
    ## @purpose  Verify retry logic works with exponential backoff (shared.retry, 177 W3.1).
    def test_retry_deliver(self, sample_payload: Payload, caplog: pytest.LogCaptureFixture) -> None:
        """Verify _retry_deliver retries on failure (sleep_fn DI — 0 реального backoff)."""
        caplog.set_level(logging.INFO)

        # 🧪 TRAP[TEST] · 2026-08-16 · Regression · _retry_deliver → shared.retry (177 W3.1)
        # · Scenario: fail → success на 2-й попытке; sleep_fn=fake (DI-шов канала, не time.sleep)
        # · Last fail: N/A (W4-B1; 177 W3.1 — retry-цикл в shared.retry, sleep_fn в __init__)
        # · Remove if: retry-контракт канала меняется
        call_count = 0
...
class TestSCPChannel:
    ...
    # endregion FUNC_test_scp_channel_requires_host
...
class TestForcedCommandChannel:
    ...
    # endregion FUNC_test_forced_command_requires_host
...
# ── Consolidated channel defaults (F5-reduction) ──


@pytest.mark.parametrize("channel_cls", [SCPChannel, ForcedCommandChannel])
def test_channel_timeout_default(channel_cls: type) -> None:
    """Both remote channels wire DEFAULT_DEPLOY_TIMEOUT when timeout is not specified."""
    assert channel_cls().timeout == DEFAULT_DEPLOY_TIMEOUT


@pytest.mark.parametrize("channel_cls", [SCPChannel, ForcedCommandChannel])
def test_channel_timeout_custom(channel_cls: type) -> None:
    """Both remote channels accept a custom timeout override."""
    assert channel_cls(timeout=300).timeout == 300


@pytest.mark.parametrize(
    ("name", "expected"),
    [
        ("DEFAULT_DEPLOY_TIMEOUT", 900),  # v1.0.1: 600→900 (холодный φ8)
        ("DEFAULT_RETRY_COUNT", 2),
        ("DEFAULT_RETRY_BACKOFF", 5),
    ],
)
def test_channel_defaults_pinned(name: str, expected: int) -> None:
    """Parametrized pin of channel default constants (consolidated from 3 constant-tests, F5)."""
    actual = {
        "DEFAULT_DEPLOY_TIMEOUT": DEFAULT_DEPLOY_TIMEOUT,
