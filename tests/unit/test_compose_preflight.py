...
class TestResolveModules:
    @pytest.mark.parametrize(
        "profiles, env, expected",
        [
            pytest.param({"postgres", "litellm"}, None, {"postgres", "litellm"}, id="with_profiles"),
            pytest.param(
                set(),
                {"COMPOSE_PROFILES": "postgres,litellm,monitoring"},
                {"postgres", "litellm", "monitoring"},
                id="with_env_profiles",
            ),
            pytest.param(set(), {}, set(), id="no_profiles"),
            pytest.param(set(), {"COMPOSE_PROFILES": ""}, set(), id="empty_env_profiles"),
        ],
    )
    def test_resolve_modules(
        ...
        """Resolve modules from explicit profiles or COMPOSE_PROFILES env (P-консолидация 168).

        Cases (1:1 из test_with_profiles/test_with_env_profiles/test_no_profiles/
        test_empty_env_profiles): explicit profiles win; COMPOSE_PROFILES parsed;
        neither → empty set; empty COMPOSE_PROFILES → empty set.
        """
        caplog.set_level(logging.INFO)
        result = resolve_modules(profiles, env=env)
...
class TestLoadEnvMap:
    ...
    # GUARD-PRESERVE (168): единственное покрытие ветки FileNotFoundError → {} graceful degradation в load_env_map
...
class TestLoadManifest:
    ...
    # GUARD-PRESERVE (168): единственное покрытие ветки missing manifest → None graceful degradation в load_manifest
...
class TestCheckSecrets:
    # GUARD-PRESERVE (168): единственное покрытие happy-path ветки check_secrets (все секреты на месте → missing == [])
    def test_all_secrets_present(self, caplog: pytest.LogCaptureFixture, sample_manifest: Path) -> None:
        """When all secrets are present, missing list is empty (DI env-dict)."""
        caplog.set_level(logging.INFO)
        secrets = load_manifest(str(sample_manifest))
        env_map: dict[str, str] = {}
        missing = check_secrets(
            {"postgres", "litellm", "hermes-agent", "monitoring"},
            secrets,
            env_map,
            env={"POSTGRES_PASSWORD": "abc", "LITELLM_MASTER_KEY": "sk-key", "TELEGRAM_BOT_TOKEN": "123:abc"},
        )  # type: ignore[arg-type]
        assert missing == []

    def test_missing_required(self, caplog: pytest.LogCaptureFixture, sample_manifest: Path) -> None:
        """Missing required secrets are reported (DI env-dict)."""
        caplog.set_level(logging.INFO)
        secrets = load_manifest(str(sample_manifest))
        env_map: dict[str, str] = {}
        missing = check_secrets(
            {"postgres", "litellm", "hermes-agent"},
            secrets,
            env_map,
            env={"LITELLM_MASTER_KEY": "sk-key"},
        )  # type: ignore[arg-type]
    ...
    def test_module_filtering(self, caplog: pytest.LogCaptureFixture, sample_manifest: Path) -> None:
        """Secrets for non-target modules are not checked."""
        caplog.set_level(logging.INFO)
        secrets = load_manifest(str(sample_manifest))
        env_map: dict[str, str] = {}
        # Only checking litellm — POSTGRES_PASSWORD is consumed by postgres+litellm
        missing = check_secrets({"litellm"}, secrets, env_map, env={})  # type: ignore[arg-type]
...
class TestValidateCharsets:
    @pytest.mark.parametrize(
        "use_env_file, env",
        [
            pytest.param(True, {}, id="all_charsets_pass"),
            pytest.param(False, {}, id="skip_empty_values"),
            pytest.param(False, {"LITELLM_MASTER_KEY": "any-value-ok"}, id="skip_missing_charset"),
        ],
    )
    def test_validate_charsets_no_errors(
        self,
        caplog: pytest.LogCaptureFixture,
        ...
        """Charset validation yields no errors for pass/skip variants (P-консолидация 168).

        Cases (1:1 из test_all_charsets_pass/test_skip_empty_values/test_skip_missing_charset):
        env-file values match charsets; empty values skipped (caught by check_secrets);
        secrets without charset field skipped.
        """
        caplog.set_level(logging.INFO)
        secrets = load_manifest(str(sample_manifest))
        env_map = load_env_map(str(sample_secrets_env)) if use_env_file else {}
        errors = validate_charsets(secrets, env_map, env=env)  # type: ignore[arg-type]
        assert errors == []

    def test_charset_violation(self, caplog: pytest.LogCaptureFixture, sample_manifest: Path) -> None:
        """A secret violating its charset produces an error."""
        caplog.set_level(logging.INFO)
        secrets = load_manifest(str(sample_manifest))
        env_map: dict[str, str] = {}
        errors = validate_charsets(
            secrets,
            env_map,
            env={
                "POSTGRES_PASSWORD": "invalid@charset!!!",
                "LITELLM_MASTER_KEY": "sk-ok",
...
class TestMainCLI:
    @pytest.mark.parametrize(
        "argv",
        [
            pytest.param(["--skip-preflight", "up", "--profile", "postgres"], id="skip_preflight"),
            pytest.param(["down"], id="down_subcommand_bypass"),
            pytest.param(["config"], id="config_subcommand_bypass"),
            pytest.param(
                ["--manifest", "/nonexistent/manifest.yaml", "up", "--profile", "postgres"],
                id="missing_manifest_graceful",
            ),
        ],
    )
    def test_main_bypass_returns_zero(self, caplog: pytest.LogCaptureFixture, argv: list[str]) -> None:
        """main() returns exit 0 for bypass/graceful-degradation paths (P-консолидация 168).

        Cases (1:1 из test_skip_preflight/test_down_subcommand_bypass/
        test_config_subcommand_bypass/test_missing_manifest_graceful):
        --skip-preflight bypass; non-'up' subcommands bypass; missing manifest → graceful exit 0.
        """
        caplog.set_level(logging.INFO)
        exit_code = main(argv)
        assert exit_code == 0
