...
@ldd_trajectory
def test_validate_no_domain_skips_without_parse(tmp_path, caplog) -> None:
    """Без domain → valid=True, try_parse НЕ вызывается."""
    compose_path = _write_compose(tmp_path)

    parse_mock = mock.MagicMock()

    result = validate_compose_networks(compose_path, domain="", compose_profiles="", parse_fn=parse_mock)
...
@ldd_trajectory
def test_validate_valid_proxy_net(tmp_path, caplog) -> None:
    ...
    result = validate_compose_networks(
        compose_path, domain="example.com", compose_profiles="", parse_fn=lambda _, **__: parsed
    )
...
@ldd_trajectory
def test_validate_invalid_no_service(tmp_path, caplog) -> None:
    ...
    result = validate_compose_networks(
        compose_path, domain="example.com", compose_profiles="", parse_fn=lambda _, **__: data
    )
...
@ldd_trajectory
def test_validate_best_effort_skip_when_parse_unavailable(tmp_path, caplog) -> None:
    """try_parse → None → valid=True (best-effort skip)."""
    compose_path = _write_compose(tmp_path)
    result = validate_compose_networks(
        compose_path, domain="example.com", compose_profiles="", parse_fn=lambda _, **__: None
    )
...
@ldd_trajectory
def test_try_parse_docker_config_cascade(tmp_path, caplog) -> None:
    ...
    data = try_parse_compose(
        compose_path,
        compose_profiles="full",
        which_fn=lambda _: "/usr/bin/docker",
        compose_config_fn=shared_mock,
    )
...
@ldd_trajectory
def test_try_parse_pyyaml_fallback(tmp_path, caplog) -> None:
...
@ldd_trajectory
def test_try_parse_none_when_yaml_malformed(tmp_path, caplog) -> None:
    ...
    data = try_parse_compose(compose_path, compose_profiles="", which_fn=lambda _: None)
...
@ldd_trajectory
def test_try_parse_missing_file_returns_none(tmp_path, caplog) -> None:
    """docker недоступен + файл отсутствует → None (best-effort skip, D-I2 закрыт)."""
    missing = tmp_path / "does-not-exist" / "compose.yaml"

    data = try_parse_compose(missing, compose_profiles="", which_fn=lambda _: None)
...
@ldd_trajectory
def test_validate_compose_not_mutated(tmp_path, caplog) -> None:
