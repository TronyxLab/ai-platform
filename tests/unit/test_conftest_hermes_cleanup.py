...
# STRUCTURE: ▶ mock _docker_ps_ids + subprocess.run → ◇ _final_hermes_test_cleanup × 3 (label rm / no-containers / name-filter negative) → ⊕ assert rm -f label-only → ⎋ LDD IMP:9
...
## @scope    Только функция _final_hermes_test_cleanup (мок _docker_ps_ids + subprocess.run —
##           без реального docker daemon). Дополняет test_session_xdist_guards.py, который
##           тестирует master-guard хука, а не внутренности свипа.
## @invariants
##   - Моки: session_mod._docker_ps_ids (возврат ID по label-фильтру) + session_mod.subprocess.run
...
def _make_fake_subprocess_run(monkeypatch: pytest.MonkeyPatch, rm_calls: list[list[str]]) -> None:
    """Patch session_mod.subprocess.run via monkeypatch: record every docker rm -f call.

    ## @purpose  Мок subprocess.run модуля session — свип не трогает реальный docker daemon.
    ##           monkeypatch.setattr гарантирует восстановление после теста (xdist-безопасно).
    ## @io       ⇥ monkeypatch, rm_calls: list (мутируемый recorder) → ⎋ None (side-effect: patch)
    ## @complexity O(1)
    """
    ...
    monkeypatch.setattr(session_mod.subprocess, "run", _fake_run)
...
## @io       monkeypatch, caplog → None (assert rm -f вызван с label-ID + единственный label-запрос)
## @complexity 1 — mocked wiring assert
...
@ldd_trajectory
def test_label_sweep_removes_labeled_containers(monkeypatch: pytest.MonkeyPatch, caplog) -> None:
    ...
    monkeypatch.setattr(session_mod, "_docker_ps_ids", _make_fake_docker_ps_ids(docker_ps_calls))
    _make_fake_subprocess_run(monkeypatch, rm_calls)
...
## @io       monkeypatch, caplog → None (assert no rm, no docker ps name-filter)
## @complexity 1 — mocked wiring assert
# 🧪 TRAP[TEST] · Regression · Scenario: нет label-контейнеров → sweep no-op без rm · Last fail: N/A · Remove if: no-op семантика изменена
@ldd_trajectory
def test_label_sweep_no_containers_no_rm(monkeypatch: pytest.MonkeyPatch, caplog) -> None:
    ...
    monkeypatch.setattr(session_mod, "_docker_ps_ids", _fake_empty)
    _make_fake_subprocess_run(monkeypatch, rm_calls)
...
## @io       monkeypatch, caplog → None (assert NO name-фильтр в запросах)
...
@ldd_trajectory
def test_label_sweep_negative_name_filter_never_used(monkeypatch: pytest.MonkeyPatch, caplog) -> None:
