...
##           in JSON-lines format, and that the shell audit_log() is preserved.
...
@pytest.fixture
def mock_context_deployer_shared():
    """DI-параметры deploy_context_projects (DevPlan 167 D3) — 0 setattr.

    ## @purpose  Позволить deploy_context_projects пройти без реальных docker/SSH-вызовов:
    ##            health_fn → False (force deploy), orchestrator_deploy_fn → синтетический
    ##            deployed-результат, facts → docker-compose.yml «существует» (bootstrap-
    ##            генерация пропускается), llm_fn → no-op. _write_audit остаётся РЕАЛЬНЫМ —
    ##            тесты патчат shared write_audit_entry (unittest.patch) для capture.
    ## @io  ⎋ dict kwargs для cd.deploy_context_projects(..., **mock_context_deployer_shared)
    ## @invariants
    ##   - health_fn → False (force deploy, не skip)
    ##   - orchestrator_deploy_fn → synthetic deployed result (channel="orchestrator")
    ##   - facts.path_isfile → True (compose «существует», bootstrap-генерация не нужна)
    ##   - llm_fn → no-op (non-fatal, uninteresting for audit tests)
    """

    class _FakeFacts:
        def path_isfile(self, _path) -> bool:
            return True  # docker-compose.yml «существует» → bootstrap-генерация пропускается

        def is_root(self) -> bool:
            return True

        def which(self, _binary: str) -> str | None:
            return None

    return {
        "health_fn": lambda _name: False,
        "orchestrator_deploy_fn": lambda **_kwargs: SimpleNamespace(
...
def test_context_deployer_writes_audit_on_deploy(
    ...
    with patch("core.internal.shared.audit_logger.write_audit_entry") as mock_audit_entry:
        results = cd.deploy_context_projects(
            node_yaml_file, "test-ctx", projects_base="/tmp/test-projects", **mock_context_deployer_shared
        )
...
# endregion TEST 1: context_deployer writes audit on deploy
...
# endregion TEST 2: audit entry contains required fields
...
# endregion TEST 3: audit format is valid JSON-lines style
...
# endregion TEST 4: docker_orchestrator writes START audit
...
# endregion TEST 5: docker_orchestrator writes FAILED audit on compose failure
...
# test_old_shell_format_unchanged asserted that the shell audit logger existed
# and contained audit_log(). That file was removed in DevPlan 089 AC7 (shell audit
...
# · Reason: shell audit deletion is intentional and permanent (DevPlan 089). Test of a
...
# endregion TEST 6: old shell audit_log format unchanged
