...
_CTX_DEPLOYER_SOURCE = Path(cd.__file__).read_text()
...
def fake_orchestrator_cls():
    ...
    class _FakeOrchestrator:
        ...
        def deploy(self, project_name: str, channel, project_dir: str):
...
@ldd_trajectory
def test_context_deployer_uses_local_channel(caplog, tmp_path, monkeypatch, fake_orchestrator_cls) -> None:
    ...
    monkeypatch.setattr(cd, "_ensure_bootstrap_compose", lambda pdir, proj: True)
    monkeypatch.setattr(cd, "DeployOrchestrator", fake_cls)

    result = cd._deploy_single_project_via_orchestrator(project, str(tmp_path / "projects"), False)
