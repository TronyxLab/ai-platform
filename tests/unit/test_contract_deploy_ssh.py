...
# NOTE: _run_bash helper (sourcing the deploy shell) removed in DevPlan 116
# B8 T5.5 — the file it sourced was deleted (DevPlan 089); the helper had no callers.
# parse_ssh_command tests removed — the deploy shell was deleted (DevPlan 089),
...
SCP_DELIVER_SH = Path(PROJECT_ROOT) / "core" / "internal" / "bootstrap" / "scp-deliver.sh"
# REMOTE_CMD_SH removed — build_ssh_cmd deleted (DevPlan 089: deploy shell migrated to Python)
...
def _print_ldd(stderr: str, stdout: str = "") -> bool:
    ...
    for source in [stderr, stdout]:
        for line in source.split("\n"):
            if "[IMP:" in line:
                imp_level = _parse_imp_level_ssh(line)
...
def _parse_imp_level_ssh(line: str) -> int | None:
    """Извлечь уровень IMP:N из строки ssh-вывода (PLW0717-хелпер).
...
def _run_scp_facade(
    ...
    proc = subprocess.run(["bash", str(script)], capture_output=True, text=True, timeout=15, env=full_env, check=False)
    return proc.stdout.strip(), proc.stderr.strip(), proc.returncode
...
# -- build_ssh_cmd tests removed (DevPlan 089: deploy shell scripts deleted,
