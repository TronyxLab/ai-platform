...
def test_invoke_unregistered_interface_fails(tmp_path: Path) -> None:
    ...
    try:
        ...
        # Формат сообщения slim-линтера W-D (split 881→198, DevPlan 163): "[invoke] interface
        # 'X' not registered for 'Y'" — прежний маркер 'NOT REGISTERED' устарел (linter удалён)
        assert "not registered for" in violations[0], f"Expected 'not registered for' in: {violations[0]}"
...
class TestLooksLikePath:
    """Unit tests for _looks_like_path() — distinguishes path-like args from flags/vars (parametrized, F5-reduction)."""

    @pytest.mark.parametrize(
        ("arg", "expected"),
        [
            ("modules/postgres/healthcheck.sh", True),  # literal path
            ("${CORE_DIR}/modules/postgres/healthcheck.sh", True),  # ruff: ignore[RUF027] — ${VAR}/path
            ("$hc_script", True),  # bare $variable (no /)
            ("${hc_script}", False),  # ${variable} without / is NOT a path
            ("-c", False),  # flag argument
            ...
            ("$*", False),  # special variable
            ("$-", False),  # special variable
            ("$0", False),  # special variable
            ("", False),  # empty string
            ('"$hc_script"', True),  # quoted bare variable
            ("${CORE_DIR}/modules/${mod_name}/healthcheck.sh", True),  # multiple vars + /
            ("$", False),  # single dollar sign
            ("$1", False),  # positional parameter
        ],
    )
    def test_looks_like_path(self, arg: str, expected: bool) -> None:
        """Parametrized: path-like vs flag/variable distinction for _looks_like_path."""
...
class TestResolveImport:
    ...
    def test_contextual_variable(self) -> None:
