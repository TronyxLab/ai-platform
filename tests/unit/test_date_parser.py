...
# Add backup-cron scripts to sys.path for DateParser import (168 F5: self-contained —
# раньше одиночный прогон падал ModuleNotFoundError, т.к. путь добавлял только
# test_backup_retention.py побочным эффектом своего модульного импорта)
...
@ldd_trajectory
# GUARD-PRESERVE (168): единственное покрытие DateParser.extract_date_from_key (7 вариантов
# ключей консолидированы в одном тесте — эвристика «1 assert» ловит цикл как false positive)
...
# GUARD-PRESERVE (168): уже параметризован (7 кейсов) — единственное покрытие is_sunday;
# эвристика «1 assert» ловит параметризованный тест как false positive
...
# GUARD-PRESERVE (168): уже параметризован (6 кейсов) — единственное покрытие is_first_of_month;
# эвристика «1 assert» ловит параметризованный тест как false positive
