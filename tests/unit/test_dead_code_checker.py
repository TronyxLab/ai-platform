...
def test_parse_blame_porcelain_committer_time(tmp_path: Path, caplog, monkeypatch) -> None:
    ...
    ts = get_line_add_timestamp(tmp_path, "f.py", 3, 12345)
...
def test_get_line_add_timestamp_fallback_mtime(tmp_path: Path, caplog, monkeypatch) -> None:
    """# 🧪 TRAP[TEST] · Regression: blame empty/error (rc!=0) → os.path.getmtime fallback (D7/D9).
    Scenario: monkeypatch subprocess → rc=128 пустой stdout (untracked/не git repo) →
    ...
    ts = get_line_add_timestamp(tmp_path, "untracked.py", 1, mtime)
...
def test_check_dead_code_clean_pass(tmp_path: Path, caplog, monkeypatch, capsys) -> None:
    ...
    main() с monkeypatched _default_project_root → return 0.
    ...
    monkeypatch.setattr("core.internal.lint.dead_code_checker._default_project_root", lambda: tmp_path)
    rc = main(["--threshold", str(THRESHOLD_DAYS)])
...
def test_check_dead_code_violation_fail(tmp_path: Path, caplog, monkeypatch, capsys) -> None:
    """# 🧪 TRAP[TEST] · Regression: violation — маркер 2 дня от mtime, threshold 0 → 1 violation,
    exit 1, STALE stdout строка присутствует.
    Scenario: tmp проект со stale маркером; main([--threshold 0]) → return 1; check_dead_code →
    ровно 1 DeadCodeViolation с корректными полями.
    Last fail: n/a (new test). Remove if: exit-code/verdict контракт изменён."""
    caplog.set_level(logging.DEBUG)
...
def test_output_format_byte_identical(tmp_path: Path, caplog, monkeypatch, capsys) -> None:
