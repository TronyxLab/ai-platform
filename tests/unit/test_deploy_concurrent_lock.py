...
def _hold_raw_flock(lock_path: str, holder_pid: int = 99999) -> int:
    ...
    """
    Path(Path(lock_path).parent).mkdir(exist_ok=True, parents=True)
...
def test_deploy_history_snapshot_atomic_prune_payload(
    ...
    remaining = sorted(f.name for f in snap_dir.iterdir() if f.name.endswith(".json"))
    ...
    tmp_leftovers = [f.name for f in snap_dir.iterdir() if f.name.endswith(".tmp")]
