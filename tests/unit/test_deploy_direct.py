...
def test_deploy_project_validation_no_ai_platform_yaml(caplog, tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [_BASH, "-c", _VALIDATE_PROJECT_SCRIPT, "--", str(project_dir)], capture_output=True, text=True, check=False
    )
...
def test_extract_org_from_path(caplog, tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [_BASH, "-c", _EXTRACT_ORG_SCRIPT, "--", str(project_dir)], capture_output=True, text=True, check=False
    )
...
def test_receive_project_only(caplog) -> None:
    ...
    logger.info("[IMP:7][test_receive_old] Testing receive myproject (no sha)")
    ...
    logger.critical("[IMP:9][test_receive_old] stdout: %s", result.stdout.strip())
    assert result.returncode == 0, f"Script failed: {result.stderr}"

    for line in result.stdout.splitlines():
        if line.startswith("PROJECT_DIR="):
            project_dir = line.split("=", 1)[1]
            logger.critical("[IMP:9][test_receive_old] PROJECT_DIR=%s", project_dir)
            assert project_dir == "/opt/projects/myproject", (
