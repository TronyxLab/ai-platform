...
## @invariants
##   - deploy_boundary fixture: 7 патчей границы (subprocess.run, _shared_retry_pull,
##     _shared_healthcheck_poll, _shared_docker_compose_up, _shared_docker_compose_ps,
##     _shared_docker_compose_images, _shared_docker_compose_down) — все через unittest.patch
##     (DevPlan 167 D3: 0 setattr; граница уже абстрагирована shared/docker_compose)
...
def deploy_boundary():
    """I/O boundary fixture (D1): subprocess.run + 6 shared docker-compose helpers пропатчены
    (все 7 — через unittest.patch, DevPlan 167 D3: 0 setattr).
    ...
    with (
        # 🧐 TRAP[DECISION] · 2026-08-15 · — · Патч-таргеты на holder'ы пакета engine/ (170 W4-B2)
        # · Rejected: патч через фасад deploy_engine._shared_* (патч атрибута фасада НЕ влияет на собственные
        #   биндинги engine/flow|lifecycle|engine — код пакета смотрит свои модульные имена в рантайме)
        # · Reason: фасад — чистый re-export без контента; holder'ы _shared_* живут в пакете
        # · Rev: если пакет engine/ вернёт биндинги в фасад — вернуть патч-таргеты на deploy_engine.*
        # патч глобального subprocess.run ловит docker_ops; _shared_* — где живут биндинги.
        patch("core.internal.deploy.engine.engine.subprocess.run", mock_run),
        ...
        patch("core.internal.deploy.engine.engine._shared_docker_compose_ps", mock_ps),
        patch("core.internal.deploy.engine.lifecycle._shared_docker_compose_images", mock_images),
        patch("core.internal.deploy.engine.engine._shared_docker_compose_down", mock_down),
    ):
        yield type(
            "Boundary",
            (),
...
def test_deploy_retry_pull_wiring(deploy_boundary, tmp_project, engine, caplog):
    ...
    b.retry_pull.side_effect = lambda *_, **k: (pulled_kwargs.append(k), True)[1]
...
def test_remove_active(deploy_boundary, caplog, engine, tmp_project):
    ...
    b.down.side_effect = lambda *_, **k: (down_flags.append(k.get("flags") or []), True)[1]
...
def test_deploy_no_engine_snapshot_files(deploy_boundary, caplog, tmp_project, engine):
    ...
    b.run.return_value = _cp(stdout="")
    snap_files: list[str] = []
    b.ps.side_effect = lambda *_, **__: (snap_files.extend(_collect_snapshot_files(tmp_project)), _cp())[1]
...
def test_deploy_engine_no_duplicate_layers_negative(caplog):
    ...
    caplog.set_level(logging.INFO)
    # 170 W4-B2: читаем класс DeployEngine (engine/engine.py) — там живёт deploy()/contextlib.chdir
    from core.internal.deploy.engine import engine as deploy_engine

    src = Path(deploy_engine.__file__).read_text(encoding="utf-8")
    tree = ast.parse(src)
    forbidden: list[str] = [
...
# 🧪 TRAP[TEST] · 2026-08-01 · D1 · result dataclasses — default values (parametrized)
# · Scenario: ServiceDeployResult/RemoveResult/StatusResult создаются с дефолтами (dataclass контракт)
# · Last fail: N/A (consolidated from 3 dataclass tests, 168 F5-reduction)
# · Remove if: result dataclass contracts change
@pytest.mark.parametrize(
    ("make_result", "defaults"),
    [
        pytest.param(
            lambda: ServiceDeployResult(success=True, project="test", ref="v1", service="app"),
            {"rollback_performed": False, "first_deploy_failed": False, "previous_image": None},
            id="ServiceDeployResult",
        ),
        pytest.param(
            lambda: RemoveResult(success=True, project="test"),
            {"already_removed": False},
            id="RemoveResult",
        ),
        pytest.param(
            lambda: StatusResult(project="test", node="node1", status="not_found"),
            {"containers": [], "last_deploy": None},
            id="StatusResult",
        ),
    ],
)
def test_result_dataclasses(make_result, defaults):
    """Deploy/Remove/Status result dataclasses create with default values."""
    r = make_result()
    for attr, value in defaults.items():
        assert getattr(r, attr) == value
    logger.critical("[IMP:9][test] %s dataclass — OK", type(r).__name__)
