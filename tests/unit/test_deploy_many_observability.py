...
# · Last fail: legacy — return (0, []) без парсинга (U-30)
...
def test_deploy_many_parses_json_counts(monkeypatch, caplog: pytest.LogCaptureFixture) -> None:
    ...
    monkeypatch.setattr(
        mod,
        "subprocess",
        type("SP", (), {"run": staticmethod(fake), "TimeoutExpired": TimeoutError, "CalledProcessError": RuntimeError}),
    )

    deployed, failed = mod._deploy_orchestrator(["mod1", "mod2", "mod3"])
...
def test_deploy_many_empty_list(monkeypatch, caplog: pytest.LogCaptureFixture) -> None:
    ...
    def _run(*a, **k):
        called.append(a)
        raise AssertionError("subprocess не должен вызываться при пустом списке")

    monkeypatch.setattr(mod, "subprocess", type("SP", (), {"run": staticmethod(_run), "TimeoutExpired": TimeoutError}))

    deployed, failed = mod._deploy_orchestrator([])
...
# · Last fail: legacy — return 0, [] при ненулевом exit
...
def test_deploy_many_returncode_nonzero(monkeypatch, caplog: pytest.LogCaptureFixture) -> None:
    ...
    deployed, failed = mod._deploy_orchestrator(["mod1", "mod2"])
...
# · Last fail: legacy — cmd включал '--scp' (дефолт SCPChannel)
...
def test_deploy_many_cmd_no_scp_flag(monkeypatch, caplog: pytest.LogCaptureFixture) -> None:
    ...
    deployed, failed = mod._deploy_orchestrator(["mod1"])
...
def test_deploy_many_non_json_stdout(monkeypatch, caplog: pytest.LogCaptureFixture) -> None:
    ...
    monkeypatch.setattr(mod, "subprocess", type("SP", (), {"run": staticmethod(fake), "TimeoutExpired": TimeoutError}))
