...
_MONITORING_RENDERER = _REPO_ROOT / "core" / "internal" / "monitoring" / "config_renderer.py"
...
## @purpose — deliver CLI: channel_factory (DI, DevPlan 167 D3) возвращает fake-канал с
##            JSON-результатом VPS (DEPLOYED) → stdout пробрасывает JSON, exit 0.
...
# · Scenario: fake-канал stdout='{"status":"DEPLOYED",...}' → _handle_deliver печатает JSON, rc 0
# · Last fail: N/A (new test)
# · Remove if: deliver CLI меняется
def test_deliver_cli_forwards_vps_json(tmp_path, caplog: pytest.LogCaptureFixture, capsys) -> None:
...
# · Scenario: fake-канал stdout='{"status":"FAILED",...}' → _handle_deliver rc 1
# · Last fail: N/A (new test)
# · Remove if: deliver CLI меняется
def test_deliver_cli_vps_failed_exit_1(tmp_path, capsys) -> None:
...
# · Scenario: _handle_deliver с --host "" → rc 1, JSON ERROR
# · Last fail: — deploy-project не передавал host → SCPChannel всегда FAILED
# · Remove if: deliver CLI меняется
...
# ═══════════════════════════════════════════════════════════════════
# region Tests: D3 — render-monitoring (DevPlan 136 W1 T1.10)
# ═══════════════════════════════════════════════════════════════════
# (D1/D2 up-safe/compose-wrapper тесты удалены — План 175 W4.2: up-safe слит в up,
#  compose-wrapper.sh удалён; preflight напрямую compose_preflight.py)
...
def test_render_monitoring_self_bootstrap_source() -> None:
    ...
    assert "python3 core/internal/monitoring/config_renderer.py" in recipe, "direct-script invocация ожидалась"

    src = _MONITORING_RENDERER.read_text(encoding="utf-8")
    assert "_PROJECT_ROOT = str(Path(__file__).resolve().parent.parent.parent.parent)" in src, (
        "D3: self-bootstrap = корень репо (4 уровня parent, 172 W5.1: файл в monitoring/)"
    )
...
def test_render_monitoring_self_bootstrap_behavioral(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    repo_root = str(Path(mcr.__file__).resolve().parent.parent.parent.parent)
    monkeypatch.delenv("PYTHONPATH", raising=False)
    stripped = [p for p in sys.path if str(Path(p).absolute()) != repo_root]
    # 🧐 TRAP[DI-KEEP] · 2026-08-14 · — · sys.path setattr — импорт-изоляция (D3 behavioral)
    # · Rejected: DI-шов (sys.path — process-global, инъекция невыразима без смены сущности
    # ·   direct-script модуля)
    # · Reason: honest-floor §4 (import-изоляция, DevPlan 167): тест проверяет self-bootstrap
    # ·   модуля при ВЫЧЕРКНУТОМ repo root из sys.path (имитация direct-script без PYTHONPATH)
    # · Rev: при введении test-import-harness (изолированного импорт-контекста) — заменить патч
...
# endregion Tests: D3 — render-monitoring (DevPlan 136 W1 T1.10)
