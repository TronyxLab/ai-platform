...
# 🧪 TRAP[TEST] · Regression: DEPLOY_PARALLEL=false must dispatch to sequential path (legacy parity)
...
def test_orchestrate_parallel_routing(caplog) -> None:
    ...
    mock_par.assert_called_once_with(["postgres"], {"postgres": ""}, "/mods", "/core", False)
...
def test_orchestrate_orchestrator_routing(caplog) -> None:
    ...
    mock_par.assert_called_once_with(["postgres", "redis"], {}, "/mods", "/core", True)
    mock_seq.assert_not_called()
...
# 🧪 TRAP[TEST] · Regression: critical module failure must escalate to exit 2 (legacy severity contract)
...
## @invariants
##   - WARN>0 must map to exit 0 with WARN log (legacy parity — warnings never fail deploy)
...
# 🧪 TRAP[TEST] · Regression: warn module failure must NOT escalate (exit 0, legacy contract)
...
# 🧪 TRAP[TEST] · Regression: no enabled modules must not crash or postflight (legacy `exit 0` parity)
...
## @purpose  _preflight must invoke context_overlay, spool_validator, status-metrics, charset validation
## @io       tmp_path, caplog → None
## @complexity 1 — mocked sub-step wiring assert
## @invariants
##   - All 4 preflight sub-steps called exactly once with expected args
##   - secrets-manifest path derived as core_dir/secrets-manifest.yaml


def test_preflight_calls_all_steps(tmp_path, caplog) -> None:
    """
    # ▶ mock 4 sub-steps → ⚡ _preflight(tmp core/node/modules) → ◇ assert each called → ⎋ pass | fail
    """
    caplog.set_level(logging.DEBUG)
    ...
    logger.info("[IMP:9][test_preflight_calls_all_steps] all 4 preflight steps wired correctly")

    _assert_ldd_imp9(caplog)


# 🧪 TRAP[TEST] · Regression: preflight must keep legacy step order + non-fatal semantics
# · Scenario: _preflight with mocked sub-steps → each called once with correct args
# · Last fail: N/A
...
## @complexity 1 — mocked sub-step wiring assert
## @invariants
##   - sudoers gets ALL module names (legacy --module-names parity)
##   - orphans get ENABLED module names (legacy --module-entries parity)
...
# 🧪 TRAP[TEST] · Regression: sequential for-loop must deploy every enabled module exactly once (legacy parity)
