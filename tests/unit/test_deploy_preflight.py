...
# GUARD-PRESERVE (168): R5-негатив — единственное покрытие fail-fast first-deploy (PlatformFatalError exit 10)
...
def test_deploy_engine_preflight_parity_negative(tmp_path: Path, monkeypatch) -> None:
    ...
    # 167 D3: run_preflight-инъекция (DI-seam _preflight_checks) вместо monkeypatch целевого модуля
    proj_dir = tmp_path / "proj"
    proj_dir.mkdir()
    engine._preflight_checks(str(proj_dir), "web", run_preflight=_fake_preflight)
