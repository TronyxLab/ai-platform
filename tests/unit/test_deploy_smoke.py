...
""",
        ...
        return subprocess.run([str(wrapper)], capture_output=True, text=True, timeout=30, check=False)
    finally:
        if wrapper.exists():
            wrapper.unlink()
...
def test_deploy_modules_no_node_yaml(tmp_path: Path) -> None:
    ...
    logger.info("%s", f"[IMP:9][I1] ✅ No NODE_YAML → exit {result.returncode} with ERROR in stderr")
...
def test_deploy_modules_missing_node_yaml_file(tmp_path: Path) -> None:
    ...
    logger.info("%s", f"[IMP:9][I2] ✅ NODE_YAML=/nonexistent → exit {result.returncode} with ERROR in stderr")
