...
##   - Zero Hardcode: DEV_CERTS_DIR always via tmp_path (monkeypatch.setenv)
...
def test_get_cert_sans_parse(tmp_path: Path, capsys) -> None:
    ...
    cert_file.write_text("dummy PEM")
...
def test_main_idempotent_noop(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    ## @io — ⇥ env vars via monkeypatch + mocked cert_is_current → ⎋ assert exit 0, no generate
    ## @complexity — O(1)
    """
    monkeypatch.setenv("DEV_CERTS_DIR", str(tmp_path))
    monkeypatch.setenv("PLATFORM_DOMAIN", DEFAULT_PLATFORM_DOMAIN)
    monkeypatch.setenv("CERT_BACKEND", "openssl")
...
# · Last fail: N/A (new test)
# · Remove if: main orchestration changes
def test_main_generate_missing(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    ## @io — ⇥ env vars + mocked cert_is_current/which/generate_openssl/verify_san → ⎋ assert exit 0
...
def test_main_unknown_backend(tmp_path: Path, monkeypatch, capsys) -> None:
    """CERT_BACKEND=invalid → exit 1 + IMP:9 error log.

    ## @purpose — Unknown backend must fail loudly, never silently fall back.
    ## @io — ⇥ env vars + mocked cert_is_current=False → ⎋ assert exit 1 + IMP:9 error
    ## @complexity — O(1)
    """
    monkeypatch.setenv("DEV_CERTS_DIR", str(tmp_path))
    monkeypatch.setenv("PLATFORM_DOMAIN", DEFAULT_PLATFORM_DOMAIN)
    monkeypatch.setenv("CERT_BACKEND", "invalid")
...
def test_main_missing_backend_tool(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    ## @io — ⇥ env vars + mocked cert_is_current=False + which=None → ⎋ assert exit 1 + IMP:9
    ## @complexity — O(1)
    """
    monkeypatch.setenv("DEV_CERTS_DIR", str(tmp_path))
    monkeypatch.setenv("PLATFORM_DOMAIN", DEFAULT_PLATFORM_DOMAIN)
    monkeypatch.setenv("CERT_BACKEND", "mkcert")
...
def test_integration_full_flow(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    ## @io — ⇥ env vars + mocked openssl subprocess (req/x509/checkend) → ⎋ assert exit 0 both runs
