...
class TestCollectHosts:
    ...
    def test_collect_hosts_dev_cert_sans_base_domains(self, caplog, tmp_path, monkeypatch) -> None:
        caplog.set_level(logging.INFO)
        certs = tmp_path / "certs"
        certs.mkdir()
        (certs / "fullchain.pem").write_text("stub", encoding="utf-8")
        monkeypatch.setattr(dev_hosts, "get_cert_sans", lambda _p: SAN_STUB)
        hosts = dev_hosts.collect_hosts(
            node_configs_dir=str(tmp_path / "configs"),
            node_name="test-node",
            dev_suffix=DEV_SUFFIX,
...
class TestAtomicWriteSudo:
    # 🧪 TRAP[TEST] · Scenario · unwritable parent → sudo mv ветка (mock _sudo_move, I4)
    # · Regression: /etc/hosts (не-writable parent) обязан идти через sudo mv, а не падать
    # · Last fail: N/A (new module)
    # · Remove if: sudo-граница (I4) изменена
    def test_atomic_write_sudo_branch_uses_sudo_move(self, monkeypatch, tmp_path) -> None:
        ...
        monkeypatch.setattr(dev_hosts, "_sudo_move", fake_sudo_move)
        try:
