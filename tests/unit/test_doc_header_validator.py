...
def test_namelint_targets(tmp_path: Path, caplog) -> None:
    """# 🧪 TRAP[TEST] · Regression: namelint allowed/lifecycle/system-исключения/неизвестный.
    Scenario: foo (allowed) pass; push-core (не зарегистрирован → FAIL, DevPlan 171 W3.3);
    restart (lifecycle) pass; test-/gate-/pre-commit-
    префиксы pass; help/venv (категория служебных таргетов, DevPlan 171 W3.6) pass; unknown → FAIL. Чистый набор → errors==[].
...
def test_namelint_help_all_service_target(tmp_path: Path, caplog) -> None:
    """# 🧪 TRAP[TEST] · Regression: help-all — системное исключение namelint (План 175 W1.3).
    Scenario: .PHONY с help-all (не в allowed_verbs) → namelint pass (категория служебных таргетов make).
    Last fail: n/a (new test). Remove if: help-all перестанет быть служебным таргетом."""
    caplog.set_level(logging.INFO)
    repo = tmp_path / "repo"
    (repo / "core").mkdir(parents=True)
    (repo / "core" / "entrypoint-manifest.yaml").write_text(
    ...
    (repo / "Makefile").write_text(".PHONY: foo restart help help-all venv\n")
