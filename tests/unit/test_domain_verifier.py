...
from core.internal.verify import domain_verifier as dv
from tests._conftest.ldd import ldd_trajectory

logger = logging.getLogger(__name__)

# ── Import the module under test ──
# План 170 W7-E1: sys.path.insert + import domain_verifier (антипаттерн, research-D §D7)
# → пакетный импорт core.internal.verify.domain_verifier (namespace-пакет; корень репо
# в sys.path через site.addsitedir tests/conftest.py). Сигнатуры/ассерты НЕ менялись.
...
# region Fixtures
# ═══════════════════════════════════════════════════════════════════
# (get_expose_domains yaml-фикстуры консолидированы в параметризацию
#  test_get_expose_domains — YAML_* константы ниже, F5-редукция 168)
...
# endregion Resolve node.yaml Tests
...
# YAML-фикстуры get_expose_domains (168 F5: фикстуры → параметризация)
YAML_WITH_EXPOSE = """\
node:
  name: test-node
...
@pytest.mark.parametrize(
    "yaml_text,project,expected",
    [
        (YAML_WITH_EXPOSE, None, ["app.test.example.com", "api.test.example.com"]),
        (YAML_NO_EXPOSE, None, []),
        # P-22: --project сужает скоуп до одного проекта (verify-race 2026-08-03)
        (YAML_WITH_EXPOSE, "frontend", ["app.test.example.com"]),
        # P-22: неизвестный проект → [] (не паника)
        (YAML_WITH_EXPOSE, "ghost-project", []),
    ],
)
@ldd_trajectory
def test_get_expose_domains(yaml_text, project, expected, tmp_path, caplog):
    """get_expose_domains: happy-path/negative по expose:true + project-фильтру (P-22)."""
    caplog.set_level(logging.DEBUG)
    yaml_path = tmp_path / "node.yaml"
    yaml_path.write_text(yaml_text)

    domains = dv.get_expose_domains(yaml_path, project=project)

    assert domains == expected, f"project={project}: expected {expected}, got {domains}"
    logger.critical("[IMP:9][test] get_expose_domains(project=%r) → %s — OK", project, domains)


# endregion get_expose_domains Tests
...
@pytest.mark.parametrize(
    ...
    [
        ("admin@test.com", "secret123", "pass"),
        # Missing credentials → None (skip gracefully, curl не вызывается)
        ("", "", None),
    ],
)
@ldd_trajectory
def test_verify_status_page(email, password, expected_status, caplog):
    """verify_status_page: happy-path (creds → pass) / negative (missing creds → None skip)."""
    caplog.set_level(logging.DEBUG)

    with patch("subprocess.run", return_value=_mock_curl("200")):
        result = dv.verify_status_page("test.example.com", email, password)

    if expected_status is None:
        assert result is None, "missing creds → verify_status_page должен вернуть None (skip)"
    else:
        assert result is not None
        assert result.status == expected_status
        assert result.http_code == 200
    logger.critical("[IMP:9][test] verify_status_page(email=%r) → %s — OK", email, expected_status)


# endregion verify_status_page Tests
...
""")
    ...
    # · Regression: --project-фильтр сломал all-domains поведение (make verify-domains NODE=... = фиктивный PASS)
...
# endregion main() CLI Integration Test
