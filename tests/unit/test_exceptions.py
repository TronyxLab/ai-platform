...
def test_exception_catch_by_base(caplog):
    ...
    for exc_class in [ConfigNotFoundError, ConfigParseError, ConfigValidationError, PlatformFatalError]:
        caught = False
        try:
            msg = f"Raised {exc_class.__name__}"
            raise exc_class(msg)
...
# F5-reduction: 5 exit-code тестов (test_platform_error_exit_code, test_config_not_found_exit_code,
# test_config_parse_error_exit_code, test_config_validation_error_exit_code,
# test_platform_fatal_error_exit_code) удалены как дубли test_platform_error_exit_codes (те же 5
# значений exit_code, instance-доступ к class-атрибуту); test_exception_inheritance_platform_error —
# дубль test_exception_inheritance (isinstance ⊆ issubclass); test_exception_str_message — дубль
# test_exception_message (те же 4 класса, str(e) == msg). Покрытие сохранено 1:1 (W3.5-5 F5).
