...
def test_static_render_contains_canonical_refs(caplog):
    ...
    assert "https://github.com/test-lab/ai-platform/blob/main/AGENTS.md#контракт-окружения-проекта" in static
...
# endregion Tests: full generate (per-node data)
...
@ldd_trajectory
def test_cli_project_dir_writes_file(caplog, tmp_path):
    """CLI: --project-dir → AI-PLATFORM.md created (exit 0) — main(argv) DI, 0 monkeypatch (167 D4)."""
    _write(tmp_path, "ai-platform.yaml", _AI_YAML)
...
# 🧪 TRAP[TEST] · Regression · dev walk-up: node.yaml резолвится БЕЗ PROJECTS_BASE
# · Scenario: ~/projects layout (context/project + context/node-configs/node/node.yaml),
# ·   PROJECTS_BASE unset → _default_node_yaml_path находит node.yaml через walk-up
# · Last fail: N/A (new test)
# · Remove if: _default_node_yaml_path walk-up fallback убирается
@ldd_trajectory
def test_generate_walkup_node_yaml_without_projects_base(caplog, tmp_path, monkeypatch):
    """Без PROJECTS_BASE: node.yaml резолвится из <project_dir>/../node-configs/<node>/node.yaml.

    Dev-машина: ~/projects/<context>/<project>/ + ~/projects/<context>/node-configs/<node>/node.yaml.
    """
    monkeypatch.delenv("PROJECTS_BASE", raising=False)

    project_dir = tmp_path / "dev-context" / "myapp"
    project_dir.mkdir(parents=True)
    node_configs = tmp_path / "dev-context" / "node-configs" / "test-node"
    node_configs.mkdir(parents=True)
    _write(project_dir, "ai-platform.yaml", _AI_YAML)  # target_node: test-node
    _write(node_configs, "node.yaml", _NODE_YAML)  # node: name: test-node, context: test-lab

    doc = generate(str(project_dir))  # node_yaml_path не передан — walk-up резолв

    assert "**Node:** test-node" in doc, "walk-up node.yaml не подхвачен (Node <unknown>?)"
    assert "**Context (org):** test-lab" in doc
    assert "nginx, postgres" in doc, "enabled modules из walk-up node.yaml не отрендерены"
    assert "_node.yaml not found_" not in doc

    logger.critical("[IMP:9][test] walk-up node.yaml OK — dev-layout резолвится без PROJECTS_BASE")


# 🧪 TRAP[TEST] · Regression · PROJECTS_BASE остаётся первичным (VPS-путь) при наличии walk-up
# · Scenario: PROJECTS_BASE задан + walk-up файл тоже существует → VPS node.yaml выигрывает
# · Last fail: N/A (new test)
# · Remove if: приоритет PROJECTS_BASE в _default_node_yaml_path меняется
@ldd_trajectory
def test_generate_projects_base_primary_over_walkup(caplog, tmp_path, monkeypatch):
    """PROJECTS_BASE задан → VPS-путь первичен (walk-up НЕ перекрывает)."""
    vps_root = tmp_path / "vps-projects-root"
    # org в PROJECTS_BASE-пути = basename(parent(project_dir)) = "dev-context"
    vps_node_yaml = vps_root / "dev-context" / "node-configs" / "test-node" / "node.yaml"
    vps_node_yaml.parent.mkdir(parents=True)
    vps_yaml = _NODE_YAML.replace("name: test-node", "name: vps-node")  # отличаем источник
    vps_node_yaml.write_text(vps_yaml)
    monkeypatch.setenv("PROJECTS_BASE", str(vps_root))

    project_dir = tmp_path / "dev-context" / "myapp"
    project_dir.mkdir(parents=True)
    walkup_node_yaml = tmp_path / "dev-context" / "node-configs" / "test-node" / "node.yaml"
    walkup_node_yaml.parent.mkdir(parents=True)
    dev_yaml = _NODE_YAML.replace("name: test-node", "name: dev-node")  # отличаем источник
    walkup_node_yaml.write_text(dev_yaml)
    _write(project_dir, "ai-platform.yaml", _AI_YAML)  # target_node: test-node

    doc = generate(str(project_dir))

    assert "**Node:** vps-node" in doc, "PROJECTS_BASE-путь не первичен (Node=%r)"
    assert "**Node:** dev-node" not in doc, "walk-up перекрыл PROJECTS_BASE — приоритет нарушен"

    logger.critical("[IMP:9][test] PROJECTS_BASE primary OK — VPS-путь выигрывает у walk-up")


# endregion Tests: graceful degradation
