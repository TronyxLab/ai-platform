...
## @invariants
##   - Обходит $PROJECTS_ROOT/*/*/ai-platform.yaml; НЕ блокируется ошибками одного проекта (WARN + continue)
##   - catalog.json — валидный JSON-массив; сортировка по (org, name) детерминирована
##   - Отсутствующий PROJECTS_ROOT → 0 (без исключений, файл не создаётся)
##   - Пустой PROJECTS_ROOT → 0 + catalog.json = []
...
## @purpose  PROJECTS_ROOT не существует → 0 (без исключений), catalog-файл НЕ создаётся.
...
def test_generate_catalog_missing_root(tmp_path, caplog) -> None:
    """PROJECTS_ROOT отсутствует → 0, файл каталога не создаётся."""
...
## @purpose  Пустой PROJECTS_ROOT (существует, без проектов) → 0 + catalog.json = [] (валидный массив).
...
def test_generate_catalog_empty_root(tmp_path, caplog) -> None:
    """Пустой PROJECTS_ROOT → 0, catalog.json = [] (валидный JSON-массив)."""
    projects_root = tmp_path / "projects"
    projects_root.mkdir()
    catalog_file = tmp_path / "catalog.json"
...
# · Scenario: env CATALOG_FILE=/env/x, PROJECTS_ROOT=/env/p; CLI --catalog-file /cli/y →
...
def test_parse_cli_args_cli_over_env(tmp_path, monkeypatch, caplog) -> None:
    ...
    monkeypatch.setenv("PROJECTS_ROOT", str(tmp_path / "env-projects"))
