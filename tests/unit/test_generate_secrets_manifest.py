...
@pytest.mark.parametrize(
    ("modules", "secret"),
    [
        ([{"name": "nginx", "env_requires": []}], "POSTGRES_PASSWORD"),  # empty env_requires
        (
            [
                {"name": "clickhouse", "env_requires": ["CLICKHOUSE_PASSWORD"]},
                {"name": "nginx", "env_requires": ["NGINX_VAR"]},
            ],
            "POSTGRES_PASSWORD",  # no module requires the queried secret
        ),
    ],
)
def test_compute_consumers_empty(caplog, modules, secret):
    """compute_consumers → [] для пустого env_requires / отсутствующего секрета (F5)."""
    result = gsm.compute_consumers(secret, modules)
    assert result == [], f"Expected empty list, got {result}"

    logger.critical("[IMP:9][test] compute_consumers empty result for secret=%s", secret)
