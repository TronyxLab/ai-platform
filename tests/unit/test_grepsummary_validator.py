...
def test_extract_keywords_scan_mode(caplog) -> None:
    ...
    line = "# GREP_SUMMARY: alpha, #beta, -->, <!--gamma, -x, --yy"
    kws = extract_keywords(line, mode="scan")
    logger.critical("[IMP:9][test] extract_keywords_scan_mode: %s — OK", kws)

    _assert_ldd(caplog)
    assert kws == ["alpha", "beta", "gamma"], f"scan-strip broken: {kws}"


def test_extract_keywords_staged_mode(caplog) -> None:
    ...
    caplog.set_level(logging.INFO)
    line = "# GREP_SUMMARY: alpha, -->, -x, gamma"
    kws = extract_keywords(line, mode="staged")
    logger.critical("[IMP:9][test] extract_keywords_staged_mode: %s — OK", kws)

    _assert_ldd(caplog)
    assert kws == ["alpha", "-->", "-x", "gamma"], f"staged raw-keep broken: {kws}"
