...
CORE_DIR = Path(__file__).resolve().parent.parent.parent / "core" / "modules"


def _read_healthcheck(module_name: str) -> str:
    """Читает healthcheck.sh указанного модуля."""
    path = Path(CORE_DIR) / module_name / "healthcheck.sh"
...
_LIB_DIR: Path = Path(__file__).resolve().parent.parent.parent / "core" / "lib"
