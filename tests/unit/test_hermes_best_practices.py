...
COMPOSE_FILES = [
    ("hermes-agent", Path(BASE_DIR) / "core" / "modules" / "hermes-agent" / "docker-compose.base.yml"),
]

CONFIG_FILES: list[tuple[str, str]] = [
    # Канон config.yaml — build/config/config.yaml (L2-образ, init.py); модульный
    # config/config.yaml удалён DevPlan 171 W1.5 (дубль без потребителей).
    ("hermes-agent", Path(BASE_DIR) / "core" / "modules" / "hermes-agent" / "build" / "config" / "config.yaml"),
]

ENV_EXAMPLE_FILES: list[tuple[str, str]] = [
    ("hermes-agent", Path(BASE_DIR) / "core" / "modules" / "hermes-agent" / ".env.example"),
]
...
@ldd_trajectory
# GUARD-PRESERVE (168): security-guard — approvals.mode != off (Hermes выполняет опасные команды без подтверждения), единственное покрытие класса дефекта
...
@ldd_trajectory
# GUARD-PRESERVE (168): security-guard — TELEGRAM_ALLOWED_USERS обязателен в .env.example (allowlist канала управления агентом), единственное покрытие класса дефекта
...
@ldd_trajectory
# GUARD-PRESERVE (168): security-guard — TELEGRAM_BOT_TOKEN в .env.example (контракт env-имён секрета, gitleaks-класс дефекта), единственное покрытие класса дефекта
...
@ldd_trajectory
# GUARD-PRESERVE (168): security-guard — HERMES_DASHBOARD_INSECURE=1 запрещён (dashboard без Basic Auth), единственное покрытие класса дефекта
def test_no_dashboard_insecure(all_compose_files, caplog) -> None:
    ...
    with caplog.at_level(logging.DEBUG):
        ...
        for name, path in all_compose_files:
            with pathlib.Path(path).open(encoding="utf-8") as f:
                content = f.read()
            violations.extend(_find_insecure_lines(name, content))
...
def _find_insecure_lines(name: str, content: str) -> list[str]:
    """Найти строки HERMES_DASHBOARD_INSECURE=1/true (PLR1702-хелпер).

    ## @io — ⇥ name, content → ⎋ list[str] "name:line" нарушений
    ## @complexity — O(L) где L = строки
    """
    violations: list[str] = []
    if "HERMES_DASHBOARD_INSECURE" not in content:
        return violations
    ...
    for i, line in enumerate(lines):
        if "HERMES_DASHBOARD_INSECURE" in line:
            ...
            if value in {"1", "true", "True"}:
                violations.append(f"{name}:{i + 1}")
                logger.warning(
                    "[IMP:4][test_best_practices][insecure] VIOLATION: %s line %d: HERMES_DASHBOARD_INSECURE=%s",
                    name,
                    i + 1,
                    value,
                )
...
@ldd_trajectory
# GUARD-PRESERVE (168): security-guard — Basic Auth env vars (HERMES_DASHBOARD_BASIC_AUTH_*) обязательны (dashboard без пароля = открытый доступ), единственное покрытие класса дефекта
...
@ldd_trajectory
# GUARD-PRESERVE (168): security-guard — секреты не в .env.example (leak-класс дефекта: sk-/Bearer/token/API_KEY паттерны), единственное покрытие класса дефекта
...
@ldd_trajectory
# GUARD-PRESERVE (168): security-guard — .env в .gitignore (secrets не должны попадать в git), единственное покрытие класса дефекта
def test_dotenv_in_gitignore(caplog) -> None:
    with caplog.at_level(logging.DEBUG):
        ...
        gitignore_paths: list[tuple[str, str]] = [
            ("hermes-agent", Path(BASE_DIR) / "core" / "modules" / "hermes-agent" / ".gitignore"),
        ]
        ...
        root_gitignore = Path(BASE_DIR) / ".gitignore"
        if pathlib.Path(root_gitignore).is_file():
            with pathlib.Path(root_gitignore).open(encoding="utf-8") as f:
                content = f.read()
            if ".env" not in content:
                logger.info(
...
@ldd_trajectory
# GUARD-PRESERVE (168): security-guard — healthcheck обязателен во всех compose (Docker без healthcheck = слепой деплой/unhealthy незамечен), единственное покрытие класса дефекта
