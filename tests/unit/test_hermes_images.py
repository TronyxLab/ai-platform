...
# STRUCTURE: ┌DI runner + paths_env┐ → ◇ test build_l1 cmd → ◇ test build_l2 CONTEXT guard (empty → False) → ◇ test build_l2 cmd → ⎋ LDD IMP:9/10
...
## @invariants
##   - Все тесты используют DI runner (build_l1(runner=...)) — no real docker (DevPlan 167 D1)
...
def test_build_l1_constructs_platform_amd64_cmd(caplog: pytest.LogCaptureFixture, paths_env: dict[str, str]) -> None:
    ...
    ok = hermes_images.build_l1(runner=_fake_run, env=paths_env)
    assert ok is True
...
def test_build_l1_subprocess_failure_returns_false(caplog: pytest.LogCaptureFixture, paths_env: dict[str, str]) -> None:
    ...
    assert hermes_images.build_l1(runner=lambda *_a, **_kw: mock_run.return_value, env=paths_env) is False
...
def test_build_l2_empty_context_fails_fast(caplog: pytest.LogCaptureFixture, paths_env: dict[str, str]) -> None:
    ...
    ok = hermes_images.build_l2("", runner=called, env=paths_env)
    assert ok is False
...
def test_build_l2_constructs_cmd_with_build_arg(caplog: pytest.LogCaptureFixture, paths_env: dict[str, str]) -> None:
    ...
    ok = hermes_images.build_l2("my-org", runner=_fake_run, env=paths_env)
    assert ok is True
...
def test_main_build_platform_dispatches_l1(paths_env: dict[str, str]) -> None:
    # 🧪 TRAP[TEST] · 2026-08-02 · test_main_build_platform_dispatches_l1 — DevPlan 118 E migration unit test
    """main(): build-platform → build_l1 через DI runner (exit 0)."""
    mock_run = mock.MagicMock(return_value=mock.MagicMock(returncode=0))
