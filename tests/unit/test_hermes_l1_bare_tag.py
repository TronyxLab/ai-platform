...
##           2026-08-14 | DevPlan 167 D2 — monkeypatch → DI-объекты (docker/compose/ghcr_org), 7 → 0 setattr
...
def _build_fakes(*, pull: bool, tag: bool, build: bool) -> tuple[dict, dict]:
    """Собрать DI-фейки для handle_hermes_agent (0 патчей, DevPlan 167 D2); вернуть kwargs + state.

    ## @purpose — Единая фабрика DI-объектов для D18-сценариев: docker-объект (inspect/pull/tag)
    ##            + compose-fn (config/check/build) + ghcr_org передаются параметрами;
    ##            tag_args/build_order фиксируют вызовы для order-assertion.
    ## @io — ⇥ pull: bool, tag: bool, build: bool → ⎋ (kwargs: dict, state: dict)
    ## @complexity — O(1)
    """
    ...
    docker = SimpleNamespace(
        docker_image_inspect_exists=lambda _img, **_kw: False,
        docker_pull=lambda _img, **_kw: pull,
        docker_tag=_tag,
    )
    compose = SimpleNamespace(
        docker_compose_config=lambda _compose_dir, **_kw: _config_result([_GHCR_L1, f"{_ORG}/{_BASE}-context:latest"]),
        check_image_exists=lambda _img: False,
        docker_compose_build=_build,
    )
    kwargs = {
        "docker": docker,
        "compose_config_fn": compose.docker_compose_config,
        "check_image_exists_fn": compose.check_image_exists,
        "compose_build_fn": compose.docker_compose_build,
        "ghcr_org": _ORG,
    }
    return kwargs, state
...
def test_handle_hermes_agent_bare_tag_after_pull(caplog: pytest.LogCaptureFixture) -> None:
    """D18: после GHCR pull L1 создаётся bare-tag hermes-agent-base:latest → build OK."""
    caplog.set_level(logging.INFO)
    kwargs, state = _build_fakes(pull=True, tag=True, build=True)

    ok = hermes_workflow.handle_hermes_agent(_COMPOSE_ARGS, "/tmp/module", "hermes-agent", **kwargs)
...
def test_handle_hermes_agent_no_bare_tag_l2_build_fail(caplog: pytest.LogCaptureFixture) -> None:
    """R5 negative (D18): без bare-tag L2 build FAIL → handle_hermes_agent False."""
    caplog.set_level(logging.INFO)
    kwargs, state = _build_fakes(pull=True, tag=False, build=False)
...
def test_handle_hermes_agent_tag_before_build_order(caplog: pytest.LogCaptureFixture) -> None:
...
def test_handle_hermes_agent_pull_fail_build_l1_source(caplog: pytest.LogCaptureFixture) -> None:
    """D18: pull fail → L1 build из source, bare-tag не требуется (tag не вызывается)."""
    caplog.set_level(logging.INFO)
    kwargs, state = _build_fakes(pull=False, tag=False, build=True)
