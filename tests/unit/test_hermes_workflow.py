...
def test_phase_hermes_build(mock_check, mock_build, tmp_path, caplog: pytest.LogCaptureFixture) -> None:
    ...
    # W5 T5.2 (wiring→behavior): вместо assert_called() — наблюдаемый эффект build-цепочки:
    # 1) L1 build из исходников (base compose + --profile + --build-arg CONTEXT) — pull не удался;
    # 2) L1→L2 build (исходные compose_args, без флагов). Порядок и аргументы — контракт
    #    handle_hermes_agent (D1): сначала L1 из source, потом L2 из L1.
    assert mock_build.call_count == 2, (
        f"Expected L1 + L1→L2 build calls, got {mock_build.call_count} — build chain broken"
    )
    l1_call, l2_call = mock_build.call_args_list
    assert l1_call.kwargs.get("flags"), "L1 source build должен нести --build-arg CONTEXT"
    assert any("CONTEXT=" in f for f in l1_call.kwargs["flags"]), (
        f"L1 build flags lack CONTEXT= build-arg: {l1_call.kwargs.get('flags')}"
    )
    assert l1_call.kwargs.get("compose_args") == compose_args, (
        f"L1 build compose_args mismatch: {l1_call.kwargs.get('compose_args')} != {compose_args}"
    )
    assert "flags" not in l2_call.kwargs, f"L1→L2 build не должен нести source-build флаги: {l2_call.kwargs}"
    assert l2_call.kwargs.get("compose_args") == compose_args, (
        f"L1→L2 build compose_args mismatch: {l2_call.kwargs.get('compose_args')} != {compose_args}"
    )
