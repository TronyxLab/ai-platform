...
def test_skip_to_fail_without_docker_negative(monkeypatch) -> None:
    """R5: без Docker + fail-режим → pytest.fail (не skip)."""
    monkeypatch.setenv("REQUIRE_HONESTY_MODE", "fail")
    # Эмулируем недоступный Docker daemon: docker info возвращает ненулевой rc
    monkeypatch.setattr("_conftest.honesty.shutil.which", lambda name: None)

    with pytest.raises(pytest.fail.Exception, match=r"Docker daemon not available"):
        require_docker_or_fail(reason="R5 negative: docker missing")
    print("[IMP:9][test_honesty] R5 PASS: Docker unavailable + fail-mode → pytest.fail")
...
def test_marker_mode_still_skips(monkeypatch) -> None:
    """marker-режим + Docker недоступен → skip (локальная разработка)."""
    monkeypatch.setenv("REQUIRE_HONESTY_MODE", "marker")
    monkeypatch.setattr("_conftest.honesty.shutil.which", lambda name: None)

    with pytest.raises(pytest.skip.Exception, match=r"Docker daemon not available"):
        require_docker_or_fail(reason="marker mode check")
    print("[IMP:9][test_honesty] marker-mode skip подтверждён (обратная совместимость)")
...
# · Remove if: REQUIRE_HONESTY_MODE=fail семантика меняется
def test_script_missing_fail_mode(monkeypatch, tmp_path) -> None:
    ...
    with pytest.raises(pytest.fail.Exception, match=r"Script not found"):
        require_script_or_fail(missing, reason="R5 negative: script missing")
    print("[IMP:9][test_honesty] R5 PASS: script missing + fail-mode → pytest.fail")
...
def test_env_missing_fail_mode(monkeypatch) -> None:
    """R5: отсутствующая env var + fail-режим → pytest.fail (не skip)."""
    monkeypatch.setenv("REQUIRE_HONESTY_MODE", "fail")
    monkeypatch.delenv("HONESTY_TEST_REQUIRED_VAR", raising=False)

    with pytest.raises(pytest.fail.Exception, match=r"Env var not set"):
        require_env_or_fail("HONESTY_TEST_REQUIRED_VAR", reason="R5 negative: env missing")
    print("[IMP:9][test_honesty] R5 PASS: env missing + fail-mode → pytest.fail")
...
def test_invalid_mode_raises(monkeypatch) -> None:
    ...
    monkeypatch.setenv("REQUIRE_HONESTY_MODE", "invalid-mode")
    # Docker недоступен — иначе _docker_available()==True и mode не валидируется
    monkeypatch.setattr("_conftest.honesty.shutil.which", lambda name: None)

    with pytest.raises(ValueError, match=r"invalid REQUIRE_HONESTY_MODE"):
        require_docker_or_fail(reason="invalid mode check")
    print("[IMP:9][test_honesty] invalid mode → ValueError подтверждён")
