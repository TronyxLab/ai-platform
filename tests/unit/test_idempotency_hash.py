...
def test_phase_needs_rerun_on_config_change(
    caplog: pytest.LogCaptureFixture, tmp_path: Path, state_file: Path, node_yaml: Path
) -> None:
    ...
    m.state.steps[phase] = StepState(name=phase, status="done", hash=m._phase_input_hash(phase, env=env))
    assert m.phase_needs_rerun(phase, env=env) is False, "тот же вход → no-op (идемпотентность)"

    _write_node_yaml(node_yaml, {"postgres": {"enabled": True}, "redis": {"enabled": True}})
    assert m.phase_needs_rerun(phase, env=env) is True, "modules change → re-run (T9.3)"
...
def test_phase_needs_rerun_only_hash_phases(
    ...
    _write_node_yaml(node_yaml, {"postgres": {"enabled": True}, "redis": {"enabled": True}})
    assert m.phase_needs_rerun(phase, env=env) is False, "не-hash фаза не перевыполняется при config change"
...
def test_run_update_reruns_done_phase_on_config_change(
    ...
    # v1: deploy_update done с hash от v1-конфига
    dep_phase = BootstrapPhase.DEPLOY_UPDATE
    v1_hash = StateMachine(state_file_path=str(state_file), env=env)._phase_input_hash(dep_phase, env=env)

    executed: list[str] = []

    class _ResumeSM(StateMachine):
        def execute_phase(self, phase_value: str, *, _env=None, _facts=None):
            executed.append(phase_value)
            return True

    m = _ResumeSM(state_file_path=str(state_file), env=env)
    m.setup_state(mode="update", node="n")

    # Все UPDATE-фазы done (deploy_update — с v1-hash)
    for pv in BootstrapPhase.UPDATE_PHASE_ORDER:
        m.state.steps[pv] = StepState(name=pv, status="done", hash=v1_hash if pv == dep_phase else "")
    m.save()

    # Конфиг меняется ПОСЛЕ сохранения hash'а (имитация правки node.yaml между прогонами)
    _write_node_yaml(node_yaml, {"postgres": {"enabled": True}, "redis": {"enabled": True}})

    rc = cli.run_update_mode(m, audit_fn=lambda _, **__: None, notify_fn=lambda _: None)
    assert rc == 0
...
@ldd_trajectory
def test_execute_phase_retries_transient_error(caplog: pytest.LogCaptureFixture, state_file: Path) -> None:
    ...
    # DI (W-H): phase_func_override= + sleep_fn= (0 патчей phases-модуля/time.sleep)
    result = m.execute_phase(BootstrapPhase.REGISTRY_UPDATE, phase_func_override=_fake_phase, sleep_fn=lambda _: None)
...
def test_execute_phase_retries_exhausted_raises(
    ...
    with pytest.raises(OSError, match="persistent failure"):
        m.execute_phase(BootstrapPhase.REGISTRY_UPDATE, phase_func_override=_always_raise, sleep_fn=lambda _: None)
