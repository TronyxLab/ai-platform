...
def test_provision_networks_no_docker_raises_platform_fatal(caplog, tmp_path) -> None:
    ...
    # DI (167 D3): which-fn — docker недоступен (D4: docker-unavailable = Fatal 10),
    # без глобального патча shutil.which
    ...
    with pytest.raises(PlatformFatalError) as excinfo:
        provision_networks(
            _FakeEnv(),
            dry_run=False,
            which_fn=lambda name: None if name == "docker" else "/usr/bin/" + name,
        )
