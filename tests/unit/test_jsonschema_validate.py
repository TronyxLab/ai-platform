...
# region FUNC_test_ci_root_key_valid
# 🧪 TRAP[TEST] · Regression · ci_root_key в node.yaml проходит schema-валидацию (DevPlan 153 T1, N6)
# · Scenario: node.yaml с node.ci_root_key (валидное поле, DevPlan 142 W1) → exit 0
# · Last fail: RC-прогон 2026-08-12 — «Additional properties are not allowed ('ci_root_key' was unexpected)» (N6)
# · Remove if: ci_root_key удаляется из схемы node.yaml
@ldd_trajectory
def test_ci_root_key_valid(tmp_path, caplog) -> None:
    """node.yaml with ci_root_key must pass validation (exit 0)."""
    yaml_path = _write(
        tmp_path,
        "node-with-ci-root.yaml",
        VALID_NODE_YAML.replace(
            "  owner_key: ssh-ed25519 AAAA test@test\n",
            "  owner_key: ssh-ed25519 AAAA test@test\n"
            "  ci_deploy_key: ssh-ed25519 AAAA ci-deploy@test\n"
            "  ci_root_key: ssh-ed25519 AAAA ci-root@test\n",
        ),
    )
    result = jsonschema_validate.main(["--yaml-file", str(yaml_path), "--schema-file", str(_NODE_SCHEMA)])
    assert result == 0, f"ci_root_key должен валидироваться (exit 0), got {result}"
    logger.info("[IMP:9][test] ci_root_key accepted by node.schema.json — OK")


# endregion FUNC_test_ci_root_key_valid


# region FUNC_test_adopted_project_type_valid
# 🧪 TRAP[TEST] · Regression · projects[].type: adopted проходит schema (DevPlan 153 N6-fix)
# · Scenario: adopt-project (project_adopter.py) пишет type: adopted — схема обязана его допускать
# · Last fail: RC-прогон 2026-08-12 (2-й) — «'type' is a required property» + adopted вне enum (скрытый дрейф)
# · Remove if: enum project type меняется
@ldd_trajectory
def test_adopted_project_type_valid(tmp_path, caplog) -> None:
    """node.yaml с projects[].type: adopted проходит валидацию (exit 0)."""
    yaml_path = _write(
        tmp_path,
        "node-with-adopted.yaml",
        """contexts:
  - name: prod
node:
  name: test-node
...
""",
    ...
    result = jsonschema_validate.main(["--yaml-file", str(yaml_path), "--schema-file", str(_NODE_SCHEMA)])
    assert result == 0, f"adopted должен валидироваться (exit 0), got {result}"
    logger.info("[IMP:9][test] projects[].type=adopted accepted by node.schema.json — OK")


# endregion FUNC_test_adopted_project_type_valid
