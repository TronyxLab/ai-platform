...
def test_reserved_project_key_not_overwritten(policy_yaml, mock_client, tmp_path, caplog):
    ...
    import core.internal.llm.key_provisioner as _kp

    with (
        patch.object(_kp, "LiteLLMAdminClient", return_value=mock_client),
        patch.object(_kp, "discover_projects") as mock_disc,
        patch.object(_kp, "get_platform_consumers") as mock_plat,
    ):
        mock_disc.return_value = [{"name": "test-backend", "llm": {"enabled": True}}]
        mock_plat.return_value = []

        result = _kp.provision_all(
            master_key="mk",
    ...
    # Идемпотентный второй прогон: ключ находится по каноничному project → 0 generate
    mock_client.generate_key.reset_mock()
    mock_client.get_key_by_metadata.return_value = {
...
def test_lock_covers_find_generate(policy_yaml, mock_client, tmp_path):
    ...
    mock_client.get_key_by_metadata.side_effect = _find_marker
