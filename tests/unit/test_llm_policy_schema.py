...
def _load_llm_schema() -> dict:
    ...
    if not LLM_POLICY_SCHEMA_PATH.exists():
        msg = f"LLM policy schema not found: {LLM_POLICY_SCHEMA_PATH}"
        raise FileNotFoundError(msg)
    with pathlib.Path(LLM_POLICY_SCHEMA_PATH).open(encoding="utf-8") as f:
        return json.load(f)
...
def valid_policy_path() -> pathlib.Path:
    ...
    if not p.exists():
        msg = f"Valid policy fixture not found: {p}"
        raise FileNotFoundError(msg)
    return p
...
def invalid_policy_path() -> pathlib.Path:
    ...
    if not p.exists():
        msg = f"Invalid policy fixture not found: {p}"
        raise FileNotFoundError(msg)
    return p
...
def test_valid_policy_jsonschema_pass(valid_policy_path, llm_policy_schema, caplog):
    ...
    with caplog.at_level(logging.DEBUG):
        ...
        with pathlib.Path(valid_policy_path).open(encoding="utf-8") as f:
...
def test_invalid_policy_jsonschema_fails(invalid_policy_path, llm_policy_schema, caplog):
    ...
    with caplog.at_level(logging.DEBUG):
        ...
        with pathlib.Path(invalid_policy_path).open(encoding="utf-8") as f:
            data = yaml.safe_load(f)
