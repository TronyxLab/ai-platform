...
# STRUCTURE: ▶ plan_steps (start×2^i, max_steps) → ◇ run_capacity (fake step_runner: насыщение
#           по error / по p99 / без насыщения / runner error / первый шаг fail) → ⎋ 8 tests
...
class TestPlanSteps:
    def test_doubling_profile(self):
        """start=2, max_steps=8 → 2..256 (×2 каждый шаг)."""
        assert plan_steps(2, 8) == [2, 4, 8, 16, 32, 64, 128, 256]

    def test_max_steps_respected(self):
        """max_steps=3 → 3 шага."""
        assert plan_steps(5, 3) == [5, 10, 20]
