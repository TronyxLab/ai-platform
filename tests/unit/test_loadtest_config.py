...
## @invariants
##   - Каждый тест изолирован: unique node name + env-дикты через env= параметр
##     (DI, DevPlan 160 E2 — без патчей окружения)
...
class TestParseScenario:
    ...
    def test_optional_env_gate(self, repo_dir):
        """optional-сценарий: выключен по умолчанию, LOAD_SCENARIO_DB=1 включает (env-дикт DI)."""
        data = load_scenarios_yaml(str(repo_dir / "core" / "loadtest" / "scenarios.yaml"))
        defaults = data["defaults"]
        assert parse_scenario("db", data["scenarios"]["db"], defaults, env={}).enabled is False
        assert parse_scenario("db", data["scenarios"]["db"], defaults, env={"LOAD_SCENARIO_DB": "1"}).enabled is True

    def test_env_rps_override_and_users_scale(self, repo_dir):
        """LOAD_RPS=25 → target_rps=25, users масштабируется до rps×2 (max(20, 50))."""
        data = load_scenarios_yaml(str(repo_dir / "core" / "loadtest" / "scenarios.yaml"))
        spec = parse_scenario("web", data["scenarios"]["web"], data["defaults"], env={"LOAD_RPS": "25"})
...
class TestParseScenarioNetwork:
    @pytest.mark.parametrize(
        ("scenario_name", "expected_network"),
        [
            ("db", "shared-db-net"),  # сеть из SoT
            ("web", "host"),  # без network в SoT → default host
        ],
    )
    def test_network_resolution(self, repo_dir, caplog, scenario_name, expected_network):
        """parse_scenario network: SoT-сеть vs default host (параметризовано, F5)."""
        caplog.set_level(logging.INFO)
        data = load_scenarios_yaml(str(repo_dir / "core" / "loadtest" / "scenarios.yaml"))
...
class TestLoadConfigNetwork:
    def test_network_env_override(self, repo_dir, node_dir, caplog):
        """LOAD_NETWORK=shared-db-net приоритетнее SoT (web → host перекрывается env-диктом)."""
        caplog.set_level(logging.INFO)
    ...
    def test_network_invalid_rejected(self, repo_dir, node_dir):
        ...
        with pytest.raises(ConfigValidationError):
            load_config(
                ...
                str(repo_dir),
                platform_root=str(repo_dir),
                env={"LOAD_NETWORK": "badnet"},
            )

    def test_db_local_runner_warns(self, repo_dir, node_dir, caplog):
        ...
        load_config(
            ...
            str(repo_dir),
            platform_root=str(repo_dir),
            env={"LOAD_SCENARIO_DB": "1"},  # optional-сценарий включён
        )
...
class TestRender:
    @pytest.mark.parametrize(
        ("domain", "expected"),
        [
            ("example.com", "https://example.com/"),  # {domain} → platform_domain
            ("", "https://1.2.3.4/"),  # пустой domain → host (invariant 2)
        ],
    )
    def test_domain_rendered(self, domain, expected):
        """{domain} плейсхолдер: platform_domain / fallback-to-host (параметризовано, F5)."""
        spec = parse_scenario("web", {"endpoint": "https://{domain}/", "target_rps": 5, "users": 10}, {})  # ruff: ignore[RUF027]
        assert render_template(spec.endpoint_template, spec, "1.2.3.4", domain) == expected
    ...
    def test_env_placeholder(self):
        """{LANGFUSE_PUBLIC_KEY} ← env-дикт; отсутствие env → ConfigValidationError."""
        ...
        rendered = render_dict(headers, spec, "h", "example.com", env={"LANGFUSE_PUBLIC_KEY": "pk-lf-123"})
        assert rendered["Authorization"] == "Bearer pk-lf-123"
        with pytest.raises(ConfigValidationError):
            render_dict(headers, spec, "h", "example.com", env={})
...
class TestLoadConfig:
    def test_full_resolution(self, repo_dir, node_dir, caplog):
    ...
    def test_invalid_runner(self, repo_dir, node_dir):
        ...
        with pytest.raises(ConfigValidationError):
            load_config(
                ...
                str(repo_dir),
                platform_root=str(repo_dir),
                env={"LOAD_RUNNER": "weird"},
            )

    def test_runner_node_env(self, repo_dir, node_dir):
        ...
        cfg = load_config(
            ...
            str(repo_dir),
            platform_root=str(repo_dir),
            env={"LOAD_RUNNER": "node", "LOAD_IMAGE": "ghcr.io/mirror/locust:2.32", "LOAD_CPUS": "4"},
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ({"LOAD_RESULTS_DIR": "/tmp/lt-results"}, "results_dir", "/tmp/lt-results"),  # gitignored results dir
        ],
    )
    def test_env_override_config(self, repo_dir, node_dir, env, attr, expected):
        """load_config env-оверрайды (LOAD_ALLOW_PROD / LOAD_RESULTS_DIR) (параметризовано, F5)."""
        cfg = load_config("web", node_dir["name"], "smoke", str(repo_dir), platform_root=str(repo_dir), env=env)
        assert str(getattr(cfg, attr)) == expected

    def test_endpoint_override_langfuse(self, repo_dir, node_dir):
        """LOAD_ENDPOINT_LANGFUSE_INGEST переопределяет SoT-endpoint (escape hatch, 146-m1 BUG-2)."""
        cfg = load_config(
            "langfuse_ingest", node_dir["name"], "smoke", str(repo_dir), platform_root=str(repo_dir), env={}
        )
        ...
        cfg = load_config(
            ...
            str(repo_dir),
            platform_root=str(repo_dir),
            env={"LOAD_ENDPOINT_LANGFUSE_INGEST": "https://n.test.local"},
        )
    ...
    def test_prometheus_host_override(self, repo_dir, node_dir, caplog):
        ...
        cfg = load_config(
            ...
            str(repo_dir),
            platform_root=str(repo_dir),
            env={"LOAD_PROMETHEUS_HOST": "localhost"},
        )
