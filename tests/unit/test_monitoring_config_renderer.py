...
# 🧪 TRAP[TEST] · deep_merge variants · Unit · Regression never · Remove if: deep_merge semantics change
@pytest.mark.parametrize(
    ("base", "overrides", "expected"),
    [
        # simple: flat dicts, override wins
        ({"a": 1, "b": 2}, [{"b": 3, "c": 4}], {"a": 1, "b": 3, "c": 4}),
        # nested 3 levels: deepest override wins
        (
            {"monitoring": {"metrics": False, "port": 3000, "retention": "7d"}},
            [{"monitoring": {"port": 8080, "alerting": True}}, {"monitoring": {"port": 9090, "dashboard": True}}],
            {"monitoring": {"metrics": False, "port": 9090, "retention": "7d", "alerting": True, "dashboard": True}},
        ),
        # base keys preserved: 'd' from base.b survives
        ({"a": 1, "b": {"c": 2, "d": 3}}, [{"b": {"c": 99}}], {"a": 1, "b": {"c": 99, "d": 3}}),
    ],
)
def test_deep_merge_variants(caplog, base, overrides, expected) -> None:
    """Parametrized: deep_merge simple/nested/base-keys-preserved (F5-reduction)."""
    ...
    _print_ldd_trajectory(caplog, "test_deep_merge_variants")
...
# ── T4.5/T4.7: missing-file loading (консолидировано, F5-reduction) ─────────


# 🧪 TRAP[TEST] · load_missing_file · Unit · Regression never · Remove if: missing-file behavior changes
@pytest.mark.parametrize(
    "load_missing",
    [
        lambda: load_l1_defaults(pathlib.Path("/tmp/nonexistent/defaults.yaml"), "backend"),
        lambda: load_l2_overrides(pathlib.Path("/tmp/nonexistent/override.yaml")),
    ],
)
def test_load_missing_file_returns_empty(caplog, load_missing) -> None:
    """Missing defaults/override file → empty dict, not exception (F5-reduction)."""
    caplog.set_level(logging.INFO)

    assert load_missing() == {}

    _print_ldd_trajectory(caplog, "test_load_missing_file_returns_empty")
...
def test_str_to_bool_variants(caplog) -> None:
    ...
    assert _str_to_bool(val=True) is True
    assert _str_to_bool(val=False) is False
