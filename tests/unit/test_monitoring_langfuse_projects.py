...
def test_create_project_created(caplog, monkeypatch) -> None:
    ...
    with mock.patch("monitoring.langfuse_projects.urllib.request.urlopen", return_value=MockResp()):
        result = create_langfuse_project(_config())
...
def test_create_project_409_skip(caplog, monkeypatch) -> None:
    ...
    with mock.patch("monitoring.langfuse_projects.urllib.request.urlopen", side_effect=_raise):
        result = create_langfuse_project(_config())
...
def test_create_project_network_error(caplog, monkeypatch) -> None:
    ...
    with mock.patch(
        "monitoring.langfuse_projects.urllib.request.urlopen",
