...
def _build_render_steps(
    mcr_module, exclude: set[str] | None = None
) -> tuple[dict[str, MagicMock], dict[str, MagicMock]]:
    """Build render-steps dict (DI 167 D3) + mocks registry — 0 monkeypatch.

    ## @purpose  DI: render-шаги заменяются MagicMock (возвращают RenderResult status="updated").
    ##            Возвращает (steps, mocks): steps — dict {component: callable} для
    ##            run_monitoring_reconfig(render_steps=...); mocks — {имя_атрибута: mock} для
    ##            per-step assert. exclude: имена атрибутов, которые НЕ мокаются (реальный рендер
    ##            в tmp_path) — W5 T5.2 wiring→behavior: platform_root-относительные шаги
    ##            рендерятся по-настоящему.
    ...
    for attr, component in _RENDER_STEPS:
        if attr in exclude:
            steps[component] = getattr(mcr_module, attr)  # реальный facade (render в tmp_path)
            continue
        mock = MagicMock(return_value=mcr.RenderResult(component=component, status="updated"))
        mocks[attr] = mock
        steps[component] = mock
    return steps, mocks


# region FUNC_test_run_monitoring_reconfig_all_steps
## @purpose  Успешный сценарий: monitoring-конфиг → все 7 render-шагов, return 0,
##            IMP:9 START/DONE присутствуют (AC W3). W5 T5.2 (wiring→behavior):
##            platform_root-относительные шаги (prometheus target, loki retention)
##            РЕНДЕРЯТСЯ ПО-НАСТОЯЩЕМУ в tmp_path — ассерты на артефакты-файлы,
##            а не на вызовы моков. Мокаются только шаги, чей output уходит в
##            /opt/* или в сеть (alert_rules/grafana → /opt, reload/langfuse → HTTP,
##            catalog → subprocess) — их эффект в unit-тесте не наблюдаем.
# 🧪 TRAP[TEST] · run_monitoring_reconfig_all_steps · Contract (§4.3) · Regression: рендер не исполняется
# · Scenario: ai-platform.yaml с monitoring-секцией → prometheus target JSON + loki retention
# ·   создаются в tmp_path (реальные артефакты), порядок шагов в caplog, return 0
# · Last fail: N/A (новый тест, DevPlan 138 W3; W5 T5.2 — real-render вместо mock-вызовов)
# · Remove if: контракт run_monitoring_reconfig меняется (шаги перестают вызываться на monitoring-конфиге)
def test_run_monitoring_reconfig_all_steps(tmp_path, caplog) -> None:
    """run_monitoring_reconfig с monitoring-конфигом → РЕАЛЬНЫЕ артефакты (prometheus/loki) в tmp_path."""
    caplog.set_level(logging.INFO)
    project_dir = _make_project(tmp_path, monitoring=True)
    platform_root = tmp_path / "platform"

    # W5 T5.2: мокаются ТОЛЬКО шаги, чей output вне tmp_path (абс. /opt/* пути или сеть) —
    # их эффект в unit-тесте не наблюдаем. prometheus/loki рендерятся по-настоящему.
    # DI (167 D3): render_steps-словарь вместо monkeypatch module-атрибутов.
    steps, mocks = _build_render_steps(mcr, exclude={"generate_prometheus_target", "update_loki_retention"})
    # reload возвращает list[RenderResult] (реальный контракт service_reload)
    mocks["reload_monitoring_services"].return_value = [mcr.RenderResult(component="reload", status="noop")]

    result = mcr.run_monitoring_reconfig(project_dir, "test-app", "test-node", platform_root, render_steps=steps)

    assert result == 0, f"best-effort: ожидался return 0, got {result}"

    # ── Наблюдаемый эффект: реальные артефакты рендера в tmp_path ──
    ...
    loki_config = platform_root / "core" / "modules" / "logging" / "config" / "loki-runtime-config.yml"
    assert loki_config.is_file(), f"Loki runtime config artifact missing: {loki_config}"
    loki_data = yaml.safe_load(loki_config.read_text(encoding="utf-8"))
    streams = loki_data["limits_config"]["retention_stream"]
    assert any(s.get("selector") == '{compose_project="test-app"}' for s in streams), (
        f"Loki retention selector for test-app missing: {streams}"
    )
    rule = next(s for s in streams if s.get("selector") == '{compose_project="test-app"}')
    assert rule["period"] == "336h", f"Loki retention period mismatch (14d → 336h): {rule}"

    # ── Порядок шагов — наблюдаемый через caplog (IMP:8 [hook] <step> render) ──
    step_order = [
        r.message.split("render:")[0].replace("[IMP:8][hook] ", "").strip()
    ...
    for attr in (
        ...
        mocks[attr].assert_called_once()  # AssertionError с деталями вызова при нарушении
    ...
    logger.info("[IMP:9][test] run_monitoring_reconfig: реальные артефакты prometheus/loki + все шаги, return 0 ✓")
...
##           W5 T5.2: моки НЕ нужны — early return ДО рендер-шагов (build_merged_config → None);
##           наблюдаемый эффект: return 0 + IMP:8 skip-лог + 0 артефактов в tmp_path.
# 🧪 TRAP[TEST] · skip_no_monitoring_section · Contract (§4.3) · Regression: рендер без monitoring-секции
# · Scenario: ai-platform.yaml БЕЗ monitoring → build_merged_config None → return 0, 0 артефактов, IMP:8 skip
# · Last fail: N/A (новый тест, DevPlan 138 W3)
# · Remove if: контракт «без секции → skip» меняется
def test_skip_no_monitoring_section(tmp_path, caplog) -> None:
    """build_merged_config → None (нет monitoring-секции) → skip, return 0, лог IMP:8, 0 артефактов."""
    caplog.set_level(logging.INFO)
    ...
    # Наблюдаемый эффект: ни одного артефакта рендера не создано (early return)
    assert not (platform_root / "prometheus-targets").exists(), "Рендер не должен исполняться без monitoring-секции"
    assert not (platform_root / "core" / "modules" / "logging").exists(), (
        "Loki config не должен писаться без monitoring-секции"
    )

    # W5 T5.4: level-agnostic content check (skip-событие «No monitoring config», IMP:8 flow-уровень)
    skip_msgs = [r.message for r in caplog.records if "No monitoring config" in r.message]
    assert skip_msgs, "Ожидался skip-лог (No monitoring config)"
...
def test_render_step_failure_non_fatal(tmp_path, caplog) -> None:
    """Сбой render-шага → лог WARN, return 0 (non-fatal), continue."""
    caplog.set_level(logging.INFO)
    project_dir = _make_project(tmp_path, monitoring=True)
    platform_root = tmp_path / "platform"

    steps, mocks = _build_render_steps(mcr)
...
def test_post_deploy_chain_calls_reconfig(tmp_path, caplog) -> None:
    """post_deploy_chain вызывает reconfig с корректными аргументами (mock, assert call)."""
    caplog.set_level(logging.INFO)
    project_dir = _make_project(tmp_path, monitoring=True)
    platform_root = tmp_path / "platform"

    # DI (W-H): run_cmd/platform_root_override/reconfig_fn параметрами (0 патчей модуля)
    ...
    orch.DeployOrchestrator(projects_base=str(tmp_path))._run_post_deploy_chain(
        ...
        node_name="test-node",
        run_cmd=MagicMock(),
        platform_root_override=str(platform_root),
        reconfig_fn=reconfig_mock,
    )
...
def test_post_deploy_chain_reconfig_failure_non_fatal(tmp_path, caplog) -> None:
    ...
    # Исключение НЕ должно проброситься — chain best-effort (WARN, continue); DI (W-H)
    orch.DeployOrchestrator(projects_base=str(tmp_path))._run_post_deploy_chain(
        "test-app",
        "sha123",
        "DEPLOYED",
