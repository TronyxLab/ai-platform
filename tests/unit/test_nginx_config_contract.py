...
DEV_CONFIG_DIR = Path(__file__).resolve().parent.parent.parent / "core" / "modules" / "nginx" / "dev-config"
...
@ldd_trajectory
# GUARD-PRESERVE (168): единственное покрытие архитектурного инварианта «каждый .conf в dev-config/ содержит server {» — удаление оставит класс дефекта без покрытия
def test_every_dev_config_has_server_block(caplog: pytest.LogCaptureFixture) -> None:
    ...
    conf_files = sorted(Path(DEV_CONFIG_DIR).glob("*.conf"))
    ...
    for f in conf_files:
        logger.info("[IMP:7][nginx-config]   %s", Path(f).name)

    # ── Фильтруем exclude ──
    target_files = [f for f in conf_files if Path(f).name not in EXCLUDE_FROM_SERVER_CHECK]
    ...
    for filepath in target_files:
        ...
        with pathlib.Path(filepath).open(encoding="utf-8") as fh:
            content = fh.read()
