...
##           migrated from the dev-certs shell). Validate idempotency, SAN generation,
...
_SCRIPT_PATH = Path(__file__).resolve().parent.parent.parent / "core" / "modules" / "nginx" / "dev_cert_generator.py"
...
def _run_script(
    ...
    *,
    ...
    return subprocess.run(
        ["python3", str(_SCRIPT_PATH)], capture_output=True, text=True, timeout=30, env=env, check=False
    )
...
def _get_cert_sans(cert_file: Path) -> set[str]:
    ...
    for line in result.stdout.split("\n"):
        ...
        for entry_raw in entries.split(","):
