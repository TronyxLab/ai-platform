...
def _get_nginx_conf_files(platform_root: str, config_dir: str = _NGINX_CONFIG_DIR) -> list[str]:
    ...
    ## @complexity — O(N)
    """
    conf_dir = Path(platform_root) / "core" / "modules" / config_dir
    if not pathlib.Path(conf_dir).is_dir():
        logger.warning("[IMP:4][get_nginx_conf] nginx config dir not found: %s", conf_dir)
        return []
    return sorted(str(p) for p in conf_dir.iterdir() if p.name.endswith(".conf") and p.is_file())
...
@pytest.mark.static_audit
@ldd_trajectory
# GUARD-PRESERVE (168): единственное покрытие класса дефекта "upstream server → unreachable host" (502)
...
@pytest.mark.static_audit
@ldd_trajectory
# GUARD-PRESERVE (168): единственное покрытие класса дефекта "vhost без сервиса" (nginx dead code / 502)
def test_vhost_files_have_corresponding_service(platform_root, all_compose_files, all_module_yamls, caplog) -> None:
    ...
    for vhost_path in vhost_files:
        vhost_basename = Path(vhost_path).name
        with pathlib.Path(vhost_path).open(encoding="utf-8") as f:
            content = f.read()

        # Strategy 0: Static/default configs that don't proxy to any service
        if vhost_basename in STATIC_VHOST_FILES:
            logger.info(
