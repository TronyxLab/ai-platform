...
# STRUCTURE: ▶ tmp_path + caplog → ◇ contexts dict array → ◇ missing → ◇ empty → ◇ context field (negative) → ◇ mixin parity (R5, H1) → ◇ atomic write (R5, H2) → ⎋ LDD IMP:9 assertions
...
##           empty handling, and negative case for the REMOVED top-level 'context' field.
...
# 🧪 TRAP[TEST] · Regression · get_context ignores top-level context field
# · Scenario: node.yaml with `context: old` + `contexts: [{name: "canon"}]` → returns "canon"
# · Last fail: N/A (canon per DevPlan 116 B6 T1 — field removed from priority)
# · Remove if: contexts[] canon semantics change
@ldd_trajectory
def test_get_context_prefers_contexts(caplog, tmp_path):
    """get_context() must read contexts[0].name, NOT the removed 'context' field."""
    yaml_file = tmp_path / "node.yaml"
    yaml_file.write_text("context: old\ncontexts:\n  - name: canon\n")
...
@ldd_trajectory
def test_write_back_atomic_negative(caplog, tmp_path):
    ...
    ## @invariants  167 D6 (DI-zero): replace_fn через конструктор NodeYaml → сбой OSError
    ##              пробрасывается, target НЕ перезаписан частично, временные .tmp файлы не
    ##              остаются в директории (0 monkeypatch).
    """
    ...
    def _boom(src: str, dst: str) -> None:
        msg = "simulated replace failure (R5 H2)"
        raise OSError(msg)

    # 🧐 TRAP[DI-SEAM] · 2026-08-14 · — · replace_fn через конструктор NodeYaml (167 D6)
    # · Rejected: monkeypatch-патч atomic_writer.os.replace — глобальный os-патч
    ...
    node = ny.NodeYaml(str(yaml_path), replace_fn=_boom)
