...
_TEST_DATA_DIR = pathlib.Path(__file__).resolve().parent.parent / "test_data"
...
"""
    ...
    result = subprocess.run(
        ["python3", "-c", python_script, str(_NODE_YAML_PATH)], capture_output=True, text=True, check=False
    )
...
"""
    ...
    result = subprocess.run(
        ["python3", "-c", python_script, str(minimal_yaml)], capture_output=True, text=True, check=False
    )
