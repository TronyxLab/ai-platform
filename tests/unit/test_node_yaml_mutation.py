...
def test_write_back_preserves_comments(caplog, tmp_path):
    ...
    """
    try:
        import ruamel.yaml  # ruff: ignore[F401]
...
def test_write_back_pyyaml_fallback(caplog, tmp_path, monkeypatch):
    ...
    # 🧐 TRAP[DI-KEEP] · 2026-08-14 · — · builtins.__import__ keep (import-path изоляция)
    # · Rejected: DI-шов (прокидывание import_fn в _write_back — 2 уровня вниз, over-engineering)
    # · Reason: тест проверяет МЕХАНИЗМ fallback при отсутствии библиотеки — реальный импорт
    # ·   ruamel.yaml должен вернуть ImportError; DI import_fn заменил бы сам тестируемый механизм
    # · Rev: при введении test-import-harness (изоляция импортов на уровне conftest)
    original_import = builtins.__import__

    def mock_import(name, *args, **kwargs):
        if name == "ruamel.yaml":
            msg = "Mock: ruamel.yaml not available"
...
def _node_with_failing_replace(yaml_path: Path) -> NodeYaml:
    """Создать NodeYaml с replace_fn, бросающим OSError (disk-full эмуляция).

    ## @purpose  DevPlan 119 E5: _write_back использует shared/atomic_writer
    ##           (tempfile.NamedTemporaryFile + os.replace). Точка коммита = replace_fn —
    ##           инжектируется через конструктор NodeYaml (167 D6: replace_fn DI — 0 monkeypatch),
    ##           той же семантикой disk-full, что раньше ловил builtins.open/os.replace.
    ## @io        ⇥ yaml_path → ⎋ NodeYaml (side-effect: replace_fn бросит OSError на записи)
    """

    def _mock_replace(*args: object, **kwargs: object) -> None:
        msg = "Mock: disk full (write)"
        raise OSError(msg)

    # 🧐 TRAP[DI-SEAM] · 2026-08-14 · — · replace_fn через конструктор NodeYaml
    # · Rejected: monkeypatch-патч atomic_writer.os.replace в 3 тестах cache-safety
    # · Reason: seam = тестируемость реального вызова — _write_back реально вызывает
    # ·   atomic_writer с инжектированным rename; конструктор-параметр (1, ≤2) обратно-совместим
    # · Rev: при переезде _write_back на другой канонический writer — убрать проброс
    return NodeYaml(str(yaml_path), replace_fn=_mock_replace)  # type: ignore[arg-type]
