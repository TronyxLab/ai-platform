...
def test_build_channel_forced_command() -> None:
    ...
    logger.info("%s", f"[IMP:9][test] channel={type(channel).__name__}")
...
def test_build_channel_scp() -> None:
    ...
    logger.info("%s", f"[IMP:9][test] channel={type(channel).__name__} metadata={channel.metadata_defaults}")
...
# · Last fail: — host молча игнорировался, LocalChannel сам-себе доставка
...
def test_build_channel_metadata_no_host() -> None:
    """Без host — metadata_defaults пуст (нет мусорного host-ключа)."""
    channel = build_channel(_ns())
    logger.info("%s", f"[IMP:9][test] channel={type(channel).__name__}")
    assert isinstance(channel, LocalChannel)
    # build_channel устанавливает metadata defaults ТОЛЬКО при наличии host —
    # без host атрибут существует, но пуст (декларация в __init__ — типизация,
    # S8-3 basedpyright reportAttributeAccessIssue)
    assert not channel.metadata_defaults, f"Без host metadata_defaults должен быть пуст: {channel.metadata_defaults}"
