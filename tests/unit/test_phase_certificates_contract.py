...
def test_phase_certificates_api_contract() -> None:
    """phase_certificates must exist with the documented signature."""
...
def test_phase_certificates_delegates_to_orchestrator(tmp_path, monkeypatch, caplog) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda core_dir_arg, node_yaml_arg: calls.append((str(core_dir_arg), str(node_yaml_arg))),
    )

    result = phases_mod.phase_certificates(str(core_dir), "test-node", str(node_yaml))
...
def test_phase_certificates_non_fatal_on_orchestrator_failure(tmp_path, monkeypatch, caplog) -> None:
    ...
    monkeypatch.setattr(phases_mod.helpers_domains, "ssl_provision_via_orchestrator", _boom)

    result = phases_mod.phase_certificates(str(tmp_path / "core"), "test-node", str(node_yaml))
...
def test_phase_certificates_missing_node_yaml_raises(tmp_path, monkeypatch, caplog) -> None:
...
def test_ssl_provision_no_domains_non_fatal(tmp_path, caplog) -> None:
    ...
    assert result is None, "ssl_provision_via_orchestrator must return None (non-fatal), got {result!r}"
