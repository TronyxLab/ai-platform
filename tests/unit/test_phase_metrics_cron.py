...
## @invariants
##   - CRON_METRICS_FILE is monkeypatched to tmp_path for every test (never touches /etc/cron.d)
...
##   - Failure path: monkeypatched os.replace raising OSError → False (non-fatal), WARN logged
## @rationale  U-03 gate fix: install_cron_metrics must be unit-testable without root —
##             the destination path is a module constant monkeypatched per test.
...
def cron_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect CRON_METRICS_FILE to tmp_path — never touches /etc/cron.d.

    ## @purpose — Isolate install_cron_metrics from the real system path.
    ## @io — ⇥ tmp_path, monkeypatch → ⎋ Path: redirected CRON_METRICS_FILE
    ## @complexity — O(1)
    """
    target = tmp_path / "etc" / "cron.d" / "platform-metrics"
    target.parent.mkdir(parents=True, exist_ok=True)
    monkeypatch.setattr(system_helpers, "CRON_METRICS_FILE", str(target))
    # /run/lock mkdir is best-effort inside install_cron_metrics — redirect to tmp to
    # avoid touching real /run/lock during tests.
    monkeypatch.setattr(system_helpers.os, "makedirs", lambda *a, **k: None)
    return target
...
def test_install_writes_file_with_contract_content(
    cron_file: Path, core_dir: Path, caplog: pytest.LogCaptureFixture
) -> None:
    ...
    ok = system_helpers.install_cron_metrics(str(core_dir))
...
def test_idempotent_second_call_does_not_rewrite(
    ...
    assert system_helpers.install_cron_metrics(str(core_dir)) is True
    first_mtime = cron_file.stat().st_mtime_ns
    first_content = cron_file.read_text()
    ...
    assert cron_file.read_text() == first_content, "Content must be unchanged on idempotent call"
...
def test_content_mutation_triggers_rewrite(cron_file: Path, core_dir: Path, caplog: pytest.LogCaptureFixture) -> None:
...
def test_write_failure_is_non_fatal(
    cron_file: Path, core_dir: Path, caplog: pytest.LogCaptureFixture, monkeypatch: pytest.MonkeyPatch
) -> None:
    """os.replace failure (e.g., non-root on read-only /etc) → False + WARN, never raises.

    Phase contract: install_cron_metrics is non-fatal — φ3 continues with WARN
    (monkeypatched write simulates permission-denied on a real node).
    """
    caplog.set_level(logging.DEBUG)
    ...
    monkeypatch.setattr(system_helpers.os, "replace", _fail_replace)
