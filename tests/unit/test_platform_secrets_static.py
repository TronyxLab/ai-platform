...
MODULE_DIR: str = Path(PLATFORM_ROOT) / "core" / "modules" / "platform-secrets"

MODULE_YAML_PATH: str = Path(MODULE_DIR) / "module.yaml"
INSTALL_SH_PATH: str = Path(MODULE_DIR) / "install.sh"
MAKEFILE_PATH: str = Path(MODULE_DIR) / "Makefile"
SERVICE_PATH: str = Path(MODULE_DIR) / "platform-secrets.service"
...
def test_module_yaml_schema(caplog: pytest.LogCaptureFixture) -> None:
    ...
    assert pathlib.Path(MODULE_YAML_PATH).is_file(), (
        f"[IMP:9][ps-yaml] FAIL: module.yaml not found at {MODULE_YAML_PATH}"
    )

    with pathlib.Path(MODULE_YAML_PATH).open(encoding="utf-8") as fh:
...
def test_module_files_present(caplog: pytest.LogCaptureFixture) -> None:
    ...
    assert pathlib.Path(MODULE_DIR).is_dir(), f"[IMP:9][ps-files] FAIL: module directory not found: {MODULE_DIR}"

    missing: list[str] = []
    for fname in REQUIRED_FILES:
        fpath = Path(MODULE_DIR) / fname
...
def test_service_execstart_path_exists(caplog: pytest.LogCaptureFixture) -> None:
    ...
    ##            The real script lives at core/internal/secrets/decrypt_secrets.py
    ##            (DevPlan 173 W1.3 — decrypt-secrets.sh facade удалён, ExecStart →
    ##            .py напрямую). Canonical deploy path is /opt/platform/core/ (not /opt/core/).
    ...
    ## @rationale — platform-secrets.service ExecStart вызывает скрипт напрямую
    ##             (exit 203/1 при неверном пути) — см. test_contract_decrypt.py.
    """
    logger.info("[IMP:7][ps-execstart] Checking ExecStart in: %s", SERVICE_PATH)

    assert pathlib.Path(SERVICE_PATH).is_file(), (
    ...
    with pathlib.Path(SERVICE_PATH).open(encoding="utf-8") as fh:
        for line in fh:
            stripped = line.strip()
    ...
    mapped_path = Path(PLATFORM_ROOT) / repo_relative
    mapped_exists = pathlib.Path(mapped_path).is_file()
...
def test_agekey_environmentfile_format_contract(caplog: pytest.LogCaptureFixture) -> None:
    ...
    INSTALLER_PY_PATH: str = Path(MODULE_DIR) / "installer.py"
    assert pathlib.Path(INSTALLER_PY_PATH).is_file(), (
        f"[IMP:9][ps-envfile] FAIL: installer.py not found at {INSTALLER_PY_PATH}"
    )
    with pathlib.Path(INSTALLER_PY_PATH).open(encoding="utf-8") as fh:
