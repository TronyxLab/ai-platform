...
class TestExtractHostPort:
    ...
    # · Last fail: None (new test for DevPlan 117 G T56)
    # · Remove if: extract_host_port regex changes
    def test_extract_host_port_var_only_returns_none(self, caplog) -> None:
        """${VAR}:YYYY without default → None."""
        caplog.set_level(0)
        assert extract_host_port("${PORT}:8080") is None
    ...
    def test_extract_host_port_empty(self) -> None:
        """Empty string → None."""
...
class TestScanComposePorts:
    ...
    # · Expect: MINIO_PORT=9000 first, MINIO_MINIO_PORT=9001 second (TRAP[BUG] 2026-07-31 fix)
    # · Last fail: None (new test for DevPlan 117 G T56)
    # · Remove if: naming scheme changes
    def test_scan_compose_ports_service_equals_module_two_ports(self, tmp_path: Path) -> None:
        """First port → MODULE_PORT, second → MODULE_SERVICE_PORT."""
        ...
        assert result["MINIO_MINIO_PORT"] == 9001
