...
def test_escalator_downgrade_audit(
    ...
    # реальный writer пишет в /var/log/platform (недоступно на dev-машине) — спай заменяет
    monkeypatch.setattr("core.internal.shared.audit_logger.write_audit_entry", fake_write)

    project = _make_mock_project(tmp_path)

    # ── upgrade: fresh → full (согласие) ──
    with caplog.at_level(logging.INFO):
        report_up = set_practices(project, "full")
    ...
    with caplog.at_level(logging.INFO):
        report_down = set_practices(project, "baseline")
