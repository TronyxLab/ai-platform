...
def test_deploy_yml_calls_reusable_workflow(caplog) -> None:
    ...
    for dyml in deploy_ymls:
        ...
        content = dyml.read_text(encoding="utf-8")
...
def test_reusable_workflow_uses_org_variable(caplog) -> None:
    ...
    content = _DEPLOY_PROJECT_YML.read_text(encoding="utf-8")
...
def test_platform_deploy_yml_deleted_from_templates(caplog) -> None:
    ...
    """
    old_files = list(_TEMPLATES_DIR.rglob("platform-deploy.yml"))

    assert not old_files, (
        f"Found {len(old_files)} platform-deploy.yml files in templates/ that should have been deleted: "
        f"{[str(f.relative_to(_PROJECT_ROOT)) for f in old_files]}"
    )
    logger.info("[IMP:9][test][no_platform_deploy] No platform-deploy.yml in templates")
...
def test_template_has_env_platform_makefile_agents(caplog, tmp_path: pathlib.Path) -> None:
    ...
    for ptype in project_types:
        ...
        # R5 negative (План 175 W4.1): удалённый таргет project-fix НЕ эмитится генератором
        # (глагол удалён с платформы — alias project-check --fix; эмиссия → «No rule to make target»)
        if "project-fix:" in makefile:
            issues.append(f"{ptype}: Makefile эмитит удалённый таргет 'project-fix' (План 175 W4.1)")
        ...
        agents_lines = len((project_dir / "AGENTS.md").read_text(encoding="utf-8").splitlines())
        if agents_lines > 60:
