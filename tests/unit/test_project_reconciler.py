...
class TestReconcileSummary:
    ...
    def test_is_success_true(self):
        """failures=0 → True."""
        summary = reconciler_projects.ReconcileSummary(node="test")
        assert summary.is_success() is True

    def test_is_success_false(self):
        """failures=1 → False."""
        summary = reconciler_projects.ReconcileSummary(node="test", failures=1)
        assert summary.is_success() is False

    def test_only_warnings_is_success(self):
        """warnings=3, failures=0 → is_success() is True (warnings don't fail)."""
        summary = reconciler_projects.ReconcileSummary(node="test", warnings=3)
        assert summary.is_success() is True
...
def _assert_imp9(caplog, min_count: int = 1):
    ...
    imp9_count = sum(
        ...
        if "[IMP:" in r.message and int(r.message.split("[IMP:")[1].split("]")[0]) >= 9
    )
    assert imp9_count >= min_count, (
        f"Expected at least {min_count} IMP:9+ log(s), found {imp9_count}"
    )
