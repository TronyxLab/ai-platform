...
def _run_via_subprocess(args: list[str], tmp_path: Path) -> subprocess.CompletedProcess:
    ...
    return subprocess.run(
        [sys.executable, str(script), *args], capture_output=True, text=True, cwd=str(tmp_path), check=False
    )
...
def test_list_projects_empty(caplog, tmp_path):
    ...
    assert result.returncode == 0
    assert not result.stdout  # No stdout output
...
def test_validate_project_name_empty(caplog, tmp_path):
    ...
    assert validate_project_name(None) is False  # type: ignore[arg-type]
...
# 🧪 TRAP[TEST] · MERGED (W2 T2.1) · U-56 verb-reserve: проект «status» невалиден
# · Scenario: validate_project_name("status") is False (verb-reserve)
# · Last fail: — не было reserve-проверки (проект «status» трактовался как SSH_ORIGINAL_COMMAND)
# · Remove if: reserve-список verb'ов меняется
@ldd_trajectory
def test_validate_project_name_rejects_status(caplog, tmp_path):
    """Проект «status» НЕвалиден (verb-reserve, U-56)."""
    from core.internal.shared.project_registry import validate_project_name

    assert validate_project_name("status") is False

    logger.critical("[IMP:9][test] validate_name_status: verb-reserve отклоняет «status» — OK")


# 🧪 TRAP[TEST] · MERGED (W2 T2.1) · U-56 reserve полнота: все verb-имена отклоняются
# · Scenario: for verb in CANONICAL_VERBS: validate_project_name(verb) is False
# · Last fail: — verb-имена регистрировались как проекты
# · Remove if: reserve-список меняется
@ldd_trajectory
def test_validate_project_name_rejects_all_verbs(caplog, tmp_path):
    """Все verb-имена отклоняются validate_project_name (reserve полнота, U-56)."""
    from core.internal.shared.project_registry import validate_project_name
    ...
    logger.critical("[IMP:9][test] validate_name_all_verbs: все %d verb-имён отклонены — OK", len(CANONICAL_VERBS))
