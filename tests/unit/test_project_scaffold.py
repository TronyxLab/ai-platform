...
def _projects_writable() -> bool:
    ...
    # Try with sudo
    if shutil.which("sudo"):
        result = subprocess.run(["sudo", "mkdir", "-p", "/opt/projects"], capture_output=True, text=True, check=False)
...
def test_converge_r3_dry_run(
    ...
    for proj_name in ("testapp", "demoapp"):
        proj_dir = pathlib.Path("/opt/projects") / proj_name
        if proj_dir.exists():
    ...
    logger.info("--- CONVERGE DRY-RUN STDERR ---")
    logger.info("%s", result.stderr)
    logger.info("--- END CONVERGE STDERR ---")
...
def test_converge_r3_scaffold(
    ...
    if not _projects_writable():
        ...
        logger.info("--- CONVERGE STDERR (no /opt write access) ---")
        logger.info("%s", result.stderr)
    ...
    logger.info("--- CONVERGE STDERR (mutation) ---")
    logger.info("%s", result.stderr)
    logger.info("--- END STDERR ---")
...
def test_converge_r3_idempotent(
    ...
    if not _projects_writable():
        ...
        logger.info("--- FIRST DRY-RUN STDERR ---")
        logger.info("%s", result1.stderr)
        logger.info("--- SECOND DRY-RUN STDERR ---")
        logger.info("%s", result2.stderr)
        logger.info("--- END ---")
    ...
    logger.info("--- FIRST CONVERGE STDERR ---")
    logger.info("%s", result1.stderr)
    logger.info("--- END STDERR ---")
    ...
    logger.info("--- SECOND CONVERGE STDERR ---")
    logger.info("%s", result2.stderr)
    logger.info("--- END STDERR ---")
...
def test_step_6b_calls_converge(
    ...
    sm_source = _STATE_MACHINE_SCRIPT.read_text(encoding="utf-8")
    ...
    for line in sm_source.splitlines():
        if "--units" in line and "R3" in line and "converge" in line.lower():
            ...
            logger.info("%s", f"[IMP:7][test][step_6b] Found converge R3 call in state_machine.py: {line.strip()}")
    ...
    converge_source = _CONVERGE_SCRIPT.read_text(encoding="utf-8")
    assert "--units" in converge_source, "converge.sh must support the --units flag in its argument parsing"
    logger.info("[IMP:7][test][step_6b] converge.sh --units flag confirmed in source")
    ...
    node_source = _NODE_LIFECYCLE_SCRIPT.read_text(encoding="utf-8")
    assert "state_machine.py" in node_source, (
        "node-lifecycle.sh must delegate to state_machine.py (DP-091 facade contract)"
    )
    logger.info("[IMP:7][test][step_6b] node-lifecycle.sh delegates to state_machine.py")
...
def test_gen_env_platform_interface(
    ...
    with pathlib.Path(env_yaml).open("w", encoding="utf-8") as f:
        yaml.dump(data, f, default_flow_style=False, sort_keys=False)
    ...
    logger.info("--- GEN-ENV-PLATFORM OUTPUT ---")
    logger.info("%s", "\n".join(lines))
    logger.info("--- END ---")
