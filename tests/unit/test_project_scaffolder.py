...
## · Reason: корневая версия каноническая; уникальные сценарии
...
def test_auto_domain_no_env(tmp_path: pathlib.Path, monkeypatch, caplog) -> None:
    ...
    result = auto_domain(name="myapp", domain="")
    assert not result, f"Expected empty string, got '{result}'"
...
def test_copy_template_excludes_platform_deploy(tmp_path: pathlib.Path, caplog) -> None:
    ...
    ## @purpose — platform-deploy.yml workflow must NOT leak into scaffolded projects
...
@ldd_trajectory
def test_new_backend_project_scaffold(tmp_path: pathlib.Path, caplog) -> None:
...
@ldd_trajectory
def test_new_frontend_project_scaffold(tmp_path: pathlib.Path, caplog) -> None:
