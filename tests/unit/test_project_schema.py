...
@pytest.mark.parametrize(
    "llm_data",
    [
        {"enabled": True},  # progressive disclosure — default chat access
        {"enabled": True, "profile": "premium"},  # explicit valid profile
        {"enabled": True, "profile": "default", "overrides": {"budget": {"daily": 5.0}, "rpm_limit": 100}},
    ],
)
def test_llm_valid_configs(project_schema, caplog, llm_data) -> None:
    """Parametrized: llm-конфиги (enabled/profile/overrides) проходят schema (F5-reduction).

    ## @purpose  Verify progressive disclosure + profile enum + overrides validation.
    """
    with caplog.at_level(logging.DEBUG):
        logger.info("[IMP:7][test_llm_valid_configs] START: llm=%s", llm_data)
        ...
        logger.critical(
            "[IMP:9][test_llm_valid_configs] ASSERT: llm errors=%d (expected 0)",
            len(errors),
        )
        assert errors == [], f"llm config should pass validation: {[e.message for e in errors]}"
