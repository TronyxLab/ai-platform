...
# · Regression: orchestrator_cli status всегда exit 0 (legacy)
# · Scenario: dispatch "status nonexistent" → rc 1; dispatch "status <existing>" → rc 0
# · Last fail: legacy — status всегда exit 0 (orchestrator_cli.py:212-215)
# · Remove if: status-контракт меняется
...
# · Last fail: legacy — raw `docker compose ps` по SSH (U-36)
