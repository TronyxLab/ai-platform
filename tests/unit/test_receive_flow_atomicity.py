#!/usr/bin/env python3
# GREP_SUMMARY: test-receive-flow-atomicity, T9.8, atomic-staging, payload-backup, rollback, payload-restore, T9.9, max-payload-bytes, streaming, reject
...
def test_receive_deploy_atomic_staging_no_leftovers(
    ...
    leftovers = [f for f in os.listdir(target_dir) if f.startswith(".payload-")]
    ...
    expected = "services:\n  web:\n    image: nginx:alpine\n"
    assert compose == expected, f"файл обязан быть ПОЛНЫМ payload (не обрезан): {compose!r}"
...
def test_rollback_restores_payload_files_from_backup(
    ...
    monkeypatch.setattr(orch, "_rollback_compose", lambda *a, **k: True)
...
def test_receive_max_payload_bytes_rejects(
    ...
    monkeypatch.setattr(rf, "MAX_PAYLOAD_BYTES", 1024)  # маленький лимит для теста
    big_payload = b"x" * (2048)  # 2× лимита
    monkeypatch.setattr("sys.stdin", type("FakeStdin", (), {"buffer": io.BytesIO(big_payload)})())

    rc = rf.ReceiveFlow(projects_base=str(tmp_path)).run(project_name="testproj", version="sha1")
...
def test_read_stdin_limited_ok(caplog: pytest.LogCaptureFixture, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("sys.stdin", type("FakeStdin", (), {"buffer": io.BytesIO(payload)})())
    data = _read_stdin_limited()
    ...
    def _capture_deploy(*args, **kwargs):
        ...
        if bdir:
            for f in os.listdir(bdir):
                backup_content[f] = Path(bdir, f).read_text(encoding="utf-8")
        return fake_result
