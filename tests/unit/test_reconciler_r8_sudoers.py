...
def test_reconcile_sudoers_converged(tmp_path, caplog, node_yaml_with_modules, sudoers_env, monkeypatch):
    ...
    with patch.object(subprocess, "run", side_effect=mock_run):
        entry = reconciler.reconcile_sudoers(
            ...
            dry_run=False,
            report_only=False,
            sudoers_dir=sudoers_env["sudoers_d_dir"],
        )
...
def test_reconcile_sudoers_drift_detected(tmp_path, caplog, node_yaml_with_modules, sudoers_env, monkeypatch):
    ...
    # Track atomic write: temp file creation + os.replace (167 D6: replace_fn seam — 0 monkeypatch)
    temp_files_created = []
    ...
    # 167 D6 (DI-zero): replace_fn передаётся в reconcile_sudoers → atomic_write_sudoers
    ...
    with patch.object(subprocess, "run", side_effect=mock_run):
        entry = reconciler.reconcile_sudoers(
            ...
            report_only=False,
            sudoers_dir=sudoers_env["sudoers_d_dir"],
            replace_fn=track_replace,
        )

    assert entry["unit"] == "R8"
    assert entry["status"] == "mutated"
    # Verify atomic write: replace_fn should have been called (tmp → target)
    assert len(temp_files_created) > 0, "replace_fn should have been called for atomic write"
...
def test_reconcile_sudoers_visudo_fail(tmp_path, caplog, node_yaml_with_modules, sudoers_env, monkeypatch):
    ...
    # 167 D6 (DI-zero): replace_fn seam — трекер rename вместо monkeypatch os.replace
    def track_replace(src, dst):
    ...
    # Original file should NOT have been replaced (replace_fn не вызван — visudo отклонил)
    ...
    assert replaced == [], f"visudo-fail: replace_fn не должен вызываться, got {replaced}"
