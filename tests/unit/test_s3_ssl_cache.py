...
##           2026-08-14 | DevPlan 167 D2 — 2 setattr (shared/ssl_certs.subprocess) → TRAP[DI-KEEP]:
##           seam в shared/ssl_certs.py (вне скоупа волны D2); прочие патчи — unittest.mock (вне метрики)
...
@ldd_trajectory
def test_upload_cert_success(caplog, tmp_path):
...
@ldd_trajectory
def test_upload_cert_missing_s3_bucket(caplog, tmp_path):
...
@ldd_trajectory
def test_download_cert_success(caplog, tmp_path):
...
@ldd_trajectory
def test_check_cert_hit(caplog, tmp_path):
...
# · Remove if: check_cert logic changes significantly
@ldd_trajectory
def test_check_cert_miss(caplog):
...
@ldd_trajectory
def test_bulk_restore_parses_yaml(caplog, tmp_path):
...
@ldd_trajectory
def test_cli_upload_command(caplog):
...
# endregion Tests: CLI entry point
...
# · Regression: G3 — data/<domain>/ layout must still resolve
# · Last fail: N/A (native replacement for test_upload_with_account_ecc_path grep)
# · Remove if: fallback removed
@ldd_trajectory
def test_find_acme_account_dir_data_fallback(caplog, tmp_path):
    ...
    assert result == str(acme / "data" / "example.com"), f"G3: data/<domain>/ fallback must resolve, got {result}"
...
def test_validate_cert_rejects_non_le_issuer(caplog, tmp_path, monkeypatch):
    ...
    # 🧐 TRAP[DI-KEEP] · 2026-08-14 · — · openssl-subprocess seam живёт в shared/ssl_certs.py
    # · Rejected: DI-шов (runner-параметр в cert_is_valid-цепочке ssl_certs)
    # · Reason: seam в core/internal/shared/ssl_certs.py — shared-модуль вне файл-скоупа волны D2
    # ·   (владелец shared/cert-волна; собственный тест test_ssl_certs.py патчит тот же subprocess
    # ·   через unittest.patch). s3_ssl_cache._validate_cert делегирует в cert_is_valid (C9) —
    # ·   runner-шов вне s3_ssl_cache.py невыразим без правки shared.
    # · Rev: когда shared/ssl_certs.cert_is_valid-цепочка получит runner-параметр —
    # ·   мигрировать оба теста на runner DI (передать fake через s3_ssl_cache._validate_cert)
...
# · Remove if: _download_s3_file сигнатура меняется
@ldd_trajectory
def test_download_s3_file_di_client_param(caplog, tmp_path):
    """_download_s3_file(s3_client=fake): скачивание через переданный клиент (без патча фабрики)."""
    mock_client = MagicMock()
    dst = tmp_path / "dst.pem"

    ok = s3_ssl_cache._download_s3_file(
        "platform/ssl-certs/x/fullchain.pem", str(dst), s3_client=mock_client, bucket="test-bucket"
    )

    assert ok is True, "download через DI-клиент обязан вернуть True"
    mock_client.download_file.assert_called_once_with("test-bucket", "platform/ssl-certs/x/fullchain.pem", str(dst))
    logger.critical("[IMP:9][test] _download_s3_file DI-клиент — вызов download_file верифицирован")


# 🧪 TRAP[TEST] · 2026-08-13 · Regression · W4b — _upload_s3_file через s3_client параметром (DI)
# · Scenario: fake-клиент параметром; upload_file получает bucket/key/path (DevPlan 160 T4.2)
# · Last fail: N/A (new — W4b factory injection)
# · Remove if: _upload_s3_file сигнатура меняется
@ldd_trajectory
def test_upload_s3_file_di_client_param(caplog, tmp_path):
    """_upload_s3_file(s3_client=fake): выгрузка через переданный клиент (без патча фабрики)."""
    mock_client = MagicMock()
    src = tmp_path / "cert.pem"
    src.write_text("content")

    ok = s3_ssl_cache._upload_s3_file(
        str(src), "platform/ssl-certs/x/fullchain.pem", s3_client=mock_client, bucket="test-bucket"
    )

    assert ok is True, "upload через DI-клиент обязан вернуть True"
    mock_client.upload_file.assert_called_once_with(str(src), "test-bucket", "platform/ssl-certs/x/fullchain.pem")
    logger.critical("[IMP:9][test] _upload_s3_file DI-клиент — вызов upload_file верифицирован")
...
@ldd_trajectory
def test_download_cert_restores_all_artifacts(caplog, tmp_path):
    ...
    def _fake_download(s3_key, local_dst):
        if local_dst.endswith(".tar.gz"):
            ...
            with Path(local_dst).open("wb") as f:
