...
# GUARD-PRESERVE (168): единственное покрытие контракта PLATFORM_* count ≥ 8 (T18 min-vars) — не редуцируется (не дубль, не параметризуем с иными свойствами)
@ldd_trajectory
...
# GUARD-PRESERVE (168): единственное покрытие идемпотентности generate_env_platform (T18 — два прогона = идентичный вывод, кроме timestamp) — не редуцируется
@ldd_trajectory
