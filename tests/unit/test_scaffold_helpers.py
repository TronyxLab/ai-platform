...
## · Reason: корневая версия каноническая; уникальные ассерты из unit
...
def test_gen_makefile_new(tmp_path: pathlib.Path, caplog) -> None:
    ...
    assert "project-check:" in content, "Makefile must keep project-check target (K1)"
    assert "help:" in content
    assert "PLATFORM_DIR" in content, "Makefile must reference PLATFORM_DIR"
    assert "test-project" in content
    assert "\t@" in content
    # R5 negative (План 175 W4.1): удалённый таргет project-fix НЕ эмитится (alias project-check --fix)
    assert "project-fix" not in content, "Makefile template must not emit removed target project-fix"
    assert "project-check --fix" in content, "Makefile must reference the canonical --fix alias"
