...
import core.internal.scripts.scripts_audit as scripts_audit
...
def test_collect_only_shebang_files(caplog: pytest.LogCaptureFixture, tmp_path: Path) -> None:
    ...
    found_imp8 = any("[IMP:8]" in r.message for r in caplog.records)
    assert found_imp8, "IMP:8 collect log expected"
...
def test_is_exception_matches_lib_and_healthcheck() -> None:
    ...
    assert scripts_audit.is_exception("core/entrypoints/deploy.sh") is False
...
deploy:
- make_target: deploy
  delegates_to: core/entrypoints/deploy.sh → core/internal/deploy/orchestrator.py
  description: Deploy project
"""
    )
    strings = scripts_audit.collect_manifest_strings(manifest)
    assert scripts_audit.is_registered("core/entrypoints/deploy.sh", strings) is True
...
def test_audit_clean_when_all_registered(caplog: pytest.LogCaptureFixture, tmp_path: Path) -> None:
    ...
    caplog.set_level(logging.INFO)
    _project, core_dir = _make_core_tree(tmp_path)
    _write_shebang(core_dir / "entrypoints" / "deploy.sh")

    manifest = core_dir / "entrypoint-manifest.yaml"
    manifest.write_text("deploy:\n- delegates_to: core/entrypoints/deploy.sh\n")

    violations = scripts_audit.audit(core_dir, _project, manifest)
    assert violations == [], f"Expected clean audit, got {violations}"
    print(f"--- audit clean, violations={violations} ---")
...
def test_main_exit_codes(monkeypatch: pytest.MonkeyPatch, tmp_path: Path, capsys) -> None:
    ...
    # Case 1: clean → exit 0
    _write_shebang(core_dir / "entrypoints" / "deploy.sh")
    (core_dir / "entrypoint-manifest.yaml").write_text("deploy:\n- delegates_to: core/entrypoints/deploy.sh\n")
