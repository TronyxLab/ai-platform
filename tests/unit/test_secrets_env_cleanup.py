...
def test_cleanup_removes_proxy_when_tor_disabled(caplog, secrets_env_file):
    ...
    on_disk = secrets_env_file.read_text()
...
def test_cleanup_keeps_proxy_when_tor_enabled(caplog, secrets_env_file):
    ...
    secrets_env_file.write_text(original)
    ...
    assert secrets_env_file.read_text() == original, "File rewritten despite TOR_ENABLED=true"
...
def test_cleanup_no_proxy_vars_unchanged(caplog, secrets_env_file):
    ...
    assert secrets_env_file.read_text() == original, "File changed despite no proxy vars present"
