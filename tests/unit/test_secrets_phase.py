...
class _FakeSecretsHelpers:
    """Fake secrets-domain I/O namespace (DI, 167 D4) — shell-фасад (lib/secrets.sh) вне unit-скоупа.

    ## @purpose — helpers-namespace injection (DI-канон 163 W-H #7): phase_secrets_provision
    ##            принимает helpers= параметром; fake реализует только decrypt/ensure —
    ##            реальный вызов фазы с 0 setattr-патчей.
    """

    def decrypt_secrets(self, _core_dir: str) -> None:
        """Fake decrypt — ничего не делает (shell-фасад вне unit-скоупа)."""

    def ensure_secrets_exist(self, _core_dir: str, *, env=None) -> None:
        """Fake ensure — принимает env= dict (DI), игнорирует."""


@pytest.fixture
def mock_secrets_helpers() -> _FakeSecretsHelpers:
    """Fake secrets-helper namespace для phase_secrets_provision (DI, 167 D4)."""
    return _FakeSecretsHelpers()
...
def test_secrets_provision_does_not_create_etc_age_key_txt(
    ...
    # DI (167 D4): helpers= fake-namespace (0 setattr-патчей) — реальный вызов фазы
    result = phase_secrets_provision(str(tmp_path), "test-node", "{}", helpers=mock_secrets_helpers)
    ...
    assert not pathlib.Path(_ETC_AGE_KEY_FILE).is_file(), (
