...
def _get_env_path(modules_dir: str) -> Path:
    """Resolve the .env file path for hermes-agent.

    ## @purpose — Single source of truth for .env location.
    ## @io — ⇥ modules_dir: str → ⎋ Path (absolute path)
    ## @complexity — O(1)
    """
    return Path(modules_dir) / "hermes-agent" / ".env"
...
def test_secrets_env_file_exists(
    ...
    # region BLOCK_Check
    file_exists = pathlib.Path(env_path).is_file()
...
def test_litellm_master_key_present(
    ...
    # endregion BLOCK_Check_REQUIRED

    # region BLOCK_Check_definitions
    import yaml

    defs_path = pathlib.Path(__file__).resolve().parent.parent.parent / "core" / "secret-definitions.yaml"
    logger.info("[IMP:7][test_litellm_master_key_present] Reading definitions: %s", defs_path)
    with pathlib.Path(defs_path).open(encoding="utf-8") as f:
        defs = yaml.safe_load(f)

    found = False
    for s in defs.get("secrets", []):
        if s.get("name") == "LITELLM_MASTER_KEY":
            ci = s.get("ci_default", "")
            logger.info("[IMP:8][test_litellm_master_key_present] LITELLM_MASTER_KEY ci_default=%s", ci)
            assert ci, "LITELLM_MASTER_KEY must have ci_default in secret-definitions.yaml"
            found = True
            break

    assert found, "LITELLM_MASTER_KEY not found in secret-definitions.yaml"
    logger.info("[IMP:9][test_litellm_master_key_present] ✅ LITELLM_MASTER_KEY has ci_default in definitions")
    # endregion BLOCK_Check_definitions
