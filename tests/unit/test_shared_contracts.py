#!/usr/bin/env python3
# GREP_SUMMARY: contracts shared DEPLOY_BEST_EFFORT EXIT_OK EXIT_FATAL exit-codes policy legacy-parity U-39 unit
...
##   - DEPLOY_BEST_EFFORT == True (фиксация политики legacy parity)
...
@ldd_trajectory
# 🧪 TRAP[TEST] · Regression · DEPLOY_BEST_EFFORT policy is True (фиксация legacy parity, DevPlan 116 B4 T1)
def test_deploy_best_effort_policy_true(caplog) -> None:
    """DEPLOY_BEST_EFFORT must be True — deploy-политика legacy parity (U-39)."""
    assert contracts.DEPLOY_BEST_EFFORT is True
    logger.info("[IMP:9][contracts] PASS: DEPLOY_BEST_EFFORT=True (legacy parity политика зафиксирована)")
