...
def test_nginx_reload_timeout(caplog: pytest.LogCaptureFixture) -> None:
    ...
    # W5 T5.4: level-agnostic content check (timed out — IMP:7 flow-строка)
    assert any("timed out" in r.message for r in caplog.records), "expected docker_ops timeout warning log"
...
def test_nginx_reload_container_missing_negative(caplog: pytest.LogCaptureFixture) -> None:
    ...
    logger.info("[IMP:9][test] R5 PASS: отсутствующий контейнер handled (no-raise)")
