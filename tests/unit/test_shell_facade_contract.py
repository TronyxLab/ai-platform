...
def test_shell_has_provisioner_delegation(caplog) -> None:
    ...
    ##           --scope networks and --scope volumes, with fallback.
    ...
    ##   - fallback (docker network create) present
    """
    caplog.set_level(logging.DEBUG)
    content = _read_deploy_modules_shell()
...
def test_shell_has_python_delegation(caplog) -> None:
    ...
    ##   - No per-module `python3 deploy/*.py` subprocess calls remain in the facade
    """
    caplog.set_level(logging.DEBUG)
    ...
    # ── 3. No per-module python3 subprocess calls remain in the facade ──
    old_ops = {
        "context_overlay.py",
    ...
    for op in old_ops:
        has_old = f"deploy/{op}" in shell_content and "python3" in shell_content
...
#           _compute_exit_code) — contract PRESERVED: CRIT>0 → 2, WARN>0 → exit 0
...
def test_shell_has_severity_exit(caplog) -> None:
    ...
    ##   - WARN maps to exit 0 (non-blocking — 33aaaeb parity)
...
def test_shell_has_context_overlay(caplog) -> None:
    ...
    ##           the preflight phase (context_overlay.py --action ensure --node-yaml parity).
