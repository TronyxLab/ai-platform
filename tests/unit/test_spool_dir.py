...
##     spool-декларации. litellm/node-metrics/service-exporters — spool_dir (/var/lib/platform/*);
...
# GUARD-PRESERVE (168): gate-контракт абсолютных путей spool_dir (модульная schema-конвенция —
# относительный путь = spool_validator/деплой ломаются на ноде); единственное покрытие свойства
...
def test_spool_dir_none_no_warn() -> None:
    ...
    logger.info("[IMP:9][test_spool_dir_none_no_warn] PASS: spool_dir: none declared and handled")
...
def test_spool_dir_missing_still_warns() -> None:
    ...
    logger.info("[IMP:9][test_spool_dir_missing_still_warns] PASS: missing spool decl still triggers WARN")
