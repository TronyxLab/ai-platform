...
def test_extra_positional_rejected_in_secrets_modes(caplog, monkeypatch) -> None:
    """T2.B P1-17 (stdin-канон plan-14): лишние НЕпустые строки stdin → fail-loud."""
    import io as _io

    # update-secrets ожидает 1 значение; подано 2 непустых → BuildModeError
    monkeypatch.setattr(ssh_cmd_builder.sys, "stdin", _io.StringIO("age-key\nEXTRA\n"))
    with pytest.raises(ssh_cmd_builder.BuildModeError, match="extra non-empty"):
        ssh_cmd_builder._dispatch_build("update-secrets", ["n1"])
    logger.info("[IMP:9][test][negative] лишние непустые stdin-строки отклоняются ✓")
