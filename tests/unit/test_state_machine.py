...
# ═══════════════════════════════════════════════════════════════════
# region Tests: P0 healthcheck fix (DevPlan 045)
# ═══════════════════════════════════════════════════════════════════
...
##    otherwise mock.patch.object(sp_mod, "run") patched subprocess.run in test module.
@ldd_trajectory
def test_run_healthchecks_calls_modules_healthcheck_sh(tmp_path, caplog):
    ...
    with mock.patch("subprocess.run") as mock_run:
        ...
        # Create a dummy node.yaml
        (tmp_path / "node.yaml").write_text("modules:\n  postgres:\n    enabled: true\n")

        from core.internal.bootstrap.lifecycle.state_machine import _run_healthchecks

        _run_healthchecks(core_dir, node_yaml=str(tmp_path / "node.yaml"))
    ...
    logger.critical("[IMP:9][test] _run_healthchecks calls modules-healthcheck.sh — P0 fix verified")
