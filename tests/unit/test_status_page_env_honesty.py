...
def test_env_requires_matches_reads() -> None:
    ...
    manifest = yaml.safe_load(
        (repo / "core" / "secrets-manifest.yaml").read_text(encoding="utf-8")
    )
    consumers_by_name = {
        entry["name"]: set(entry.get("consumers") or []) for entry in manifest["secrets"]
    }
    ...
    for mod in _CHECKED:
        ...
        assert not unregistered, (
            f"{mod}: env_requires без регистрации в secrets-manifest consumers: {unregistered}"
        )
