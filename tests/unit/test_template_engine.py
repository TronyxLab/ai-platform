...
# STRUCTURE: ┌19 atomic tests┐ → ◇ render basic → ◇ strict grammar → ◇ error cases → ◇ edge cases → ◇ check_all
...
@pytest.mark.parametrize(
    "pairs,expected",
    [
        (["A=1", "B=2", "NAME=hello"], {"A": "1", "B": "2", "NAME": "hello"}),  # базовое преобразование
        (["X=first", "X=second"], {"X": "second"}),  # duplicate key: last wins
    ],
)
def test_parse_vars(caplog, pairs, expected):
    """parse_vars converts KEY=val list to dict; на duplicate key побеждает последнее значение."""
    # 🧪 TRAP[TEST] · 2026-07-18 · REGRESSION · CLI var parsing + duplicate key resolution
    # · Last fail: N/A (preventive)
    # · Remove if: parse_vars signature / duplicate policy changes
    result = parse_vars(pairs)
    assert result == expected
    logger.critical("[IMP:9][test][parse] parse_vars OK: %s", pairs)
...
def test_deterministic_output(caplog, tmp_path):
    ...
    tmpl.write_text("{{A}}-{{B}}")
    template_vars = {"A": "x", "B": "y"}
    r1 = render_template(str(tmpl), vars=template_vars, dry_run=True)
    r2 = render_template(str(tmpl), vars=template_vars, dry_run=True)
    assert r1 == r2
    assert r1 == "x-y"
