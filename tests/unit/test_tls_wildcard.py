...
VHOST_CONFIG_DIR_REL: str = Path("core") / "modules" / "nginx" / "config"
...
# W3.5-1 (164): issue-cert.sh → issue_cert.py (Python-модуль — acme.sh executor)
TLS_SCRIPT_PATHS: list[str] = [
    Path("core") / "internal" / "bootstrap" / "issue_cert.py",
]
...
def _read_conf(platform_root: str, filename: str) -> str:
    ...
    """
    config_dir = Path(platform_root) / VHOST_CONFIG_DIR_REL
    filepath = Path(config_dir) / filename
    if not pathlib.Path(filepath).is_file():
        msg = f"[IMP:9][_read_conf] Vhost config not found: {filepath}"
...
def _platform_root() -> str:
    ...
    ## @complexity — O(1)
    ## @invariants — Uses pathlib.Path(__file__).resolve().parent.parent.parent
    """
...
def _tls_script_paths(_platform_root: str) -> list[str]:
    ...
    """
    paths = [Path(_platform_root) / rel for rel in TLS_SCRIPT_PATHS]
...
def test_vhost_configs_use_wildcard_cert_path(
    ...
    for filename in sorted(SSL_VHOST_FILES):
        config_dir = Path(_platform_root) / VHOST_CONFIG_DIR_REL
        filepath = Path(config_dir) / filename
        ...
        if not paths and "include /etc/nginx/conf.d/ssl-params.conf" in content:
            ...
            for candidate in ("ssl-params.conf", "ssl-params.conf.template"):
                ssl_params_path = Path(config_dir) / candidate
                if pathlib.Path(ssl_params_path).is_file():
                    with pathlib.Path(ssl_params_path).open(encoding="utf-8") as sp_f:
...
def test_hermes_vhost_conditionally_deployed(
    ...
    # endregion BLOCK_CheckHttpRedirect
    ...
    # endregion BLOCK_CheckHttpsProxy
...
## @purpose  Verify TLS-related executors (issue_cert.py) exist on disk.
##           These handle certificate issuance, renewal, and acme.sh integration.
...
## @rationale  Contract test: verifies the real TLS executors are present in the project tree.
...
## @purpose  Verify TLS-related executors are syntactically valid: bash -n для .sh фасадов,
##           python3 -m py_compile для .py модулей (W3.5-1: issue-cert.sh → issue_cert.py).
##           A syntax error in issue_cert.py would block wildcard certificate
##           issuance and renewal, breaking TLS for all platform services.
## @io       ⇥ _tls_script_paths → ⎋ None (asserts syntax-check returncode == 0 for each)
## @complexity  O(N) where N = number of scripts
## @invariants
##   - .sh → bash -n (без выполнения); .py → python3 -m py_compile
##   - returncode != 0 means syntax error → deployment + TLS renewal broken
## @rationale  Contract test: catches syntax regressions in critical TLS executors
##             via REAL interpreter, not simulation.
...
def test_tls_scripts_syntax(_tls_script_paths: list[str]) -> None:
    """
    # ▶ _tls_script_paths → ∋ each path → ⚡ bash -n | python3 -m py_compile → ◇ rc == 0? → ⊕ results → ⎋ pass | fail
    """
    ...
    for script_path in _tls_script_paths:
        ...
        # W3.5-1: диспетч проверки по суффиксу (.sh → bash -n; .py → py_compile)
        ...
        logger.info("%s", f"[IMP:7][test_tls_scripts_syntax] {checker} {script_path} → exit={result.returncode}")
        if result.stderr:
            for line in result.stderr.splitlines():
                logger.info("%s", f"[IMP:7][{checker}/stderr] {line}")
...
def test_acme_sh_available() -> None:
    ...
    if acme_path is None:
        ...
        if not (pathlib.Path("/opt/platform").is_dir() or os.environ.get("NODE")):
...
@pytest.mark.contract
# GUARD-PRESERVE (168): единственное покрытие класса дефекта Simulator-код в тестах (TRAP[BUG] 2026-07-10 Epic 2 T2.1)
