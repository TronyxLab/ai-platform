...
# 🧪 TRAP[TEST] · 2026-08-02 · Regression · plan_install: все пакеты установлены → [] (D2)
# · Scenario: present = все 4 пакета → план пуст (shell SKIP-ветка)
# · Last fail: N/A (new — D2 test-first)
# · Remove if: plan_install semantics change
def test_plan_install_all_present() -> None:
    """Все пакеты (tor/privoxy/obfs4proxy/webtunnel) установлены → пустой план."""
    plan = tor_setup.plan_install(
        present={"tor", "privoxy", "obfs4proxy", "webtunnel"},
        repo={"webtunnel", "obfs4proxy"},
    )
    assert plan == []


# 🧪 TRAP[TEST] · 2026-08-02 · Regression · plan_install: webtunnel доступен → в плане (D2)
# · Scenario: webtunnel отсутствует на диске, но есть в репозиториях → план включает webtunnel
# · Last fail: N/A (new — D2 test-first)
# · Remove if: webtunnel plan-логика меняется
def test_plan_install_webtunnel_available() -> None:
    """webtunnel отсутствует + доступен в репозиториях → план включает webtunnel."""
    plan = tor_setup.plan_install(
        present={"tor", "privoxy", "obfs4proxy"},
        repo={"webtunnel", "obfs4proxy"},
    )
    assert plan == ["webtunnel"]
...
# 🧪 TRAP[TEST] · 2026-08-02 · Regression · plan_install: все отсутствуют + webtunnel в репо (D2)
# · Scenario: пустой present + полный repo → план = tor, privoxy, obfs4proxy, webtunnel
# · Last fail: N/A (new — D2 test-first; DevPlan test_tor_package_all_absent)
# · Remove if: порядок плана меняется
def test_tor_package_all_absent() -> None:
    """Всё отсутствует, webtunnel доступен → полный план из 4 пакетов (порядок сохранён)."""
    plan = tor_setup.plan_install(present=set(), repo={"webtunnel", "obfs4proxy"})
    assert plan == ["tor", "privoxy", "obfs4proxy", "webtunnel"]
...
# 🧪 TRAP[TEST] · 2026-08-02 · Regression · plan_install: частично установлено (D2)
# · Scenario: только tor установлен → план = privoxy, obfs4proxy (+webtunnel если в репо)
# · Last fail: N/A (new — D2 test-first)
# · Remove if: plan_install semantics change
def test_plan_install_partial() -> None:
    """Частичная установка: только tor на месте → план = недостающие базовые + webtunnel."""
    plan = tor_setup.plan_install(present={"tor"}, repo={"webtunnel", "obfs4proxy"})
    assert plan == ["privoxy", "obfs4proxy", "webtunnel"]
