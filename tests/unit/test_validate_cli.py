...
##   - Golden strings match the legacy PYOF heredoc format exactly (2-space prefix, " > " path)
...
# Golden baselines — byte-identical to legacy PYOF heredoc output (DevPlan 093 AC1)
...
def test_cli_valid_yaml_exit0_silent(tmp_path) -> None:
    ...
    # · Scenario: valid node.yaml → CLI must stay fully silent (legacy PYOF printed nothing)
    ...
    assert result.stdout == "", f"FAIL: stdout must be empty, got: {result.stdout!r}"
    assert result.stderr == "", f"FAIL: stderr must be empty (byte-identical), got: {result.stderr!r}"
...
def test_cli_missing_field_exit1_golden(tmp_path) -> None:
    ...
    # · Scenario: missing 'modules' → exact legacy error line
    # · Last fail: N/A (new CLI)
    ...
    assert result.stdout == "", f"FAIL: stdout must stay empty, got: {result.stdout!r}"
...
def test_cli_type_mismatch_exit1_golden(tmp_path) -> None:
    ...
    # · Scenario: name: 123 → "  Error at 'node > name': ..." (legacy " > " separator)
    # · Last fail: N/A (new CLI)
