...
def test_cli_exit_code(caplog, tmp_path):
    ...
    result_valid = subprocess.run(
        [sys.executable, str(_SCRIPT_PATH), str(valid_path)], capture_output=True, text=True, check=False
    )
    ...
    result_invalid = subprocess.run(
        [sys.executable, str(_SCRIPT_PATH), str(invalid_path)], capture_output=True, text=True, check=False
    )
