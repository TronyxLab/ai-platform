...
class TestMainCLI:
    def test_all_passes_on_valid(self, tmp_path, tmp_schema, tmp_dotenv, tmp_secrets_manifest, caplog):
        """--all with valid modules → exit 0 (пути через CLI-аргументы, 0 monkeypatch)."""
        ...
        # DI (167 D4): пути инжектятся CLI-аргументами (--schema-path/--modules-dir/
        # --secrets-manifest) — уже-DI шов main(); 0 setattr-патчей констант.
        rc = main([
            "--all",
            "--schema-path",
            str(tmp_schema),
    ...
    def test_strict_mode_detects_issues(self, tmp_path, tmp_schema, tmp_dotenv, tmp_secrets_manifest, caplog):  # ruff: ignore[ARG002]
