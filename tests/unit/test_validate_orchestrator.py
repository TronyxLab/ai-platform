"""
# GREP_SUMMARY: test-validate-orchestrator, discovery, schema-routing, extension-check, detect-validator, exit-codes, caplog, LDD, subprocess-DI, D2-lint, D3-discovery, D5-extension
# STRUCTURE: ▶ tmp_path fixtures (valid/invalid node.yaml + ai-platform.yml) → ◇ unit functions (discover/resolve/detect/extension) → ◇ main() flows with run_cmd DI → ⊕ stderr golden asserts → ◇ LDD IMP:9 via ldd_trajectory → ⎋ exit-code asserts
...
##           через run_cmd DI (W-H DevPlan 163), граница subprocess не тестируется через subprocess).
...
@dataclass
class _FakeProc:
    """Фейковый CompletedProcess для run_cmd DI (граница subprocess)."""
...
class _FakeFacts:
    """EnvironmentFacts-fake (DevPlan 160 W4b): which('ajv') через параметр detect_validator(facts=...)."""
    ...
    def which(self, binary: str) -> str | None:
        return self._ajv if binary == "ajv" else None
...
def test_check_project_extension_allows_yaml(tmp_path, caplog, capsys) -> None:
    ...
    assert not capsys.readouterr().err, "FAIL: allow-path не должен эмитить stderr"


# endregion TEST_EXTENSION_ALLOWS


# region TEST_DETECT_PRIORITY
@ldd_trajectory
...
# endregion TEST_DETECT_PRIORITY


# region TEST_DETECT_NONE
@ldd_trajectory
def test_detect_validator_none_exits_1(caplog, capsys) -> None:
    ...
    with pytest.raises(PlatformFatalError) as exc_info:
        validate_orchestrator.detect_validator(facts=_FakeFacts(ajv_path=None), find_spec_fn=lambda _name: None)
...
@ldd_trajectory
def test_validate_with_python_ok(tmp_path, caplog, capsys) -> None:
    ...
    captured: list = []
    ok = validate_orchestrator.validate_with_python(yaml_f, _NODE_SCHEMA, run_cmd=_make_run(captured, returncode=0))
...
@ldd_trajectory
def test_validate_with_python_fail_format(tmp_path, caplog, capsys) -> None:
    ...
    captured: list = []
    ok = validate_orchestrator.validate_with_python(
        yaml_f, _NODE_SCHEMA, run_cmd=_make_run(captured, returncode=1, stderr=golden_err)
    )
...
@ldd_trajectory
def test_main_flag_only_skips_discovery(caplog, capsys) -> None:
    ...
    discovery_mock = mock.Mock(return_value=[])

    rc = validate_orchestrator.main(["--lint"], validator="python", discover_fn=discovery_mock)
...
@ldd_trajectory
def test_main_explicit_file_validates(tmp_path, caplog, capsys) -> None:
    ...
    rc = validate_orchestrator.main([str(yaml_f)], validator="python", run_cmd=_make_run(captured, returncode=0))
...
@ldd_trajectory
def test_main_error_aggregation_exit_1(tmp_path, caplog, capsys) -> None:
    ...
    rc = validate_orchestrator.main(
        [str(yaml_f)],
        validator="python",
        run_cmd=_make_run(captured, returncode=1, stderr="  Error at '(root)': 'modules' is a required property"),
    )
...
@ldd_trajectory
def test_main_check_fqdn_delegates(caplog, capsys) -> None:
    ...
    rc = validate_orchestrator.main(["--check-fqdn", project_dir], run_cmd=_make_run(captured, returncode=1))
...
@ldd_trajectory
def test_main_check_ports_default_base(caplog, capsys) -> None:
    ...
    captured: list = []
    fake_run = _make_run(captured, returncode=0)

    # Scenario A: PROJECTS_BASE env (DI env-dict, W-H — 0 setenv)
    rc_a = validate_orchestrator.main(["--check-ports"], run_cmd=fake_run, env={"PROJECTS_BASE": "/custom/projects"})
    assert rc_a == 0
    cmd_a = captured[-1][0]
    ...
    rc_b = validate_orchestrator.main(["--check-ports"], run_cmd=fake_run, env={})
    assert rc_b == 0
    cmd_b = captured[-1][0]
...
@ldd_trajectory
def test_aiplatform_yml_extension_error_plus_validation(tmp_path, caplog, capsys) -> None:
