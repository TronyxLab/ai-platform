...
##           NO real subprocess/network. monkeypatch — только внутренние функции openssl-парсинга.
## @invariants
##   - DI: curl_runner/s_client_runner/ssh_runner инжектятся (паттерн vps_readiness)
##   - Каждый тест: @pytest.mark.unit + @ldd_trajectory + # 🧪 TRAP[TEST] (R1/R2 честность)
...
def test_collect_remote_via_ssh(caplog, tmp_path, monkeypatch) -> None:
    ...
    def fake_ssh(host: str, user: str, cmd: str, timeout: int) -> tuple[int, str]:
        assert "cat /etc/nginx/conf.d/overlay/*.conf" in cmd
...
## @purpose — T14: check_tls через s_client_runner DI + monkeypatch SAN/expiry — полный ok
## @io — ⇥ DI s_client_runner + monkeypatch → ⎋ None (asserts chain/san/days/verdict)
## @complexity — O(1)
@pytest.mark.unit
@ldd_trajectory
...
def test_check_tls_verdict_ok(caplog, monkeypatch) -> None:
    ...
    monkeypatch.setattr(verify_sweep, "_cert_san_matches", lambda *a, **k: True)
    monkeypatch.setattr(verify_sweep, "_cert_days_left", lambda *a, **k: 20)
    monkeypatch.setattr(ssl_certs, "cert_check_expiry", lambda *a, **k: True)
    monkeypatch.setattr(ssl_certs, "cert_is_le_issuer", lambda *a, **k: True)
...
## @io — ⇥ DI + monkeypatch _cert_days_left=-1 → ⎋ None (asserts fail)
## @complexity — O(1)
@pytest.mark.unit
@ldd_trajectory
...
def test_check_tls_expired_fails(caplog, monkeypatch) -> None:
    ...
    monkeypatch.setattr(verify_sweep, "_cert_san_matches", lambda *a, **k: True)
    monkeypatch.setattr(verify_sweep, "_cert_days_left", lambda *a, **k: -1)
    monkeypatch.setattr(ssl_certs, "cert_check_expiry", lambda *a, **k: False)
    monkeypatch.setattr(ssl_certs, "cert_is_le_issuer", lambda *a, **k: True)
...
## @io — ⇥ DI + monkeypatch _cert_san_matches=False → ⎋ None (asserts fail)
## @complexity — O(1)
@pytest.mark.unit
@ldd_trajectory
...
def test_check_tls_san_mismatch_fails(caplog, monkeypatch) -> None:
    ...
    monkeypatch.setattr(verify_sweep, "_cert_san_matches", lambda *a, **k: False)
    monkeypatch.setattr(verify_sweep, "_cert_days_left", lambda *a, **k: 30)
    monkeypatch.setattr(ssl_certs, "cert_check_expiry", lambda *a, **k: True)
    monkeypatch.setattr(ssl_certs, "cert_is_le_issuer", lambda *a, **k: True)
