...
# STRUCTURE: fixtures(tmp_path project factory) → ◇ configure_vhost ┌empty domain → SKIP (False) без мутаций┐ → ◇ primary vhost_renderer (real call, D-I1 закрыт) → ◇ fallback subprocess (add-vhost.sh) → ◇ update_yaml_for_vhost (needs.domain + expose:true) → ◇ resolve_node_configs_dir (walk-up | PROJECTS_BASE env) → ⎋ LDD IMP:9
...
##           node-configs dir отсутствует), resolve_node_configs_dir (walk-up + PROJECTS_BASE fallback).
## @invariants
...
##            2026-08-14 | DevPlan 167 D2 — runner/update_yaml/resolve_configs DI-параметры (7 → 0 setattr)
...
# · Scenario: configure_vhost(domain="") → False; update_yaml mocked → assert NOT called;
...
@ldd_trajectory
def test_configure_vhost_empty_domain_skips_no_mutation(tmp_path, caplog) -> None:
    ...
    result = vc.configure_vhost(project_dir, domain="", org="org", yaml_file=yaml_file, update_yaml=update_mock)
...
@ldd_trajectory
def test_configure_vhost_renderer_primary_path(tmp_path, caplog) -> None:
...
@ldd_trajectory
def test_configure_vhost_renderer_false_triggers_subprocess(tmp_path, caplog) -> None:
...
def test_configure_vhost_subprocess_node_configs_missing(tmp_path, monkeypatch, caplog) -> None:
    ...
    result = vc.configure_vhost(
        ...
        org="org",
        yaml_file=yaml_file,
        node_configs_dir=None,
        runner=subprocess_mock,
        resolve_configs=lambda project_dir, org: None,  # ruff: ignore[ARG005]
    )
...
## @purpose  PROJECTS_BASE env fallback: parent.name != org → `PROJECTS_BASE/org/node-configs` (is_dir).
# 🧪 TRAP[TEST] · resolve_node_configs_dir_env_fallback · Contract (D4) · Regression: PROJECTS_BASE fallback сломан
# · Scenario: project вне org-структуры; PROJECTS_BASE=tmp; tmp/org/node-configs создан → resolve вернёт его
# · Last fail: N/A (новый тест W4.1)
# · Remove if: PROJECTS_BASE fallback удаляется из resolve_node_configs_dir
@ldd_trajectory
def test_resolve_node_configs_dir_env_fallback(tmp_path, monkeypatch, caplog) -> None:
    """PROJECTS_BASE env → `PROJECTS_BASE/org/node-configs`."""
    ...
    logger.info("[IMP:9][test] resolve_node_configs_dir: PROJECTS_BASE fallback → %s ✓", resolved)
...
# · Scenario: proj вне org-структуры, PROJECTS_BASE не задан, нет node-configs → None (без raise)
# · Last fail: N/A (новый negative-тест W4.1)
...
def test_resolve_node_configs_dir_none(tmp_path, monkeypatch, caplog) -> None:
    """Нет walk-up и нет PROJECTS_BASE → None (graceful)."""
    monkeypatch.delenv("PROJECTS_BASE", raising=False)
    proj_dir = tmp_path / "elsewhere" / "proj"
    proj_dir.mkdir(parents=True)
