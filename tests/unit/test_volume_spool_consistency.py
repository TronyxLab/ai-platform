...
@ldd_trajectory
# GUARD-PRESERVE (168): контракт spool→volume — каждый spool_dir/spool_volume из module.yaml обязан
# иметь named volume с matching device (иначе bind-mount fail на деплое); единственное покрытие инварианта
...
def test_deploy_modules_knows_all_spools(platform_root, all_module_yamls, caplog) -> None:
    ...
    spool_validator = Path(platform_root) / "core" / "internal" / "bootstrap" / "deploy" / "spool_validator.py"
    assert pathlib.Path(spool_validator).is_file(), f"spool_validator.py not found at {spool_validator}"

    with pathlib.Path(spool_validator).open(encoding="utf-8") as f:
...
@pytest.mark.static_audit
@ldd_trajectory
# GUARD-PRESERVE (168): контракт документирования named volumes — недокументированный volume
# (не spool_dir/spool_volume и не known-host-dir) = tech debt; единственное покрытие orphan-детекции
