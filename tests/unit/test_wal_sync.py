...
# STRUCTURE: ┌FakeBoto (head/put/list/delete)┐ → ○ scan filter (incl .history, excl .backup) → ○ wal key wal/{node}/<timeline>/ → ○ HEAD 404→PUT / ok→skip → ○ safe-delete (old+in-S3→rm; old+NOT-in-S3→keep R5) → ○ rate-limit → ○ dry-run 0 mutations → ○ S3-fail→exit 1
...
## @changes  2026-08-13 | DevPlan 162 W1-4 — timeline namespace: scan включает .history (PITR),
##             S3-ключ wal/{node}/{timeline}/{name}; тесты обновлены на вложенные ключи
...
class FakeBoto:
    ...
    def head_object(self, Bucket: str | None = None, Key: str | None = None) -> dict:  # ruff: ignore[ARG002]
    ...
    def list_objects_v2(self, Bucket=None, Prefix=None, MaxKeys=None, ContinuationToken=None) -> dict:  # ruff: ignore[ARG002]
    ...
    def delete_objects(self, Bucket=None, Delete=None) -> dict:  # ruff: ignore[ARG002]
...
# 🧪 TRAP[TEST] · REGRESSION · WAL timeline namespace 162 W1-4
# · Scenario: scan-фильтр ^([0-9A-F]{24}|[0-9A-F]{8}\.history)$ — 24-hex сегменты И .history файлы
# ·   ВКЛЮЧЕНЫ (timeline-переходы нужны для PITR); .backup/random.txt/прочие не-WAL — исключены
# · Last fail: до 162 W1-4 — .history исключались регуляркой ^[0-9A-F]{24}$ → PITR после
# ·   timeline-switch неполон (restore через timeline-переход невозможен)
# · Remove if: WAL segment pattern changes
def test_scan_local_includes_history_files(tmp_path: Path, caplog) -> None:
    """24-hex сегменты + .history включены (PITR); .backup/random.txt/прочие исключены."""
    caplog.set_level(logging.INFO)
    ...
    assert names == ["000000000000000000000000", "00000002.history"], (
        f"scan: 24-hex сегмент + .history включены; .backup/прочие исключены, got {names}"
    )
    assert any("[IMP:8][wal_sync][scan] Timeline history file included" in r.message for r in caplog.records), (
        "IMP:8-лог включения .history (PITR-трасса)"
    )
    logger.info("[IMP:9][test_wal_sync] scan filter (incl .history, excl .backup/random) PASS")
...
# region WAL_TIMELINE_NAMESPACE


# 🧪 TRAP[TEST] · REGRESSION · WAL timeline namespace 162 W1-4
# · Scenario: WAL_SEGMENT_RE матчит 24-hex сегменты И .history файлы (timeline-переходы, PITR);
# ·   отклоняет не-WAL имена (лишний символ, .txt)
# · Last fail: до 162 W1-4 — регулярка ^[0-9A-F]{24}$ исключала .history → PITR через
# ·   timeline-переход неполон
# · Remove if: WAL segment pattern changes
def test_wal_segment_re_matches_segments_and_history(caplog) -> None:
    """WAL_SEGMENT_RE: 24-hex сегменты и .history принимаются; не-WAL имена отклоняются."""
    caplog.set_level(logging.INFO)
    assert wal_sync.WAL_SEGMENT_RE.fullmatch("000000010000000000000001"), "24-hex WAL-сегмент должен матчиться"
    assert wal_sync.WAL_SEGMENT_RE.fullmatch("00000002.history"), ".history файл (timeline-переход) должен матчиться"
    assert not wal_sync.WAL_SEGMENT_RE.fullmatch("000000010000000000000001x"), "25 hex — reject"
    assert not wal_sync.WAL_SEGMENT_RE.fullmatch("wal.txt"), "не-WAL имя — reject"
    logger.info("[IMP:9][test_wal_sync] WAL_SEGMENT_RE (segment + .history) PASS")


# 🧪 TRAP[TEST] · REGRESSION · WAL timeline namespace 162 W1-4 (parametrize)
# · Scenario: _wal_key парсит PG timeline ID из первых 8 hex → вложенный ключ wal/{node}/{timeline}/{name}:
# ·   WAL-сегмент 000000010000000000000001 → 00000001; .history 00000002.history → 00000002 (PITR)
# · Last fail: до 162 W1-4 — ключ был плоским wal/{node}/{name} → коллизия после реинсталла
# ·   (новый инстанс нумерует WAL с #1, старые #1-#42 в S3 → skip по HEAD → PITR сломан)
# · Remove if: ключевая схема S3 меняется
@pytest.mark.parametrize(
    ("name", "expected_key"),
    [
        (
            "000000010000000000000001",
            "platform/backups/wal/node1/00000001/000000010000000000000001",
        ),
        ("00000002.history", "platform/backups/wal/node1/00000002/00000002.history"),
    ],
    ids=["segment", "history"],
)
def test_wal_key_timeline_namespace(caplog, name: str, expected_key: str) -> None:
    """_wal_key: timeline namespace из первых 8 hex имени WAL-сегмента / .history (plan W1-4)."""
    caplog.set_level(logging.INFO)
    assert wal_sync._wal_key("platform/backups", "node1", name) == expected_key, (
        f"timeline namespace: {name} → {expected_key}"
    )
    logger.info("[IMP:9][test_wal_sync] _wal_key(%s) timeline namespace PASS", name)


# 🧪 TRAP[TEST] · REGRESSION · WAL timeline namespace 162 W1-4 (plan validation item 3)
# · Scenario: WAL с timeline 00000001 и 00000002 → РАЗНЫЕ S3-пути (никакой коллизии после реинсталла)
# · Last fail: до 162 W1-4 — оба сегмента писались в wal/{node}/ → HEAD-skip нового инстанса
# ·   пропускал upload, PITR нового кластера сломан
# · Remove if: ключевая схема S3 меняется
def test_wal_key_different_timelines_different_s3_paths(caplog) -> None:
    """Разные timeline → разные S3-ключи (инвариант namespace, plan W1-4 validation 3)."""
    caplog.set_level(logging.INFO)
    key_t1 = wal_sync._wal_key("platform/backups", "node1", "000000010000000000000001")
    key_t2 = wal_sync._wal_key("platform/backups", "node1", "000000020000000000000002")
    assert key_t1 != key_t2, "timeline 00000001 и 00000002 → разные S3-пути"
    assert key_t1.startswith("platform/backups/wal/node1/00000001/")
    assert key_t2.startswith("platform/backups/wal/node1/00000002/")
    logger.info("[IMP:9][test_wal_sync] timeline → distinct S3 paths PASS")


# endregion WAL_TIMELINE_NAMESPACE
...
def test_sync_uploads_when_head_404(tmp_path: Path, caplog) -> None:
    ...
    assert uploaded == 1
    # timeline namespace (162 W1-4): "A"*24 → timeline "aaaaaaaa"
    assert fake.put_calls == ["platform/backups/wal/node1/aaaaaaaa/" + wal], "HEAD 404 → PUT expected"
...
def test_sync_skips_when_head_ok(tmp_path: Path, caplog) -> None:
    ...
    client = _make_client(fake)
    wal = "B" * 24
    fake.objects["platform/backups/wal/node1/bbbbbbbb/" + wal] = {"content": b"x", "lm": datetime.now(timezone.utc)}
    (tmp_path / wal).write_bytes(b"data")
    entries = wal_sync.scan_local(str(tmp_path))
...
def test_local_retention_removes_old_confirmed(tmp_path: Path, caplog) -> None:
    ...
    fake.objects["platform/backups/wal/node1/cccccccc/" + wal] = {"content": b"x", "lm": datetime.now(timezone.utc)}
    entries = wal_sync.scan_local(str(tmp_path))
...
def test_local_retention_keeps_young(tmp_path: Path, caplog) -> None:
    ...
    fake.objects["platform/backups/wal/node1/eeeeeeee/" + wal] = {"content": b"x", "lm": datetime.now(timezone.utc)}
    entries = wal_sync.scan_local(str(tmp_path))
...
def test_s3_retention_purges_old_objects(caplog) -> None:
    ...
    # Nested timeline keys (162 W1-4): wal/node1/{timeline}/ + имя WAL-сегмента
    fake.objects[prefix + "ffffffff/" + "F" * 24] = {"content": b"x", "lm": old}
    fake.objects[prefix + "gggggggg/" + "G" * 24] = {"content": b"x", "lm": fresh}
    fake.objects["platform/backups/wal/node2/" + "hhhhhhhh/" + "H" * 24] = {"content": b"x", "lm": old}

    purged = wal_sync.apply_s3_retention(client, "platform/backups", "node1", 14)

    assert purged == 1, "только старый объект node1"
    assert fake.deleted == [prefix + "ffffffff/" + "F" * 24]
    assert prefix + "gggggggg/" + "G" * 24 in fake.objects, "свежий объект остаётся"
    assert "platform/backups/wal/node2/" + "hhhhhhhh/" + "H" * 24 in fake.objects, "чужой node не тронут"
...
def test_dry_run_zero_mutations(tmp_path: Path, caplog) -> None:
    ...
    fake.objects["platform/backups/wal/node1/bbbbbbbb/" + wal_old] = {"content": b"x", "lm": datetime.now(timezone.utc)}
    entries = wal_sync.scan_local(str(tmp_path))
...
def test_s3_failure_in_sync_exits_1(tmp_path: Path, caplog) -> None:
    """S3-ошибка в sync → main возвращает 1 + IMP:10 «S3 FAIL» (DI: env-dict + client_factory, 167 D4)."""
    caplog.set_level(logging.INFO)
    wal = "C" * 24
    (tmp_path / wal).write_bytes(b"data")
    ...
    # DI (DevPlan 167 D4): WAL_ARCHIVE_DIR → tmp_path (реальный scan_local), client_factory →
    # FakeBoto (реальный sync). 0 setattr-патчей — env-dict + factory инжектируются в main().
    exit_code = wal_sync.main(
        [],
        env={"WAL_ARCHIVE_DIR": str(tmp_path)},
